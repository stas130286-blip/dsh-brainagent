 ...
 *
 * v0.7.0: фабрика createGoalStack(workspaceDir, config?) — цели,
 * желания, журнал решений и exploration-бусты в замыкании инстанса;
 * свободные функции — обёртки над общим ленивым инстансом.
 * Чистая buildGoalContext и no-op resetExtractionThrottle остаются
 * на уровне модуля. Без workspaceDir фабрика создаёт detached-инстанс
...
export type GoalStackInstance = {
  ...
  makeVoluntaryDecision(options: string[], context: string): VoluntaryDecision | undefined;
  buildVolitionContext(): string | undefined;
  getDesires(): Desire[];
  weakenDesiresAfterFire(consecutiveFires: number): void;
  satisfyDesiresOnUserResponse(): void;
  getDecisionLog(): VoluntaryDecision[];
  boostExploration(domain: string, multiplier: number, durationCycles: number): void;
  getEffectiveExplorationRate(context: string): number;
  tickExplorationBoosts(): void;
  escalateStaleDesires(): Array<{
    desireId: string;
    oldStrength: number;
    newStrength: number;
  }>;
...
/**
 * Create a goal-stack instance with isolated state.
 * Without workspaceDir the instance has no persistence (detached) —
 * identical to pre-init module behavior.
 */
export function createGoalStack(workspaceDir: string, config?: BrainAgentConfig): GoalStackInstance {
  // ── State (closure) ─────────────────────────────────────────────
  const storageDir = workspaceDir ? join(workspaceDir, ".brainagent", "goals") : "";

  const gcfg = config?.goalStack;
  const maxGoals = gcfg?.maxGoals ?? 20;
  const defaultTTLMs = gcfg?.defaultTTLMs ?? 24 * 60 * 60 * 1000;
   ...
   */
  function resolveDesireCompetition(context: string): Desire | undefined {
   ...
   */
  function makeVoluntaryDecision(
    options: string[],
    context: string,
   ...
   */
  function buildVolitionContext(): string | undefined {
    if (desires.length === 0) return undefined;
   ...
   */
  function weakenDesiresAfterFire(consecutiveFires: number): void {
   ...
   */
  function satisfyDesiresOnUserResponse(): void {
  ...
  function getDecisionLog(): VoluntaryDecision[] {
    return [...decisionLog];
  }
   ...
   */
  function boostExploration(domain: string, multiplier: number, durationCycles: number): void {
  ...
  function getEffectiveExplorationRate(context: string): number {
    const contextLower = context.toLowerCase();
  ...
  function tickExplorationBoosts(): void {
   ...
   */
  function escalateStaleDesires(): Array<{
    desireId: string;
    oldStrength: number;
    newStrength: number;
  }> {
   ...
   */
  async function extractGoalsFromConversation(
    userMessage: string,
    config: NeuroClawConfig,
    logger?: { info: (msg: string) => void },
  ): Promise<Goal[]> {
   ...
   */
  function checkAutonomousGoals(idleMs?: number): Goal[] {
    const now = Date.now();
    const triggered: Goal[] = [];
...
let active: GoalStackInstance | null = null;

function current(): GoalStackInstance {
  if (!active) active = createGoalStack("");
  return active;
}

export function initGoalStack(workspaceDir: string, config: BrainAgentConfig): void {
  active = createGoalStack(workspaceDir, config);
}
...
export function checkGoalTriggers(
  ...
  return current().checkGoalTriggers(input, currentEmotion, currentDomain);
}
...
export function getGoalStackStats(): {
  ...
  return current().getGoalStackStats();
}
...
export function addDesire(
  ...
  return current().addDesire(type, description, strength, source);
}

export function resolveDesireCompetition(context: string): Desire | undefined {
  return current().resolveDesireCompetition(context);
}
...
export function escalateStaleDesires(): Array<{
  ...
  return current().escalateStaleDesires();
}
...
export async function extractGoalsFromConversation(
  ...
  return current().extractGoalsFromConversation(userMessage, config, logger);
}
