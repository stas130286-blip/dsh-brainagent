...
export type InteractionOutcomeParams = {
...
export type DopamineSystemInstance = {
  /** Обработать исход цикла и разослать нейромодуляторный сигнал. */
  processOutcome(params: InteractionOutcomeParams, config: BrainAgentConfig): DopamineSignal;
  /** Отметить наблюдение как новое (для ацетилхолина). */
  markNovelty(): void;
  /** Текущие уровни нейромодуляторов (копия). */
  getState(): NeuromodulatorState;
  /** Эффективная скорость обучения (ацетилхолин + дофамин). */
  getEffectiveLearningRate(baseLearningRate: number, config: BrainAgentConfig): number;
  /** Толерантность к риску (серотонин). */
  getRiskTolerance(): number;
  /** Уровень внимания (норадреналин). */
  ...
  /** Сбросить отложенную персистенцию на диск. */
  stop(): void;
...
export function createDopamineSystem(workspaceDir: string): DopamineSystemInstance {
  const storageDir = join(workspaceDir, ".brainagent", "neuromodulators");
  ...
  function updateAcetylcholine(_domain: string, _config: BrainAgentConfig): void {
   ...
   */
  function processOutcome(params: InteractionOutcomeParams, config: BrainAgentConfig): DopamineSignal {
    totalInteractions++;
  ...
  return {
    ...
    getEffectiveLearningRate: (baseLearningRate, config) => {
      const achBoost =
        1 +
        (state.acetylcholine - 0.5) * (config.neuromodulators.acetylcholineLearningBoost - 1);
    ...
    getStats,
    stop: () => {
      // Нечего отписывать (модуль только эмитит); отложенная запись остаётся в persist-очереди
    },
    dispose: () => {
      // см. stop
    },
 ...
 */
function computeCreditAssignment(modules: string[], issues: string[]): Record<string, number> {
...
let active: DopamineSystemInstance | undefined;

// ── Совместимый API ─────────────────────────────────────────────────

export function initDopamineSystem(workspaceDir: string): void {
  active?.dispose();
  active = createDopamineSystem(workspaceDir);
}

/** Остановить и снять инстанс (для симметрии жизненного цикла). */
export function stopDopamineSystem(): void {
  active?.stop();
  active = undefined;
}
 ...
 * Process the outcome of an interaction cycle and compute neuromodulator updates.
 * Returns the dopamine signal that gets broadcast to all modules.
 * До инициализации возвращает нейтральный сигнал без сайд-эффектов.
 */
export function processInteractionOutcome(
  params: InteractionOutcomeParams,
  ...
  return active.processOutcome(params, config);
}
