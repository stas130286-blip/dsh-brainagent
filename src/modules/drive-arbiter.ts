 ...
 *
 * v0.6.9: фабрика createDriveArbiter() с per-instance состоянием;
...
export interface DriveArbiterInstance {
  getStats: () => DriveArbiterStats;
  getLastSelectedDrive: () => DriveId | null;
  buildContext: () => string | null;
  stop: () => void;
}

/**
 * Create a Drive Arbiter instance with its own state.
 * Подписки на шину оформляются сразу при создании (эквивалент init).
 */
export function createDriveArbiter(
  workspaceDir: string,
  cfg: BrainAgentConfig,
  getters: DriveStatGetters,
  log?: { info: (msg: string) => void },
): DriveArbiterInstance {
  ...
  // Реентрантность-гард арбитража (сбрасывается в finally)
  ...
  function arbitrateInner(): void {
    ...
    const scores: DriveScore[] = drives.map((d) => {
      ...
      if (d.id === lastSelectedDrive && lastSelectionTime > 0) {
        ...
        const decayFactor = Math.pow(config.recencyDecay, timeSinceLast / (5 * 60 * 1000));
  ...
  function getStats(): DriveArbiterStats {
    return {
      driveWeights: { ...driveWeights },
      lastSelectedDrive,
      totalArbitrations,
      recentConflicts: conflictLog.filter((e) => Date.now() - e.timestamp < 60 * 60 * 1000)
        .length,
    };
  }

  function getLastSelectedDrive(): DriveId | null {
...
let active: DriveArbiterInstance | undefined;
...
export function initDriveArbiter(
  ...
  active?.stop();
  active = createDriveArbiter(workspaceDir, cfg, getters, log);
}
...
export function getLastSelectedDrive(): DriveId | null {
  return active?.getLastSelectedDrive() ?? null;
}
