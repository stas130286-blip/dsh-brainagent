...
let config: BrainAgentConfig["autonomousResearch"] | undefined;
...
export async function executeResearch(topic: string): Promise<ResearchResult | null> {
  ...
  const provider = resolveProvider(deps.gatewayConfig);
...
function resolveProvider(gatewayConfig: NeuroClawConfig): SearchProvider {
...
async function searchBrave(queries: string[]): Promise<LinkSearchResponse | null> {
  ...
  for (const query of queries) {
    try {
      ...
      if (!response.ok) {
        deps.logger.info(`BrainAgent AutonomousResearch: Brave search failed (${response.status})`);
        continue;
      }
...
async function searchGrok(queries: string[]): Promise<TextSearchResponse | null> {
  ...
  try {
    ...
    if (!response.ok) {
      deps.logger.info(`BrainAgent AutonomousResearch: Grok search failed (${response.status})`);
      return null;
    }
