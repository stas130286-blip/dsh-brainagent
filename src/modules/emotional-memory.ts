...
export function initEmotionalMemory(workspaceDir: string, _config: BrainAgentConfig): void {
  storageDir = join(workspaceDir, ".brainagent", "emotional-memory");
...
export function computeEmotionMatchBonus(
  ...
  return config.emotionalMemory.emotionMatchBonus;
}
...
export async function generateQualiaAsync(
  ...
  if (intensity <= LLM_INTENSITY_THRESHOLD || !config) {
  ...
  try {
    ...
    const response = await callLLM(QUALIA_METAPHOR_PROMPT, userMessage, config, logger, 200);
