...
// Autobiographical Self state
let autobiographicalMemories: AutobiographicalMemory[] = [];
let maxAutobioMemories = 100;
let significantRewardThreshold = 0.8;
let significantEmotionThreshold = 0.7;

// ── Initialization ──────────────────────────────────────────────────

export function initAgentIdentity(workspaceDir: string, config: BrainAgentConfig): void {
  storageDir = join(workspaceDir, ".brainagent", "identity");
  if (!existsSync(storageDir)) {
    mkdirSync(storageDir, { recursive: true });
  }
  snapshotInterval = config.agentIdentity.snapshotInterval;
  maxSnapshots = config.agentIdentity.maxSnapshots;
  maxAutobioMemories = config.agentIdentity.maxAutobiographicalMemories ?? 100;
  significantRewardThreshold = config.agentIdentity.significantRewardThreshold ?? 0.8;
  significantEmotionThreshold = config.agentIdentity.significantEmotionThreshold ?? 0.7;
