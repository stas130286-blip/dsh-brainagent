...
import { markNovelty, getEffectiveLearningRate, getRiskTolerance } from "./dopamine-system.ts";
...
// ── Synaptic weight state (Hebbian learning) ────────────────────────

let storageDir = "";
let currentConfig: BrainAgentConfig | null = null;

/** Current synaptic weights for all pathways */
let synapticState: SynapticState = createDefaultSynapticState();

/** Track which pathways were activated in the current cycle */
let activatedPathways: Set<PathwayName> = new Set();
...
// ── State tracking for pathway operations ───────────────────────────
...
let currentNeuroState: NeuromodulatorState = {
  dopamine: 0.5,
  serotonin: 0.6,
  norepinephrine: 0.3,
  acetylcholine: 0.4,
...
// Logger reference for pathway operations
let pathwayLogger: { info: (msg: string) => void; warn: (msg: string) => void } | undefined;

/** Unsubscribe functions for all registered listeners */
let pathwayUnsubs: Array<() => void> = [];
...
export function initNeuralPathways(
  ...
  // Clean up any previously registered listeners (idempotent init)
  for (const unsub of pathwayUnsubs) unsub();
  pathwayUnsubs = [];
  activatedPathways.clear();

  pathwayLogger = logger;
  currentConfig = config;

  // Setup storage for synaptic weights
  storageDir = join(workspaceDir, ".brainagent", "synapses");
  if (!existsSync(storageDir)) {
    mkdirSync(storageDir, { recursive: true });
  }

  // Load persisted synaptic state
  loadSynapticState();
...
export function buildNeuromodulatorContext(): string | undefined {
  ...
  const significantDev =
    Math.abs(currentNeuroState.dopamine - 0.5) > 0.2 ||
    Math.abs(currentNeuroState.norepinephrine - 0.3) > 0.2 ||
    Math.abs(currentNeuroState.serotonin - 0.6) > 0.15;
