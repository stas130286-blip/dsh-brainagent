...
let storageDir = "";
let config: BrainAgentConfig["temporalAwareness"] | undefined;
...
export function stopTemporalAwareness(): void {
  persistState();
  logger?.info("BrainAgent TemporalAwareness: stopped.");
}

// ── Core: called on each interaction ──────────────────────────────
...
function computeDensity(now: number): number {
  if (!config || timestamps.length < 2) return 0;
...
export function buildTemporalContext(): string | null {
  if (!config || timestamps.length < 2) return null;
