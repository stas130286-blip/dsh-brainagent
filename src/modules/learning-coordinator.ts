...
export type LearningCoordinatorInstance = {
  getStats(): LearningStats;
  getLatestCycleReport(): LearningCycleReport | undefined;
  buildContext(): string | undefined;
  recordRecurringIssue(issueType: string): LearningInsight | undefined;
  recordDomainPerformance(domain: MessageDomain, reward: number, errorIssues?: string[]): void;
  assessCapability(domain: MessageDomain): CapabilityAssessment;
  getDomainPerformance(domain: MessageDomain): DomainPerformance | undefined;
  buildCapabilityContext(currentDomain?: MessageDomain): string | undefined;
  /** Отписка от шины. */
  stop(): void;
...
/** Создать координатора обучения с собственным состоянием и подписками. */
export function createLearningCoordinator(
  workspaceDir: string,
  config: BrainAgentConfig,
): LearningCoordinatorInstance {
  const storageDir = join(workspaceDir, ".brainagent", "learning");
  ...
  function generateInsights(cfg: BrainAgentConfig): void {
  ...
  function generateCycleReport(lastSignal: DopamineSignal): void {
    const perModuleMetrics: Record<string, import("./types.ts").ModulePerformanceMetrics> = {};
  ...
  function getOrCreateMetrics(moduleName: string): ModuleMetrics {
  ...
  function getLatestCycleReport(): LearningCycleReport | undefined {
   ...
   */
  function recordRecurringIssue(issueType: string): LearningInsight | undefined {
   ...
   */
  function recordDomainPerformance(
   ...
   */
  function assessCapability(domain: MessageDomain): CapabilityAssessment {
   ...
   */
  function getDomainPerformance(domain: MessageDomain): DomainPerformance | undefined {
   ...
   */
  function buildCapabilityContext(currentDomain?: MessageDomain): string | undefined {
...
let active: LearningCoordinatorInstance | undefined;

export function initLearningCoordinator(workspaceDir: string, config: BrainAgentConfig): void {
  active?.dispose();
  active = createLearningCoordinator(workspaceDir, config);
}

export function stopLearningCoordinator(): void {
  active?.stop();
  active = undefined;
}
...
export function getLatestCycleReport(): LearningCycleReport | undefined {
  return active?.getLatestCycleReport();
}
...
export function recordRecurringIssue(issueType: string): LearningInsight | undefined {
  return active?.recordRecurringIssue(issueType);
}
...
export function recordDomainPerformance(
  ...
  active?.recordDomainPerformance(domain, reward, errorIssues);
}
...
export function assessCapability(domain: MessageDomain): CapabilityAssessment {
  if (active) return active.assessCapability(domain);
  return {
    domain,
    confidenceLevel: 0.5,
    reasoning: `Insufficient data for "${domain}" domain (0 cycles). Using default confidence.`,
  };
}
...
export function getDomainPerformance(domain: MessageDomain): DomainPerformance | undefined {
  return active?.getDomainPerformance(domain);
}
...
export function buildCapabilityContext(currentDomain?: MessageDomain): string | undefined {
  return active?.buildCapabilityContext(currentDomain);
}
