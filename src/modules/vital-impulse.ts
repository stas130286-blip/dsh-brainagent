 ...
 *
 * v0.7.0: фабрика createVitalImpulse(workspaceDir, cfg?, log?, deps?) —
 * давление, привыкание, GABA-торможение и адаптивные веса сигналов
 * в замыкании инстанса; подписки на шину оформляются при создании
 * и снимаются в stop() (lifecycle-семейство). Свободные функции —
 * обёртки над активным инстансом. До init модуль был инертен
 * (без слушателей шины) — обёртки сохраняют ровно это поведение.
 */
...
export type VitalImpulseInstance = {
  forceImpulse(reason?: string): void;
  consumeMotivation(): string | null;
  resetConsecutiveFires(): void;
  getConsecutiveAutonomousFires(): number;
  getStats(): VitalImpulseStats;
  stop(): void;
};
...
/**
 * Create a vital-impulse instance with isolated state.
 * Subscribes to bus events immediately; stop() removes the
 * subscriptions and flushes persisted state.
 * Without cfg the instance stays inert (signals are ignored) —
 * identical to pre-init module behavior.
 */
export function createVitalImpulse(
  workspaceDir: string,
  cfg?: BrainAgentConfig,
  log?: { info: (msg: string) => void },
  injectedDeps?: VitalImpulseDeps,
): VitalImpulseInstance {
  // ── Состояние (closure) ─────────────────────────────────────────
  const storageDir = workspaceDir ? join(workspaceDir, ".brainagent", "vital-impulse") : "";
  const config: BrainAgentConfig["vitalImpulse"] | undefined = cfg?.vitalImpulse;
  const circadianEnabled = cfg?.circadian?.enabled ?? false;
  const logger = log;
  const deps: VitalImpulseDeps | undefined = injectedDeps;
   ...
   */
  function consumeMotivation(): string | null {
   ...
   */
  function resetConsecutiveFires(): void {
  ...
  function getConsecutiveAutonomousFires(): number {
    return consecutiveAutonomousFires;
  }

  function getStats(): VitalImpulseStats {
...
let active: VitalImpulseInstance | undefined;

function current(): VitalImpulseInstance | undefined {
  return active;
}
...
export function initVitalImpulse(
  ...
  active?.stop();
  active = createVitalImpulse(workspaceDir, cfg, log, injectedDeps);
}
...
export function forceImpulse(reason?: string): void {
  current()?.forceImpulse(reason);
}

export function consumeMotivation(): string | null {
  return current()?.consumeMotivation() ?? null;
}

export function resetConsecutiveFires(): void {
  current()?.resetConsecutiveFires();
}

export function getConsecutiveAutonomousFires(): number {
  return current()?.getConsecutiveAutonomousFires() ?? 0;
}

export function getVitalImpulseStats(): VitalImpulseStats {
  const inst = current();
  if (!inst) {
    // Pre-init behavior: инертная статистика без слушателей шины
    return {
      currentPressure: 0,
