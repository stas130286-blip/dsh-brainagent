...
function buildHostConfig(options = {}) {
  ...
  const config2 = { models: { providers } };
  if (options.model) {
    config2.agents = { defaults: { model: options.model } };
  }
  return config2;
}
...
function mergeBrainConfig(config2) {
  const modules = { ...DEFAULT_CONFIG.modules };
  for (const [key, value] of Object.entries(config2.modules)) {
  ...
  return {
    ...
    dualProcess: {
      ...
      ...config2.dualProcess?.fastModel ? { fastModel: config2.dualProcess.fastModel } : {},
      ...config2.dualProcess?.slowModel ? { slowModel: config2.dualProcess.slowModel } : {}
    },
    circadian: {
      ...DEFAULT_CONFIG.circadian,
      enabled: config2.circadian.enabled
    },
    contextInjection: {
      ...DEFAULT_CONFIG.contextInjection,
      ...config2.contextInjection
    },
    learningLoop: {
      rewardLedger: {
        ...DEFAULT_CONFIG.learningLoop.rewardLedger,
        ...config2.learningLoop?.rewardLedger ?? {}
      },
      strategyBandit: {
        ...DEFAULT_CONFIG.learningLoop.strategyBandit,
        ...config2.learningLoop?.strategyBandit ?? {}
      }
    }
  };
}
...
function parseUserModelSelection(config2) {
...
function resolveProvider(config2) {
  const providers = config2.models?.providers;
  if (!providers) return null;
  const userSelection = parseUserModelSelection(config2);
  if (userSelection) {
...
function createLLMClient() {
  ...
  function isAIProviderAvailable3(config2) {
    return (availabilityHook?.() ?? false) || resolveProvider(config2) !== null;
  }
  async function callLLM2(systemPrompt, userText, config2, logger2, maxTokens = 500) {
    if (callBackend) {
      try {
        const bridged = await callBackend(systemPrompt, userText, config2, logger2, maxTokens);
        if (bridged !== void 0) return bridged;
      } catch (error) {
        logger2?.info(`BrainAgent LLM: bridge failed, falling back \u2014 ${String(error)}`);
      }
    }
    const provider = resolveProvider(config2);
    if (!provider) {
      logger2?.info("BrainAgent LLM: no AI provider configured, skipping");
      return null;
    }
    const userSelection = parseUserModelSelection(config2);
    if (userSelection) {
      logger2?.info(`BrainAgent LLM: calling ${provider.name} (${provider.model}) [user-selected]`);
    } else {
      logger2?.info(
...
${userText}` }] }],
        ...
        logger2?.info(`BrainAgent LLM: Google error ${response.status}: ${errorText}`);
        return null;
      } else {
      ...
      if (!response.ok) {
        const errorText = await response.text();
        logger2?.info(`BrainAgent LLM: ${provider.name} error ${response.status}: ${errorText}`);
        return null;
      }
      ...
      logger2?.info(`BrainAgent LLM: error \u2014 ${String(error)}`);
      return null;
    }
  }
...
function isAIProviderAvailable(config2) {
  return current().isAIProviderAvailable(config2);
}
async function callLLM(systemPrompt, userText, config2, logger2, maxTokens = 500) {
  return current().callLLM(systemPrompt, userText, config2, logger2, maxTokens);
}
...
async function fetchOpenAIEmbeddings(texts, provider, logger2) {
...
async function fetchGoogleEmbeddings(texts, provider, logger2) {
  ...
  if (!response.ok) {
    const errorText = await response.text();
    logger2?.info(`BrainAgent Embeddings: Google API error ${response.status}: ${errorText}`);
    return null;
  }
  const data = await response.json();
...
function createAIEmbeddings() {
  ...
  async function getEmbeddings(texts, config2, logger2) {
    const provider = resolveEmbeddingProvider(config2);
    if (!provider) {
      return null;
    }
    ...
    try {
      const result = provider.format === "google" ? await fetchGoogleEmbeddings(texts, provider, logger2) : await fetchOpenAIEmbeddings(texts, provider, logger2);
      if (result === null && provider.name === "Ollama") {
        ollamaUnreachable = true;
      }
      return result;
    } catch (error) {
      logger2?.info(`BrainAgent Embeddings: ${provider.name} error \u2014 ${String(error)}`);
      if (provider.name === "Ollama") ollamaUnreachable = true;
      return null;
    }
  }
  async function getEmbedding2(text, config2, logger2) {
    const result = await getEmbeddings([text], config2, logger2);
...
async function getEmbedding(text, config2, logger2) {
  return current2().getEmbedding(text, config2, logger2);
}
...
function createHippocampus(workspaceDir) {
  ...
  function scheduleEmbeddingBackfill() {
    ...
    const config2 = embeddingsConfig;
    const missing = [];
    ...
    void (async () => {
      for (let i = 0; i < missing.length; i++) {
        ...
        try {
          const vec = await getEmbedding(item.text, config2, embeddingsLogger);
          if (vec) {
  ...
  function initEmbeddings2(config2, logger2) {
    const provider = resolveEmbeddingProvider(config2);
    if (!provider) {
      logger2?.info("BrainAgent Hippocampus: no embedding provider \u2014 using TF-IDF");
      return;
    }
    embeddingsConfig = config2;
    embeddingsLogger = logger2;
  ...
  function updateEmbeddingsConfig2(config2) {
  ...
  function storeEpisode2(event, summary, emotionalContext = "neutral", entities = [], emotionIntensity = 0) {
    ...
    if (embeddingsAvailable && embeddingsConfig) {
      const config2 = embeddingsConfig;
      void getEmbedding(`${event} ${summary}`, config2, embeddingsLogger).then((vec) => {
        if (vec) {
  ...
  function storeFact2(content, category, sourceEpisodeIds = [], relatedIds = []) {
    ...
    if (embeddingsAvailable && embeddingsConfig) {
      const config2 = embeddingsConfig;
      void getEmbedding(`${content} ${category}`, config2, embeddingsLogger).then((vec) => {
        if (vec) {
  ...
  function reviseFact(existing, newContent, newConfidence, sourceEpisodeIds, reason) {
    ...
    if (embeddingsAvailable && embeddingsConfig) {
      const config2 = embeddingsConfig;
      void getEmbedding(`${newContent} ${existing.category}`, config2, embeddingsLogger).then(
        (vec) => {
          if (vec) {
  ...
  async function consolidate2(config2, neuroClawConfig, logger2, intensity = 0.5, skipAIReview = false) {
    let merged = 0;
    let pruned = 0;
    let strengthened = 0;
    ...
    if (semanticStore.length > config2.memory.maxSemanticMemories) {
      semanticStore.sort((a, b) => b.confidence - a.confidence);
      const removed = semanticStore.splice(config2.memory.maxSemanticMemories);
    ...
    if (!skipAIReview && neuroClawConfig && isAIProviderAvailable(neuroClawConfig) && semanticStore.length >= 5) {
      try {
        ...
        logger2?.info(`BrainAgent Consolidation: AI review error \u2014 ${String(err)}`);
      }
    }
    persistEpisodic();
    persistSemantic();
...
function initEmbeddings(config2, logger2) {
  current3().initEmbeddings(config2, logger2);
}
function updateEmbeddingsConfig(config2) {
  current3().updateEmbeddingsConfig(config2);
}
...
async function consolidate(config2, neuroClawConfig, logger2, intensity = 0.5, skipAIReview = false) {
  return current3().consolidate(config2, neuroClawConfig, logger2, intensity, skipAIReview);
}
function getStats() {
...
async function assessWithAI(text, config2, logger2) {
  const content = await callLLM(EMOTION_PROMPT, text, config2, logger2, 100);
...
function createMirrorNeurons(workspaceDir) {
  const storageDir2 = workspaceDir ? join3(workspaceDir, ".brainagent", "users") : "";
  const userModels = /* @__PURE__ */ new Map();
  if (storageDir2 && !existsSync2(storageDir2)) {
    mkdirSync2(storageDir2, { recursive: true });
  }
  try {
    const indexPath = join3(storageDir2, "index.json");
    if (storageDir2 && existsSync2(indexPath)) {
  ...
  function persistModels() {
    ...
    try {
      ...
      writeFileSync3(join3(storageDir2, "index.json"), JSON.stringify(data, null, 2), "utf-8");
    } catch {
    }
  }
  ...
  function observe2(userId, text, amygdalaResult, config2) {
    ...
    if (model.emotionHistory.length > config2.empathy.emotionHistoryLength) {
      model.emotionHistory = model.emotionHistory.slice(-config2.empathy.emotionHistoryLength);
    }
    ...
    applyTheoryOfMindUpdates(model, text, amygdalaResult, config2);
    persistModels();
  ...
  async function observeWithAI2(userId, text, amygdalaResult, config2, neuroConfig, logger2) {
    const model = getOrCreateModel(userId);
    model.emotionHistory.push({
      timestamp: Date.now(),
      emotion: amygdalaResult.emotion,
    ...
    const alpha = 0.3;
    model.stressLevel = model.stressLevel * (1 - alpha) + (isStressed ? amygdalaResult.emotionIntensity : 0) * alpha;
    const content = await callLLM(STYLE_PROMPT, text, neuroConfig, logger2, 100);
    if (content) {
      try {
        const jsonMatch = content.match(/\{[\s\S]*\}/);
        if (jsonMatch) {
...
function applyTheoryOfMindUpdates(model, text, amygdalaResult, config2) {
  ensureToMFields(model);
  const maxIntentHistory = config2.empathy.maxIntentHistory ?? 20;
  const domainLimit = config2.empathy.knowledgeModelDomainLimit ?? 15;
...
function updateKnowledgeModelToM(model, text, domainLimit) {
  ...
  for (const domain of domainSignals) {
    const current25 = model.knowledgeModel[domain];
    if (!current25) {
      model.knowledgeModel[domain] = "beginner";
    } else if (current25 === "beginner") {
...
function observe(userId, text, amygdalaResult, config2) {
  return current4().observe(userId, text, amygdalaResult, config2);
}
...
function observeWithAI(userId, text, amygdalaResult, config2, neuroConfig, logger2) {
  return current4().observeWithAI(userId, text, amygdalaResult, config2, neuroConfig, logger2);
}
...
function createPredictiveEngine(workspaceDir) {
  const storageDir2 = workspaceDir ? join4(workspaceDir, ".brainagent", "predictions") : "";
  ...
  let lastDomain;
  if (storageDir2 && !existsSync3(storageDir2)) {
    mkdirSync3(storageDir2, { recursive: true });
  }
  function loadPatterns() {
    if (!storageDir2) return;
    try {
      const tPath = join4(storageDir2, "temporal.json");
    ...
    try {
      const sPath = join4(storageDir2, "sequential.json");
    ...
    try {
      const cPath = join4(storageDir2, "contextual.json");
...
function createBasalGanglia(workspaceDir) {
  const storageDir2 = workspaceDir ? join5(workspaceDir, ".brainagent", "habits") : "";
  let habits = [];
  ...
  if (storageDir2 && !existsSync4(storageDir2)) {
    mkdirSync4(storageDir2, { recursive: true });
  }
  function loadHabits() {
    if (!storageDir2) return;
    try {
      const path = join5(storageDir2, "habits.json");
      if (existsSync4(path)) {
  ...
  function persistHabits() {
    if (!storageDir2) return;
    try {
      writeFileSync5(join5(storageDir2, "habits.json"), JSON.stringify(habits, null, 2), "utf-8");
    } catch {
    }
  }
...
async function detectReinforcementWithAI(text, config2, logger2) {
  const content = await callLLM(REINFORCEMENT_PROMPT, text, config2, logger2, 100);
...
function createIntrospection(workspaceDir, config2) {
  const storageDir2 = workspaceDir ? join6(workspaceDir, ".brainagent", "introspection") : "";
  const traces = [];
  let currentTrace = null;
  let maxTraces = config2?.introspection.maxTraces ?? 3;
  const injectConfidence = config2?.introspection.injectConfidence ?? true;
  const selfDialogue = [];
  const metaSnapshots = [];
  let maxSelfDialogue = config2?.introspection.maxSelfDialogue ?? 10;
  let maxMetaSnapshots = config2?.introspection.maxMetaSnapshots ?? 5;
  function loadState2() {
    if (!storageDir2) return;
    try {
      const path = join6(storageDir2, "traces.json");
      if (existsSync5(path)) {
  ...
  if (storageDir2) {
    if (!existsSync5(storageDir2)) {
      mkdirSync5(storageDir2, { recursive: true });
    }
    maxTraces = config2?.introspection.maxTraces ?? maxTraces;
    maxSelfDialogue = config2?.introspection.maxSelfDialogue ?? maxSelfDialogue;
    maxMetaSnapshots = config2?.introspection.maxMetaSnapshots ?? maxMetaSnapshots;
    loadState2();
  }
  return {
...
function initIntrospection(workspaceDir, config2) {
  active7 = createIntrospection(workspaceDir, config2);
}
...
function createDopamineSystem(workspaceDir) {
  const storageDir2 = join7(workspaceDir, ".brainagent", "neuromodulators");
  ...
  loadState2();
  function updateDopamine(predictionError, config2) {
    const baseline = config2.neuromodulators.baselineDopamine;
    const decay = config2.neuromodulators.dopamineDecayRate;
  ...
  function processOutcome(params, config2) {
    totalInteractions++;
    let reward = 0;
    ...
    updateDopamine(predictionError, config2);
    updateSerotonin(reward);
    updateNorepinephrine(params.complexity, params.emotion);
    updateAcetylcholine(params.domain, config2);
  ...
  return {
    ...
    getState: () => ({ ...state }),
    getEffectiveLearningRate: (baseLearningRate, config2) => {
      const achBoost = 1 + (state.acetylcholine - 0.5) * (config2.neuromodulators.acetylcholineLearningBoost - 1);
...
function processInteractionOutcome(params, config2) {
  if (!active8) {
    return {
      reward: 0,
      predictionError: 0,
  ...
  return active8.processOutcome(params, config2);
}
...
function createNeuralPathways(workspaceDir, config2, logger2) {
  const storageDir2 = join8(workspaceDir, ".brainagent", "synapses");
  ...
  function applyHebbianLearning(reward) {
    ...
    const { learningRate, decayRate, minWeight, maxWeight } = config2.synapticPlasticity;
    const now = Date.now();
    for (const pathwayName of activatedPathways) {
      ...
      if (weight.recentActivations.length > config2.synapticPlasticity.activationHistorySize) {
        weight.recentActivations = weight.recentActivations.slice(
          -config2.synapticPlasticity.activationHistorySize
        );
      }
  ...
  loadSynapticState();
  synapticState.learningRate = config2.synapticPlasticity.learningRate;
  synapticState.decayRate = config2.synapticPlasticity.decayRate;
  const unsubs = [];
  unsubs.push(
...
var active9;
function initNeuralPathways(workspaceDir, config2, logger2) {
  active9?.dispose();
  active9 = createNeuralPathways(workspaceDir, config2, logger2);
}
...
function createWorkingMemory(workspaceDir, opts) {
  const storageDir2 = workspaceDir ? join9(workspaceDir, ".brainagent", "working-memory") : "";
  const maxEntries = opts.maxEntries;
  const summaryMaxLength = opts.summaryMaxLength;
  let entries = [];
  if (storageDir2 && !existsSync8(storageDir2)) {
    mkdirSync8(storageDir2, { recursive: true });
  }
  function loadState2() {
    if (!storageDir2) return;
    try {
      const path = join9(storageDir2, "state.json");
      if (existsSync8(path)) {
  ...
  function persistState2() {
    if (!storageDir2) return;
    try {
      writeFileSync8(join9(storageDir2, "state.json"), JSON.stringify(entries, null, 2), "utf-8");
    } catch {
    }
  }
  loadState2();
...
function initWorkingMemoryStorage(workspaceDir, config2) {
  active10 = createWorkingMemory(workspaceDir, {
    maxEntries: config2.workingMemory.maxEntries,
    summaryMaxLength: config2.workingMemory.summaryMaxLength
  });
}
...
async function extractFactsWithAI(text, config2, logger2) {
  const content = await callLLM(EXTRACTION_PROMPT, text, config2, logger2, 500);
  if (!content) return [];
  const facts = parseFactsFromResponse(content);
  if (facts.length > 0) {
    logger2?.info(`BrainAgent AI Extractor: found ${facts.length} facts`);
  }
  return facts;
}
...
function isAIProviderAvailable2(config2) {
  return isAIProviderAvailable(config2);
}
...
async function validateAsync(response, originalInput, config2, classification, assessment, userModel, logger2) {
...
\u041E\u0442\u0432\u0435\u0442: ${response}`;
    const aiResponse = await callLLM(QUALITY_PROMPT, userText, config2, logger2, 300);
    if (aiResponse) {
    ...
    logger2?.info(`BrainAgent Cerebellum: AI validation error \u2014 ${String(err)}`);
  }
  return heuristicResult;
}
...
async function extractProcedureAsync(text, config2, classification, logger2) {
  if (isAIProviderAvailable(config2)) {
    try {
      const aiResponse = await callLLM(PROCEDURE_PROMPT, text, config2, logger2, 300);
      if (aiResponse) {
        const parsed = parseProcedureResponse(aiResponse, classification);
        if (parsed) return parsed;
      }
    } catch (err) {
      logger2?.info(`BrainAgent Procedural: AI extraction error \u2014 ${String(err)}`);
    }
  }
...
function createStructuralPlasticity(workspaceDir, config2, log) {
  const storageDir2 = workspaceDir ? join10(workspaceDir, ".brainagent", "structural") : "";
  let currentConfig = config2 ?? null;
  const logger2 = log;
...
function initStructuralPlasticity(workspaceDir, config2, log) {
  active11 = createStructuralPlasticity(workspaceDir, config2, log);
}
...
function createLearningCoordinator(workspaceDir, config2) {
  const storageDir2 = join11(workspaceDir, ".brainagent", "learning");
  if (!existsSync10(storageDir2)) {
    mkdirSync10(storageDir2, { recursive: true });
  }
  ...
  function loadState2() {
    try {
      const path = join11(storageDir2, "coordinator.json");
  ...
  function persistState2() {
    try {
      ...
      writeFileSync10(
        join11(storageDir2, "coordinator.json"),
        JSON.stringify(
          {
  ...
  function processRewardSignal(signal) {
    ...
    if (cycleCount >= config2.learning.minCyclesForInsights && cycleCount % 5 === 0) {
      generateInsights(config2);
    }
    if (cycleCount % 10 === 0) {
...
var active12;
function initLearningCoordinator(workspaceDir, config2) {
  active12?.dispose();
  active12 = createLearningCoordinator(workspaceDir, config2);
}
...
function createEmergentModules(workspaceDir, config2, log) {
  const storageDir2 = workspaceDir ? join12(workspaceDir, ".brainagent", "emergent") : "";
  let currentConfig = config2 ?? null;
  const logger2 = log;
  let state = createDefaultState3();
  let patternHistory = [];
  ...
  function saveState() {
    if (!storageDir2) return;
    try {
      writeFileSync11(join12(storageDir2, "state.json"), JSON.stringify(state, null, 2), "utf-8");
    } catch {
    }
  }
  ...
  if (storageDir2) {
    ...
    state.minOccurrences = config2?.emergentModules.minOccurrences ?? state.minOccurrences;
    state.minReward = config2?.emergentModules.minRewardForEstablishment ?? state.minReward;
    loadState2();
    logger2?.info(`EmergentModules: initialized with ${state.modules.length} discovered patterns`);
  }
  return {
...
function initEmergentModules(workspaceDir, config2, log) {
  active13 = createEmergentModules(workspaceDir, config2, log);
}
...
function createMetabolicBudget(workspaceDir, config2, log) {
  const storageDir2 = join13(workspaceDir, ".brainagent", "metabolic");
  if (!existsSync12(storageDir2)) {
    mkdirSync12(storageDir2, { recursive: true });
  }
  const state = createDefaultState();
  state.totalBudget = config2.metabolicBudget.totalBudget;
  state.regenRate = config2.metabolicBudget.regenRate;
  const periodStats = {};
  ...
  function loadState2() {
    try {
      const path = join13(storageDir2, "state.json");
  ...
  function saveState() {
    try {
      writeFileSync12(join13(storageDir2, "state.json"), JSON.stringify(state, null, 2), "utf-8");
    } catch {
    }
  }
  loadState2();
  ...
  function consumeEnergy2(module, amount) {
    ...
    if (moduleEnergy.energy < config2.metabolicBudget.lowPowerThreshold) {
  ...
  function endCycle3() {
    ...
    for (const module of ALL_MODULES2) {
      ...
      if (moduleEnergy.lowPowerMode && moduleEnergy.energy > config2.metabolicBudget.lowPowerThreshold * 2) {
    ...
    if (state.cyclesSinceRebalance >= config2.metabolicBudget.rebalanceInterval) {
      rebalanceEnergy();
...
var active14;
function initMetabolicBudget(workspaceDir, config2, log) {
  active14?.dispose();
  active14 = createMetabolicBudget(workspaceDir, config2, log);
}
...
function createAgentIdentity(workspaceDir, config2) {
  const storageDir2 = workspaceDir ? join14(workspaceDir, ".brainagent", "identity") : "";
  const icfg = config2?.agentIdentity;
  ...
  function loadState2() {
    if (!storageDir2) return;
    try {
      const path = join14(storageDir2, "state.json");
  ...
  if (storageDir2) {
    if (!existsSync13(storageDir2)) {
      mkdirSync13(storageDir2, { recursive: true });
    }
    loadState2();
  }
  return {
...
function initAgentIdentity(workspaceDir, config2) {
  active15 = createAgentIdentity(workspaceDir, config2);
}
...
function createEmotionalMemory(workspaceDir, config2) {
  const storageDir2 = workspaceDir ? join15(workspaceDir, ".brainagent", "emotional-memory") : "";
  if (storageDir2 && !existsSync14(storageDir2)) {
    mkdirSync14(storageDir2, { recursive: true });
  }
  let state = { flashbulbCount: 0, emotionMatchBoosts: 0 };
  let qualiaHistory = [];
  const maxQualiaHistory = config2.emotionalMemory.maxQualiaHistory ?? 10;
  const metaphorCache = /* @__PURE__ */ new Map();
  let lastLLMCallTimestamp = 0;
  function loadState2() {
    if (!storageDir2) return;
    try {
      const path = join15(storageDir2, "state.json");
  ...
  async function generateQualiaAsync2(emotion, intensity, domain, neuromodulators, llmConfig, logger2) {
...
${neuroDesc}`;
    try {
      ...
      const response = await callLLM(QUALIA_METAPHOR_PROMPT, userMessage, llmConfig, logger2, 200);
      if (!response) {
        logger2?.info("BrainAgent Qualia: LLM returned null, falling back to template");
        return generateQualia(emotion, intensity, domain, neuromodulators);
      }
      const parsed = parseLLMQualiaResponse(response);
      if (!parsed) {
        logger2?.info("BrainAgent Qualia: failed to parse LLM response, falling back to template");
        return generateQualia(emotion, intensity, domain, neuromodulators);
      }
      const qualia = {
        timestamp: now,
      ...
      persistState2();
      logger2?.info(`BrainAgent Qualia: LLM-generated metaphor for ${emotion}/${domain}`);
      return qualia;
    } catch (err) {
      logger2?.info(`BrainAgent Qualia: AI metaphor generation failed \u2014 ${String(err)}`);
      return generateQualia(emotion, intensity, domain, neuromodulators);
    }
  }
  function dispose() {
...
function initEmotionalMemory(workspaceDir, config2) {
  active16?.dispose();
  active16 = createEmotionalMemory(workspaceDir, config2);
}
function tagEmotionalContext(emotion, intensity) {
  return current12().tagEmotionalContext(emotion, intensity);
}
async function generateQualiaAsync(emotion, intensity, domain, neuromodulators, config2, logger2) {
  return current12().generateQualiaAsync(emotion, intensity, domain, neuromodulators, config2, logger2);
}
...
function createQualiaSimulator(workspaceDir, config2) {
  const storageDir2 = workspaceDir ? join16(workspaceDir, ".brainagent", "qualia-simulator") : "";
  let currentQualia = null;
  const qualiaLog = [];
  const maxLog = 20;
  const minIntensityForInjection = config2?.qualiaSimulator.minIntensityForInjection ?? 0.5;
  function loadState2() {
    if (!storageDir2) return;
    try {
      const path = join16(storageDir2, "state.json");
  ...
  if (storageDir2) {
    if (!existsSync15(storageDir2)) {
      mkdirSync15(storageDir2, { recursive: true });
    }
    loadState2();
  }
...
function initQualiaSimulator(workspaceDir, config2) {
  active17 = createQualiaSimulator(workspaceDir, config2);
}
...
function createDMN(workspaceDir, config2, log) {
  const storageDir2 = workspaceDir ? join17(workspaceDir, ".brainagent", "dmn") : "";
  const insights = [];
  ...
  let maxBackgroundThoughts = config2?.dmn?.maxBackgroundThoughts ?? 20;
  function loadState2() {
    if (!storageDir2) return;
    try {
      const path = join17(storageDir2, "state.json");
...
${lines}
  ...
  if (storageDir2) {
    if (!existsSync16(storageDir2)) {
      mkdirSync16(storageDir2, { recursive: true });
    }
    currentConfig = config2 ?? null;
    loadState2();
  }
  return {
    init,
...
function initDMN(workspaceDir, config2, log) {
  active18?.stop();
  active18 = createDMN(workspaceDir, config2, log);
}
function runAssociationFinding(config2) {
  return current14().runAssociationFinding(config2);
}
function getDMNStats() {
  return current14().getStats();
}
function generateBackgroundThoughts(config2, unresolvedQuestions, recentEmotions, knowledgeGaps) {
...
function createTemporalBinding(workspaceDir, opts) {
  const storageDir2 = workspaceDir ? join18(workspaceDir, ".brainagent", "temporal-binding") : "";
  const maxMoments = opts.maxMoments;
  let moments = [];
  let idCounter = 0;
  if (storageDir2 && !existsSync17(storageDir2)) {
    mkdirSync17(storageDir2, { recursive: true });
  }
  function loadState2() {
    if (!storageDir2) return;
    try {
      const path = join18(storageDir2, "state.json");
  ...
  function persistState2() {
    if (!storageDir2) return;
    try {
      writeFileSync17(join18(storageDir2, "state.json"), JSON.stringify(moments, null, 2), "utf-8");
    } catch {
    }
  }
  loadState2();
...
function initTemporalBinding(workspaceDir, config2) {
  active19 = createTemporalBinding(workspaceDir, { maxMoments: config2.temporalBinding.maxMoments });
}
...
function createSessionBridge(workspaceDir, config2, _logger) {
  const storageDir2 = workspaceDir ? join19(workspaceDir, ".brainagent", "sessions") : "";
  if (storageDir2 && !existsSync18(storageDir2)) {
    mkdirSync18(storageDir2, { recursive: true });
  }
  const gapThresholdMs = config2.sessionBridge.gapThresholdMs;
  const maxSummaryTopics = config2.sessionBridge.maxSummaryTopics;
  ...
  function loadState2() {
    if (!storageDir2) return;
    try {
      const currentPath = join19(storageDir2, "current.json");
    ...
    try {
      const lastPath = join19(storageDir2, "last.json");
  ...
  function persistCurrent() {
    ...
    try {
      writeFileSync18(
        join19(storageDir2, "current.json"),
  ...
  function persistLast() {
    if (!storageDir2) return;
    try {
      writeFileSync18(join19(storageDir2, "last.json"), JSON.stringify(lastSession, null, 2), "utf-8");
    } catch {
    }
  }
...
function initSessionBridge(workspaceDir, config2, logger2) {
  active20?.dispose();
  active20 = createSessionBridge(workspaceDir, config2, logger2);
}
...
function createCuriosityDrive(workspaceDir, config2) {
  const storageDir2 = workspaceDir ? join20(workspaceDir, ".brainagent", "curiosity") : "";
  const gaps = [];
  let totalDetected = 0;
  let questionsGenerated = 0;
  let gapsFilled = 0;
  let maxGaps = config2?.curiosity?.maxGaps ?? 15;
  let minGapConfidence = config2?.curiosity?.minGapConfidence ?? 0.3;
  let askProbability = config2?.curiosity?.askProbability ?? 0.1;
  let idCounter = 0;
  function loadState2() {
    if (!storageDir2) return;
    try {
      const path = join20(storageDir2, "state.json");
...
${question}`;
  ...
  if (storageDir2) {
    if (!existsSync19(storageDir2)) {
      mkdirSync19(storageDir2, { recursive: true });
    }
    if (config2) {
      maxGaps = config2.curiosity.maxGaps;
      minGapConfidence = config2.curiosity.minGapConfidence;
      askProbability = config2.curiosity.askProbability;
    }
    loadState2();
  }
  return {
...
function initCuriosityDrive(workspaceDir, config2) {
  active21?.stop();
  active21 = createCuriosityDrive(workspaceDir, config2);
}
...
function createGoalStack(workspaceDir, config2) {
  const storageDir2 = workspaceDir ? join21(workspaceDir, ".brainagent", "goals") : "";
  const gcfg = config2?.goalStack;
  ...
  function loadState2() {
    if (!storageDir2) return;
    try {
      const path = join21(storageDir2, "state.json");
  ...
  function checkGoalTriggers2(input, currentEmotion, currentDomain) {
    ...
    if (triggered.length > 0) persistState2();
    return triggered;
  }
  function expireGoals2() {
    ...
    if (goals.length > 60) {
      const active40 = goals.filter((g) => g.status === "pending" || g.status === "triggered");
      const inactive = goals.filter((g) => g.status === "completed" || g.status === "expired").slice(-30);
      goals = [...active40, ...inactive];
      changed = true;
    }
    if (changed) persistState2();
  }
  ...
  function weakenDesiresAfterFire2(consecutiveFires) {
    ...
    if (desires.length !== before || changed) persistState2();
  }
  ...
  function escalateStaleDesires() {
    ...
    if (escalated.length > 0) persistState2();
    return escalated;
  }
  async function extractGoalsFromConversation3(userMessage, config3, logger2) {
    if (!isAIProviderAvailable(config3)) {
      logger2?.info("BrainAgent GoalStack: no AI provider available, skipping goal extraction");
      return [];
    }
    logger2?.info("BrainAgent GoalStack: extracting goals from conversation...");
    const response = await callLLM(GOAL_EXTRACTION_PROMPT, userMessage, config3, logger2, 300);
    if (!response) {
      logger2?.info("BrainAgent GoalStack: LLM returned null/empty response");
      return [];
    }
    logger2?.info(`BrainAgent GoalStack: LLM response received (${response.length} chars)`);
    let parsed;
    try {
      let jsonStr = response.trim();
      const jsonMatch = jsonStr.match(/\[[\s\S]*\]/);
      ...
      logger2?.info("BrainAgent GoalStack: failed to parse LLM goal extraction response");
      return [];
    }
    const createdGoals = [];
  ...
  if (storageDir2) {
    if (!existsSync20(storageDir2)) {
      mkdirSync20(storageDir2, { recursive: true });
    }
    loadState2();
  }
  return {
...
function initGoalStack(workspaceDir, config2) {
  active22 = createGoalStack(workspaceDir, config2);
}
...
async function extractGoalsFromConversation(userMessage, config2, logger2) {
  return current18().extractGoalsFromConversation(userMessage, config2, logger2);
}
...
function createCircadianRhythm(workspaceDir, config2, log) {
  const storageDir2 = join22(workspaceDir, ".brainagent", "circadian");
  if (!existsSync21(storageDir2)) {
    mkdirSync21(storageDir2, { recursive: true });
  }
  const maxSleepConsolidations = config2.circadian.maxSleepConsolidations ?? 5;
  ...
  function loadState2() {
    try {
      const path = join22(storageDir2, "state.json");
  ...
  function persistState2() {
    try {
      writeFileSync21(join22(storageDir2, "state.json"), JSON.stringify(state, null, 2), "utf-8");
    } catch {
    }
  }
  function evaluatePhase() {
    if (!config2.circadian.enabled) return;
    const now = Date.now();
    const idleTime = now - lastActivityTime;
    state.idleTime = idleTime;
    const activityWindow = config2.circadian.activityWindowMs;
    state.activityLevel = Math.max(0, 1 - idleTime / activityWindow);
    const phaseDuration = now - state.phaseStartedAt;
    const { idleThresholdMs, minWakeDurationMs, minSleepDurationMs, transitionDurationMs } = config2.circadian;
    switch (state.phase) {
      case "wake":
      ...
      case "sleep":
        if (consolidationCallback && phaseDuration > 3e4 && state.sleepConsolidations < maxSleepConsolidations) {
          const consolidationInterval = config2.circadian.sleepConsolidationIntervalMs ?? 6e4;
  ...
  loadState2();
  updateModulationFromPhase();
  const evalInterval = config2.circadian.evaluationIntervalMs ?? 3e4;
  const evaluationTimer = setInterval(() => evaluatePhase(), evalInterval);
  log?.info(
    `Circadian: initialized in ${state.phase} phase (idle threshold: ${config2.circadian.idleThresholdMs / 1e3}s)`
  );
  function stopTimer() {
  ...
  return {
    recordActivity: () => {
      ...
      if (state.phase === "sleep" || state.phase === "transition-to-sleep") {
        ...
        const minSleep = config2.circadian.minSleepDurationMs ?? 6e4;
...
function initCircadianRhythm(workspaceDir, config2, log) {
  active23?.dispose();
  active23 = createCircadianRhythm(workspaceDir, config2, log);
...
var DriveEngine = class {
  constructor(spec, config2, workspaceDir, circadianEnabled2, deps2, logger2) {
    this.spec = spec;
    this.config = config2;
    this.circadianEnabled = circadianEnabled2;
    this.deps = deps2;
    this.logger = logger2;
    const storageDir2 = join23(workspaceDir, ".brainagent", spec.id);
    if (!existsSync22(storageDir2)) {
      mkdirSync22(storageDir2, { recursive: true });
    }
    this.stateFile = join23(storageDir2, "state.json");
    cancelPersist(this.stateFile);
    this.loadState();
    this.wireCoreListeners();
    logger2.info(
      `BrainAgent ${spec.logName}: initialized (satiation=${this.satiation.toFixed(2)}, decay=${config2.baseDecayRate}/${config2.decayIntervalMs}ms, domains=${config2.rewardDomains.join(",")})`
    );
  }
  spec;
  config;
...
function createMasteryDrive(workspaceDir, cfg, log, injectedDeps) {
  const storageDir2 = workspaceDir ? join24(workspaceDir, ".brainagent", "mastery-drive") : "";
  const config2 = cfg?.masteryDrive;
  const circadianEnabled2 = cfg?.circadian?.enabled ?? false;
  const logger2 = log;
  const deps2 = injectedDeps;
  ...
  function getOrCreateDomain(domain) {
    ...
    if (!entry) {
      entry = {
        satiation: config2?.initialSatiation ?? 0.5,
        lastActivityTime: Date.now(),
        totalRewards: 0
      };
  ...
  function pruneDomainsIfNeeded() {
    if (!config2) return;
    while (domainSatiations.size > config2.maxTrackedDomains) {
      let maxSatiation = -1;
      let maxDomain = "";
  ...
  function boostDomain(domain, amount) {
    ...
    const boost = Math.min(config2?.maxSatiationBoost ?? 0.6, Math.max(0, amount));
  ...
  function getAggregateSatiation() {
    if (domainSatiations.size === 0) return config2?.initialSatiation ?? 0.5;
    let min = Infinity;
  ...
  function evaluateDecay() {
    if (!config2) return;
    const now = Date.now();
    const elapsed = (now - lastDecayEvaluationTime) / config2.decayIntervalMs;
    ...
    const serotoninMod = 0.7 + neuroState.serotonin * 0.6;
    for (const [, mastery] of domainSatiations) {
      let effectiveRate = config2.baseDecayRate * adaptiveDecayModifier * circadianMod / serotoninMod;
      const inactiveMs = now - mastery.lastActivityTime;
      if (inactiveMs > 10 * 60 * 1e3) {
        effectiveRate *= config2.inactiveDomainDecayMultiplier;
      }
      const decayFactor = Math.pow(1 - effectiveRate, elapsed);
  ...
  function computeNeedLevel() {
    ...
    if (aggregate < config2.needThresholds.urgent) return "urgent";
    if (aggregate < config2.needThresholds.strong) return "strong";
    if (aggregate < config2.needThresholds.moderate) return "moderate";
    if (aggregate < config2.needThresholds.mild) return "mild";
    return "none";
  }
  ...
  function emitNeedSignals() {
    ...
    if (needLevelRank(currentNeedLevel) >= needLevelRank("moderate") && now - lastDesireUpdateTime > config2.desireUpdateIntervalMs) {
      lastDesireUpdateTime = now;
      updateMasteryDesire(weakest?.domain);
    }
    if (needLevelRank(currentNeedLevel) >= needLevelRank("strong") && now - lastDMNBiasTime > config2.dmnBiasIntervalMs) {
      lastDMNBiasTime = now;
  ...
  function updateMasteryDesire(weakestDomain) {
    ...
    const existing = deps2.getDesires().find((d) => d.type === "mastery" && d.source === "mastery-drive");
    if (existing) {
      if (existing.strength < targetStrength) {
        existing.strength = targetStrength;
      }
      ...
      deps2.addDesire("mastery", description, targetStrength, "mastery-drive");
    }
    logger2?.info(
  ...
  function biasDMNToMasteryThoughts(weakestDomain) {
    ...
    const topics = [];
    if (weakestDomain) {
      const skillFacts = deps2.getFactsByCategory("skill", 2);
    ...
    deps2.generateMasteryThought(topics);
    logger2?.info(`BrainAgent MasteryDrive: biased DMN toward ${topics.length} mastery topic(s)`);
  }
  function onMasteryReward(signal) {
    ...
    if (signal.predictionError > 0) {
      const boost = Math.min(
        ...
        Math.max(0, signal.predictionError * config2.improvementRewardMultiplier)
      );
      const entry = getOrCreateDomain(domain);
      entry.satiation = Math.min(1, entry.satiation + boost);
      entry.lastActivityTime = Date.now();
  ...
  function wireEventListeners() {
    ...
    const unsubPredError = bus.on("dopamine:prediction-error", (data) => {
      ...
      if (!domain) return;
      if (data.error > 0) {
        const boost = Math.min(config2.maxSatiationBoost, data.error * 0.15);
    ...
    unsubscribers2.push(unsubPredError);
    ...
    unsubscribers2.push(unsubCapability);
    ...
    unsubscribers2.push(unsubAmygdala);
  }
  function getStats2() {
    evaluateDecay();
    const aggregate = getAggregateSatiation();
  ...
  function loadState2() {
    if (!storageDir2) return;
    try {
      const filePath = join24(storageDir2, "state.json");
        ...
        if (raw.domainSatiations) {
          ...
          for (const [domain, mastery] of Object.entries(raw.domainSatiations)) {
            domainSatiations.set(domain, {
              satiation: mastery.satiation ?? config2?.initialSatiation ?? 0.5,
  ...
  function persistState2() {
    if (!storageDir2) return;
    schedulePersist(join24(storageDir2, "state.json"), () => {
      const domainMap = {};
      for (const [domain, mastery] of domainSatiations) {
  ...
  function stop() {
    ...
    if (storageDir2) {
      persistState2();
      flushPersist(join24(storageDir2, "state.json"));
    }
    logger2?.info("BrainAgent MasteryDrive: stopped.");
  }
  if (storageDir2) {
    if (!existsSync23(storageDir2)) {
      mkdirSync23(storageDir2, { recursive: true });
    }
    cancelPersist(join24(storageDir2, "state.json"));
    loadState2();
  }
  wireEventListeners();
  if (cfg && config2) {
    logger2?.info(
      `BrainAgent MasteryDrive: initialized (domains=${domainSatiations.size}, decay=${config2.baseDecayRate}/${config2.decayIntervalMs}ms, maxDomains=${config2.maxTrackedDomains})`
    );
  }
  return {
    getStats: getStats2,
...
function collectActiveDrives(drives, threshold) {
  ...
  if (drives.social) {
    ...
    if (s.need >= threshold) active40.push({ name: "social", need: s.need, needLevel: s.needLevel });
  }
  if (drives.cognitive) {
    const c = drives.cognitive();
    if (c.need >= threshold) active40.push({ name: "cognitive", need: c.need, needLevel: c.needLevel });
  }
  if (drives.creative) {
    const c = drives.creative();
    if (c.need >= threshold) active40.push({ name: "creative", need: c.need, needLevel: c.needLevel });
  }
  if (drives.mastery) {
    const m = drives.mastery();
    if (m.need >= threshold) active40.push({ name: "mastery", need: m.need, needLevel: m.needLevel });
  }
  return active40;
}
function synthesizeAutonomousCycleState(brainConfig, drives, cycle) {
  const active40 = collectActiveDrives(drives, 0.5);
  if (active40.length === 0) return;
  const ranked = active40.map((d) => {
...
function createAutonomousDeliverer(deps2) {
  const { state, brainConfig, minGapMs, logger: logger2 } = deps2;
  return function enqueueAutonomousIntent(text) {
    const trimmed = text.trim();
    if (!trimmed) return;
    if (state.previousCycleWasAutonomous) {
      logger2.info("BrainAgent Autonomy: intent suppressed \u2014 previous cycle was autonomous");
      return;
    }
    if (Date.now() - state.lastAutonomousDeliveryAt < minGapMs) {
      logger2.info("BrainAgent Autonomy: intent suppressed \u2014 minimum gap not elapsed");
      return;
    }
    if (brainConfig.modules.proactiveFeedback) {
      const intentDomain = deps2.classifyDomain(trimmed).domain;
      if (deps2.isDomainSuppressed(intentDomain)) {
        logger2.info(
    ...
    const agent = deps2.pickAgent();
    if (!agent) {
      logger2.warn("BrainAgent Autonomy: no live agent \u2014 autonomous intent dropped");
      return;
    }
    const rejectionHints = brainConfig.modules.proactiveFeedback ? deps2.getSuppressedDomainHints() : [];
    ...
    deps2.deliver(agent, framed);
  };
}
function createAutonomousIntentResolver(deps2) {
  const { state, brainConfig, drives } = deps2;
  return () => {
    if (brainConfig.modules.goalStack && deps2.goalStack) {
      const stats = deps2.goalStack.getGoalStackStats();
      if (stats.pending > 0) {
        const idleMs = brainConfig.circadian.enabled ? deps2.circadian?.getCircadianState().idleTime : void 0;
        const triggered = deps2.goalStack.checkAutonomousGoals(idleMs);
        if (triggered.length > 0) {
          const goalCtx = deps2.goalStack.buildGoalContext(triggered);
          if (goalCtx) {
    ...
    if (brainConfig.modules.dmn && deps2.dmn) {
      const unused = deps2.dmn.getRecentUnusedInsights(30 * 60 * 1e3);
    ...
    if (brainConfig.modules.goalStack && deps2.goalStack) {
      const strong = deps2.goalStack.getDesires().filter((d) => d.strength >= 0.9);
      if (strong.length > 0) {
    ...
    const active40 = collectActiveDrives(drives, 0.8);
    if (active40.length > 0) {
      const strongest = active40.sort((a, b) => b.need - a.need)[0];
...
function createCycleEngine(deps2) {
  const { config: config2, brainConfig, getHostConfig, logger: logger2, markActivation, state } = deps2;
  ...
  async function endCycle3(key) {
    ...
    const aiAvailable = config2.modules.aiEnrichment && isAIProviderAvailable2(getHostConfig());
    if (brainConfig.modules.cerebellum && cycle.responseText.trim()) {
      ...
      if (result.issues.length > 0) {
        logger2.warn(`BrainAgent Cerebellum: quality issues \u2014 ${result.issues.join("; ")}`);
    ...
    if (config2.modules.semanticExtraction && semanticSource.length > 15 && isFactWorthy(semanticSource, cycle.classification)) {
      let factsStored = false;
      if (aiAvailable) {
        try {
          const aiFacts = await extractFactsWithAI(semanticSource, getHostConfig(), logger2);
          if (aiFacts.length > 0) {
          ...
          logger2.info(`BrainAgent Semantic: AI extraction failed, falling back \u2014 ${String(err)}`);
        }
      }
      if (!factsStored) {
      ...
      if (factsStored && brainConfig.modules.curiosityDrive) {
        ...
        for (const gap of getOpenGaps()) {
          if (inputLower.includes(gap.topic.toLowerCase())) {
            ...
            logger2.info(`BrainAgent Curiosity: gap filled for "${gap.topic}" via new fact storage`);
          }
        }
      }
    }
    if (config2.modules.proceduralExtraction && input.length > 10 && isProcedural(input, cycle.classification)) {
      const procedure = await extractProcedureAsync(input, getHostConfig(), cycle.classification, logger2);
      if (procedure && procedure.confidence > 0.5) {
        storeWorkflow(procedure.description, procedure.triggerPattern, procedure.steps);
        logger2.info(`BrainAgent Procedural: stored workflow "${procedure.description}"`);
      }
    }
    ...
    if (brainConfig.modules.neuromodulatorSystem && input.length > 5) {
      ...
      if (config2.modules.structuralPlasticity) {
        endCycle(reward);
      }
    ...
    if (brainConfig.modules.dmn) {
      ...
      if (wakeInteractionCount >= brainConfig.dmn.wakeThoughtInterval) {
        ...
        void runAssociationFinding(brainConfig).catch((err) => {
          logger2.info(`BrainAgent DMN: association finding error: ${err}`);
        });
      }
    }
    if (brainConfig.modules.goalStack && input.length > 10) {
      ...
      if (goalExtractionCounter >= brainConfig.goalStack.extractionInterval) {
        ...
        void extractGoalsFromConversation(input, getHostConfig(), logger2).catch((err) => {
          logger2.info(`BrainAgent GoalStack: extraction error: ${err}`);
        });
      }
    }
    if (brainConfig.modules.goalStack) {
...
function createStrategyBandit(workspaceDir, config2) {
  const storageDir2 = join25(workspaceDir, ".brainagent", "bandit");
  if (!existsSync24(storageDir2)) {
    mkdirSync24(storageDir2, { recursive: true });
  }
  const storageFile = join25(storageDir2, "state.json");
  const explorationConstant = config2.learningLoop.strategyBandit.explorationConstant;
  const attributionWindowMs = config2.learningLoop.strategyBandit.attributionWindowMs;
  let state = {};
  let lastChoices = {};
  function loadState2() {
...
var active28;
function initStrategyBandit(workspaceDir, config2) {
  active28?.dispose();
  active28 = createStrategyBandit(workspaceDir, config2);
}
...
function initVitalImpulse(workspaceDir, cfg, log, injectedDeps) {
  storageDir = join26(workspaceDir, ".brainagent", "vital-impulse");
...
function createDriveArbiter(workspaceDir, cfg, getters, log) {
  const config2 = cfg.driveArbiter;
  const statGetters = getters;
  const logger2 = log;
  const unsubscribers2 = [];
  const storageDir2 = join27(workspaceDir, ".brainagent");
  if (!existsSync26(storageDir2)) {
    mkdirSync26(storageDir2, { recursive: true });
  }
  let driveWeights = {
    social: 1,
    cognitive: 1,
  ...
  function loadState2() {
    try {
      const path = join27(storageDir2, "drive-arbiter.json");
  ...
  function persistState2() {
    if (!storageDir2) return;
    schedulePersist(join27(storageDir2, "drive-arbiter.json"), () => {
      const data = {
        driveWeights,
        lastSelectedDrive,
        lastSelectionTime,
  ...
  cancelPersist(join27(storageDir2, "drive-arbiter.json"));
  loadState2();
  function arbitrate() {
  ...
  function arbitrateInner() {
    ...
    const drives = [];
    if (social && social.need >= config2.minDriveNeed) {
      drives.push({ id: "social", need: social.need });
    }
    if (cognitive && cognitive.need >= config2.minDriveNeed) {
      drives.push({ id: "cognitive", need: cognitive.need });
    }
    if (creative && creative.need >= config2.minDriveNeed) {
      drives.push({ id: "creative", need: creative.need });
    }
    if (mastery && mastery.need >= config2.minDriveNeed) {
    ...
    const scores = drives.map((d) => {
      ...
      if (d.id === lastSelectedDrive && lastSelectionTime > 0) {
        ...
        const decayFactor = Math.pow(config2.recencyDecay, timeSinceLast / (5 * 60 * 1e3));
    ...
    let winner = scores[0];
    if (scores.length > 1 && Math.random() < config2.explorationRate) {
    ...
    if (conflictLog.length > (config2.maxConflictLog ?? 50)) {
      conflictLog = conflictLog.slice(-config2.maxConflictLog);
    }
  ...
  unsubscribers2.push(unsubReward);
  logger2?.info("BrainAgent DriveArbiter: initialized");
  function getStats2() {
    return {
  ...
  function stop() {
    ...
    unsubscribers2.length = 0;
    persistState2();
    flushPersist(join27(storageDir2, "drive-arbiter.json"));
    logger2?.info("BrainAgent DriveArbiter: stopped.");
  }
  return {
    getStats: getStats2,
...
function initDriveArbiter(workspaceDir, cfg, getters, log) {
  active29?.stop();
  active29 = createDriveArbiter(workspaceDir, cfg, getters, log);
}
...
function buildArbiterContext() {
  return active29?.buildContext() ?? null;
}
...
function initInteroception(getters, log) {
  active30?.dispose();
  active30 = createInteroception(getters, log);
}
...
function createTemporalAwareness(workspaceDir, cfg, log) {
  const config2 = cfg.temporalAwareness;
  let timestamps = [];
  let typicalGapMs = 0;
  let totalInteractions = 0;
  const storageDir2 = join28(workspaceDir, ".brainagent");
  if (!existsSync27(storageDir2)) {
    mkdirSync27(storageDir2, { recursive: true });
  }
  try {
    const path = join28(storageDir2, "temporal-awareness.json");
  ...
  function persistState2() {
    try {
      ...
      writeFileSync22(
        join28(storageDir2, "temporal-awareness.json"),
  ...
  function computeDensity(now) {
    ...
    const windowStart = now - config2.densityWindowMs;
  ...
  function recordInteraction2() {
    ...
    timestamps.push(now);
    if (timestamps.length > config2.gapHistorySize) {
      timestamps = timestamps.slice(-config2.gapHistorySize);
    }
    totalInteractions++;
    ...
    if (lastTime > 0 && gapMs > 0) {
      if (typicalGapMs === 0) {
        ...
        typicalGapMs = typicalGapMs * (1 - config2.gapEmaAlpha) + gapMs * config2.gapEmaAlpha;
      }
      const temporalSurprise = typicalGapMs > 0 ? gapMs / typicalGapMs : 1;
      if (temporalSurprise >= config2.longAbsenceMultiplier && gapMs > 60 * 1e3) {
    ...
    if (density >= config2.highDensityThreshold) {
  ...
  function buildContext() {
    ...
    const density = computeDensity(now);
    const lines = [];
    if (temporalSurprise >= config2.longAbsenceMultiplier && currentGapMs > 60 * 1e3) {
      lines.push(
...
function initTemporalAwareness(workspaceDir, cfg, log) {
  active31?.dispose();
  active31 = createTemporalAwareness(workspaceDir, cfg, log);
}
...
function recordInteraction() {
  active31?.recordInteraction();
}
function getTemporalAwarenessStats() {
  return active31?.getStats() ?? {
    typicalGapMs: 0,
    lastInteractionTime: 0,
    currentGapMs: 0,
...
function createAttentionGate(workspaceDir) {
  const storageDir2 = workspaceDir ? join29(workspaceDir, ".brainagent", "attention") : "";
  let totalProcessed = 0;
  let totalDropped = 0;
  let totalRelevanceSum = 0;
  if (storageDir2 && !existsSync28(storageDir2)) {
    mkdirSync28(storageDir2, { recursive: true });
  }
  function loadState2() {
    if (!storageDir2) return;
    try {
      const path = join29(storageDir2, "state.json");
  ...
  loadState2();
  function filter(injections, currentInput, norepinephrineLevel, config2, maxTokenBudget) {
    ...
    const effectiveThreshold = config2.attentionGate.minRelevanceScore * (1 - norepinephrineLevel * 0.5);
    scored.sort((a, b) => b.score - a.score);
    let kept = [];
    let dropped = 0;
    for (const item of scored) {
      if (kept.length < config2.attentionGate.maxContextSections && item.score >= effectiveThreshold) {
        kept.push(item.text);
...
var active32;
function current20() {
  return active32 ?? (active32 = createAttentionGate(""));
}
function initAttentionGate(workspaceDir, _config) {
  active32 = createAttentionGate(workspaceDir);
}
function filterContextInjections(injections, currentInput, norepinephrineLevel, config2, maxTokenBudget) {
  return current20().filter(injections, currentInput, norepinephrineLevel, config2, maxTokenBudget);
}
function getAttentionStats() {
  return current20().getStats();
}
...
function decideProcessingPath(classification, config2) {
  const threshold = COMPLEXITY_ORDER2[config2.dualProcess.system2Threshold];
  ...
  const modelOverride = useSlow ? config2.dualProcess.slowModel : config2.dualProcess.fastModel;
  const result = {
...
function createAutonomousResearch(cfg, log, injectedDeps) {
  let config2 = cfg?.autonomousResearch;
  let deps2 = injectedDeps;
  let stats = {
    totalCycles: 0,
...
${knownContext}` : `Topic: ${topic}`;
    const result = await deps2.callLLM(systemPrompt, userText, deps2.gatewayConfig, deps2.logger, 300);
    if (!result) return null;
    try {
      const cleaned = result.replace(/```json?\s*/g, "").replace(/```/g, "").trim();
      const parsed = JSON.parse(cleaned);
      if (Array.isArray(parsed)) {
        return parsed.filter((q) => typeof q === "string" && q.length > 2).slice(0, config2.maxSearchQueries);
      }
    } catch {
      const lines = result.split("\n").map((l) => l.replace(/^[-*\d.)\s]+/, "").trim()).filter((l) => l.length > 3 && l.length < 200);
      if (lines.length > 0) return lines.slice(0, config2.maxSearchQueries);
    }
    return null;
  }
  ...
  async function searchBrave(queries) {
    ...
    if (!apiKey) {
      deps2.logger.info("BrainAgent AutonomousResearch: no Brave API key available");
      return null;
    }
    const allResults = [];
    for (const query of queries) {
      try {
        const params = new URLSearchParams({
          q: query,
          count: String(config2.maxPagesPerQuery)
        });
  ...
  async function searchTavily(queries) {
    ...
    if (!apiKey) {
      deps2.logger.info("BrainAgent AutonomousResearch: no Tavily API key available");
      return null;
    }
    const allResults = [];
    for (const query of queries) {
      try {
        const response = await fetch("https://api.tavily.com/search", {
          ...
          body: JSON.stringify({
            ...
            max_results: config2.maxPagesPerQuery
          }),
          signal: AbortSignal.timeout(1e4)
        });
        if (!response.ok) {
  ...
  async function searchPerplexity(queries) {
    ...
    if (!apiKey) {
      deps2.logger.info("BrainAgent AutonomousResearch: no Perplexity API key available");
      return null;
    }
    const combinedQuery = queries.join("; ");
    const baseUrl = resolvePerplexityBaseUrl(apiKey, deps2.gatewayConfig);
    const model = resolvePerplexityModel(deps2.gatewayConfig);
    try {
      ...
      deps2.logger.info(`BrainAgent AutonomousResearch: Perplexity error \u2014 ${String(err)}`);
      return null;
    }
  }
  async function searchGrok(queries) {
    ...
    if (!apiKey) {
      deps2.logger.info("BrainAgent AutonomousResearch: no xAI/Grok API key available");
      return null;
    }
    const combinedQuery = queries.join("; ");
    const model = resolveGrokModel(deps2.gatewayConfig);
    try {
      ...
      deps2.logger.info(`BrainAgent AutonomousResearch: Grok error \u2014 ${String(err)}`);
      return null;
    }
  }
  function resolveApiKey(provider) {
    if (!deps2) return null;
    const search = deps2.gatewayConfig.tools;
  ...
  async function fetchPages(results) {
    ...
    const pages = [];
    for (const result of results) {
      if (totalChars >= config2.maxTotalChars) break;
      try {
        ...
        const truncated = text.slice(0, config2.maxPageChars);
...
${truncated}`);
  ...
  async function extractAndStore(topic, content, queriesExecuted, pagesRead) {
    ...
    const truncatedContent = content.slice(0, config2.maxTotalChars);
    const userText = `Topic: ${topic}

Web content:
${truncatedContent}`;
    const result = await deps2.callLLM(
      systemPrompt,
      userText,
      deps2.gatewayConfig,
      deps2.logger,
      config2.extractMaxTokens
    );
    if (!result) {
      deps2.logger.info("BrainAgent AutonomousResearch: extraction LLM call failed");
      return { summary: "", factsStored: 0, queriesExecuted, pagesRead };
    }
    let facts = [];
    let summary = "";
    try {
      ...
      deps2.logger.info("BrainAgent AutonomousResearch: extraction JSON parse failed, using raw");
    }
    for (const fact of facts) {
      deps2.storeFact(fact.content, fact.category || "knowledge", [], ["autonomous-research"]);
    }
    stats.totalCycles++;
  ...
  async function executeResearch2(topic) {
    if (!config2?.enabled || !deps2) return null;
    const now = Date.now();
    if (now - stats.lastResearchTime < config2.cooldownMs) {
    ...
    const existingFacts = deps2.recallFacts(topic, 5);
...
${existingFacts.map((f) => `- ${f.content}`).join("\n")}` : "";
    ...
    if (!queries || queries.length === 0) {
      deps2.logger.info("BrainAgent AutonomousResearch: no queries planned, skipping");
      return null;
    }
    const provider = resolveSearchProvider(deps2.gatewayConfig);
    deps2.logger.info(`BrainAgent AutonomousResearch: using search provider "${provider}"`);
    const searchResponse = await searchWeb(queries, provider);
    if (!searchResponse) {
      deps2.logger.info("BrainAgent AutonomousResearch: search returned nothing, skipping");
      return null;
    }
    let content;
    let pagesRead = 0;
    if (searchResponse.type === "text") {
      content = searchResponse.content.slice(0, config2.maxTotalChars);
      if (searchResponse.citations.length > 0) {
        content += "\n\nSources:\n" + searchResponse.citations.map((c) => `- ${c}`).join("\n");
      }
    } else {
      if (searchResponse.results.length === 0) {
        deps2.logger.info("BrainAgent AutonomousResearch: no search results, skipping");
        return null;
      }
  ...
  function stop() {
    deps2?.logger.info("BrainAgent AutonomousResearch: stopped.");
  }
  if (cfg) {
...
var active33 = null;
function current21() {
  if (!active33) active33 = createAutonomousResearch();
  return active33;
}
function initAutonomousResearch(cfg, log, injectedDeps) {
  active33 = createAutonomousResearch(cfg, log, injectedDeps);
}
function stopAutonomousResearch() {
  current21().stop();
}
async function executeResearch(topic) {
  return current21().executeResearch(topic);
}
...
function createPreStepHandler(deps2) {
  const { config: config2, brainConfig, getHostConfig, logger: logger2, state, cycles, startCycle } = deps2;
  return async (payload, next) => {
    ...
    if (config2.modules.aiEnrichment && isAIProviderAvailable2(getHostConfig())) {
      const hc = getHostConfig();
      const tasks = [];
      if (brainConfig.modules.amygdala) {
        tasks.push(
          assessWithAI(input, hc, logger2).then((aiAssessment) => {
      ...
      if (brainConfig.modules.mirrorNeurons && cyc.assessment) {
        ...
        tasks.push(
          observeWithAI("default", input, snapshot, brainConfig, hc, logger2).then((model) => {
            cyc.userModel = model;
          }).catch(() => {
          })
        );
      ...
      if (brainConfig.modules.basalGanglia) {
        tasks.push(
          detectReinforcementWithAI(input, hc, logger2).then((aiSignal) => {
      ...
      ]).catch((err) => logger2.warn(String(err)));
    }
    ...
    const episodicLimit = Math.max(1, Math.round(config2.recall.episodicLimit * (0.5 + attentionLevel)));
    const semanticLimit = Math.max(1, Math.round(config2.recall.semanticLimit * (0.5 + attentionLevel)));
    ...
    if (brainConfig.modules.basalGanglia && cyc.classification) {
      ...
      if (habitMatch) {
        ...
        deps2.sessionHabits.set(key, habitMatch.habit.id);
...
function initGoalExecutor(_cfg, log) {
  active34 = createGoalExecutor(log);
...
function createAutonomyEnricher(cfg, log, injectedDeps) {
  ...
  function enrichWithMemories(motivation) {
    if (!deps2 || !config2) return;
    const desires = deps2.getDesires();
    ...
    const allSemantic = [];
    for (const query of queries) {
      const recalled = deps2.recallMemories(query, 2, 3);
    ...
    if (uniqueEpisodic.length === 0 && uniqueSemantic.length === 0) {
      logger2?.info("BrainAgent AutonomyEnricher: no relevant memories found, skipping enrichment");
      return;
    }
    const memoryContext = buildMemoryContext2(uniqueEpisodic, uniqueSemantic);
    deps2.enqueueSystemEvent(memoryContext, { contextKey: "autonomy-enricher" });
    logger2?.info(
  ...
  function stop() {
    ...
    unsubscribers2.length = 0;
    logger2?.info("BrainAgent AutonomyEnricher: stopped.");
  }
  wireEventListeners();
  logger2.info("BrainAgent AutonomyEnricher: initialized (memory-driven autonomy)");
  return { stop };
}
...
function initAutonomyEnricher(cfg, log, injectedDeps) {
  active35?.stop();
  active35 = createAutonomyEnricher(cfg, log, injectedDeps);
}
...
function createDreamMode() {
  ...
  async function runConsolidation(config2, logger2, neuroClawConfig, circadianTriggered = false) {
    ...
    try {
      ...
      const result = await consolidate(config2, neuroClawConfig, logger2, intensityMultiplier, skipAI);
      ...
      logger2?.info(`BrainAgent DreamMode: error during consolidation \u2014 ${String(err)}`);
    } finally {
      isConsolidating = false;
    }
  }
  function start(config2, logger2, neuroClawConfig) {
    if (dreamInterval) return;
    storedConfig = config2;
    storedLogger = logger2;
    storedNeuroClawConfig = neuroClawConfig;
    const intervalMs = config2.memory.dreamIntervalMinutes * 60 * 1e3;
    logger2?.info(
      `BrainAgent DreamMode: starting (interval: ${config2.memory.dreamIntervalMinutes}min)`
    );
    if (config2.circadian?.enabled) {
      setConsolidationCallback(async () => {
        await runConsolidation(config2, logger2, neuroClawConfig, true);
      });
      logger2?.info("BrainAgent DreamMode: registered with circadian rhythm system");
    }
    setTimeout(() => void runConsolidation(config2, logger2, neuroClawConfig, false), 3e4);
    dreamInterval = setInterval(() => {
      void runConsolidation(config2, logger2, neuroClawConfig, false);
    }, intervalMs);
  }
  function stop() {
    if (dreamInterval) {
  ...
  async function forceConsolidation2(config2, logger2, neuroClawConfig, intensity) {
    const sleepSettings = getSleepSettings();
    const effectiveIntensity = intensity ?? sleepSettings.consolidationIntensity;
    const result = await consolidate(config2, neuroClawConfig, logger2, effectiveIntensity);
    lastConsolidation = Date.now();
    if (logger2) {
      logger2.info(
...
var active36 = null;
function current22() {
  if (!active36) active36 = createDreamMode();
  return active36;
}
function startDreamMode(config2, logger2, neuroClawConfig) {
  current22().start(config2, logger2, neuroClawConfig);
}
function stopDreamMode() {
  current22().stop();
}
function forceConsolidation(config2, logger2, neuroClawConfig, intensity) {
  return current22().forceConsolidation(config2, logger2, neuroClawConfig, intensity);
}
...
function createProactiveFeedback(workspaceDir, cfg, log) {
  const storageDir2 = workspaceDir ? join30(workspaceDir, ".brainagent", "proactive-feedback") : "";
  const stateFile = storageDir2 ? join30(storageDir2, "state.json") : "";
  let config2 = cfg?.proactiveFeedback ?? null;
  const logger2 = log;
  ...
  function applyDecay2() {
    ...
    for (const entry of domainFeedback.values()) {
      ...
      entry.suppressionScore = Math.max(0, entry.suppressionScore - days * config2.decayPerDay);
      entry.lastDecayTime = now;
    }
  }
  function enforceDomainLimit() {
    if (!config2 || domainFeedback.size <= config2.maxTrackedDomains) return;
    const sorted = [...domainFeedback.entries()].sort(
      (a, b) => a[1].suppressionScore - b[1].suppressionScore
    );
    while (domainFeedback.size > config2.maxTrackedDomains) {
  ...
  function recordProactiveReaction2(domain, reactionText) {
    ...
    if (signal === "rejection") {
      entry.rejections += 1;
      entry.suppressionScore += config2.rejectionStep;
      entry.lastRejectionTime = Date.now();
      entry.lastHits = classification.hits;
    } else if (signal === "negative") {
      entry.rejections += 1;
      entry.suppressionScore += config2.negativeStep;
      entry.lastRejectionTime = Date.now();
      entry.lastHits = classification.hits;
    } else if (signal === "positive") {
      entry.accepts += 1;
      entry.suppressionScore = Math.max(0, entry.suppressionScore - config2.positiveStep);
    }
  ...
  function isDomainSuppressed2(domain) {
    ...
    if (entry.suppressionScore < config2.suppressionThreshold) return false;
    return Date.now() - entry.lastRejectionTime < config2.cooldownMs;
  }
  ...
  function stop() {
    ...
    logger2?.info("BrainAgent ProactiveFeedback: stopped.");
  }
  if (storageDir2) {
    if (!existsSync29(storageDir2)) {
      mkdirSync29(storageDir2, { recursive: true });
    }
    cancelPersist(stateFile);
    loadState2();
    if (config2) {
      logger2?.info(
        `BrainAgent ProactiveFeedback: initialized (threshold=${config2.suppressionThreshold}, cooldown=${config2.cooldownMs}ms, decay=${config2.decayPerDay}/day, tracked=${domainFeedback.size})`
      );
    }
  }
  return {
...
var active37 = null;
function current23() {
  if (!active37) active37 = createProactiveFeedback("");
  return active37;
}
function initProactiveFeedback(workspaceDir, cfg, log) {
  active37 = createProactiveFeedback(workspaceDir, cfg, log);
}
...
function recordProactiveReaction(domain, reactionText) {
  return current23().recordProactiveReaction(domain, reactionText);
}
function isDomainSuppressed(domain) {
  return current23().isDomainSuppressed(domain);
}
function getSuppressedDomainHints() {
  return current23().getSuppressedDomainHints();
}
...
function createRewardLedger(workspaceDir, config2) {
  const storageDir2 = join31(workspaceDir, ".brainagent", "reward");
  if (!existsSync30(storageDir2)) {
    mkdirSync30(storageDir2, { recursive: true });
  }
  const storageFile = join31(storageDir2, "ledger.json");
  cancelPersist(storageFile);
  const maxEntries = config2.learningLoop.rewardLedger.maxEntries;
  let entries = [];
  let idCounter = 0;
...
var active38;
function initRewardLedger(workspaceDir, config2) {
  active38?.dispose();
  active38 = createRewardLedger(workspaceDir, config2);
}
...
function createThalamicGate(config2, signalProviders) {
  let gateConfig = config2;
...
var active39 = null;
function current24() {
  if (!active39) active39 = createThalamicGate();
  return active39;
}
function initThalamicGate(config2, signalProviders) {
  active39 = createThalamicGate(config2, signalProviders);
}
...
function attachLlmBridge(ctx, preferredModel) {
  ...
  setCallLLMBackend(async (systemPrompt, userText, _config, logger2, maxTokens) => {
    ...
    try {
      ...
      logger2?.info(`BrainAgent LLM bridge: ctx.llm call failed \u2014 ${String(error)}`);
      return void 0;
    }
  });
  void resolveRoute();
...
function registerBrainAgentCommands(api, config2) {
  api.registerCommand({
    name: "brainagent",
    ...
    handler: async (ctx) => {
      ...
      if (args === "status" || args === "") {
        return buildStatusReport(config2);
      }
      if (args === "dream") {
        const result = await forceConsolidation(config2, api.logger, api.config);
        return {
          text: [
...
function buildStatusReport(config2) {
  ...
  const lines = [
    ...
    "",
    "**Core Modules:**",
    `  Thalamus (classifier):        ${config2.modules.thalamus ? "ON" : "OFF"}`,
    `  Amygdala (emotion/priority):   ${config2.modules.amygdala ? "ON" : "OFF"}`,
    `  Hippocampus (memory):          ${config2.modules.hippocampus ? "ON" : "OFF"}`,
    `  Prefrontal Cortex (reasoning): ${config2.modules.prefrontalCortex ? "ON" : "OFF"}`,
    `  Cerebellum (quality):          ${config2.modules.cerebellum ? "ON" : "OFF"}`,
    `  Mirror Neurons (empathy):      ${config2.modules.mirrorNeurons ? "ON" : "OFF"}`,
    `  Predictive Engine (anticip.):  ${config2.modules.predictiveEngine ? "ON" : "OFF"}`,
    `  Basal Ganglia (habits):        ${config2.modules.basalGanglia ? "ON" : "OFF"}`,
    `  Dream Mode (consolidation):    ${config2.modules.dreamMode ? "ON" : "OFF"}`,
    "",
    "**Integration Modules:**",
    `  Dopamine System (reward):      ${config2.modules.neuromodulatorSystem ? "ON" : "OFF"}`,
    `  Learning Coordinator (meta):   ${config2.modules.learningCoordinator ? "ON" : "OFF"}`,
    `  Neural Pathways (connections):  ${config2.modules.neuralPathways ? "ON" : "OFF"}`,
    "",
    "**Consciousness Modules:**",
    `  Working Memory (continuity):   ${config2.modules.workingMemory ? "ON" : "OFF"}`,
    `  Session Bridge (cross-sess.):  ${config2.modules.sessionBridge ? "ON" : "OFF"}`,
    `  Emotional Memory (flashbulb):  ${config2.modules.emotionalMemory ? "ON" : "OFF"}`,
    `  Attention Gate (filtering):    ${config2.modules.attentionGate ? "ON" : "OFF"}`,
    `  DMN (idle thinking):           ${config2.modules.dmn ? "ON" : "OFF"}`,
    `  Introspection (self-trace):    ${config2.modules.introspection ? "ON" : "OFF"}`,
    `  Agent Identity (self-model):   ${config2.modules.agentIdentity ? "ON" : "OFF"}`,
    `  Goal Stack (proactive):        ${config2.modules.goalStack ? "ON" : "OFF"}`,
    `  Curiosity Drive (gaps):        ${config2.modules.curiosityDrive ? "ON" : "OFF"}`,
    `  Temporal Binding (stream):     ${config2.modules.temporalBinding ? "ON" : "OFF"}`,
    `  Qualia Simulator (subjective): ${config2.modules.qualiaSimulator ? "ON" : "OFF"}`,
    `  Vital Impulse (autonomous):   ${config2.modules.vitalImpulse ? "ON" : "OFF"}`,
    `  Goal Executor (autonomous):   ${config2.modules.goalStack ? "ON" : "OFF"}`,
    "",
    "**Autonomic Modules:**",
    `  Metabolic Budget (energy):     ${config2.modules.metabolicBudget ? "ON" : "OFF"}`,
    `  Emergent Modules (patterns):   ${config2.modules.emergentModules ? "ON" : "OFF"}`,
    `  Interoception (inner state):   ${config2.modules.interoception ? "ON" : "OFF"}`,
    `  Proactive Feedback (\xAB\u043D\u0435 \u0437\u0430\u0448\u043B\u043E\xBB): ${config2.modules.proactiveFeedback ? "ON" : "OFF"}`,
    "",
    "**Memory:**",
  ...
  if (config2.modules.neuromodulatorSystem) {
    const dStats = getDopamineStats();
    const ns = dStats.currentState;
    lines.push(
      "",
  ...
  if (config2.modules.learningCoordinator) {
    const lStats = getLearningStats();
    lines.push(
      "",
  ...
  if (identityStatsGetter && config2.modules.agentIdentity) {
  ...
  if (qualiaSimulatorStatsGetter && config2.modules.qualiaSimulator) {
  ...
  if (vitalImpulseStatsGetter && config2.modules.vitalImpulse) {
    const vi = vitalImpulseStatsGetter();
  ...
  if (goalExecutorStatsGetter && config2.modules.goalStack) {
  ...
  if (config2.modules.proactiveFeedback) {
  ...
  if (socialDriveStatsGetter && config2.modules.socialDrive) {
  ...
  if (cognitiveHungerStatsGetter && config2.modules.cognitiveHunger) {
  ...
  if (creativeDriveStatsGetter && config2.modules.creativeDrive) {
  ...
  if (masteryDriveStatsGetter && config2.modules.masteryDrive) {
  ...
  if (driveArbiterStatsGetter && config2.modules.driveArbiter) {
  ...
  if (temporalAwarenessStatsGetter && config2.modules.temporalAwareness) {
  ...
  if (autonomousResearchStatsGetter && config2.modules.autonomousResearch) {
...
function apply(ctx, config2) {
  ...
  const brainConfig = mergeBrainConfig(config2);
  const dataDir = config2.dataDir;
  ...
  const enqueueAutonomousIntent = createAutonomousDeliverer({
    state,
    brainConfig,
    minGapMs: config2.autonomousMinGapMs ?? 10 * 60 * 1e3,
    logger: logger2,
    pickAgent,
  ...
  initEmbeddings(hostConfig(), logger2);
  if (brainConfig.modules.workingMemory) {
  ...
  if (brainConfig.modules.neuralPathways) {
    initNeuralPathways(dataDir, brainConfig, logger2);
  }
  if (config2.modules.structuralPlasticity) {
    initStructuralPlasticity(dataDir, brainConfig, logger2);
  }
  if (brainConfig.modules.emotionalMemory) {
    initEmotionalMemory(dataDir, brainConfig);
  }
  if (brainConfig.modules.sessionBridge) {
    initSessionBridge(dataDir, brainConfig, logger2);
  }
  if (brainConfig.modules.dmn) {
    initDMN(dataDir, brainConfig, logger2);
  }
  if (brainConfig.modules.goalStack) {
  ...
  if (brainConfig.modules.socialDrive) {
    initSocialDrive(dataDir, brainConfig, logger2, {
      ...driveInitBase,
  ...
  if (brainConfig.modules.cognitiveHunger) {
    initCognitiveHunger(dataDir, brainConfig, logger2, {
      ...driveInitBase,
  ...
  if (brainConfig.modules.creativeDrive) {
    initCreativeDrive(dataDir, brainConfig, logger2, {
      ...driveInitBase,
  ...
  if (brainConfig.modules.masteryDrive) {
    initMasteryDrive(dataDir, brainConfig, logger2, {
      ...driveInitBase,
  ...
  if (brainConfig.modules.vitalImpulse) {
    initVitalImpulse(dataDir, brainConfig, logger2, {
  ...
  if (brainConfig.modules.goalStack) {
    initGoalExecutor(brainConfig, logger2);
  }
  if (brainConfig.modules.actionDispatcher && brainConfig.modules.vitalImpulse) {
    initAutonomyEnricher(brainConfig, logger2, {
  ...
  if (brainConfig.modules.autonomousResearch) {
    initAutonomousResearch(brainConfig, logger2, {
      callLLM,
      storeFact,
  ...
  if (brainConfig.circadian.enabled) {
    initCircadianRhythm(dataDir, brainConfig, logger2);
  }
  if (brainConfig.modules.dreamMode) {
    startDreamMode(brainConfig, logger2, hostConfig());
  }
  if (brainConfig.modules.metabolicBudget) {
    initMetabolicBudget(dataDir, brainConfig, logger2);
  }
  if (brainConfig.modules.emergentModules) {
    initEmergentModules(dataDir, brainConfig, logger2);
  }
  if (brainConfig.modules.proactiveFeedback) {
    initProactiveFeedback(dataDir, brainConfig, logger2);
  }
  ...
  if (brainConfig.modules.temporalAwareness) {
    initTemporalAwareness(dataDir, brainConfig, logger2);
  }
  if (brainConfig.modules.interoception) {
    initInteroception(
      {
  ...
  ctx.on("session/event", (_session, event) => {
    ...
    if (event.type === "turn/end") {
      void endCycle3(key).catch((err) => logger2.warn(`BrainAgent endCycle: ${String(err)}`));
    }
  });
