...
function createMetricsCollector(workspaceDir, options = {}) {
  ...
  const filePath = workspaceDir ? join30(workspaceDir, ".brainagent", "metrics.json") : "";
  let updates = 0;
  if (filePath) {
    mkdirSync29(join30(workspaceDir, ".brainagent"), { recursive: true });
  }
  function collect() {
    const sections = {};
    const errors = {};
...
import { existsSync as existsSync30, mkdirSync as mkdirSync30, readFileSync as readFileSync28 } from "node:fs";
import { join as join31 } from "node:path";
function createProactiveFeedback(workspaceDir, cfg, log) {
  const storageDir = workspaceDir ? join31(workspaceDir, ".brainagent", "proactive-feedback") : "";
  const stateFile = storageDir ? join31(storageDir, "state.json") : "";
...
import { existsSync as existsSync31, mkdirSync as mkdirSync31, readFileSync as readFileSync29 } from "node:fs";
...
function createRewardLedger(workspaceDir, config) {
  const storageDir = join32(workspaceDir, ".brainagent", "reward");
  if (!existsSync31(storageDir)) {
    mkdirSync31(storageDir, { recursive: true });
  }
  const storageFile = join32(storageDir, "ledger.json");
  cancelPersist(storageFile);
  ...
  if (existsSync31(storageFile)) {
    try {
      const data = JSON.parse(readFileSync29(storageFile, "utf-8"));
