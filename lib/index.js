...
async function callLLM(systemPrompt, userText, config7, logger16, maxTokens = 500) {
  if (callBackend) {
    try {
      const bridged = await callBackend(systemPrompt, userText, config7, logger16, maxTokens);
...
async function assessWithAI(text, config7, logger16) {
  const content = await callLLM(EMOTION_PROMPT, text, config7, logger16, 100);
...
async function getEmbedding(text, config7, logger16) {
  const result = await getEmbeddings([text], config7, logger16);
  return result?.[0] ?? null;
}
async function getEmbeddings(texts, config7, logger16) {
...
function initEmbeddings(config7, logger16) {
  const provider = resolveEmbeddingProvider(config7);
  if (!provider) {
...
async function consolidate(config7, neuroClawConfig, logger16, intensity = 0.5, skipAIReview = false) {
  let merged = 0;
  let pruned = 0;
  let strengthened = 0;
...
function decideProcessingPath(classification, config7) {
  const threshold = COMPLEXITY_ORDER[config7.dualProcess.system2Threshold];
...
async function validateAsync(response, originalInput, config7, classification, assessment, userModel, logger16) {
...
\u041E\u0442\u0432\u0435\u0442: ${response}`;
    const aiResponse = await callLLM(QUALITY_PROMPT, userText, config7, logger16, 300);
    if (aiResponse) {
...
async function observeWithAI(userId, text, amygdalaResult, config7, neuroConfig, logger16) {
  const model = getOrCreateModel(userId);
  model.emotionHistory.push({
    timestamp: Date.now(),
    emotion: amygdalaResult.emotion,
...
async function detectReinforcementWithAI(text, config7, logger16) {
  const content = await callLLM(REINFORCEMENT_PROMPT, text, config7, logger16, 100);
...
import { renameSync, writeFileSync as writeFileSync7 } from "node:fs";
...
function writeFileSyncSafe(path, data) {
  writeFileSync7(path, data, "utf-8");
}
...
var pathwayLogger;
var pathwayUnsubs = [];
function initNeuralPathways(workspaceDir, config7, logger16) {
...
async function generateQualiaAsync(emotion, intensity, domain, neuromodulators, config7, logger16) {
...
${neuroDesc}`;
  try {
    ...
    const response = await callLLM(QUALIA_METAPHOR_PROMPT, userMessage, config7, logger16, 200);
    if (!response) {
...
async function extractFactsWithAI(text, config7, logger16) {
  const content = await callLLM(EXTRACTION_PROMPT, text, config7, logger16, 500);
  if (!content) return [];
...
async function extractProcedureAsync(text, config7, classification, logger16) {
  if (isAIProviderAvailable(config7)) {
    try {
      const aiResponse = await callLLM(PROCEDURE_PROMPT, text, config7, logger16, 300);
      if (aiResponse) {
...
async function extractGoalsFromConversation(userMessage, config7, logger16) {
  if (!isAIProviderAvailable(config7)) {
  ...
  const response = await callLLM(GOAL_EXTRACTION_PROMPT, userMessage, config7, logger16, 300);
  if (!response) {
...
var DriveEngine = class {
  constructor(spec, config7, workspaceDir, circadianEnabled3, deps5, logger16) {
    this.spec = spec;
    this.config = config7;
    this.circadianEnabled = circadianEnabled3;
    this.deps = deps5;
    this.logger = logger16;
    const storageDir26 = join17(workspaceDir, ".brainagent", spec.id);
    if (!existsSync17(storageDir26)) {
      mkdirSync17(storageDir26, { recursive: true });
    }
    this.stateFile = join17(storageDir26, "state.json");
...
function startDreamMode(config7, logger16, neuroClawConfig) {
  ...
  if (config7.circadian?.enabled) {
    setConsolidationCallback(async () => {
      await runConsolidation(config7, logger16, neuroClawConfig, true);
    });
    logger16?.info("BrainAgent DreamMode: registered with circadian rhythm system");
  }
  setTimeout(() => void runConsolidation(config7, logger16, neuroClawConfig, false), 3e4);
  dreamInterval = setInterval(() => {
    void runConsolidation(config7, logger16, neuroClawConfig, false);
  }, intervalMs);
}
...
async function runConsolidation(config7, logger16, neuroClawConfig, circadianTriggered = false) {
  ...
  try {
    ...
    const result = await consolidate(config7, neuroClawConfig, logger16, intensityMultiplier, skipAI);
...
async function forceConsolidation(config7, logger16, neuroClawConfig, intensity) {
  const sleepSettings = getSleepSettings();
  const effectiveIntensity = intensity ?? sleepSettings.consolidationIntensity;
  const result = await consolidate(config7, neuroClawConfig, logger16, effectiveIntensity);
  lastConsolidation = Date.now();
  if (logger16) {
    logger16.info(
...
function initTemporalBinding(workspaceDir, config7) {
  ...
  maxMoments = config7.temporalBinding.maxMoments;
  moments = [];
  idCounter5 = 0;
  loadState17();
}
...
function initMetabolicBudget(workspaceDir, config7, log) {
  ...
  logger14 = log;
  storageDir24 = join26(workspaceDir, ".brainagent", "metabolic");
...
function loadState20() {
  try {
    const path = join26(storageDir24, "state.json");
...
function saveState2() {
  try {
    writeFileSync22(join26(storageDir24, "state.json"), JSON.stringify(state5, null, 2), "utf-8");
  } catch {
  }
}
...
function initEmergentModules(workspaceDir, config7, log) {
  ...
  logger15 = log;
  storageDir25 = join27(workspaceDir, ".brainagent", "emergent");
  if (!existsSync27(storageDir25)) {
    mkdirSync27(storageDir25, { recursive: true });
  }
  state6.minOccurrences = config7.emergentModules.minOccurrences;
  state6.minReward = config7.emergentModules.minRewardForEstablishment;
  loadState21();
...
function saveState3() {
  try {
    writeFileSync23(join27(storageDir25, "state.json"), JSON.stringify(state6, null, 2), "utf-8");
  } catch {
  }
}
...
var Config = Schema.object({
  dataDir: Schema.string().default(join28(homedir(), ".brainagent")),
...
function mergeBrainConfig(config7) {
  return {
    ...
    modules: {
      ...
      thalamus: config7.modules.thalamus,
      amygdala: config7.modules.amygdala,
      hippocampus: config7.modules.hippocampus,
      prefrontalCortex: config7.modules.prefrontalCortex,
      cerebellum: config7.modules.cerebellum,
      workingMemory: config7.modules.workingMemory,
      attentionGate: config7.modules.attentionGate,
      mirrorNeurons: config7.modules.mirrorNeurons,
      predictiveEngine: config7.modules.predictiveEngine,
      basalGanglia: config7.modules.basalGanglia,
      neuromodulatorSystem: config7.modules.neuromodulatorSystem,
      learningCoordinator: config7.modules.learningCoordinator,
      neuralPathways: config7.modules.neuralPathways,
      emotionalMemory: config7.modules.emotionalMemory,
      sessionBridge: config7.modules.sessionBridge,
      dmn: config7.modules.dmn,
      dreamMode: config7.modules.dreamMode,
      goalStack: config7.modules.goalStack,
      curiosityDrive: config7.modules.curiosityDrive,
      vitalImpulse: config7.modules.vitalImpulse,
      socialDrive: config7.modules.socialDrive,
      cognitiveHunger: config7.modules.cognitiveHunger,
      creativeDrive: config7.modules.creativeDrive,
      masteryDrive: config7.modules.masteryDrive,
      actionDispatcher: config7.modules.autonomyEnricher,
      driveArbiter: config7.modules.driveArbiter,
      autonomousResearch: config7.modules.autonomousResearch,
      introspection: config7.modules.introspection,
      agentIdentity: config7.modules.agentIdentity,
      temporalBinding: config7.modules.temporalBinding,
      qualiaSimulator: config7.modules.qualiaSimulator,
      temporalAwareness: config7.modules.temporalAwareness,
      thalamicGate: config7.modules.thalamicGate,
      metabolicBudget: config7.modules.metabolicBudget,
      emergentModules: config7.modules.emergentModules,
      interoception: config7.modules.interoception
    },
    dualProcess: {
...
function apply(ctx, config7) {
  ...
  mkdirSync28(dataDir, { recursive: true });
  ...
  function enqueueAutonomousIntent(text) {
    ...
    const minGapMs = config7.autonomousMinGapMs ?? 10 * 60 * 1e3;
  ...
  async function endCycle3(key) {
    ...
    const aiAvailable = config7.modules.aiEnrichment && isAIProviderAvailable2(hostConfig());
  ...
  ctx.on("agent/pre-step", async (payload, next) => {
    ...
    if (config7.modules.aiEnrichment && isAIProviderAvailable2(hostConfig())) {
      const hc = hostConfig();
      const tasks = [];
      if (brainConfig.modules.amygdala) {
        tasks.push(
