...
async function callLLM(systemPrompt, userText, config7, logger9, maxTokens = 500) {
  if (callBackend) {
    try {
      const bridged = await callBackend(systemPrompt, userText, config7, logger9, maxTokens);
      if (bridged !== void 0) return bridged;
    } catch (error) {
      logger9?.info(`BrainAgent LLM: bridge failed, falling back \u2014 ${String(error)}`);
    }
  }
  const provider = resolveProvider(config7);
  if (!provider) {
    logger9?.info("BrainAgent LLM: no AI provider configured, skipping");
    return null;
  }
  const userSelection = parseUserModelSelection(config7);
  if (userSelection) {
    logger9?.info(`BrainAgent LLM: calling ${provider.name} (${provider.model}) [user-selected]`);
  } else {
    logger9?.info(`BrainAgent LLM: calling ${provider.name} (${provider.model}) [auto-detected]`);
  }
  try {
    let response;
...
${userText}` }] }],
      ...
      logger9?.info(`BrainAgent LLM: Google error ${response.status}: ${errorText}`);
      return null;
    } else {
    ...
    if (!response.ok) {
      const errorText = await response.text();
      logger9?.info(`BrainAgent LLM: ${provider.name} error ${response.status}: ${errorText}`);
      return null;
    }
    ...
    logger9?.info(`BrainAgent LLM: error \u2014 ${String(error)}`);
    return null;
  }
}
...
async function getEmbedding(text, config7, logger9) {
  const result = await getEmbeddings([text], config7, logger9);
  return result?.[0] ?? null;
}
async function getEmbeddings(texts, config7, logger9) {
  ...
  try {
    const result = provider.format === "google" ? await fetchGoogleEmbeddings(texts, provider, logger9) : await fetchOpenAIEmbeddings(texts, provider, logger9);
    if (result === null && provider.name === "Ollama") {
      ollamaUnreachable = true;
    }
    return result;
  } catch (error) {
    logger9?.info(`BrainAgent Embeddings: ${provider.name} error \u2014 ${String(error)}`);
    if (provider.name === "Ollama") ollamaUnreachable = true;
    return null;
  }
}
async function fetchOpenAIEmbeddings(texts, provider, logger9) {
...
async function fetchGoogleEmbeddings(texts, provider, logger9) {
  ...
  if (!response.ok) {
    const errorText = await response.text();
    logger9?.info(`BrainAgent Embeddings: Google API error ${response.status}: ${errorText}`);
    return null;
  }
  const data = await response.json();
...
function createHippocampus(workspaceDir) {
  ...
  function initEmbeddings2(config7, logger9) {
    const provider = resolveEmbeddingProvider(config7);
    if (!provider) {
      logger9?.info("BrainAgent Hippocampus: no embedding provider \u2014 using TF-IDF");
      return;
    }
    embeddingsConfig = config7;
    embeddingsLogger = logger9;
  ...
  async function consolidate2(config7, neuroClawConfig, logger9, intensity = 0.5, skipAIReview = false) {
    let merged = 0;
    let pruned = 0;
    let strengthened = 0;
    ...
    if (!skipAIReview && neuroClawConfig && isAIProviderAvailable(neuroClawConfig) && semanticStore.length >= 5) {
      try {
        ...
        logger9?.info(`BrainAgent Consolidation: AI review error \u2014 ${String(err)}`);
      }
    }
    persistEpisodic();
    persistSemantic();
...
function initEmbeddings(config7, logger9) {
  current().initEmbeddings(config7, logger9);
}
...
async function consolidate(config7, neuroClawConfig, logger9, intensity = 0.5, skipAIReview = false) {
  return current().consolidate(config7, neuroClawConfig, logger9, intensity, skipAIReview);
}
function getStats() {
...
async function assessWithAI(text, config7, logger9) {
  const content = await callLLM(EMOTION_PROMPT, text, config7, logger9, 100);
...
function createMirrorNeurons(workspaceDir) {
  const storageDir13 = workspaceDir ? join3(workspaceDir, ".brainagent", "users") : "";
  const userModels = /* @__PURE__ */ new Map();
  if (storageDir13 && !existsSync2(storageDir13)) {
    mkdirSync2(storageDir13, { recursive: true });
  }
  try {
    const indexPath = join3(storageDir13, "index.json");
    if (storageDir13 && existsSync2(indexPath)) {
  ...
  function persistModels() {
    ...
    try {
      ...
      writeFileSync3(join3(storageDir13, "index.json"), JSON.stringify(data, null, 2), "utf-8");
    } catch {
    }
  }
  ...
  async function observeWithAI2(userId, text, amygdalaResult, config7, neuroConfig, logger9) {
    const model = getOrCreateModel(userId);
    model.emotionHistory.push({
      timestamp: Date.now(),
      emotion: amygdalaResult.emotion,
    ...
    const alpha = 0.3;
    model.stressLevel = model.stressLevel * (1 - alpha) + (isStressed ? amygdalaResult.emotionIntensity : 0) * alpha;
    const content = await callLLM(STYLE_PROMPT, text, neuroConfig, logger9, 100);
    if (content) {
      try {
        const jsonMatch = content.match(/\{[\s\S]*\}/);
        if (jsonMatch) {
...
function updateKnowledgeModelToM(model, text, domainLimit) {
  ...
  for (const domain of domainSignals) {
    const current10 = model.knowledgeModel[domain];
    if (!current10) {
      model.knowledgeModel[domain] = "beginner";
    } else if (current10 === "beginner") {
...
function observeWithAI(userId, text, amygdalaResult, config7, neuroConfig, logger9) {
  return current2().observeWithAI(userId, text, amygdalaResult, config7, neuroConfig, logger9);
}
...
function createPredictiveEngine(workspaceDir) {
  const storageDir13 = workspaceDir ? join4(workspaceDir, ".brainagent", "predictions") : "";
  ...
  let lastDomain;
  if (storageDir13 && !existsSync3(storageDir13)) {
    mkdirSync3(storageDir13, { recursive: true });
  }
  function loadPatterns() {
    if (!storageDir13) return;
    try {
      const tPath = join4(storageDir13, "temporal.json");
    ...
    try {
      const sPath = join4(storageDir13, "sequential.json");
    ...
    try {
      const cPath = join4(storageDir13, "contextual.json");
...
function createBasalGanglia(workspaceDir) {
  const storageDir13 = workspaceDir ? join5(workspaceDir, ".brainagent", "habits") : "";
  let habits = [];
  ...
  if (storageDir13 && !existsSync4(storageDir13)) {
    mkdirSync4(storageDir13, { recursive: true });
  }
  function loadHabits() {
    if (!storageDir13) return;
    try {
      const path = join5(storageDir13, "habits.json");
      if (existsSync4(path)) {
  ...
  function persistHabits() {
    if (!storageDir13) return;
    try {
      writeFileSync5(join5(storageDir13, "habits.json"), JSON.stringify(habits, null, 2), "utf-8");
    } catch {
    }
  }
...
async function detectReinforcementWithAI(text, config7, logger9) {
  const content = await callLLM(REINFORCEMENT_PROMPT, text, config7, logger9, 100);
...
function createDopamineSystem(workspaceDir) {
  const storageDir13 = join7(workspaceDir, ".brainagent", "neuromodulators");
  if (!existsSync6(storageDir13)) {
    mkdirSync6(storageDir13, { recursive: true });
  }
  cancelPersist(join7(storageDir13, "state.json"));
  let state3 = {
    dopamine: 0.5,
    serotonin: 0.6,
    norepinephrine: 0.3,
  ...
  function loadState13() {
    try {
      const path = join7(storageDir13, "state.json");
      if (existsSync6(path)) {
...
function createNeuralPathways(workspaceDir, config7, logger9) {
  const storageDir13 = join8(workspaceDir, ".brainagent", "synapses");
  if (!existsSync7(storageDir13)) {
    mkdirSync7(storageDir13, { recursive: true });
  }
  ...
  function loadSynapticState() {
    try {
      const path = join8(storageDir13, "weights.json");
      if (existsSync7(path)) {
  ...
  function saveSynapticState() {
    try {
      writeFileSync7(
        join8(storageDir13, "weights.json"),
...
var active6;
function initNeuralPathways(workspaceDir, config7, logger9) {
  active6?.dispose();
  active6 = createNeuralPathways(workspaceDir, config7, logger9);
}
...
function createWorkingMemory(workspaceDir, opts) {
  const storageDir13 = workspaceDir ? join9(workspaceDir, ".brainagent", "working-memory") : "";
  const maxEntries = opts.maxEntries;
  const summaryMaxLength = opts.summaryMaxLength;
  let entries = [];
  if (storageDir13 && !existsSync8(storageDir13)) {
    mkdirSync8(storageDir13, { recursive: true });
  }
  function loadState13() {
    if (!storageDir13) return;
    try {
      const path = join9(storageDir13, "state.json");
      if (existsSync8(path)) {
  ...
  function persistState11() {
    if (!storageDir13) return;
    try {
      writeFileSync8(join9(storageDir13, "state.json"), JSON.stringify(entries, null, 2), "utf-8");
    } catch {
    }
  }
  loadState13();
...
async function extractFactsWithAI(text, config7, logger9) {
  const content = await callLLM(EXTRACTION_PROMPT, text, config7, logger9, 500);
  if (!content) return [];
  const facts = parseFactsFromResponse(content);
  if (facts.length > 0) {
    logger9?.info(`BrainAgent AI Extractor: found ${facts.length} facts`);
  }
  return facts;
}
...
async function validateAsync(response, originalInput, config7, classification, assessment, userModel, logger9) {
...
\u041E\u0442\u0432\u0435\u0442: ${response}`;
    const aiResponse = await callLLM(QUALITY_PROMPT, userText, config7, logger9, 300);
    if (aiResponse) {
    ...
    logger9?.info(`BrainAgent Cerebellum: AI validation error \u2014 ${String(err)}`);
  }
  return heuristicResult;
}
...
async function extractProcedureAsync(text, config7, classification, logger9) {
  if (isAIProviderAvailable(config7)) {
    try {
      const aiResponse = await callLLM(PROCEDURE_PROMPT, text, config7, logger9, 300);
      if (aiResponse) {
        const parsed = parseProcedureResponse(aiResponse, classification);
        if (parsed) return parsed;
      }
    } catch (err) {
      logger9?.info(`BrainAgent Procedural: AI extraction error \u2014 ${String(err)}`);
    }
  }
...
function createLearningCoordinator(workspaceDir, config7) {
  const storageDir13 = join11(workspaceDir, ".brainagent", "learning");
  if (!existsSync10(storageDir13)) {
    mkdirSync10(storageDir13, { recursive: true });
  }
  ...
  function loadState13() {
    try {
      const path = join11(storageDir13, "coordinator.json");
  ...
  function persistState11() {
    try {
      ...
      writeFileSync10(
        join11(storageDir13, "coordinator.json"),
        JSON.stringify(
          {
...
function createMetabolicBudget(workspaceDir, config7, log) {
  const storageDir13 = join13(workspaceDir, ".brainagent", "metabolic");
  if (!existsSync12(storageDir13)) {
    mkdirSync12(storageDir13, { recursive: true });
  }
  ...
  function loadState13() {
    try {
      const path = join13(storageDir13, "state.json");
  ...
  function saveState3() {
    try {
      writeFileSync12(join13(storageDir13, "state.json"), JSON.stringify(state3, null, 2), "utf-8");
    } catch {
    }
  }
  loadState13();
...
function createEmotionalMemory(workspaceDir, config7) {
  const storageDir13 = workspaceDir ? join15(workspaceDir, ".brainagent", "emotional-memory") : "";
  if (storageDir13 && !existsSync14(storageDir13)) {
  ...
  function loadState13() {
    if (!storageDir13) return;
    try {
      const path = join15(storageDir13, "state.json");
  ...
  function persistState11() {
    ...
    try {
      writeFileSync14(
        join15(storageDir13, "state.json"),
  ...
  async function generateQualiaAsync2(emotion, intensity, domain, neuromodulators, llmConfig, logger9) {
...
${neuroDesc}`;
    try {
      ...
      const response = await callLLM(QUALIA_METAPHOR_PROMPT, userMessage, llmConfig, logger9, 200);
      if (!response) {
        logger9?.info("BrainAgent Qualia: LLM returned null, falling back to template");
        return generateQualia(emotion, intensity, domain, neuromodulators);
      }
      const parsed = parseLLMQualiaResponse(response);
      if (!parsed) {
        logger9?.info("BrainAgent Qualia: failed to parse LLM response, falling back to template");
        return generateQualia(emotion, intensity, domain, neuromodulators);
      }
      const qualia = {
        timestamp: now,
      ...
      persistState11();
      logger9?.info(`BrainAgent Qualia: LLM-generated metaphor for ${emotion}/${domain}`);
      return qualia;
    } catch (err) {
      logger9?.info(`BrainAgent Qualia: AI metaphor generation failed \u2014 ${String(err)}`);
      return generateQualia(emotion, intensity, domain, neuromodulators);
    }
  }
  function dispose() {
...
async function generateQualiaAsync(emotion, intensity, domain, neuromodulators, config7, logger9) {
  return current6().generateQualiaAsync(emotion, intensity, domain, neuromodulators, config7, logger9);
}
...
function initDMN(workspaceDir, config7, log) {
  storageDir6 = join17(workspaceDir, ".brainagent", "dmn");
  if (!existsSync16(storageDir6)) {
    mkdirSync16(storageDir6, { recursive: true });
  }
  currentConfig3 = config7;
  logger3 = log;
  insights = [];
  lastRunTimestamp = 0;
  totalAssociationsFound = 0;
  innerMonologue = [];
  maxBackgroundThoughts = config7.dmn.maxBackgroundThoughts ?? 20;
  loadState6();
}
function loadState6() {
  if (!storageDir6) return;
  try {
    const path = join17(storageDir6, "state.json");
...
async function runAssociationFinding(config7) {
  const maxInsights = config7.dmn.maxInsightsPerCycle;
  const minSimilarity = config7.dmn.minSimilarityForAssociation;
  const categories = [
    "user_info",
    "user_preference",
    "definition",
  ...
  persistState4();
  logger3?.info(`DMN: found ${newInsights.length} cross-domain associations`);
  return newInsights;
}
...
function generateBackgroundThoughts(config7, unresolvedQuestions, recentEmotions, knowledgeGaps) {
  const maxPerCycle = config7.dmn.maxThoughtsPerCycle ?? 5;
  const newThoughts = [];
  ...
  persistState4();
  logger3?.info(`DMN: generated ${newThoughts.length} background thoughts`);
  return newThoughts;
}
...
function createTemporalBinding(workspaceDir, opts) {
  const storageDir13 = workspaceDir ? join18(workspaceDir, ".brainagent", "temporal-binding") : "";
  const maxMoments = opts.maxMoments;
  let moments = [];
  let idCounter3 = 0;
  if (storageDir13 && !existsSync17(storageDir13)) {
    mkdirSync17(storageDir13, { recursive: true });
  }
  function loadState13() {
    if (!storageDir13) return;
    try {
      const path = join18(storageDir13, "state.json");
  ...
  function persistState11() {
    if (!storageDir13) return;
    try {
      writeFileSync17(join18(storageDir13, "state.json"), JSON.stringify(moments, null, 2), "utf-8");
    } catch {
    }
  }
  loadState13();
...
var active11;
function current7() {
  return active11 ?? (active11 = createTemporalBinding("", { maxMoments: DEFAULT_MAX_MOMENTS }));
}
function initTemporalBinding(workspaceDir, config7) {
  active11 = createTemporalBinding(workspaceDir, { maxMoments: config7.temporalBinding.maxMoments });
}
function createMoment(input, thoughts, emotion, emotionIntensity, activeMemoryIds, intentions, confidence, domain) {
  return current7().createMoment(
    input,
    thoughts,
    emotion,
    emotionIntensity,
...
function buildTemporalContext(n = 3) {
  return current7().buildContext(n);
}
...
function createSessionBridge(workspaceDir, config7, _logger) {
  const storageDir13 = workspaceDir ? join19(workspaceDir, ".brainagent", "sessions") : "";
  if (storageDir13 && !existsSync18(storageDir13)) {
    mkdirSync18(storageDir13, { recursive: true });
  }
  ...
  function loadState13() {
    if (!storageDir13) return;
    try {
      const currentPath = join19(storageDir13, "current.json");
    ...
    try {
      const lastPath = join19(storageDir13, "last.json");
  ...
  function persistCurrent() {
    ...
    try {
      writeFileSync18(
        join19(storageDir13, "current.json"),
  ...
  function persistLast() {
    if (!storageDir13) return;
    try {
      writeFileSync18(join19(storageDir13, "last.json"), JSON.stringify(lastSession, null, 2), "utf-8");
    } catch {
    }
  }
...
function current8() {
  if (!active12) {
    active12 = createSessionBridge("", DEFAULT_CONFIG);
  }
  return active12;
}
function initSessionBridge(workspaceDir, config7, logger9) {
  active12?.dispose();
  active12 = createSessionBridge(workspaceDir, config7, logger9);
}
function recordCycleForSession(input, classification, assessment, reward) {
  current8().recordCycleForSession(input, classification, assessment, reward);
}
function checkSessionGap() {
  return current8().checkSessionGap();
}
function buildSessionBridgeContext() {
  return current8().buildSessionBridgeContext();
}
function getSessionBridgeStats() {
  return current8().getSessionBridgeStats();
}
...
function initCuriosityDrive(workspaceDir, config7) {
  storageDir7 = join20(workspaceDir, ".brainagent", "curiosity");
  if (!existsSync19(storageDir7)) {
    mkdirSync19(storageDir7, { recursive: true });
  }
...
function loadState7() {
  ...
  try {
    const path = join20(storageDir7, "state.json");
...
function initGoalStack(workspaceDir, config7) {
  storageDir8 = join21(workspaceDir, ".brainagent", "goals");
  if (!existsSync20(storageDir8)) {
    mkdirSync20(storageDir8, { recursive: true });
  }
...
function loadState8() {
  ...
  try {
    const path = join21(storageDir8, "state.json");
...
function checkGoalTriggers(input, currentEmotion, currentDomain) {
  ...
  if (triggered.length > 0) persistState6();
  return triggered;
}
function expireGoals() {
  ...
  if (goals.length > 60) {
    const active23 = goals.filter((g) => g.status === "pending" || g.status === "triggered");
    const inactive = goals.filter((g) => g.status === "completed" || g.status === "expired").slice(-30);
    goals = [...active23, ...inactive];
    changed = true;
  }
  if (changed) persistState6();
}
...
function weakenDesiresAfterFire(consecutiveFires) {
  ...
  if (desires.length !== before || changed) persistState6();
}
...
async function extractGoalsFromConversation(userMessage, config7, logger9) {
  if (!isAIProviderAvailable(config7)) {
    logger9?.info("BrainAgent GoalStack: no AI provider available, skipping goal extraction");
    return [];
  }
  logger9?.info("BrainAgent GoalStack: extracting goals from conversation...");
  const response = await callLLM(GOAL_EXTRACTION_PROMPT, userMessage, config7, logger9, 300);
  if (!response) {
    logger9?.info("BrainAgent GoalStack: LLM returned null/empty response");
    return [];
  }
  logger9?.info(`BrainAgent GoalStack: LLM response received (${response.length} chars)`);
  let parsed;
  try {
    let jsonStr = response.trim();
    const jsonMatch = jsonStr.match(/\[[\s\S]*\]/);
    ...
    logger9?.info("BrainAgent GoalStack: failed to parse LLM goal extraction response");
    return [];
  }
  const createdGoals = [];
...
function createCircadianRhythm(workspaceDir, config7, log) {
  const storageDir13 = join22(workspaceDir, ".brainagent", "circadian");
  if (!existsSync21(storageDir13)) {
    mkdirSync21(storageDir13, { recursive: true });
  }
  ...
  function loadState13() {
    try {
      const path = join22(storageDir13, "state.json");
  ...
  function persistState11() {
    try {
      writeFileSync21(join22(storageDir13, "state.json"), JSON.stringify(state3, null, 2), "utf-8");
    } catch {
    }
  }
...
function initCircadianRhythm(workspaceDir, config7, log) {
  active13?.dispose();
  active13 = createCircadianRhythm(workspaceDir, config7, log);
  if (pendingConsolidationCallback) {
    active13.setConsolidationCallback(pendingConsolidationCallback);
  }
}
...
function setConsolidationCallback(callback) {
  ...
  active13?.setConsolidationCallback(callback);
}
function recordActivity() {
  active13?.recordActivity();
}
function getCircadianState() {
  return active13?.getState() ?? createDefaultState4();
}
function getSleepSettings() {
  return active13?.getSleepSettings() ?? {
    consolidationIntensity: 0.5,
    pruningAggressiveness: 0.3,
    synapticNormalization: false
  };
...
function isInSleepPhase() {
  return active13?.isInSleepPhase() ?? false;
}
function isInWakePhase() {
  return active13?.isInWakePhase() ?? true;
}
function forcePhase(phase) {
  active13?.forcePhase(phase);
}
function getCircadianStats() {
  const st = active13?.getState() ?? createDefaultState4();
  return {
    phase: st.phase,
...
var DriveEngine = class {
  constructor(spec, config7, workspaceDir, circadianEnabled3, deps5, logger9) {
    this.spec = spec;
    this.config = config7;
    this.circadianEnabled = circadianEnabled3;
    this.deps = deps5;
    this.logger = logger9;
    const storageDir13 = join23(workspaceDir, ".brainagent", spec.id);
    if (!existsSync22(storageDir13)) {
      mkdirSync22(storageDir13, { recursive: true });
    }
    this.stateFile = join23(storageDir13, "state.json");
...
function initSocialDrive(workspaceDir, cfg, log, injectedDeps) {
  active14?.dispose();
  active14 = createSocialDrive(workspaceDir, cfg, log, injectedDeps);
}
...
function getSatiation() {
  return active14?.getSatiation() ?? 0;
}
...
function initCognitiveHunger(workspaceDir, cfg, log, injectedDeps) {
  active15?.dispose();
  active15 = createCognitiveHunger(workspaceDir, cfg, log, injectedDeps);
}
...
function initCreativeDrive(workspaceDir, cfg, log, injectedDeps) {
  active16?.dispose();
  active16 = createCreativeDrive(workspaceDir, cfg, log, injectedDeps);
}
...
function initMasteryDrive(workspaceDir, cfg, log, injectedDeps) {
  storageDir9 = join24(workspaceDir, ".brainagent", "mastery-drive");
  if (!existsSync23(storageDir9)) {
    mkdirSync23(storageDir9, { recursive: true });
  }
  ...
  totalNeedSignals = 0;
  adaptiveDecayModifier = 1;
  cancelPersist(join24(storageDir9, "state.json"));
  loadState9();
  wireEventListeners();
  logger4.info(
...
function stopMasteryDrive() {
  ...
  unsubscribers.length = 0;
  persistState7();
  flushPersist(join24(storageDir9, "state.json"));
  logger4?.info("BrainAgent MasteryDrive: stopped.");
}
...
function biasDMNToMasteryThoughts(weakestDomain) {
  ...
  logger4?.info(`BrainAgent MasteryDrive: biased DMN toward ${topics.length} mastery topic(s)`);
}
...
function loadState9() {
  try {
    const filePath = join24(storageDir9, "state.json");
...
function persistState7() {
  schedulePersist(join24(storageDir9, "state.json"), () => {
    const domainMap = {};
    for (const [domain, mastery] of domainSatiations) {
...
function collectActiveDrives(drives, threshold) {
  ...
  if (drives.social) {
    ...
    if (s.need >= threshold) active23.push({ name: "social", need: s.need, needLevel: s.needLevel });
  }
  if (drives.cognitive) {
    const c = drives.cognitive();
    if (c.need >= threshold) active23.push({ name: "cognitive", need: c.need, needLevel: c.needLevel });
  }
  if (drives.creative) {
    const c = drives.creative();
    if (c.need >= threshold) active23.push({ name: "creative", need: c.need, needLevel: c.needLevel });
  }
  if (drives.mastery) {
    const m = drives.mastery();
    if (m.need >= threshold) active23.push({ name: "mastery", need: m.need, needLevel: m.needLevel });
  }
  return active23;
}
function synthesizeAutonomousCycleState(brainConfig, drives, cycle) {
  const active23 = collectActiveDrives(drives, 0.5);
  if (active23.length === 0) return;
  const ranked = active23.map((d) => {
...
function createAutonomousDeliverer(deps5) {
  const { state: state3, brainConfig, minGapMs, logger: logger9 } = deps5;
  return function enqueueAutonomousIntent(text) {
    const trimmed = text.trim();
    if (!trimmed) return;
    if (state3.previousCycleWasAutonomous) {
      logger9.info("BrainAgent Autonomy: intent suppressed \u2014 previous cycle was autonomous");
      return;
    }
    if (Date.now() - state3.lastAutonomousDeliveryAt < minGapMs) {
      logger9.info("BrainAgent Autonomy: intent suppressed \u2014 minimum gap not elapsed");
      return;
    }
    ...
    if (!agent) {
      logger9.warn("BrainAgent Autonomy: no live agent \u2014 autonomous intent dropped");
      return;
    }
...
function createAutonomousIntentResolver(deps5) {
  ...
  return () => {
    ...
    const active23 = collectActiveDrives(drives, 0.8);
    if (active23.length > 0) {
      const strongest = active23.sort((a, b) => b.need - a.need)[0];
...
function createCycleEngine(deps5) {
  const { config: config7, brainConfig, getHostConfig, logger: logger9, markActivation, state: state3 } = deps5;
  ...
  async function endCycle3(key) {
    ...
    if (brainConfig.modules.cerebellum && cycle.responseText.trim()) {
      ...
      if (result.issues.length > 0) {
        logger9.warn(`BrainAgent Cerebellum: quality issues \u2014 ${result.issues.join("; ")}`);
    ...
    if (config7.modules.semanticExtraction && semanticSource.length > 15 && isFactWorthy(semanticSource, cycle.classification)) {
      ...
      if (aiAvailable) {
        try {
          const aiFacts = await extractFactsWithAI(semanticSource, getHostConfig(), logger9);
          if (aiFacts.length > 0) {
          ...
          logger9.info(`BrainAgent Semantic: AI extraction failed, falling back \u2014 ${String(err)}`);
        }
      }
      if (!factsStored) {
      ...
      if (factsStored && brainConfig.modules.curiosityDrive) {
        ...
        for (const gap of getOpenGaps()) {
          if (inputLower.includes(gap.topic.toLowerCase())) {
            ...
            logger9.info(`BrainAgent Curiosity: gap filled for "${gap.topic}" via new fact storage`);
          }
        }
      }
    }
    if (config7.modules.proceduralExtraction && input.length > 10 && isProcedural(input, cycle.classification)) {
      const procedure = await extractProcedureAsync(input, getHostConfig(), cycle.classification, logger9);
      if (procedure && procedure.confidence > 0.5) {
        storeWorkflow(procedure.description, procedure.triggerPattern, procedure.steps);
        logger9.info(`BrainAgent Procedural: stored workflow "${procedure.description}"`);
      }
    }
    ...
    if (brainConfig.modules.dmn) {
      ...
      if (wakeInteractionCount >= brainConfig.dmn.wakeThoughtInterval) {
        ...
        void runAssociationFinding(brainConfig).catch((err) => {
          logger9.info(`BrainAgent DMN: association finding error: ${err}`);
        });
      }
    }
    if (brainConfig.modules.goalStack && input.length > 10) {
      ...
      if (goalExtractionCounter >= brainConfig.goalStack.extractionInterval) {
        ...
        void extractGoalsFromConversation(input, getHostConfig(), logger9).catch((err) => {
          logger9.info(`BrainAgent GoalStack: extraction error: ${err}`);
        });
      }
    }
    if (brainConfig.modules.goalStack) {
...
function createStrategyBandit(workspaceDir, config7) {
  const storageDir13 = join25(workspaceDir, ".brainagent", "bandit");
  if (!existsSync24(storageDir13)) {
    mkdirSync24(storageDir13, { recursive: true });
  }
  const storageFile = join25(storageDir13, "state.json");
...
function initStrategyBandit(workspaceDir, config7) {
  active17?.dispose();
  active17 = createStrategyBandit(workspaceDir, config7);
}
...
function chooseArm(decisionPoint, arms) {
  ...
  return active17.chooseArm(decisionPoint, arms);
}
...
function initVitalImpulse(workspaceDir, cfg, log, injectedDeps) {
  storageDir10 = join26(workspaceDir, ".brainagent", "vital-impulse");
  if (!existsSync25(storageDir10)) {
    mkdirSync25(storageDir10, { recursive: true });
  }
  ...
  cancelPersist(join26(storageDir10, "state.json"));
  loadState10();
  wireSignalListeners();
  logger5.info(
...
function stopVitalImpulse() {
  ...
  unsubscribers2.length = 0;
  persistState8();
  flushPersist(join26(storageDir10, "state.json"));
  logger5?.info("BrainAgent VitalImpulse: stopped.");
}
...
function forceImpulse(reason) {
  ...
  recentSignals = [];
  persistState8();
  logger5?.info("BrainAgent VitalImpulse: forced impulse fired.");
}
...
function loadState10() {
  try {
    const filePath = join26(storageDir10, "state.json");
...
function persistState8() {
  schedulePersist(join26(storageDir10, "state.json"), () => {
    const state3 = {
      lastFireTime,
      totalFires,
      totalSignalsReceived,
...
function initDriveArbiter(workspaceDir, cfg, getters, log) {
  ...
  storageDir11 = join27(workspaceDir, ".brainagent");
  if (!existsSync26(storageDir11)) {
    mkdirSync26(storageDir11, { recursive: true });
  }
  cancelPersist(join27(storageDir11, "drive-arbiter.json"));
  loadState11();
  const driveEvents = [
  ...
  logger6?.info("BrainAgent DriveArbiter: initialized");
}
function stopDriveArbiter() {
  ...
  unsubscribers3.length = 0;
  persistState9();
  flushPersist(join27(storageDir11, "drive-arbiter.json"));
  logger6?.info("BrainAgent DriveArbiter: stopped.");
}
function arbitrate() {
...
function loadState11() {
  try {
    const path = join27(storageDir11, "drive-arbiter.json");
...
function persistState9() {
  ...
  schedulePersist(join27(storageDir11, "drive-arbiter.json"), () => {
    const data = {
      driveWeights,
      lastSelectedDrive,
      lastSelectionTime,
...
function initInteroception(getters, log) {
  active18?.dispose();
  active18 = createInteroception(getters, log);
}
...
function getInteroceptiveState() {
  return active18?.getState() ?? null;
}
function buildInteroceptionContext() {
  return active18?.buildContext() ?? null;
}
...
function createTemporalAwareness(workspaceDir, cfg, log) {
  ...
  let typicalGapMs = 0;
  let totalInteractions = 0;
  const storageDir13 = join28(workspaceDir, ".brainagent");
  if (!existsSync27(storageDir13)) {
    mkdirSync27(storageDir13, { recursive: true });
  }
  try {
    const path = join28(storageDir13, "temporal-awareness.json");
  ...
  function persistState11() {
    try {
      ...
      writeFileSync22(
        join28(storageDir13, "temporal-awareness.json"),
...
function initTemporalAwareness(workspaceDir, cfg, log) {
  active19?.dispose();
  active19 = createTemporalAwareness(workspaceDir, cfg, log);
}
...
function createAttentionGate(workspaceDir) {
  const storageDir13 = workspaceDir ? join29(workspaceDir, ".brainagent", "attention") : "";
  let totalProcessed = 0;
  let totalDropped = 0;
  let totalRelevanceSum = 0;
  if (storageDir13 && !existsSync28(storageDir13)) {
    mkdirSync28(storageDir13, { recursive: true });
  }
  function loadState13() {
    if (!storageDir13) return;
    try {
      const path = join29(storageDir13, "state.json");
  ...
  function persistState11() {
    ...
    try {
      writeFileSync23(
        join29(storageDir13, "state.json"),
...
var active20;
function current9() {
  return active20 ?? (active20 = createAttentionGate(""));
}
function initAttentionGate(workspaceDir, _config) {
  active20 = createAttentionGate(workspaceDir);
}
function filterContextInjections(injections, currentInput, norepinephrineLevel, config7, maxTokenBudget) {
  return current9().filter(injections, currentInput, norepinephrineLevel, config7, maxTokenBudget);
}
...
function createPreStepHandler(deps5) {
  const { config: config7, brainConfig, getHostConfig, logger: logger9, state: state3, cycles, startCycle } = deps5;
  return async (payload, next) => {
    ...
    if (config7.modules.aiEnrichment && isAIProviderAvailable2(getHostConfig())) {
      ...
      if (brainConfig.modules.amygdala) {
        tasks.push(
          assessWithAI(input, hc, logger9).then((aiAssessment) => {
      ...
      if (brainConfig.modules.mirrorNeurons && cyc.assessment) {
        ...
        tasks.push(
          observeWithAI("default", input, snapshot, brainConfig, hc, logger9).then((model) => {
            cyc.userModel = model;
          }).catch(() => {
          })
        );
      ...
      if (brainConfig.modules.basalGanglia) {
        tasks.push(
          detectReinforcementWithAI(input, hc, logger9).then((aiSignal) => {
      ...
      ]).catch((err) => logger9.warn(String(err)));
    }
...
function initGoalExecutor(_cfg, log) {
  active21 = createGoalExecutor(log);
...
function initAutonomyEnricher(cfg, log, injectedDeps) {
  ...
  wireEventListeners2();
  logger7.info("BrainAgent AutonomyEnricher: initialized (memory-driven autonomy)");
}
function stopAutonomyEnricher() {
  ...
  unsubscribers4.length = 0;
  logger7?.info("BrainAgent AutonomyEnricher: stopped.");
}
...
function enrichWithMemories(motivation) {
  ...
  if (uniqueEpisodic.length === 0 && uniqueSemantic.length === 0) {
    logger7?.info("BrainAgent AutonomyEnricher: no relevant memories found, skipping enrichment");
    return;
  }
...
function startDreamMode(config7, logger9, neuroClawConfig) {
  if (dreamInterval) return;
  storedConfig = config7;
  storedLogger = logger9;
  ...
  if (config7.circadian?.enabled) {
    setConsolidationCallback(async () => {
      await runConsolidation(config7, logger9, neuroClawConfig, true);
    });
    logger9?.info("BrainAgent DreamMode: registered with circadian rhythm system");
  }
  setTimeout(() => void runConsolidation(config7, logger9, neuroClawConfig, false), 3e4);
  dreamInterval = setInterval(() => {
    void runConsolidation(config7, logger9, neuroClawConfig, false);
  }, intervalMs);
}
...
async function runConsolidation(config7, logger9, neuroClawConfig, circadianTriggered = false) {
  ...
  try {
    ...
    const result = await consolidate(config7, neuroClawConfig, logger9, intensityMultiplier, skipAI);
    ...
    logger9?.info(`BrainAgent DreamMode: error during consolidation \u2014 ${String(err)}`);
  } finally {
    isConsolidating = false;
  }
}
async function forceConsolidation(config7, logger9, neuroClawConfig, intensity) {
  const sleepSettings = getSleepSettings();
  const effectiveIntensity = intensity ?? sleepSettings.consolidationIntensity;
  const result = await consolidate(config7, neuroClawConfig, logger9, effectiveIntensity);
  lastConsolidation = Date.now();
  if (logger9) {
    logger9.info(
...
function initProactiveFeedback(workspaceDir, cfg, log) {
  storageDir12 = join30(workspaceDir, ".brainagent", "proactive-feedback");
  if (!existsSync29(storageDir12)) {
    mkdirSync29(storageDir12, { recursive: true });
  }
  stateFile = join30(storageDir12, "state.json");
...
function stopProactiveFeedback() {
  ...
  logger8?.info("BrainAgent ProactiveFeedback: stopped.");
}
...
function createRewardLedger(workspaceDir, config7) {
  const storageDir13 = join31(workspaceDir, ".brainagent", "reward");
  if (!existsSync30(storageDir13)) {
    mkdirSync30(storageDir13, { recursive: true });
  }
  const storageFile = join31(storageDir13, "ledger.json");
  cancelPersist(storageFile);
...
function initRewardLedger(workspaceDir, config7) {
  active22?.dispose();
  active22 = createRewardLedger(workspaceDir, config7);
}
...
function attachLlmBridge(ctx, preferredModel) {
  ...
  setCallLLMBackend(async (systemPrompt, userText, _config, logger9, maxTokens) => {
    ...
    try {
      ...
      logger9?.info(`BrainAgent LLM bridge: ctx.llm call failed \u2014 ${String(error)}`);
      return void 0;
    }
  });
  void resolveRoute();
...
function apply(ctx, config7) {
  ...
  initEmbeddings(hostConfig(), logger9);
  if (brainConfig.modules.workingMemory) {
  ...
  if (brainConfig.modules.neuralPathways) {
    initNeuralPathways(dataDir, brainConfig, logger9);
  }
  if (config7.modules.structuralPlasticity) {
    initStructuralPlasticity(dataDir, brainConfig, logger9);
  }
  if (brainConfig.modules.emotionalMemory) {
    initEmotionalMemory(dataDir, brainConfig);
  }
  if (brainConfig.modules.sessionBridge) {
    initSessionBridge(dataDir, brainConfig, logger9);
  }
  if (brainConfig.modules.dmn) {
    initDMN(dataDir, brainConfig, logger9);
  }
  if (brainConfig.modules.goalStack) {
  ...
  if (brainConfig.modules.socialDrive) {
    initSocialDrive(dataDir, brainConfig, logger9, {
      ...driveInitBase,
  ...
  if (brainConfig.modules.cognitiveHunger) {
    initCognitiveHunger(dataDir, brainConfig, logger9, {
      ...driveInitBase,
  ...
  if (brainConfig.modules.creativeDrive) {
    initCreativeDrive(dataDir, brainConfig, logger9, {
      ...driveInitBase,
  ...
  if (brainConfig.modules.masteryDrive) {
    initMasteryDrive(dataDir, brainConfig, logger9, {
      ...driveInitBase,
  ...
  if (brainConfig.modules.vitalImpulse) {
    initVitalImpulse(dataDir, brainConfig, logger9, {
  ...
  if (brainConfig.modules.goalStack) {
    initGoalExecutor(brainConfig, logger9);
  }
  if (brainConfig.modules.actionDispatcher && brainConfig.modules.vitalImpulse) {
    initAutonomyEnricher(brainConfig, logger9, {
  ...
  if (brainConfig.modules.autonomousResearch) {
    initAutonomousResearch(brainConfig, logger9, {
      callLLM,
      storeFact,
  ...
  if (brainConfig.circadian.enabled) {
    initCircadianRhythm(dataDir, brainConfig, logger9);
  }
  if (brainConfig.modules.dreamMode) {
    startDreamMode(brainConfig, logger9, hostConfig());
  }
  if (brainConfig.modules.metabolicBudget) {
    initMetabolicBudget(dataDir, brainConfig, logger9);
  }
  if (brainConfig.modules.emergentModules) {
    initEmergentModules(dataDir, brainConfig, logger9);
  }
  if (brainConfig.modules.proactiveFeedback) {
    initProactiveFeedback(dataDir, brainConfig, logger9);
  }
  ...
  if (brainConfig.modules.temporalAwareness) {
    initTemporalAwareness(dataDir, brainConfig, logger9);
  }
  if (brainConfig.modules.interoception) {
    initInteroception(
      {
  ...
  ctx.on("session/event", (_session, event) => {
    ...
    if (event.type === "turn/end") {
      void endCycle3(key).catch((err) => logger9.warn(`BrainAgent endCycle: ${String(err)}`));
    }
  });
  ...
  logger9.info("BrainAgent: cognitive architecture initialized");
  logger9.info(
