...
function mergeBrainConfig(config8) {
  ...
  for (const [key, value] of Object.entries(config8.modules)) {
  ...
  return {
    ...
    learningLoop: {
      rewardLedger: {
        ...
        ...config8.learningLoop?.rewardLedger ?? {}
      },
      strategyBandit: {
        ...DEFAULT_CONFIG.learningLoop.strategyBandit,
        ...config8.learningLoop?.strategyBandit ?? {}
      }
    }
  };
}
...
async function callLLM(systemPrompt, userText, config8, logger13, maxTokens = 500) {
  if (callBackend) {
    try {
      const bridged = await callBackend(systemPrompt, userText, config8, logger13, maxTokens);
      if (bridged !== void 0) return bridged;
    } catch (error) {
      logger13?.info(`BrainAgent LLM: bridge failed, falling back \u2014 ${String(error)}`);
    }
  }
  const provider = resolveProvider(config8);
  if (!provider) {
    logger13?.info("BrainAgent LLM: no AI provider configured, skipping");
    return null;
  }
  const userSelection = parseUserModelSelection(config8);
  if (userSelection) {
    logger13?.info(`BrainAgent LLM: calling ${provider.name} (${provider.model}) [user-selected]`);
  } else {
    logger13?.info(`BrainAgent LLM: calling ${provider.name} (${provider.model}) [auto-detected]`);
  }
  try {
    let response;
...
${userText}` }] }],
      ...
      logger13?.info(`BrainAgent LLM: Google error ${response.status}: ${errorText}`);
      return null;
    } else {
    ...
    if (!response.ok) {
      const errorText = await response.text();
      logger13?.info(`BrainAgent LLM: ${provider.name} error ${response.status}: ${errorText}`);
      return null;
    }
    ...
    logger13?.info(`BrainAgent LLM: error \u2014 ${String(error)}`);
    return null;
  }
}
...
async function getEmbedding(text, config8, logger13) {
  const result = await getEmbeddings([text], config8, logger13);
  return result?.[0] ?? null;
}
async function getEmbeddings(texts, config8, logger13) {
  ...
  try {
    const result = provider.format === "google" ? await fetchGoogleEmbeddings(texts, provider, logger13) : await fetchOpenAIEmbeddings(texts, provider, logger13);
    if (result === null && provider.name === "Ollama") {
      ollamaUnreachable = true;
    }
    return result;
  } catch (error) {
    logger13?.info(`BrainAgent Embeddings: ${provider.name} error \u2014 ${String(error)}`);
    if (provider.name === "Ollama") ollamaUnreachable = true;
    return null;
  }
}
async function fetchOpenAIEmbeddings(texts, provider, logger13) {
...
async function fetchGoogleEmbeddings(texts, provider, logger13) {
  ...
  if (!response.ok) {
    const errorText = await response.text();
    logger13?.info(`BrainAgent Embeddings: Google API error ${response.status}: ${errorText}`);
    return null;
  }
  const data = await response.json();
...
function initEmbeddings(config8, logger13) {
  const provider = resolveEmbeddingProvider(config8);
  if (!provider) {
    logger13?.info("BrainAgent Hippocampus: no embedding provider \u2014 using TF-IDF");
    return;
  }
  embeddingsConfig = config8;
  embeddingsLogger = logger13;
...
async function consolidate(config8, neuroClawConfig, logger13, intensity = 0.5, skipAIReview = false) {
  let merged = 0;
  let pruned = 0;
  let strengthened = 0;
  ...
  if (!skipAIReview && neuroClawConfig && isAIProviderAvailable(neuroClawConfig) && semanticStore.length >= 5) {
    try {
      ...
      logger13?.info(`BrainAgent Consolidation: AI review error \u2014 ${String(err)}`);
    }
  }
  persistEpisodic();
  persistSemantic();
...
async function assessWithAI(text, config8, logger13) {
  const content = await callLLM(EMOTION_PROMPT, text, config8, logger13, 100);
...
function createMirrorNeurons(workspaceDir) {
  const storageDir21 = workspaceDir ? join3(workspaceDir, ".brainagent", "users") : "";
  const userModels = /* @__PURE__ */ new Map();
  if (storageDir21 && !existsSync2(storageDir21)) {
    mkdirSync2(storageDir21, { recursive: true });
  }
  try {
    const indexPath = join3(storageDir21, "index.json");
    if (storageDir21 && existsSync2(indexPath)) {
  ...
  function persistModels() {
    ...
    try {
      ...
      writeFileSync3(join3(storageDir21, "index.json"), JSON.stringify(data, null, 2), "utf-8");
    } catch {
    }
  }
  ...
  function observe2(userId, text, amygdalaResult, config8) {
  ...
  async function observeWithAI2(userId, text, amygdalaResult, config8, neuroConfig, logger13) {
    const model = getOrCreateModel(userId);
    model.emotionHistory.push({
      timestamp: Date.now(),
      emotion: amygdalaResult.emotion,
    ...
    const alpha = 0.3;
    model.stressLevel = model.stressLevel * (1 - alpha) + (isStressed ? amygdalaResult.emotionIntensity : 0) * alpha;
    const content = await callLLM(STYLE_PROMPT, text, neuroConfig, logger13, 100);
    if (content) {
      try {
        const jsonMatch = content.match(/\{[\s\S]*\}/);
        if (jsonMatch) {
...
function observe(userId, text, amygdalaResult, config8) {
  return current().observe(userId, text, amygdalaResult, config8);
}
...
function observeWithAI(userId, text, amygdalaResult, config8, neuroConfig, logger13) {
  return current().observeWithAI(userId, text, amygdalaResult, config8, neuroConfig, logger13);
}
...
function createPredictiveEngine(workspaceDir) {
  const storageDir21 = workspaceDir ? join4(workspaceDir, ".brainagent", "predictions") : "";
  ...
  let lastDomain;
  if (storageDir21 && !existsSync3(storageDir21)) {
    mkdirSync3(storageDir21, { recursive: true });
  }
  function loadPatterns() {
    if (!storageDir21) return;
    try {
      const tPath = join4(storageDir21, "temporal.json");
    ...
    try {
      const sPath = join4(storageDir21, "sequential.json");
    ...
    try {
      const cPath = join4(storageDir21, "contextual.json");
...
function createBasalGanglia(workspaceDir) {
  const storageDir21 = workspaceDir ? join5(workspaceDir, ".brainagent", "habits") : "";
  let habits = [];
  let habitIndex = new VectorIndex();
  let idCounter5 = 0;
  function nextHabitId() {
    return `hab-${Date.now()}-${++idCounter5}`;
  }
  if (storageDir21 && !existsSync4(storageDir21)) {
    mkdirSync4(storageDir21, { recursive: true });
  }
  function loadHabits() {
    if (!storageDir21) return;
    try {
      const path = join5(storageDir21, "habits.json");
      if (existsSync4(path)) {
  ...
  function persistHabits() {
    if (!storageDir21) return;
    try {
      writeFileSync5(join5(storageDir21, "habits.json"), JSON.stringify(habits, null, 2), "utf-8");
    } catch {
    }
  }
...
async function detectReinforcementWithAI(text, config8, logger13) {
  const content = await callLLM(REINFORCEMENT_PROMPT, text, config8, logger13, 100);
...
var pathwayLogger;
var pathwayUnsubs = [];
function initNeuralPathways(workspaceDir, config8, logger13) {
  for (const unsub of pathwayUnsubs) unsub();
  pathwayUnsubs = [];
  activatedPathways.clear();
  pathwayLogger = logger13;
...
function createWorkingMemory(workspaceDir, opts) {
  const storageDir21 = workspaceDir ? join9(workspaceDir, ".brainagent", "working-memory") : "";
  const maxEntries2 = opts.maxEntries;
  const summaryMaxLength = opts.summaryMaxLength;
  let entries2 = [];
  if (storageDir21 && !existsSync8(storageDir21)) {
    mkdirSync8(storageDir21, { recursive: true });
  }
  function loadState21() {
    if (!storageDir21) return;
    try {
      const path = join9(storageDir21, "state.json");
      if (existsSync8(path)) {
        const data = JSON.parse(readFileSync8(path, "utf-8"));
        entries2 = Array.isArray(data) ? data : [];
      }
    } catch {
      entries2 = [];
    }
  ...
  function persistState17() {
    if (!storageDir21) return;
    try {
      writeFileSync8(join9(storageDir21, "state.json"), JSON.stringify(entries2, null, 2), "utf-8");
    } catch {
    }
  }
  loadState21();
  ...
  function buildContext(_currentInput) {
    ...
    bus.emitSync("working-memory:context-built", { entriesUsed: entries2.length });
    return lines.join("\n");
  }
  function clear() {
    entries2 = [];
  ...
  function getStats2() {
    return {
      ...
      oldestTimestamp: entries2.length > 0 ? entries2[0].timestamp : null,
      newestTimestamp: entries2.length > 0 ? entries2[entries2.length - 1].timestamp : null
    };
  }
...
async function extractFactsWithAI(text, config8, logger13) {
  const content = await callLLM(EXTRACTION_PROMPT, text, config8, logger13, 500);
  if (!content) return [];
  const facts = parseFactsFromResponse(content);
  if (facts.length > 0) {
    logger13?.info(`BrainAgent AI Extractor: found ${facts.length} facts`);
  }
  return facts;
}
...
async function validateAsync(response, originalInput, config8, classification, assessment, userModel, logger13) {
...
\u041E\u0442\u0432\u0435\u0442: ${response}`;
    const aiResponse = await callLLM(QUALITY_PROMPT, userText, config8, logger13, 300);
    if (aiResponse) {
    ...
    logger13?.info(`BrainAgent Cerebellum: AI validation error \u2014 ${String(err)}`);
  }
  return heuristicResult;
}
...
async function extractProcedureAsync(text, config8, classification, logger13) {
  if (isAIProviderAvailable(config8)) {
    try {
      const aiResponse = await callLLM(PROCEDURE_PROMPT, text, config8, logger13, 300);
      if (aiResponse) {
        const parsed = parseProcedureResponse(aiResponse, classification);
        if (parsed) return parsed;
      }
    } catch (err) {
      logger13?.info(`BrainAgent Procedural: AI extraction error \u2014 ${String(err)}`);
    }
  }
...
function initEmergentModules(workspaceDir, config8, log) {
  ...
  state3.minOccurrences = config8.emergentModules.minOccurrences;
  state3.minReward = config8.emergentModules.minRewardForEstablishment;
  loadState5();
...
function initMetabolicBudget(workspaceDir, config8, log) {
  ...
  state4.totalBudget = config8.metabolicBudget.totalBudget;
  state4.regenRate = config8.metabolicBudget.regenRate;
  loadState6();
  initPeriodStats();
  logger3?.info(
...
async function generateQualiaAsync(emotion, intensity, domain, neuromodulators, config8, logger13) {
...
${neuroDesc}`;
  try {
    ...
    const response = await callLLM(QUALIA_METAPHOR_PROMPT, userMessage, config8, logger13, 200);
    if (!response) {
      logger13?.info("BrainAgent Qualia: LLM returned null, falling back to template");
      return generateQualia(emotion, intensity, domain, neuromodulators);
    }
    const parsed = parseLLMQualiaResponse(response);
    if (!parsed) {
      logger13?.info("BrainAgent Qualia: failed to parse LLM response, falling back to template");
      return generateQualia(emotion, intensity, domain, neuromodulators);
    }
    const qualia = {
      timestamp: now,
    ...
    persistState5();
    logger13?.info(`BrainAgent Qualia: LLM-generated metaphor for ${emotion}/${domain}`);
    return qualia;
  } catch (err) {
    logger13?.info(`BrainAgent Qualia: AI metaphor generation failed \u2014 ${String(err)}`);
    return generateQualia(emotion, intensity, domain, neuromodulators);
  }
}
...
function createTemporalBinding(workspaceDir, opts) {
  const storageDir21 = workspaceDir ? join18(workspaceDir, ".brainagent", "temporal-binding") : "";
  const maxMoments = opts.maxMoments;
  let moments = [];
  let idCounter5 = 0;
  if (storageDir21 && !existsSync17(storageDir21)) {
    mkdirSync17(storageDir21, { recursive: true });
  }
  function loadState21() {
    if (!storageDir21) return;
    try {
      const path = join18(storageDir21, "state.json");
  ...
  function persistState17() {
    if (!storageDir21) return;
    try {
      writeFileSync17(join18(storageDir21, "state.json"), JSON.stringify(moments, null, 2), "utf-8");
    } catch {
    }
  }
  loadState21();
...
function initTemporalBinding(workspaceDir, config8) {
  active5 = createTemporalBinding(workspaceDir, { maxMoments: config8.temporalBinding.maxMoments });
}
...
function expireGoals() {
  ...
  if (goals.length > 60) {
    const active11 = goals.filter((g) => g.status === "pending" || g.status === "triggered");
    const inactive = goals.filter((g) => g.status === "completed" || g.status === "expired").slice(-30);
    goals = [...active11, ...inactive];
    changed = true;
  }
...
async function extractGoalsFromConversation(userMessage, config8, logger13) {
  if (!isAIProviderAvailable(config8)) {
    logger13?.info("BrainAgent GoalStack: no AI provider available, skipping goal extraction");
    return [];
  }
  logger13?.info("BrainAgent GoalStack: extracting goals from conversation...");
  const response = await callLLM(GOAL_EXTRACTION_PROMPT, userMessage, config8, logger13, 300);
  if (!response) {
    logger13?.info("BrainAgent GoalStack: LLM returned null/empty response");
    return [];
  }
  logger13?.info(`BrainAgent GoalStack: LLM response received (${response.length} chars)`);
  let parsed;
  try {
    let jsonStr = response.trim();
    const jsonMatch = jsonStr.match(/\[[\s\S]*\]/);
    ...
    logger13?.info("BrainAgent GoalStack: failed to parse LLM goal extraction response");
    return [];
  }
  const createdGoals = [];
...
function initCircadianRhythm(workspaceDir, config8, log) {
  ...
  logger5?.info(
    `Circadian: initialized in ${state6.phase} phase (idle threshold: ${config8.circadian.idleThresholdMs / 1e3}s)`
  );
}
...
var DriveEngine = class {
  constructor(spec, config8, workspaceDir, circadianEnabled3, deps5, logger13) {
    this.spec = spec;
    this.config = config8;
    this.circadianEnabled = circadianEnabled3;
    this.deps = deps5;
    this.logger = logger13;
    const storageDir21 = join23(workspaceDir, ".brainagent", spec.id);
    if (!existsSync22(storageDir21)) {
      mkdirSync22(storageDir21, { recursive: true });
    }
    this.stateFile = join23(storageDir21, "state.json");
...
function collectActiveDrives(drives, threshold) {
  ...
  if (drives.social) {
    ...
    if (s.need >= threshold) active11.push({ name: "social", need: s.need, needLevel: s.needLevel });
  }
  if (drives.cognitive) {
    const c = drives.cognitive();
    if (c.need >= threshold) active11.push({ name: "cognitive", need: c.need, needLevel: c.needLevel });
  }
  if (drives.creative) {
    const c = drives.creative();
    if (c.need >= threshold) active11.push({ name: "creative", need: c.need, needLevel: c.needLevel });
  }
  if (drives.mastery) {
    const m = drives.mastery();
    if (m.need >= threshold) active11.push({ name: "mastery", need: m.need, needLevel: m.needLevel });
  }
  return active11;
}
function synthesizeAutonomousCycleState(brainConfig, drives, cycle) {
  const active11 = collectActiveDrives(drives, 0.5);
  if (active11.length === 0) return;
  const ranked = active11.map((d) => {
...
function createAutonomousDeliverer(deps5) {
  const { state: state8, brainConfig, minGapMs, logger: logger13 } = deps5;
  return function enqueueAutonomousIntent(text) {
    const trimmed = text.trim();
    if (!trimmed) return;
    if (state8.previousCycleWasAutonomous) {
      logger13.info("BrainAgent Autonomy: intent suppressed \u2014 previous cycle was autonomous");
      return;
    }
    if (Date.now() - state8.lastAutonomousDeliveryAt < minGapMs) {
      logger13.info("BrainAgent Autonomy: intent suppressed \u2014 minimum gap not elapsed");
      return;
    }
    ...
    if (!agent) {
      logger13.warn("BrainAgent Autonomy: no live agent \u2014 autonomous intent dropped");
      return;
    }
...
function createAutonomousIntentResolver(deps5) {
  ...
  return () => {
    ...
    const active11 = collectActiveDrives(drives, 0.8);
    if (active11.length > 0) {
      const strongest = active11.sort((a, b) => b.need - a.need)[0];
...
function createCycleEngine(deps5) {
  const { config: config8, brainConfig, getHostConfig, logger: logger13, markActivation, state: state8 } = deps5;
  ...
  async function endCycle3(key) {
    ...
    const aiAvailable = config8.modules.aiEnrichment && isAIProviderAvailable2(getHostConfig());
    if (brainConfig.modules.cerebellum && cycle.responseText.trim()) {
      ...
      if (result.issues.length > 0) {
        logger13.warn(`BrainAgent Cerebellum: quality issues \u2014 ${result.issues.join("; ")}`);
    ...
    if (config8.modules.semanticExtraction && semanticSource.length > 15 && isFactWorthy(semanticSource, cycle.classification)) {
      ...
      if (aiAvailable) {
        try {
          const aiFacts = await extractFactsWithAI(semanticSource, getHostConfig(), logger13);
          if (aiFacts.length > 0) {
          ...
          logger13.info(`BrainAgent Semantic: AI extraction failed, falling back \u2014 ${String(err)}`);
        }
      }
      if (!factsStored) {
      ...
      if (factsStored && brainConfig.modules.curiosityDrive) {
        ...
        for (const gap of getOpenGaps()) {
          if (inputLower.includes(gap.topic.toLowerCase())) {
            ...
            logger13.info(`BrainAgent Curiosity: gap filled for "${gap.topic}" via new fact storage`);
          }
        }
      }
    }
    if (config8.modules.proceduralExtraction && input.length > 10 && isProcedural(input, cycle.classification)) {
      const procedure = await extractProcedureAsync(input, getHostConfig(), cycle.classification, logger13);
      if (procedure && procedure.confidence > 0.5) {
        storeWorkflow(procedure.description, procedure.triggerPattern, procedure.steps);
        logger13.info(`BrainAgent Procedural: stored workflow "${procedure.description}"`);
      }
    }
    ...
    if (brainConfig.modules.dmn) {
      ...
      if (wakeInteractionCount >= brainConfig.dmn.wakeThoughtInterval) {
        ...
        void runAssociationFinding(brainConfig).catch((err) => {
          logger13.info(`BrainAgent DMN: association finding error: ${err}`);
        });
      }
    }
    if (brainConfig.modules.goalStack && input.length > 10) {
      ...
      if (goalExtractionCounter >= brainConfig.goalStack.extractionInterval) {
        ...
        void extractGoalsFromConversation(input, getHostConfig(), logger13).catch((err) => {
          logger13.info(`BrainAgent GoalStack: extraction error: ${err}`);
        });
      }
    }
    if (brainConfig.modules.goalStack) {
...
var lastChoices = {};
function initStrategyBandit(workspaceDir, config8) {
  stopStrategyBandit();
  const storageDir21 = join25(workspaceDir, ".brainagent", "bandit");
  if (!existsSync24(storageDir21)) {
    mkdirSync24(storageDir21, { recursive: true });
  }
  storageFile = join25(storageDir21, "state.json");
  cancelPersist(storageFile);
  explorationConstant = config8.learningLoop.strategyBandit.explorationConstant;
  attributionWindowMs = config8.learningLoop.strategyBandit.attributionWindowMs;
  state7 = {};
  lastChoices = {};
  loadState16();
  unsubscribers2.push(
...
function initInteroception(getters, log) {
  ...
  logger9?.info("BrainAgent Interoception: initialized");
}
function stopInteroception() {
  ...
  unsubscribers5.length = 0;
  logger9?.info("BrainAgent Interoception: stopped.");
}
function evaluate() {
...
function initTemporalAwareness(workspaceDir, cfg, log) {
  config4 = cfg.temporalAwareness;
  logger10 = log;
  timestamps = [];
  typicalGapMs = 0;
  totalInteractions2 = 0;
  storageDir19 = join28(workspaceDir, ".brainagent");
  if (!existsSync27(storageDir19)) {
    mkdirSync27(storageDir19, { recursive: true });
  }
  loadState19();
  logger10?.info(
...
function stopTemporalAwareness() {
  persistState15();
  logger10?.info("BrainAgent TemporalAwareness: stopped.");
}
function recordInteraction() {
  ...
  timestamps.push(now);
  if (timestamps.length > config4.gapHistorySize) {
    timestamps = timestamps.slice(-config4.gapHistorySize);
  }
  totalInteractions2++;
  ...
  if (lastTime > 0 && gapMs > 0) {
    if (typicalGapMs === 0) {
      ...
      typicalGapMs = typicalGapMs * (1 - config4.gapEmaAlpha) + gapMs * config4.gapEmaAlpha;
    }
    const temporalSurprise = typicalGapMs > 0 ? gapMs / typicalGapMs : 1;
    if (temporalSurprise >= config4.longAbsenceMultiplier && gapMs > 60 * 1e3) {
  ...
  if (density >= config4.highDensityThreshold) {
...
function computeDensity(now) {
  if (!config4 || timestamps.length < 2) return 0;
  const windowStart = now - config4.densityWindowMs;
...
function loadState19() {
  try {
    const path = join28(storageDir19, "temporal-awareness.json");
...
function persistState15() {
  ...
  try {
    ...
    writeFileSync22(
      join28(storageDir19, "temporal-awareness.json"),
...
function buildTemporalContext2() {
  if (!config4 || timestamps.length < 2) return null;
  const now = Date.now();
  ...
  const density = computeDensity(now);
  const lines = [];
  if (temporalSurprise >= config4.longAbsenceMultiplier && currentGapMs > 60 * 1e3) {
    lines.push(
...
function createAttentionGate(workspaceDir) {
  const storageDir21 = workspaceDir ? join29(workspaceDir, ".brainagent", "attention") : "";
  let totalProcessed = 0;
  let totalDropped = 0;
  let totalRelevanceSum = 0;
  if (storageDir21 && !existsSync28(storageDir21)) {
    mkdirSync28(storageDir21, { recursive: true });
  }
  function loadState21() {
    if (!storageDir21) return;
    try {
      const path = join29(storageDir21, "state.json");
  ...
  function persistState17() {
    ...
    try {
      writeFileSync23(
        join29(storageDir21, "state.json"),
  ...
  loadState21();
  function filter(injections, currentInput, norepinephrineLevel, config8, maxTokenBudget) {
...
var active9;
function current6() {
  return active9 ?? (active9 = createAttentionGate(""));
}
function initAttentionGate(workspaceDir, _config) {
  active9 = createAttentionGate(workspaceDir);
}
function filterContextInjections(injections, currentInput, norepinephrineLevel, config8, maxTokenBudget) {
  return current6().filter(injections, currentInput, norepinephrineLevel, config8, maxTokenBudget);
}
...
function decideProcessingPath(classification, config8) {
  const threshold = COMPLEXITY_ORDER2[config8.dualProcess.system2Threshold];
...
async function executeResearch(topic) {
  if (!config5?.enabled || !deps3) return null;
  const now = Date.now();
...
async function fetchPages(results) {
  ...
  const pages = [];
  for (const result of results) {
    if (totalChars >= config5.maxTotalChars) break;
    try {
...
function createPreStepHandler(deps5) {
  const { config: config8, brainConfig, getHostConfig, logger: logger13, state: state8, cycles, startCycle } = deps5;
  return async (payload, next) => {
    ...
    if (config8.modules.aiEnrichment && isAIProviderAvailable2(getHostConfig())) {
      const hc = getHostConfig();
      const tasks = [];
      if (brainConfig.modules.amygdala) {
        tasks.push(
          assessWithAI(input, hc, logger13).then((aiAssessment) => {
      ...
      if (brainConfig.modules.mirrorNeurons && cyc.assessment) {
        ...
        tasks.push(
          observeWithAI("default", input, snapshot, brainConfig, hc, logger13).then((model) => {
            cyc.userModel = model;
          }).catch(() => {
          })
        );
      ...
      if (brainConfig.modules.basalGanglia) {
        tasks.push(
          detectReinforcementWithAI(input, hc, logger13).then((aiSignal) => {
      ...
      ]).catch((err) => logger13.warn(String(err)));
    }
...
function initGoalExecutor(_cfg, log) {
  active10 = createGoalExecutor(log);
...
function wireEventListeners2() {
  ...
  unsubscribers6.push(unsubFired);
}
function enrichWithMemories(motivation) {
  if (!deps4 || !config6) return;
...
function startDreamMode(config8, logger13, neuroClawConfig) {
  if (dreamInterval) return;
  storedConfig = config8;
  storedLogger = logger13;
  ...
  if (config8.circadian?.enabled) {
    setConsolidationCallback(async () => {
      await runConsolidation(config8, logger13, neuroClawConfig, true);
    });
    logger13?.info("BrainAgent DreamMode: registered with circadian rhythm system");
  }
  setTimeout(() => void runConsolidation(config8, logger13, neuroClawConfig, false), 3e4);
  dreamInterval = setInterval(() => {
    void runConsolidation(config8, logger13, neuroClawConfig, false);
  }, intervalMs);
}
...
async function runConsolidation(config8, logger13, neuroClawConfig, circadianTriggered = false) {
  ...
  try {
    ...
    const result = await consolidate(config8, neuroClawConfig, logger13, intensityMultiplier, skipAI);
    ...
    logger13?.info(`BrainAgent DreamMode: error during consolidation \u2014 ${String(err)}`);
  } finally {
    isConsolidating = false;
  }
}
async function forceConsolidation(config8, logger13, neuroClawConfig, intensity) {
  const sleepSettings = getSleepSettings();
  const effectiveIntensity = intensity ?? sleepSettings.consolidationIntensity;
  const result = await consolidate(config8, neuroClawConfig, logger13, effectiveIntensity);
  lastConsolidation = Date.now();
  if (logger13) {
    logger13.info(
...
function initProactiveFeedback(workspaceDir, cfg, log) {
  storageDir20 = join30(workspaceDir, ".brainagent", "proactive-feedback");
  if (!existsSync29(storageDir20)) {
    mkdirSync29(storageDir20, { recursive: true });
  }
  stateFile = join30(storageDir20, "state.json");
...
function stopProactiveFeedback() {
  ...
  logger12?.info("BrainAgent ProactiveFeedback: stopped.");
}
...
function initRewardLedger(workspaceDir, config8) {
  stopRewardLedger();
  const storageDir21 = join31(workspaceDir, ".brainagent", "reward");
  if (!existsSync30(storageDir21)) {
    mkdirSync30(storageDir21, { recursive: true });
  }
  storageFile2 = join31(storageDir21, "ledger.json");
  cancelPersist(storageFile2);
  maxEntries = config8.learningLoop.rewardLedger.maxEntries;
  entries = [];
  idCounter4 = 0;
  loadLedger();
  unsubscribers7.push(
...
function recordReward(source, contribution, context) {
...
function loadLedger() {
  if (!storageFile2 || !existsSync30(storageFile2)) return;
  try {
...
function attachLlmBridge(ctx, preferredModel) {
  ...
  setCallLLMBackend(async (systemPrompt, userText, _config, logger13, maxTokens) => {
    ...
    try {
      ...
      logger13?.info(`BrainAgent LLM bridge: ctx.llm call failed \u2014 ${String(error)}`);
      return void 0;
    }
  });
  void resolveRoute();
...
function apply(ctx, config8) {
  ...
  const enqueueAutonomousIntent = createAutonomousDeliverer({
    state: state8,
    brainConfig,
    minGapMs: config8.autonomousMinGapMs ?? 10 * 60 * 1e3,
    logger: logger13,
    pickAgent,
  ...
  initEmbeddings(hostConfig(), logger13);
  if (brainConfig.modules.workingMemory) {
  ...
  if (brainConfig.modules.neuralPathways) {
    initNeuralPathways(dataDir, brainConfig, logger13);
  }
  if (config8.modules.structuralPlasticity) {
    initStructuralPlasticity(dataDir, brainConfig, logger13);
  }
  if (brainConfig.modules.emotionalMemory) {
    initEmotionalMemory(dataDir, brainConfig);
  }
  if (brainConfig.modules.sessionBridge) {
    initSessionBridge(dataDir, brainConfig, logger13);
  }
  if (brainConfig.modules.dmn) {
    initDMN(dataDir, brainConfig, logger13);
  }
  if (brainConfig.modules.goalStack) {
  ...
  if (brainConfig.modules.socialDrive) {
    initSocialDrive(dataDir, brainConfig, logger13, {
      ...driveInitBase,
  ...
  if (brainConfig.modules.cognitiveHunger) {
    initCognitiveHunger(dataDir, brainConfig, logger13, {
      ...driveInitBase,
  ...
  if (brainConfig.modules.creativeDrive) {
    initCreativeDrive(dataDir, brainConfig, logger13, {
      ...driveInitBase,
  ...
  if (brainConfig.modules.masteryDrive) {
    initMasteryDrive(dataDir, brainConfig, logger13, {
      ...driveInitBase,
  ...
  if (brainConfig.modules.vitalImpulse) {
    initVitalImpulse(dataDir, brainConfig, logger13, {
  ...
  if (brainConfig.modules.goalStack) {
    initGoalExecutor(brainConfig, logger13);
  }
  if (brainConfig.modules.actionDispatcher && brainConfig.modules.vitalImpulse) {
    initAutonomyEnricher(brainConfig, logger13, {
  ...
  if (brainConfig.modules.autonomousResearch) {
    initAutonomousResearch(brainConfig, logger13, {
      callLLM,
      storeFact,
  ...
  if (brainConfig.circadian.enabled) {
    initCircadianRhythm(dataDir, brainConfig, logger13);
  }
  if (brainConfig.modules.dreamMode) {
    startDreamMode(brainConfig, logger13, hostConfig());
  }
  if (brainConfig.modules.metabolicBudget) {
    initMetabolicBudget(dataDir, brainConfig, logger13);
  }
  if (brainConfig.modules.emergentModules) {
    initEmergentModules(dataDir, brainConfig, logger13);
  }
  if (brainConfig.modules.proactiveFeedback) {
    initProactiveFeedback(dataDir, brainConfig, logger13);
  }
  ...
  if (brainConfig.modules.temporalAwareness) {
    initTemporalAwareness(dataDir, brainConfig, logger13);
  }
  if (brainConfig.modules.interoception) {
    initInteroception(
      {
  ...
  ctx.on("session/event", (_session, event) => {
    ...
    if (event.type === "turn/end") {
      void endCycle3(key).catch((err) => logger13.warn(`BrainAgent endCycle: ${String(err)}`));
    }
  });
  ...
  logger13.info("BrainAgent: cognitive architecture initialized");
  logger13.info(
