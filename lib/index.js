...
function buildHostConfig(options = {}) {
  ...
  const config6 = { models: { providers: providers2 } };
  if (options.model) {
...
function resolveProvider(config6) {
  const providers2 = config6.models?.providers;
...
function createMirrorNeurons(workspaceDir) {
  const storageDir10 = workspaceDir ? join3(workspaceDir, ".brainagent", "users") : "";
  const userModels = /* @__PURE__ */ new Map();
  if (storageDir10 && !existsSync2(storageDir10)) {
    mkdirSync2(storageDir10, { recursive: true });
  }
  try {
    const indexPath = join3(storageDir10, "index.json");
    if (storageDir10 && existsSync2(indexPath)) {
  ...
  function persistModels() {
    ...
    try {
      ...
      writeFileSync3(join3(storageDir10, "index.json"), JSON.stringify(data, null, 2), "utf-8");
    } catch {
    }
  }
...
function updateKnowledgeModelToM(model, text, domainLimit) {
  ...
  for (const domain of domainSignals) {
    const current13 = model.knowledgeModel[domain];
    if (!current13) {
      model.knowledgeModel[domain] = "beginner";
    } else if (current13 === "beginner") {
...
function createPredictiveEngine(workspaceDir) {
  const storageDir10 = workspaceDir ? join4(workspaceDir, ".brainagent", "predictions") : "";
  ...
  let lastDomain;
  if (storageDir10 && !existsSync3(storageDir10)) {
    mkdirSync3(storageDir10, { recursive: true });
  }
  function loadPatterns() {
    if (!storageDir10) return;
    try {
      const tPath = join4(storageDir10, "temporal.json");
    ...
    try {
      const sPath = join4(storageDir10, "sequential.json");
    ...
    try {
      const cPath = join4(storageDir10, "contextual.json");
...
function createBasalGanglia(workspaceDir) {
  const storageDir10 = workspaceDir ? join5(workspaceDir, ".brainagent", "habits") : "";
  let habits = [];
  ...
  if (storageDir10 && !existsSync4(storageDir10)) {
    mkdirSync4(storageDir10, { recursive: true });
  }
  function loadHabits() {
    if (!storageDir10) return;
    try {
      const path = join5(storageDir10, "habits.json");
      if (existsSync4(path)) {
  ...
  function persistHabits() {
    if (!storageDir10) return;
    try {
      writeFileSync5(join5(storageDir10, "habits.json"), JSON.stringify(habits, null, 2), "utf-8");
    } catch {
    }
  }
...
function initIntrospection(workspaceDir, config6) {
  storageDir = join6(workspaceDir, ".brainagent", "introspection");
  if (!existsSync5(storageDir)) {
    mkdirSync5(storageDir, { recursive: true });
  }
  maxTraces = config6.introspection.maxTraces;
  injectConfidence = config6.introspection.injectConfidence;
  maxSelfDialogue = config6.introspection.maxSelfDialogue;
  maxMetaSnapshots = config6.introspection.maxMetaSnapshots;
  traces = [];
  currentTrace = null;
  selfDialogue = [];
  metaSnapshots = [];
...
function createDopamineSystem(workspaceDir) {
  const storageDir10 = join7(workspaceDir, ".brainagent", "neuromodulators");
  if (!existsSync6(storageDir10)) {
    mkdirSync6(storageDir10, { recursive: true });
  }
  cancelPersist(join7(storageDir10, "state.json"));
  let state3 = {
    dopamine: 0.5,
    serotonin: 0.6,
    norepinephrine: 0.3,
  ...
  function loadState10() {
    try {
      const path = join7(storageDir10, "state.json");
      if (existsSync6(path)) {
...
function initDopamineSystem(workspaceDir) {
  active5?.dispose();
  active5 = createDopamineSystem(workspaceDir);
}
function processInteractionOutcome(params, config6) {
  ...
  return active5.processOutcome(params, config6);
}
function markNovelty() {
  active5?.markNovelty();
}
function getNeuromodulatorState() {
  return active5?.getState() ?? {
    dopamine: 0.5,
    serotonin: 0.6,
    norepinephrine: 0.3,
    acetylcholine: 0.4
...
function getAttentionLevel() {
  return active5?.getAttentionLevel() ?? 0.3;
}
function getDopamineStats() {
  return active5?.getStats() ?? {
...
function createNeuralPathways(workspaceDir, config6, logger7) {
  const storageDir10 = join8(workspaceDir, ".brainagent", "synapses");
  if (!existsSync7(storageDir10)) {
    mkdirSync7(storageDir10, { recursive: true });
  }
  ...
  function loadSynapticState() {
    try {
      const path = join8(storageDir10, "weights.json");
      if (existsSync7(path)) {
  ...
  function saveSynapticState() {
    try {
      writeFileSync7(
        join8(storageDir10, "weights.json"),
...
function initNeuralPathways(workspaceDir, config6, logger7) {
  active6?.dispose();
  active6 = createNeuralPathways(workspaceDir, config6, logger7);
}
function resetCycleState() {
  active6?.resetCycleState();
}
function getPathwayStats() {
  return active6?.getPathwayStats() ?? {
    pathwayCount: 8,
...
function getSynapticStats() {
  return active6?.getSynapticStats() ?? computeSynapticStats(createDefaultSynapticState());
}
function buildNeuromodulatorContext() {
  return active6?.buildNeuromodulatorContext();
}
...
function createWorkingMemory(workspaceDir, opts) {
  const storageDir10 = workspaceDir ? join9(workspaceDir, ".brainagent", "working-memory") : "";
  const maxEntries = opts.maxEntries;
  const summaryMaxLength = opts.summaryMaxLength;
  let entries = [];
  if (storageDir10 && !existsSync8(storageDir10)) {
    mkdirSync8(storageDir10, { recursive: true });
  }
  function loadState10() {
    if (!storageDir10) return;
    try {
      const path = join9(storageDir10, "state.json");
      if (existsSync8(path)) {
  ...
  function persistState8() {
    if (!storageDir10) return;
    try {
      writeFileSync8(join9(storageDir10, "state.json"), JSON.stringify(entries, null, 2), "utf-8");
    } catch {
    }
  }
  loadState10();
...
function initStructuralPlasticity(workspaceDir, config6, log) {
  ...
  logger = log;
  storageDir2 = join10(workspaceDir, ".brainagent", "structural");
  if (!existsSync9(storageDir2)) {
    mkdirSync9(storageDir2, { recursive: true });
  }
  loadState2();
  logger?.info(
...
function loadState2() {
  try {
    const path = join10(storageDir2, "state.json");
    if (existsSync9(path)) {
...
function saveState() {
  try {
    writeFileSync9(join10(storageDir2, "state.json"), JSON.stringify(state, null, 2), "utf-8");
  } catch {
  }
}
...
function createLearningCoordinator(workspaceDir, config6) {
  const storageDir10 = join11(workspaceDir, ".brainagent", "learning");
  if (!existsSync10(storageDir10)) {
    mkdirSync10(storageDir10, { recursive: true });
  }
  ...
  function loadState10() {
    try {
      const path = join11(storageDir10, "coordinator.json");
  ...
  function persistState8() {
    try {
      ...
      writeFileSync10(
        join11(storageDir10, "coordinator.json"),
        JSON.stringify(
          {
...
function initLearningCoordinator(workspaceDir, config6) {
  active8?.dispose();
  active8 = createLearningCoordinator(workspaceDir, config6);
}
...
function buildLearningContext() {
  return active8?.buildContext();
}
function recordRecurringIssue(issueType) {
  return active8?.recordRecurringIssue(issueType);
}
function recordDomainPerformance(domain, reward, errorIssues = []) {
  active8?.recordDomainPerformance(domain, reward, errorIssues);
}
function buildCapabilityContext(currentDomain) {
  return active8?.buildCapabilityContext(currentDomain);
}
...
function initEmergentModules(workspaceDir, config6, log) {
  ...
  logger2 = log;
  storageDir3 = join12(workspaceDir, ".brainagent", "emergent");
  if (!existsSync11(storageDir3)) {
    mkdirSync11(storageDir3, { recursive: true });
  }
...
function loadState3() {
  try {
    const path = join12(storageDir3, "state.json");
...
function saveState2() {
  try {
    writeFileSync11(join12(storageDir3, "state.json"), JSON.stringify(state2, null, 2), "utf-8");
  } catch {
  }
}
...
function createMetabolicBudget(workspaceDir, config6, log) {
  const storageDir10 = join13(workspaceDir, ".brainagent", "metabolic");
  if (!existsSync12(storageDir10)) {
    mkdirSync12(storageDir10, { recursive: true });
  }
  ...
  function loadState10() {
    try {
      const path = join13(storageDir10, "state.json");
  ...
  function saveState3() {
    try {
      writeFileSync12(join13(storageDir10, "state.json"), JSON.stringify(state3, null, 2), "utf-8");
    } catch {
    }
  }
  loadState10();
...
function initMetabolicBudget(workspaceDir, config6, log) {
  active9?.dispose();
  active9 = createMetabolicBudget(workspaceDir, config6, log);
}
function consumeEnergy(module, amount) {
  return active9?.consumeEnergy(module, amount) ?? true;
}
function recordPerformance(module, reward) {
  active9?.recordPerformance(module, reward);
}
function endCycle2() {
  active9?.endCycle();
}
...
function initAgentIdentity(workspaceDir, config6) {
  storageDir4 = join14(workspaceDir, ".brainagent", "identity");
  if (!existsSync13(storageDir4)) {
    mkdirSync13(storageDir4, { recursive: true });
  }
...
function loadState4() {
  ...
  try {
    const path = join14(storageDir4, "state.json");
...
function createEmotionalMemory(workspaceDir, config6) {
  const storageDir10 = workspaceDir ? join15(workspaceDir, ".brainagent", "emotional-memory") : "";
  if (storageDir10 && !existsSync14(storageDir10)) {
    mkdirSync14(storageDir10, { recursive: true });
  }
  ...
  function loadState10() {
    if (!storageDir10) return;
    try {
      const path = join15(storageDir10, "state.json");
  ...
  function persistState8() {
    ...
    try {
      writeFileSync14(
        join15(storageDir10, "state.json"),
...
function current6() {
  if (!active10) {
    active10 = createEmotionalMemory("", DEFAULT_CONFIG);
  }
  return active10;
}
function initEmotionalMemory(workspaceDir, config6) {
  active10?.dispose();
  active10 = createEmotionalMemory(workspaceDir, config6);
}
function tagEmotionalContext(emotion, intensity) {
  return current6().tagEmotionalContext(emotion, intensity);
}
async function generateQualiaAsync(emotion, intensity, domain, neuromodulators, config6, logger7) {
  return current6().generateQualiaAsync(emotion, intensity, domain, neuromodulators, config6, logger7);
}
...
function initQualiaSimulator(workspaceDir, config6) {
  storageDir5 = join16(workspaceDir, ".brainagent", "qualia-simulator");
  if (!existsSync15(storageDir5)) {
    mkdirSync15(storageDir5, { recursive: true });
  }
...
function loadState5() {
  ...
  try {
    const path = join16(storageDir5, "state.json");
...
function createDMN(workspaceDir, config6, log) {
  const storageDir10 = workspaceDir ? join17(workspaceDir, ".brainagent", "dmn") : "";
  const insights = [];
  ...
  function loadState10() {
    if (!storageDir10) return;
    try {
      const path = join17(storageDir10, "state.json");
  ...
  function persistState8() {
    ...
    try {
      writeFileSync16(
        join17(storageDir10, "state.json"),
        JSON.stringify(
          {
...
${lines}
  ...
  if (storageDir10) {
    if (!existsSync16(storageDir10)) {
      mkdirSync16(storageDir10, { recursive: true });
    }
...
function buildBackgroundThoughtContext() {
  return current7().buildBackgroundThoughtContext();
}
function getRecentUnusedInsights(maxAge = 24 * 60 * 60 * 1e3) {
  return current7().getRecentUnusedInsights(maxAge);
}
...
function createTemporalBinding(workspaceDir, opts) {
  const storageDir10 = workspaceDir ? join18(workspaceDir, ".brainagent", "temporal-binding") : "";
  const maxMoments = opts.maxMoments;
  let moments = [];
  let idCounter2 = 0;
  if (storageDir10 && !existsSync17(storageDir10)) {
    mkdirSync17(storageDir10, { recursive: true });
  }
  function loadState10() {
    if (!storageDir10) return;
    try {
      const path = join18(storageDir10, "state.json");
  ...
  function persistState8() {
    if (!storageDir10) return;
    try {
      writeFileSync17(join18(storageDir10, "state.json"), JSON.stringify(moments, null, 2), "utf-8");
    } catch {
    }
  }
  loadState10();
...
var active12;
function current8() {
  return active12 ?? (active12 = createTemporalBinding("", { maxMoments: DEFAULT_MAX_MOMENTS }));
}
function initTemporalBinding(workspaceDir, config6) {
  active12 = createTemporalBinding(workspaceDir, { maxMoments: config6.temporalBinding.maxMoments });
}
function createMoment(input, thoughts, emotion, emotionIntensity, activeMemoryIds, intentions, confidence, domain) {
  return current8().createMoment(
    input,
    thoughts,
    emotion,
    emotionIntensity,
...
function buildTemporalContext(n = 3) {
  return current8().buildContext(n);
}
...
function createSessionBridge(workspaceDir, config6, _logger) {
  const storageDir10 = workspaceDir ? join19(workspaceDir, ".brainagent", "sessions") : "";
  if (storageDir10 && !existsSync18(storageDir10)) {
    mkdirSync18(storageDir10, { recursive: true });
  }
  ...
  function loadState10() {
    if (!storageDir10) return;
    try {
      const currentPath = join19(storageDir10, "current.json");
    ...
    try {
      const lastPath = join19(storageDir10, "last.json");
  ...
  function persistCurrent() {
    ...
    try {
      writeFileSync18(
        join19(storageDir10, "current.json"),
  ...
  function persistLast() {
    if (!storageDir10) return;
    try {
      writeFileSync18(join19(storageDir10, "last.json"), JSON.stringify(lastSession, null, 2), "utf-8");
    } catch {
    }
  }
...
function current9() {
  if (!active13) {
    active13 = createSessionBridge("", DEFAULT_CONFIG);
  }
  return active13;
}
function initSessionBridge(workspaceDir, config6, logger7) {
  active13?.dispose();
  active13 = createSessionBridge(workspaceDir, config6, logger7);
}
function recordCycleForSession(input, classification, assessment, reward) {
  current9().recordCycleForSession(input, classification, assessment, reward);
}
function checkSessionGap() {
  return current9().checkSessionGap();
}
function buildSessionBridgeContext() {
  return current9().buildSessionBridgeContext();
}
function getSessionBridgeStats() {
  return current9().getSessionBridgeStats();
}
...
function createCuriosityDrive(workspaceDir, config6) {
  const storageDir10 = workspaceDir ? join20(workspaceDir, ".brainagent", "curiosity") : "";
  const gaps = [];
  let totalDetected = 0;
  ...
  function loadState10() {
    if (!storageDir10) return;
    try {
      const path = join20(storageDir10, "state.json");
  ...
  function persistState8() {
    ...
    try {
      writeFileSync19(
        join20(storageDir10, "state.json"),
        JSON.stringify(
...
${question}`;
  ...
  if (storageDir10) {
    if (!existsSync19(storageDir10)) {
      mkdirSync19(storageDir10, { recursive: true });
    }
    if (config6) {
...
var active14 = null;
function current10() {
  if (!active14) active14 = createCuriosityDrive("");
  return active14;
}
function initCuriosityDrive(workspaceDir, config6) {
  active14?.stop();
  active14 = createCuriosityDrive(workspaceDir, config6);
}
function detectKnowledgeGap(topic, domain, recallWasEmpty, predictionConfidence) {
  current10().detectKnowledgeGap(topic, domain, recallWasEmpty, predictionConfidence);
}
function buildCuriosityContext(serotoninLevel, _acetylcholineLevel) {
  return current10().buildCuriosityContext(serotoninLevel, _acetylcholineLevel);
}
function markGapFilled(topic) {
  current10().markGapFilled(topic);
}
function getCuriosityStats() {
  return current10().getStats();
}
function getOpenGaps() {
  return current10().getOpenGaps();
}
...
function initGoalStack(workspaceDir, config6) {
  storageDir6 = join21(workspaceDir, ".brainagent", "goals");
  if (!existsSync20(storageDir6)) {
    mkdirSync20(storageDir6, { recursive: true });
  }
...
function loadState6() {
  ...
  try {
    const path = join21(storageDir6, "state.json");
...
function checkGoalTriggers(input, currentEmotion, currentDomain) {
  ...
  if (triggered.length > 0) persistState4();
  return triggered;
}
function expireGoals() {
  ...
  if (goals.length > 60) {
    const active27 = goals.filter((g) => g.status === "pending" || g.status === "triggered");
    const inactive = goals.filter((g) => g.status === "completed" || g.status === "expired").slice(-30);
    goals = [...active27, ...inactive];
    changed = true;
  }
  if (changed) persistState4();
}
...
function weakenDesiresAfterFire(consecutiveFires) {
  ...
  if (desires.length !== before || changed) persistState4();
}
...
function createCircadianRhythm(workspaceDir, config6, log) {
  const storageDir10 = join22(workspaceDir, ".brainagent", "circadian");
  if (!existsSync21(storageDir10)) {
    mkdirSync21(storageDir10, { recursive: true });
  }
  ...
  function loadState10() {
    try {
      const path = join22(storageDir10, "state.json");
  ...
  function persistState8() {
    try {
      writeFileSync21(join22(storageDir10, "state.json"), JSON.stringify(state3, null, 2), "utf-8");
    } catch {
    }
  }
...
function initCircadianRhythm(workspaceDir, config6, log) {
  active15?.dispose();
  active15 = createCircadianRhythm(workspaceDir, config6, log);
  if (pendingConsolidationCallback) {
    active15.setConsolidationCallback(pendingConsolidationCallback);
  }
}
...
function setConsolidationCallback(callback) {
  ...
  active15?.setConsolidationCallback(callback);
}
function recordActivity() {
  active15?.recordActivity();
}
function getCircadianState() {
  return active15?.getState() ?? createDefaultState4();
}
function getSleepSettings() {
  return active15?.getSleepSettings() ?? {
    consolidationIntensity: 0.5,
    pruningAggressiveness: 0.3,
    synapticNormalization: false
  };
...
function isInSleepPhase() {
  return active15?.isInSleepPhase() ?? false;
}
function isInWakePhase() {
  return active15?.isInWakePhase() ?? true;
}
function forcePhase(phase) {
  active15?.forcePhase(phase);
}
function getCircadianStats() {
  const st = active15?.getState() ?? createDefaultState4();
  return {
    phase: st.phase,
...
var DriveEngine = class {
  constructor(spec, config6, workspaceDir, circadianEnabled3, deps5, logger7) {
    ...
    this.deps = deps5;
    this.logger = logger7;
    const storageDir10 = join23(workspaceDir, ".brainagent", spec.id);
    if (!existsSync22(storageDir10)) {
      mkdirSync22(storageDir10, { recursive: true });
    }
    this.stateFile = join23(storageDir10, "state.json");
...
function initSocialDrive(workspaceDir, cfg, log, injectedDeps) {
  active16?.dispose();
  active16 = createSocialDrive(workspaceDir, cfg, log, injectedDeps);
}
...
function getSatiation() {
  return active16?.getSatiation() ?? 0;
}
...
function initCognitiveHunger(workspaceDir, cfg, log, injectedDeps) {
  active17?.dispose();
  active17 = createCognitiveHunger(workspaceDir, cfg, log, injectedDeps);
}
...
function initCreativeDrive(workspaceDir, cfg, log, injectedDeps) {
  active18?.dispose();
  active18 = createCreativeDrive(workspaceDir, cfg, log, injectedDeps);
}
...
function initMasteryDrive(workspaceDir, cfg, log, injectedDeps) {
  storageDir7 = join24(workspaceDir, ".brainagent", "mastery-drive");
  if (!existsSync23(storageDir7)) {
    mkdirSync23(storageDir7, { recursive: true });
  }
  ...
  totalNeedSignals = 0;
  adaptiveDecayModifier = 1;
  cancelPersist(join24(storageDir7, "state.json"));
  loadState7();
  wireEventListeners();
  logger3.info(
...
function stopMasteryDrive() {
  ...
  unsubscribers.length = 0;
  persistState5();
  flushPersist(join24(storageDir7, "state.json"));
...
function loadState7() {
  try {
    const filePath = join24(storageDir7, "state.json");
...
function persistState5() {
  schedulePersist(join24(storageDir7, "state.json"), () => {
    const domainMap = {};
    for (const [domain, mastery] of domainSatiations) {
...
function collectActiveDrives(drives, threshold) {
  ...
  if (drives.social) {
    ...
    if (s.need >= threshold) active27.push({ name: "social", need: s.need, needLevel: s.needLevel });
  }
  if (drives.cognitive) {
    const c = drives.cognitive();
    if (c.need >= threshold) active27.push({ name: "cognitive", need: c.need, needLevel: c.needLevel });
  }
  if (drives.creative) {
    const c = drives.creative();
    if (c.need >= threshold) active27.push({ name: "creative", need: c.need, needLevel: c.needLevel });
  }
  if (drives.mastery) {
    const m = drives.mastery();
    if (m.need >= threshold) active27.push({ name: "mastery", need: m.need, needLevel: m.needLevel });
  }
  return active27;
}
function synthesizeAutonomousCycleState(brainConfig, drives, cycle) {
  const active27 = collectActiveDrives(drives, 0.5);
  if (active27.length === 0) return;
  const ranked = active27.map((d) => {
...
function createAutonomousIntentResolver(deps5) {
  ...
  return () => {
    ...
    const active27 = collectActiveDrives(drives, 0.8);
    if (active27.length > 0) {
      const strongest = active27.sort((a, b) => b.need - a.need)[0];
...
function createStrategyBandit(workspaceDir, config6) {
  const storageDir10 = join25(workspaceDir, ".brainagent", "bandit");
  if (!existsSync24(storageDir10)) {
    mkdirSync24(storageDir10, { recursive: true });
  }
  const storageFile = join25(storageDir10, "state.json");
...
function initStrategyBandit(workspaceDir, config6) {
  active19?.dispose();
  active19 = createStrategyBandit(workspaceDir, config6);
}
...
function chooseArm(decisionPoint, arms) {
  ...
  return active19.chooseArm(decisionPoint, arms);
}
...
function initVitalImpulse(workspaceDir, cfg, log, injectedDeps) {
  storageDir8 = join26(workspaceDir, ".brainagent", "vital-impulse");
  if (!existsSync25(storageDir8)) {
    mkdirSync25(storageDir8, { recursive: true });
  }
  ...
  cancelPersist(join26(storageDir8, "state.json"));
  loadState8();
  wireSignalListeners();
  logger4.info(
...
function stopVitalImpulse() {
  ...
  unsubscribers2.length = 0;
  persistState6();
  flushPersist(join26(storageDir8, "state.json"));
...
function loadState8() {
  try {
    const filePath = join26(storageDir8, "state.json");
...
function persistState6() {
  schedulePersist(join26(storageDir8, "state.json"), () => {
    const state3 = {
      lastFireTime,
      totalFires,
      totalSignalsReceived,
...
function createDriveArbiter(workspaceDir, cfg, getters, log) {
  ...
  const logger7 = log;
  const unsubscribers4 = [];
  const storageDir10 = join27(workspaceDir, ".brainagent");
  if (!existsSync26(storageDir10)) {
    mkdirSync26(storageDir10, { recursive: true });
  }
  let driveWeights = {
    social: 1,
    cognitive: 1,
  ...
  function loadState10() {
    try {
      const path = join27(storageDir10, "drive-arbiter.json");
  ...
  function persistState8() {
    if (!storageDir10) return;
    schedulePersist(join27(storageDir10, "drive-arbiter.json"), () => {
      const data = {
        driveWeights,
        lastSelectedDrive,
        lastSelectionTime,
  ...
  cancelPersist(join27(storageDir10, "drive-arbiter.json"));
  loadState10();
  function arbitrate() {
  ...
  function stop() {
    ...
    unsubscribers4.length = 0;
    persistState8();
    flushPersist(join27(storageDir10, "drive-arbiter.json"));
...
function initDriveArbiter(workspaceDir, cfg, getters, log) {
  active20?.stop();
  active20 = createDriveArbiter(workspaceDir, cfg, getters, log);
}
...
function buildArbiterContext() {
  return active20?.buildContext() ?? null;
}
...
function initInteroception(getters, log) {
  active21?.dispose();
  active21 = createInteroception(getters, log);
}
...
function createTemporalAwareness(workspaceDir, cfg, log) {
  ...
  let typicalGapMs = 0;
  let totalInteractions = 0;
  const storageDir10 = join28(workspaceDir, ".brainagent");
  if (!existsSync27(storageDir10)) {
    mkdirSync27(storageDir10, { recursive: true });
  }
  try {
    const path = join28(storageDir10, "temporal-awareness.json");
  ...
  function persistState8() {
    try {
      ...
      writeFileSync22(
        join28(storageDir10, "temporal-awareness.json"),
...
function initTemporalAwareness(workspaceDir, cfg, log) {
  active22?.dispose();
  active22 = createTemporalAwareness(workspaceDir, cfg, log);
}
...
function recordInteraction() {
  active22?.recordInteraction();
}
function getTemporalAwarenessStats() {
  return active22?.getStats() ?? {
    typicalGapMs: 0,
    lastInteractionTime: 0,
    currentGapMs: 0,
...
function createAttentionGate(workspaceDir) {
  const storageDir10 = workspaceDir ? join29(workspaceDir, ".brainagent", "attention") : "";
  let totalProcessed = 0;
  let totalDropped = 0;
  let totalRelevanceSum = 0;
  if (storageDir10 && !existsSync28(storageDir10)) {
    mkdirSync28(storageDir10, { recursive: true });
  }
  function loadState10() {
    if (!storageDir10) return;
    try {
      const path = join29(storageDir10, "state.json");
  ...
  function persistState8() {
    ...
    try {
      writeFileSync23(
        join29(storageDir10, "state.json"),
...
var active23;
function current11() {
  return active23 ?? (active23 = createAttentionGate(""));
}
function initAttentionGate(workspaceDir, _config) {
  active23 = createAttentionGate(workspaceDir);
}
function filterContextInjections(injections, currentInput, norepinephrineLevel, config6, maxTokenBudget) {
  return current11().filter(injections, currentInput, norepinephrineLevel, config6, maxTokenBudget);
}
...
function initGoalExecutor(_cfg, log) {
  active24 = createGoalExecutor(log);
...
var active25 = null;
function current12() {
  if (!active25) active25 = createDreamMode();
  return active25;
}
function startDreamMode(config6, logger7, neuroClawConfig) {
  current12().start(config6, logger7, neuroClawConfig);
}
function stopDreamMode() {
  current12().stop();
}
function forceConsolidation(config6, logger7, neuroClawConfig, intensity) {
  return current12().forceConsolidation(config6, logger7, neuroClawConfig, intensity);
}
...
function initProactiveFeedback(workspaceDir, cfg, log) {
  storageDir9 = join30(workspaceDir, ".brainagent", "proactive-feedback");
  if (!existsSync29(storageDir9)) {
    mkdirSync29(storageDir9, { recursive: true });
  }
  stateFile = join30(storageDir9, "state.json");
...
function createRewardLedger(workspaceDir, config6) {
  const storageDir10 = join31(workspaceDir, ".brainagent", "reward");
  if (!existsSync30(storageDir10)) {
    mkdirSync30(storageDir10, { recursive: true });
  }
  const storageFile = join31(storageDir10, "ledger.json");
  cancelPersist(storageFile);
...
function initRewardLedger(workspaceDir, config6) {
  active26?.dispose();
  active26 = createRewardLedger(workspaceDir, config6);
}
