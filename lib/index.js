...
async function callLLM(systemPrompt, userText, config7, logger11, maxTokens = 500) {
  if (callBackend) {
    try {
      const bridged = await callBackend(systemPrompt, userText, config7, logger11, maxTokens);
      if (bridged !== void 0) return bridged;
    } catch (error) {
      logger11?.info(`BrainAgent LLM: bridge failed, falling back \u2014 ${String(error)}`);
    }
  }
  const provider = resolveProvider(config7);
  if (!provider) {
    logger11?.info("BrainAgent LLM: no AI provider configured, skipping");
    return null;
  }
  const userSelection = parseUserModelSelection(config7);
  if (userSelection) {
    logger11?.info(`BrainAgent LLM: calling ${provider.name} (${provider.model}) [user-selected]`);
  } else {
    logger11?.info(`BrainAgent LLM: calling ${provider.name} (${provider.model}) [auto-detected]`);
  }
  try {
    let response;
...
${userText}` }] }],
      ...
      logger11?.info(`BrainAgent LLM: Google error ${response.status}: ${errorText}`);
      return null;
    } else {
    ...
    if (!response.ok) {
      const errorText = await response.text();
      logger11?.info(`BrainAgent LLM: ${provider.name} error ${response.status}: ${errorText}`);
      return null;
    }
    ...
    logger11?.info(`BrainAgent LLM: error \u2014 ${String(error)}`);
    return null;
  }
}
...
async function getEmbedding(text, config7, logger11) {
  const result = await getEmbeddings([text], config7, logger11);
  return result?.[0] ?? null;
}
async function getEmbeddings(texts, config7, logger11) {
  ...
  try {
    const result = provider.format === "google" ? await fetchGoogleEmbeddings(texts, provider, logger11) : await fetchOpenAIEmbeddings(texts, provider, logger11);
    if (result === null && provider.name === "Ollama") {
      ollamaUnreachable = true;
    }
    return result;
  } catch (error) {
    logger11?.info(`BrainAgent Embeddings: ${provider.name} error \u2014 ${String(error)}`);
    if (provider.name === "Ollama") ollamaUnreachable = true;
    return null;
  }
}
async function fetchOpenAIEmbeddings(texts, provider, logger11) {
...
async function fetchGoogleEmbeddings(texts, provider, logger11) {
  ...
  if (!response.ok) {
    const errorText = await response.text();
    logger11?.info(`BrainAgent Embeddings: Google API error ${response.status}: ${errorText}`);
    return null;
  }
  const data = await response.json();
...
function initEmbeddings(config7, logger11) {
  const provider = resolveEmbeddingProvider(config7);
  if (!provider) {
    logger11?.info("BrainAgent Hippocampus: no embedding provider \u2014 using TF-IDF");
    return;
  }
  embeddingsConfig = config7;
  embeddingsLogger = logger11;
...
async function consolidate(config7, neuroClawConfig, logger11, intensity = 0.5, skipAIReview = false) {
  let merged = 0;
  let pruned = 0;
  let strengthened = 0;
  ...
  if (!skipAIReview && neuroClawConfig && isAIProviderAvailable(neuroClawConfig) && semanticStore.length >= 5) {
    try {
      ...
      logger11?.info(`BrainAgent Consolidation: AI review error \u2014 ${String(err)}`);
    }
  }
  persistEpisodic();
  persistSemantic();
...
async function assessWithAI(text, config7, logger11) {
  const content = await callLLM(EMOTION_PROMPT, text, config7, logger11, 100);
...
function createMirrorNeurons(workspaceDir) {
  const storageDir20 = workspaceDir ? join3(workspaceDir, ".brainagent", "users") : "";
  const userModels = /* @__PURE__ */ new Map();
  if (storageDir20 && !existsSync2(storageDir20)) {
    mkdirSync2(storageDir20, { recursive: true });
  }
  try {
    const indexPath = join3(storageDir20, "index.json");
    if (storageDir20 && existsSync2(indexPath)) {
  ...
  function persistModels() {
    ...
    try {
      ...
      writeFileSync3(join3(storageDir20, "index.json"), JSON.stringify(data, null, 2), "utf-8");
    } catch {
    }
  }
  ...
  async function observeWithAI2(userId, text, amygdalaResult, config7, neuroConfig, logger11) {
    const model = getOrCreateModel(userId);
    model.emotionHistory.push({
      timestamp: Date.now(),
      emotion: amygdalaResult.emotion,
    ...
    const alpha = 0.3;
    model.stressLevel = model.stressLevel * (1 - alpha) + (isStressed ? amygdalaResult.emotionIntensity : 0) * alpha;
    const content = await callLLM(STYLE_PROMPT, text, neuroConfig, logger11, 100);
    if (content) {
      try {
        const jsonMatch = content.match(/\{[\s\S]*\}/);
        if (jsonMatch) {
...
function observeWithAI(userId, text, amygdalaResult, config7, neuroConfig, logger11) {
  return current().observeWithAI(userId, text, amygdalaResult, config7, neuroConfig, logger11);
}
...
function createPredictiveEngine(workspaceDir) {
  const storageDir20 = workspaceDir ? join4(workspaceDir, ".brainagent", "predictions") : "";
  ...
  let lastDomain;
  if (storageDir20 && !existsSync3(storageDir20)) {
    mkdirSync3(storageDir20, { recursive: true });
  }
  function loadPatterns() {
    if (!storageDir20) return;
    try {
      const tPath = join4(storageDir20, "temporal.json");
    ...
    try {
      const sPath = join4(storageDir20, "sequential.json");
    ...
    try {
      const cPath = join4(storageDir20, "contextual.json");
...
function createBasalGanglia(workspaceDir) {
  const storageDir20 = workspaceDir ? join5(workspaceDir, ".brainagent", "habits") : "";
  let habits = [];
  ...
  if (storageDir20 && !existsSync4(storageDir20)) {
    mkdirSync4(storageDir20, { recursive: true });
  }
  function loadHabits() {
    if (!storageDir20) return;
    try {
      const path = join5(storageDir20, "habits.json");
      if (existsSync4(path)) {
  ...
  function persistHabits() {
    if (!storageDir20) return;
    try {
      writeFileSync5(join5(storageDir20, "habits.json"), JSON.stringify(habits, null, 2), "utf-8");
    } catch {
    }
  }
...
async function detectReinforcementWithAI(text, config7, logger11) {
  const content = await callLLM(REINFORCEMENT_PROMPT, text, config7, logger11, 100);
...
function initDopamineSystem(workspaceDir) {
  storageDir2 = join7(workspaceDir, ".brainagent", "neuromodulators");
  if (!existsSync6(storageDir2)) {
    mkdirSync6(storageDir2, { recursive: true });
  }
  cancelPersist(join7(storageDir2, "state.json"));
  loadState2();
}
function loadState2() {
  if (!storageDir2) return;
  try {
    const path = join7(storageDir2, "state.json");
    if (existsSync6(path)) {
...
var pathwayLogger;
var pathwayUnsubs = [];
function initNeuralPathways(workspaceDir, config7, logger11) {
  for (const unsub of pathwayUnsubs) unsub();
  pathwayUnsubs = [];
  activatedPathways.clear();
  pathwayLogger = logger11;
  currentConfig = config7;
  storageDir3 = join8(workspaceDir, ".brainagent", "synapses");
  if (!existsSync7(storageDir3)) {
    mkdirSync7(storageDir3, { recursive: true });
  }
  loadSynapticState();
...
function loadSynapticState() {
  ...
  try {
    const path = join8(storageDir3, "weights.json");
    if (existsSync7(path)) {
...
function saveSynapticState() {
  ...
  try {
    writeFileSync7(
      join8(storageDir3, "weights.json"),
...
function createWorkingMemory(workspaceDir, opts) {
  const storageDir20 = workspaceDir ? join9(workspaceDir, ".brainagent", "working-memory") : "";
  const maxEntries = opts.maxEntries;
  const summaryMaxLength = opts.summaryMaxLength;
  let entries = [];
  if (storageDir20 && !existsSync8(storageDir20)) {
    mkdirSync8(storageDir20, { recursive: true });
  }
  function loadState20() {
    if (!storageDir20) return;
    try {
      const path = join9(storageDir20, "state.json");
      if (existsSync8(path)) {
  ...
  function persistState16() {
    if (!storageDir20) return;
    try {
      writeFileSync8(join9(storageDir20, "state.json"), JSON.stringify(entries, null, 2), "utf-8");
    } catch {
    }
  }
  loadState20();
...
async function extractFactsWithAI(text, config7, logger11) {
  const content = await callLLM(EXTRACTION_PROMPT, text, config7, logger11, 500);
  if (!content) return [];
  const facts = parseFactsFromResponse(content);
  if (facts.length > 0) {
    logger11?.info(`BrainAgent AI Extractor: found ${facts.length} facts`);
  }
  return facts;
}
...
async function validateAsync(response, originalInput, config7, classification, assessment, userModel, logger11) {
...
\u041E\u0442\u0432\u0435\u0442: ${response}`;
    const aiResponse = await callLLM(QUALITY_PROMPT, userText, config7, logger11, 300);
    if (aiResponse) {
    ...
    logger11?.info(`BrainAgent Cerebellum: AI validation error \u2014 ${String(err)}`);
  }
  return heuristicResult;
}
...
async function extractProcedureAsync(text, config7, classification, logger11) {
  if (isAIProviderAvailable(config7)) {
    try {
      const aiResponse = await callLLM(PROCEDURE_PROMPT, text, config7, logger11, 300);
      if (aiResponse) {
        const parsed = parseProcedureResponse(aiResponse, classification);
        if (parsed) return parsed;
      }
    } catch (err) {
      logger11?.info(`BrainAgent Procedural: AI extraction error \u2014 ${String(err)}`);
    }
  }
...
function initStructuralPlasticity(workspaceDir, config7, log) {
  ...
  logger = log;
  storageDir4 = join10(workspaceDir, ".brainagent", "structural");
  if (!existsSync9(storageDir4)) {
    mkdirSync9(storageDir4, { recursive: true });
  }
  loadState3();
  logger?.info(
...
function endCycle(reward) {
  ...
  state2.totalCycles++;
  checkForNewPathways();
  if (state2.totalCycles % currentConfig2.structuralPlasticity.pruningThreshold === 0) {
    pruneUnusedPathways();
  }
...
function loadState3() {
  try {
    const path = join10(storageDir4, "state.json");
    if (existsSync9(path)) {
...
function saveState() {
  try {
    writeFileSync9(join10(storageDir4, "state.json"), JSON.stringify(state2, null, 2), "utf-8");
  } catch {
  }
}
...
function initLearningCoordinator(workspaceDir, config7) {
  storageDir5 = join11(workspaceDir, ".brainagent", "learning");
  if (!existsSync10(storageDir5)) {
    mkdirSync10(storageDir5, { recursive: true });
  }
...
function loadState4() {
  ...
  try {
    const path = join11(storageDir5, "coordinator.json");
...
function persistState3() {
  ...
  try {
    ...
    writeFileSync10(
      join11(storageDir5, "coordinator.json"),
      JSON.stringify(
        {
...
var logger2;
var state3 = createDefaultState2();
var patternHistory = [];
...
function initEmergentModules(workspaceDir, config7, log) {
  ...
  logger2 = log;
  storageDir6 = join12(workspaceDir, ".brainagent", "emergent");
  if (!existsSync11(storageDir6)) {
    mkdirSync11(storageDir6, { recursive: true });
  }
  state3.minOccurrences = config7.emergentModules.minOccurrences;
  state3.minReward = config7.emergentModules.minRewardForEstablishment;
  loadState5();
  logger2?.info(`EmergentModules: initialized with ${state3.modules.length} discovered patterns`);
}
...
function updateExistingModule(module, reward) {
  ...
  if (module.status === "emerging" && module.occurrences >= state3.minOccurrences * 2 && module.avgReward >= state3.minReward) {
...
function checkForNewModule(participants, domain) {
  ...
  const matchingPatterns = patternHistory.filter((p) => {
    ...
    return sortedP.length === sortedParticipants.length && sortedP.every((m, i) => m === sortedParticipants[i]) && p.reward >= state3.minReward * 0.8;
  });
  if (matchingPatterns.length < state3.minOccurrences) return;
  const activeModules = state3.modules.filter((m) => m.status !== "deprecated").length;
  if (activeModules >= currentConfig4.emergentModules.maxEmergentModules) {
    const weakest = state3.modules.filter((m) => m.status !== "deprecated").sort((a, b) => a.avgReward - b.avgReward)[0];
    if (weakest && weakest.avgReward < state3.minReward * 0.5) {
  ...
  if (avgReward < state3.minReward * 0.8) return;
  const newModule = {
...
function deprecateUnusedModules() {
  ...
  for (const module of state3.modules) {
...
function getEmergentStats() {
  const emerging = state3.modules.filter((m) => m.status === "emerging").length;
  const established = state3.modules.filter((m) => m.status === "established").length;
  const deprecated = state3.modules.filter((m) => m.status === "deprecated").length;
  const topModules = state3.modules.filter((m) => m.status !== "deprecated").sort((a, b) => b.avgReward - a.avgReward).slice(0, 5).map((m) => ({
    name: m.name,
  ...
  return {
    totalDiscovered: state3.modules.length,
    emerging,
    established,
    deprecated,
    topModules
...
function loadState5() {
  try {
    const path = join12(storageDir6, "state.json");
    if (existsSync11(path)) {
      const data = JSON.parse(readFileSync11(path, "utf-8"));
      state3.modules = data.modules ?? [];
    }
  } catch {
  }
}
function saveState2() {
  try {
    writeFileSync11(join12(storageDir6, "state.json"), JSON.stringify(state3, null, 2), "utf-8");
  } catch {
  }
}
...
var logger3;
var state4 = createDefaultState3();
var periodStats = {};
...
function initMetabolicBudget(workspaceDir, config7, log) {
  ...
  logger3 = log;
  storageDir7 = join13(workspaceDir, ".brainagent", "metabolic");
  if (!existsSync12(storageDir7)) {
    mkdirSync12(storageDir7, { recursive: true });
  }
  state4.totalBudget = config7.metabolicBudget.totalBudget;
  state4.regenRate = config7.metabolicBudget.regenRate;
  loadState6();
  initPeriodStats();
  logger3?.info(
    `MetabolicBudget: initialized with total budget ${state4.totalBudget.toFixed(1)} energy units`
  );
}
function consumeEnergy(module, amount) {
  const moduleEnergy = state4.moduleEnergies[module];
  ...
  if (currentConfig5 && moduleEnergy.energy < currentConfig5.metabolicBudget.lowPowerThreshold) {
...
function endCycle2() {
  ...
  state4.cyclesSinceRebalance++;
  for (const module of ALL_MODULES2) {
    const moduleEnergy = state4.moduleEnergies[module];
    const regenAmount = state4.regenRate * moduleEnergy.performance;
    moduleEnergy.energy = Math.min(moduleEnergy.baseEnergy, moduleEnergy.energy + regenAmount);
    if (moduleEnergy.lowPowerMode && moduleEnergy.energy > currentConfig5.metabolicBudget.lowPowerThreshold * 2) {
  ...
  if (state4.cyclesSinceRebalance >= currentConfig5.metabolicBudget.rebalanceInterval) {
    rebalanceEnergy();
    state4.cyclesSinceRebalance = 0;
    saveState3();
  }
}
function rebalanceEnergy() {
  ...
  for (const perf of performances) {
    const moduleEnergy = state4.moduleEnergies[perf.module];
    const normalizedScore = Math.max(0.1, perf.score + 1) / totalScore;
    const newBaseEnergy = state4.totalBudget / ALL_MODULES2.length * (0.5 + normalizedScore);
...
function getMetabolicStats() {
  const usedEnergy = ALL_MODULES2.reduce(
    (sum, m) => sum + (state4.moduleEnergies[m]?.baseEnergy ?? 0) - (state4.moduleEnergies[m]?.energy ?? 0),
    0
  );
  const modules = ALL_MODULES2.map((m) => ({
    name: m,
    energy: state4.moduleEnergies[m].energy,
    baseEnergy: state4.moduleEnergies[m].baseEnergy,
    performance: state4.moduleEnergies[m].performance,
    lowPowerMode: state4.moduleEnergies[m].lowPowerMode
  }));
  const lowPowerModules = ALL_MODULES2.filter((m) => state4.moduleEnergies[m].lowPowerMode);
  const topPerformers = [...ALL_MODULES2].sort((a, b) => state4.moduleEnergies[b].performance - state4.moduleEnergies[a].performance).slice(0, 3);
  return {
    totalBudget: state4.totalBudget,
    usedEnergy,
    cyclesSinceRebalance: state4.cyclesSinceRebalance,
    modules,
    lowPowerModules,
    topPerformers
  };
...
function loadState6() {
  try {
    const path = join13(storageDir7, "state.json");
    if (existsSync12(path)) {
      const data = JSON.parse(readFileSync12(path, "utf-8"));
      for (const module of ALL_MODULES2) {
        if (data.moduleEnergies[module]) {
          state4.moduleEnergies[module] = data.moduleEnergies[module];
        }
      }
      state4.cyclesSinceRebalance = data.cyclesSinceRebalance ?? 0;
    }
  } catch {
  }
}
function saveState3() {
  try {
    writeFileSync12(join13(storageDir7, "state.json"), JSON.stringify(state4, null, 2), "utf-8");
  } catch {
  }
}
...
function initAgentIdentity(workspaceDir, config7) {
  storageDir8 = join14(workspaceDir, ".brainagent", "identity");
  if (!existsSync13(storageDir8)) {
    mkdirSync13(storageDir8, { recursive: true });
  }
...
function loadState7() {
  ...
  try {
    const path = join14(storageDir8, "state.json");
...
var storageDir9 = "";
var state5 = { flashbulbCount: 0, emotionMatchBoosts: 0 };
var qualiaHistory = [];
...
function initEmotionalMemory(workspaceDir, _config) {
  storageDir9 = join15(workspaceDir, ".brainagent", "emotional-memory");
  if (!existsSync14(storageDir9)) {
    mkdirSync14(storageDir9, { recursive: true });
  }
  state5 = { flashbulbCount: 0, emotionMatchBoosts: 0 };
  qualiaHistory = [];
...
function persistState5() {
  ...
  try {
    writeFileSync14(
      ...
      JSON.stringify({ ...state5, qualiaHistory }, null, 2),
      "utf-8"
    );
  } catch {
  }
...
async function generateQualiaAsync(emotion, intensity, domain, neuromodulators, config7, logger11) {
...
${neuroDesc}`;
  try {
    ...
    const response = await callLLM(QUALIA_METAPHOR_PROMPT, userMessage, config7, logger11, 200);
    if (!response) {
      logger11?.info("BrainAgent Qualia: LLM returned null, falling back to template");
      return generateQualia(emotion, intensity, domain, neuromodulators);
    }
    const parsed = parseLLMQualiaResponse(response);
    if (!parsed) {
      logger11?.info("BrainAgent Qualia: failed to parse LLM response, falling back to template");
      return generateQualia(emotion, intensity, domain, neuromodulators);
    }
    const qualia = {
      timestamp: now,
    ...
    persistState5();
    logger11?.info(`BrainAgent Qualia: LLM-generated metaphor for ${emotion}/${domain}`);
    return qualia;
  } catch (err) {
    logger11?.info(`BrainAgent Qualia: AI metaphor generation failed \u2014 ${String(err)}`);
    return generateQualia(emotion, intensity, domain, neuromodulators);
  }
}
...
function initQualiaSimulator(workspaceDir, config7) {
  storageDir10 = join16(workspaceDir, ".brainagent", "qualia-simulator");
  if (!existsSync15(storageDir10)) {
    mkdirSync15(storageDir10, { recursive: true });
  }
...
function loadState9() {
  ...
  try {
    const path = join16(storageDir10, "state.json");
...
function persistState6() {
  ...
  try {
    writeFileSync15(
      join16(storageDir10, "state.json"),
...
function describeNeuromodulatorFeel(state8) {
  const { dopamine, serotonin, norepinephrine, acetylcholine } = state8;
  const parts = [];
...
function initDMN(workspaceDir, config7, log) {
  storageDir11 = join17(workspaceDir, ".brainagent", "dmn");
  if (!existsSync16(storageDir11)) {
    mkdirSync16(storageDir11, { recursive: true });
  }
  currentConfig6 = config7;
  logger4 = log;
  insights = [];
  lastRunTimestamp = 0;
...
function loadState10() {
  ...
  try {
    const path = join17(storageDir11, "state.json");
...
function persistState7() {
  ...
  try {
    writeFileSync16(
      join17(storageDir11, "state.json"),
      JSON.stringify(
        {
...
async function runAssociationFinding(config7) {
  ...
  persistState7();
  logger4?.info(`DMN: found ${newInsights.length} cross-domain associations`);
  return newInsights;
}
...
function generateBackgroundThoughts(config7, unresolvedQuestions, recentEmotions, knowledgeGaps) {
  ...
  persistState7();
  logger4?.info(`DMN: generated ${newThoughts.length} background thoughts`);
  return newThoughts;
}
...
function createTemporalBinding(workspaceDir, opts) {
  const storageDir20 = workspaceDir ? join18(workspaceDir, ".brainagent", "temporal-binding") : "";
  const maxMoments = opts.maxMoments;
  let moments = [];
  let idCounter4 = 0;
  if (storageDir20 && !existsSync17(storageDir20)) {
    mkdirSync17(storageDir20, { recursive: true });
  }
  function loadState20() {
    if (!storageDir20) return;
    try {
      const path = join18(storageDir20, "state.json");
  ...
  function persistState16() {
    if (!storageDir20) return;
    try {
      writeFileSync17(join18(storageDir20, "state.json"), JSON.stringify(moments, null, 2), "utf-8");
    } catch {
    }
  }
  loadState20();
...
var active5;
function current5() {
  return active5 ?? (active5 = createTemporalBinding("", { maxMoments: DEFAULT_MAX_MOMENTS }));
}
function initTemporalBinding(workspaceDir, config7) {
  active5 = createTemporalBinding(workspaceDir, { maxMoments: config7.temporalBinding.maxMoments });
}
...
function initSessionBridge(workspaceDir, config7, _logger) {
  storageDir12 = join19(workspaceDir, ".brainagent", "sessions");
  if (!existsSync18(storageDir12)) {
...
function initCuriosityDrive(workspaceDir, config7) {
  storageDir13 = join20(workspaceDir, ".brainagent", "curiosity");
  if (!existsSync19(storageDir13)) {
    mkdirSync19(storageDir13, { recursive: true });
  }
...
function loadState12() {
  ...
  try {
    const path = join20(storageDir13, "state.json");
...
function persistState8() {
  ...
  try {
    writeFileSync19(
      join20(storageDir13, "state.json"),
      JSON.stringify(
...
function initGoalStack(workspaceDir, config7) {
  storageDir14 = join21(workspaceDir, ".brainagent", "goals");
  if (!existsSync20(storageDir14)) {
    mkdirSync20(storageDir14, { recursive: true });
  }
...
function loadState13() {
  ...
  try {
    const path = join21(storageDir14, "state.json");
...
function persistState9() {
  ...
  try {
    writeFileSync20(
      join21(storageDir14, "state.json"),
...
function checkGoalTriggers(input, currentEmotion, currentDomain) {
  ...
  if (triggered.length > 0) persistState9();
  return triggered;
}
function expireGoals() {
  ...
  if (goals.length > 60) {
    const active14 = goals.filter((g) => g.status === "pending" || g.status === "triggered");
    const inactive = goals.filter((g) => g.status === "completed" || g.status === "expired").slice(-30);
    goals = [...active14, ...inactive];
    changed = true;
  }
  if (changed) persistState9();
}
...
function weakenDesiresAfterFire(consecutiveFires) {
  ...
  if (desires.length !== before || changed) persistState9();
}
...
async function extractGoalsFromConversation(userMessage, config7, logger11) {
  if (!isAIProviderAvailable(config7)) {
    logger11?.info("BrainAgent GoalStack: no AI provider available, skipping goal extraction");
    return [];
  }
  logger11?.info("BrainAgent GoalStack: extracting goals from conversation...");
  const response = await callLLM(GOAL_EXTRACTION_PROMPT, userMessage, config7, logger11, 300);
  if (!response) {
    logger11?.info("BrainAgent GoalStack: LLM returned null/empty response");
    return [];
  }
  logger11?.info(`BrainAgent GoalStack: LLM response received (${response.length} chars)`);
  let parsed;
  try {
    let jsonStr = response.trim();
    const jsonMatch = jsonStr.match(/\[[\s\S]*\]/);
    ...
    logger11?.info("BrainAgent GoalStack: failed to parse LLM goal extraction response");
    return [];
  }
  const createdGoals = [];
...
function initCircadianRhythm(workspaceDir, config7, log) {
  storageDir15 = join22(workspaceDir, ".brainagent", "circadian");
  if (!existsSync21(storageDir15)) {
    mkdirSync21(storageDir15, { recursive: true });
  }
  currentConfig7 = config7;
  logger5 = log;
  maxSleepConsolidations = config7.circadian.maxSleepConsolidations ?? 5;
  state6 = createDefaultState4();
  loadState14();
  ...
  logger5?.info(
    `Circadian: initialized in ${state6.phase} phase (idle threshold: ${config7.circadian.idleThresholdMs / 1e3}s)`
  );
}
...
function loadState14() {
  ...
  try {
    const path = join22(storageDir15, "state.json");
    if (existsSync21(path)) {
      const data = JSON.parse(readFileSync21(path, "utf-8"));
      state6 = { ...createDefaultState4(), ...data };
      state6.phaseStartedAt = Date.now();
      state6.idleTime = 0;
...
function persistState10() {
  if (!storageDir15) return;
  try {
    writeFileSync21(join22(storageDir15, "state.json"), JSON.stringify(state6, null, 2), "utf-8");
  } catch {
  }
}
function evaluatePhase() {
  if (!currentConfig7?.circadian.enabled) return;
  const now = Date.now();
  const idleTime = now - lastActivityTime;
  state6.idleTime = idleTime;
  const activityWindow = currentConfig7.circadian.activityWindowMs;
  state6.activityLevel = Math.max(0, 1 - idleTime / activityWindow);
  const phaseDuration = now - state6.phaseStartedAt;
  const { idleThresholdMs, minWakeDurationMs, minSleepDurationMs, transitionDurationMs } = currentConfig7.circadian;
  switch (state6.phase) {
    case "wake":
    ...
    case "transition-to-sleep":
      state6.phaseProgress = Math.min(1, phaseDuration / transitionDurationMs);
      updateModulationFromPhase();
      if (activityCounter > 0) {
        activityCounter = 0;
        transitionTo("wake");
      } else if (state6.phaseProgress >= 1) {
        transitionTo("sleep");
      }
      break;
    case "sleep":
      if (consolidationCallback && phaseDuration > 3e4 && state6.sleepConsolidations < maxSleepConsolidations) {
        const consolidationInterval = currentConfig7?.circadian.sleepConsolidationIntervalMs ?? 6e4;
        const triggerWindow = consolidationInterval / 3;
        const shouldConsolidate = phaseDuration % consolidationInterval < triggerWindow && state6.sleepConsolidations === 0;
        if (shouldConsolidate || phaseDuration > state6.sleepConsolidations * consolidationInterval) {
...
function transitionTo(newPhase) {
  const oldPhase = state6.phase;
  if (oldPhase === newPhase) return;
  logger5?.info(`Circadian: ${oldPhase} \u2192 ${newPhase}`);
  state6.phase = newPhase;
  state6.phaseStartedAt = Date.now();
  state6.phaseProgress = 0;
  if (newPhase === "wake") {
    state6.wakeInteractions = 0;
    bus.emitSync("circadian:wake-started", { idleTime: state6.idleTime });
  } else if (newPhase === "sleep") {
    state6.sleepConsolidations = 0;
    bus.emitSync("circadian:sleep-started", { wakeInteractions: state6.wakeInteractions });
  }
  updateModulationFromPhase();
...
function updateModulationFromPhase() {
  ...
  const cfg = currentConfig7.circadian;
  switch (state6.phase) {
    case "wake":
      state6.wakeModulation = {
    ...
    case "transition-to-sleep": {
      ...
      const sleepProgress = state6.phaseProgress;
      state6.wakeModulation = {
    ...
    case "transition-to-wake": {
      ...
      const wakeProgress = state6.phaseProgress;
      state6.wakeModulation = {
...
async function triggerSleepConsolidation() {
  ...
  state6.sleepConsolidations++;
  logger5?.info(
    `Circadian: sleep consolidation #${state6.sleepConsolidations} (intensity: ${(state6.sleepSettings.consolidationIntensity * 100).toFixed(0)}%)`
  );
  try {
    await consolidationCallback();
  } catch (err) {
    logger5?.warn(`Circadian: consolidation error \u2014 ${String(err)}`);
  }
}
function recordActivity() {
  lastActivityTime = Date.now();
  activityCounter++;
  state6.wakeInteractions++;
  bus.emitSync("circadian:activity-detected", { activityLevel: state6.activityLevel });
  if (state6.phase === "sleep" || state6.phase === "transition-to-sleep") {
    const phaseDuration = Date.now() - state6.phaseStartedAt;
    const minSleep = currentConfig7?.circadian.minSleepDurationMs ?? 6e4;
    if (state6.phase === "transition-to-sleep" || phaseDuration >= minSleep) {
      transitionTo("transition-to-wake");
    }
  }
}
...
function getSleepSettings() {
  return { ...state6.sleepSettings };
}
function isInSleepPhase() {
  return state6.phase === "sleep" || state6.phase === "transition-to-sleep";
}
function isInWakePhase() {
  return state6.phase === "wake" || state6.phase === "transition-to-wake";
}
function forcePhase(phase) {
  logger5?.info(`Circadian: forced phase change to ${phase}`);
  transitionTo(phase);
}
function getCircadianStats() {
  return {
    phase: state6.phase,
    phaseProgress: state6.phaseProgress,
    phaseDuration: Date.now() - state6.phaseStartedAt,
    idleTime: state6.idleTime,
    activityLevel: state6.activityLevel,
    wakeInteractions: state6.wakeInteractions,
    sleepConsolidations: state6.sleepConsolidations,
    modulation: {
      dopamine: state6.wakeModulation.dopamineBoost,
      serotonin: state6.wakeModulation.serotoninBoost,
      acetylcholine: state6.wakeModulation.acetylcholineBoost,
      norepinephrine: state6.wakeModulation.norepinephrineBoost
    },
    sleepSettings: { ...state6.sleepSettings }
  };
}
...
var DriveEngine = class {
  constructor(spec, config7, workspaceDir, circadianEnabled3, deps5, logger11) {
    this.spec = spec;
    this.config = config7;
    this.circadianEnabled = circadianEnabled3;
    this.deps = deps5;
    this.logger = logger11;
    const storageDir20 = join23(workspaceDir, ".brainagent", spec.id);
    if (!existsSync22(storageDir20)) {
      mkdirSync22(storageDir20, { recursive: true });
    }
    this.stateFile = join23(storageDir20, "state.json");
  ...
  persistState() {
    schedulePersist(this.stateFile, () => {
      ...
      return JSON.stringify(state8, null, 2);
    });
  }
};
...
function initSocialDrive(workspaceDir, cfg, log, injectedDeps) {
  active6?.dispose();
  active6 = createSocialDrive(workspaceDir, cfg, log, injectedDeps);
}
...
function initCognitiveHunger(workspaceDir, cfg, log, injectedDeps) {
  active7?.dispose();
  active7 = createCognitiveHunger(workspaceDir, cfg, log, injectedDeps);
}
...
function initCreativeDrive(workspaceDir, cfg, log, injectedDeps) {
  active8?.dispose();
  active8 = createCreativeDrive(workspaceDir, cfg, log, injectedDeps);
}
...
function initMasteryDrive(workspaceDir, cfg, log, injectedDeps) {
  storageDir16 = join24(workspaceDir, ".brainagent", "mastery-drive");
  if (!existsSync23(storageDir16)) {
    mkdirSync23(storageDir16, { recursive: true });
  }
  ...
  totalNeedSignals = 0;
  adaptiveDecayModifier = 1;
  cancelPersist(join24(storageDir16, "state.json"));
  loadState15();
  wireEventListeners();
  logger6.info(
...
function stopMasteryDrive() {
  ...
  unsubscribers.length = 0;
  persistState11();
  flushPersist(join24(storageDir16, "state.json"));
  logger6?.info("BrainAgent MasteryDrive: stopped.");
}
...
function biasDMNToMasteryThoughts(weakestDomain) {
  ...
  logger6?.info(`BrainAgent MasteryDrive: biased DMN toward ${topics.length} mastery topic(s)`);
}
...
function loadState15() {
  try {
    const filePath = join24(storageDir16, "state.json");
...
function persistState11() {
  schedulePersist(join24(storageDir16, "state.json"), () => {
    const domainMap = {};
    for (const [domain, mastery] of domainSatiations) {
...
function collectActiveDrives(drives, threshold) {
  ...
  if (drives.social) {
    ...
    if (s.need >= threshold) active14.push({ name: "social", need: s.need, needLevel: s.needLevel });
  }
  if (drives.cognitive) {
    const c = drives.cognitive();
    if (c.need >= threshold) active14.push({ name: "cognitive", need: c.need, needLevel: c.needLevel });
  }
  if (drives.creative) {
    const c = drives.creative();
    if (c.need >= threshold) active14.push({ name: "creative", need: c.need, needLevel: c.needLevel });
  }
  if (drives.mastery) {
    const m = drives.mastery();
    if (m.need >= threshold) active14.push({ name: "mastery", need: m.need, needLevel: m.needLevel });
  }
  return active14;
}
function synthesizeAutonomousCycleState(brainConfig, drives, cycle) {
  const active14 = collectActiveDrives(drives, 0.5);
  if (active14.length === 0) return;
  const ranked = active14.map((d) => {
...
function createAutonomousDeliverer(deps5) {
  const { state: state8, brainConfig, minGapMs, logger: logger11 } = deps5;
  return function enqueueAutonomousIntent(text) {
    const trimmed = text.trim();
    if (!trimmed) return;
    if (state8.previousCycleWasAutonomous) {
      logger11.info("BrainAgent Autonomy: intent suppressed \u2014 previous cycle was autonomous");
      return;
    }
    if (Date.now() - state8.lastAutonomousDeliveryAt < minGapMs) {
      logger11.info("BrainAgent Autonomy: intent suppressed \u2014 minimum gap not elapsed");
      return;
    }
    ...
    if (!agent) {
      logger11.warn("BrainAgent Autonomy: no live agent \u2014 autonomous intent dropped");
      return;
    }
    ...
    state8.lastAutonomousDeliveryAt = Date.now();
    deps5.deliver(agent, framed);
  };
}
function createAutonomousIntentResolver(deps5) {
  const { state: state8, brainConfig, drives } = deps5;
  return () => {
    if (brainConfig.modules.goalStack && deps5.goalStack) {
      ...
      if (stats2.pending > 0) {
        ...
        if (triggered.length > 0) {
          ...
          if (goalCtx) {
            state8.lastAutonomousSource = `goal:${triggered[0].id}`;
            return {
              text: [
                AUTONOMOUS_TAG,
    ...
    if (brainConfig.modules.dmn && deps5.dmn) {
      ...
      if (novel.length > 0) {
        ...
        state8.lastAutonomousSource = "dmn:insight";
        return {
          text: [
            AUTONOMOUS_TAG,
            best.insightText,
    ...
    if (brainConfig.modules.goalStack && deps5.goalStack) {
      ...
      if (strong.length > 0) {
        ...
        state8.lastAutonomousSource = `desire:${top.id}`;
        return {
          text: [
            AUTONOMOUS_TAG,
            top.description,
    ...
    const active14 = collectActiveDrives(drives, 0.8);
    if (active14.length > 0) {
      const strongest = active14.sort((a, b) => b.need - a.need)[0];
      state8.lastAutonomousSource = `drive:${strongest.name}`;
      return {
        text: [
          AUTONOMOUS_TAG,
...
function createCycleEngine(deps5) {
  const { config: config7, brainConfig, getHostConfig, logger: logger11, markActivation, state: state8 } = deps5;
  ...
  async function endCycle3(key) {
    ...
    if (brainConfig.modules.cerebellum && cycle.responseText.trim()) {
      ...
      if (result.issues.length > 0) {
        logger11.warn(`BrainAgent Cerebellum: quality issues \u2014 ${result.issues.join("; ")}`);
    ...
    if (brainConfig.modules.hippocampus && input.length > 5) {
      if (isAutonomousCycle && cycle.responseText.trim()) {
        ...
        state8.lastAutonomousEpisodeId = episode.id;
        state8.lastAutonomousDomain = cycle.classification?.domain ?? "unknown";
        state8.previousCycleWasAutonomous = true;
    ...
    if (config7.modules.semanticExtraction && semanticSource.length > 15 && isFactWorthy(semanticSource, cycle.classification)) {
      ...
      if (aiAvailable) {
        try {
          const aiFacts = await extractFactsWithAI(semanticSource, getHostConfig(), logger11);
          if (aiFacts.length > 0) {
          ...
          logger11.info(`BrainAgent Semantic: AI extraction failed, falling back \u2014 ${String(err)}`);
        }
      }
      if (!factsStored) {
      ...
      if (factsStored && brainConfig.modules.curiosityDrive) {
        ...
        for (const gap of getOpenGaps()) {
          if (inputLower.includes(gap.topic.toLowerCase())) {
            ...
            logger11.info(`BrainAgent Curiosity: gap filled for "${gap.topic}" via new fact storage`);
          }
        }
      }
    }
    if (config7.modules.proceduralExtraction && input.length > 10 && isProcedural(input, cycle.classification)) {
      const procedure = await extractProcedureAsync(input, getHostConfig(), cycle.classification, logger11);
      if (procedure && procedure.confidence > 0.5) {
        storeWorkflow(procedure.description, procedure.triggerPattern, procedure.steps);
        logger11.info(`BrainAgent Procedural: stored workflow "${procedure.description}"`);
      }
    }
    ...
    if (brainConfig.modules.dmn) {
      ...
      if (wakeInteractionCount >= brainConfig.dmn.wakeThoughtInterval) {
        ...
        void runAssociationFinding(brainConfig).catch((err) => {
          logger11.info(`BrainAgent DMN: association finding error: ${err}`);
        });
      }
    }
    if (brainConfig.modules.goalStack && input.length > 10) {
      ...
      if (goalExtractionCounter >= brainConfig.goalStack.extractionInterval) {
        ...
        void extractGoalsFromConversation(input, getHostConfig(), logger11).catch((err) => {
          logger11.info(`BrainAgent GoalStack: extraction error: ${err}`);
        });
      }
    }
    if (brainConfig.modules.goalStack) {
...
function initStrategyBandit(workspaceDir, config7) {
  stopStrategyBandit();
  const storageDir20 = join25(workspaceDir, ".brainagent", "bandit");
  if (!existsSync24(storageDir20)) {
    mkdirSync24(storageDir20, { recursive: true });
  }
  storageFile = join25(storageDir20, "state.json");
...
function chooseArm(decisionPoint, arms) {
  ...
  const point = state7[decisionPoint] ?? (state7[decisionPoint] = {});
...
function loadState16() {
  ...
  try {
    ...
    state7 = data && typeof data === "object" ? data : {};
  } catch {
  }
}
function persistState12() {
  if (!storageFile) return;
  schedulePersist(storageFile, () => JSON.stringify(state7, null, 2));
}
...
function initVitalImpulse(workspaceDir, cfg, log, injectedDeps) {
  storageDir17 = join26(workspaceDir, ".brainagent", "vital-impulse");
  if (!existsSync25(storageDir17)) {
    mkdirSync25(storageDir17, { recursive: true });
  }
  ...
  cancelPersist(join26(storageDir17, "state.json"));
  loadState17();
  wireSignalListeners();
  logger7.info(
...
function stopVitalImpulse() {
  ...
  unsubscribers3.length = 0;
  persistState13();
  flushPersist(join26(storageDir17, "state.json"));
  logger7?.info("BrainAgent VitalImpulse: stopped.");
}
...
function forceImpulse(reason) {
  ...
  recentSignals = [];
  persistState13();
  logger7?.info("BrainAgent VitalImpulse: forced impulse fired.");
}
...
function loadState17() {
  try {
    const filePath = join26(storageDir17, "state.json");
...
function persistState13() {
  schedulePersist(join26(storageDir17, "state.json"), () => {
    const state8 = {
      lastFireTime,
      totalFires,
      totalSignalsReceived,
...
function initDriveArbiter(workspaceDir, cfg, getters, log) {
  ...
  storageDir18 = join27(workspaceDir, ".brainagent");
  if (!existsSync26(storageDir18)) {
    mkdirSync26(storageDir18, { recursive: true });
  }
  cancelPersist(join27(storageDir18, "drive-arbiter.json"));
  loadState18();
  const driveEvents = [
...
function stopDriveArbiter() {
  ...
  unsubscribers4.length = 0;
  persistState14();
  flushPersist(join27(storageDir18, "drive-arbiter.json"));
...
function loadState18() {
  try {
    const path = join27(storageDir18, "drive-arbiter.json");
...
function persistState14() {
  if (!storageDir18) return;
  schedulePersist(join27(storageDir18, "drive-arbiter.json"), () => {
    const data = {
      driveWeights,
      lastSelectedDrive,
      lastSelectionTime,
...
function createInteroception(getters, log) {
  ...
  function evaluate() {
    ...
    const changed = !lastState || lastState.pattern !== state8.pattern || Math.abs(lastState.confidence - state8.confidence) > 0.15;
    lastState = state8;
    if (changed) {
      bus.emitSync("interoception:state-updated", {
        pattern: state8.pattern,
        confidence: state8.confidence,
        description: state8.description,
        aggregateNeed: state8.aggregateNeed
      });
      log?.info(
        `BrainAgent Interoception: ${state8.pattern} (confidence=${(state8.confidence * 100).toFixed(0)}%) \u2014 ${state8.description}`
      );
    }
  }
  unsubscribers6.push(
...
function initInteroception(getters, log) {
  active9?.dispose();
  active9 = createInteroception(getters, log);
}
...
function getInteroceptiveState() {
  return active9?.getState() ?? null;
}
function buildInteroceptionContext() {
  return active9?.buildContext() ?? null;
}
...
function createTemporalAwareness(workspaceDir, cfg, log) {
  ...
  let timestamps = [];
  let typicalGapMs = 0;
  let totalInteractions2 = 0;
  const storageDir20 = join28(workspaceDir, ".brainagent");
  if (!existsSync27(storageDir20)) {
    mkdirSync27(storageDir20, { recursive: true });
  }
  try {
    const path = join28(storageDir20, "temporal-awareness.json");
  ...
  function persistState16() {
    try {
      ...
      writeFileSync22(
        join28(storageDir20, "temporal-awareness.json"),
...
function initTemporalAwareness(workspaceDir, cfg, log) {
  active10?.dispose();
  active10 = createTemporalAwareness(workspaceDir, cfg, log);
}
...
function buildTemporalContext2() {
  return active10?.buildContext() ?? null;
}
...
function createAttentionGate(workspaceDir) {
  const storageDir20 = workspaceDir ? join29(workspaceDir, ".brainagent", "attention") : "";
  let totalProcessed = 0;
  let totalDropped = 0;
  let totalRelevanceSum = 0;
  if (storageDir20 && !existsSync28(storageDir20)) {
    mkdirSync28(storageDir20, { recursive: true });
  }
  function loadState20() {
    if (!storageDir20) return;
    try {
      const path = join29(storageDir20, "state.json");
  ...
  function persistState16() {
    ...
    try {
      writeFileSync23(
        join29(storageDir20, "state.json"),
...
var active11;
function current6() {
  return active11 ?? (active11 = createAttentionGate(""));
}
function initAttentionGate(workspaceDir, _config) {
  active11 = createAttentionGate(workspaceDir);
}
...
function assembleContext(state8) {
  const sections = [];
  if (state8.classification) {
    const meta = buildMetacognitiveInstruction(state8.classification);
    if (meta) sections.push(meta);
  }
  if (state8.priority) {
    const empathy = buildEmpathyGuidance(state8.priority);
    if (empathy) sections.push(empathy);
  }
  if (state8.userModel) {
    const userCtx = buildUserContext(state8.userModel);
    if (userCtx) sections.push(userCtx);
  }
  const memCtx = buildMemoryContext(state8.relevantMemories);
  if (memCtx) sections.push(memCtx);
  if (state8.relevantMemories.procedural.length > 0) {
    const procCtx = buildProceduralContext(state8.relevantMemories.procedural);
    if (procCtx) sections.push(procCtx);
  }
  if (state8.contextInjections.length > 0) {
    sections.push(...state8.contextInjections);
  }
...
function createInjectionMetrics() {
  const state8 = zeroState();
  function record(sectionCount, chars, budgetChars) {
    state8.totalCycles++;
    state8.totalChars += chars;
    state8.totalSections += sectionCount;
    state8.lastChars = chars;
    state8.lastSections = sectionCount;
    if (chars > state8.maxChars) state8.maxChars = chars;
    if (sectionCount > state8.maxSections) state8.maxSections = sectionCount;
    if (budgetChars !== void 0 && chars > budgetChars) state8.overBudgetCycles++;
  }
  function get() {
    const avgChars = state8.totalCycles > 0 ? Math.round(state8.totalChars / state8.totalCycles) : 0;
    const avgSections = state8.totalCycles > 0 ? Math.round(state8.totalSections / state8.totalCycles * 10) / 10 : 0;
    return {
      cycles: state8.totalCycles,
      avgChars,
      maxChars: state8.maxChars,
      lastChars: state8.lastChars,
      avgSections,
      maxSections: state8.maxSections,
      lastSections: state8.lastSections,
      avgEstTokens: Math.ceil(avgChars / 4),
      overBudgetCycles: state8.overBudgetCycles
    };
  }
  function reset() {
    Object.assign(state8, zeroState());
  }
...
function createPreStepHandler(deps5) {
  const { config: config7, brainConfig, getHostConfig, logger: logger11, state: state8, cycles, startCycle } = deps5;
  return async (payload, next) => {
    ...
    const cyc = cycle;
    const isAutonomousCycle = isAutonomousInput(input);
    if (brainConfig.modules.autonomousResearch && isAutonomousCycle && isResearchIntent(state8.lastAutonomousSource, input)) {
      ...
      logger11.info(
        `BrainAgent AutonomousResearch: detected research intent (source=${state8.lastAutonomousSource}), running isolated pipeline for "${topic}"`
      );
    ...
    if (config7.modules.aiEnrichment && isAIProviderAvailable2(getHostConfig())) {
      ...
      if (brainConfig.modules.amygdala) {
        tasks.push(
          assessWithAI(input, hc, logger11).then((aiAssessment) => {
      ...
      if (brainConfig.modules.mirrorNeurons && cyc.assessment) {
        ...
        tasks.push(
          observeWithAI("default", input, snapshot, brainConfig, hc, logger11).then((model) => {
            cyc.userModel = model;
          }).catch(() => {
          })
        );
      ...
      if (brainConfig.modules.basalGanglia) {
        tasks.push(
          detectReinforcementWithAI(input, hc, logger11).then((aiSignal) => {
      ...
      ]).catch((err) => logger11.warn(String(err)));
    }
...
function initGoalExecutor(_cfg, log) {
  active12 = createGoalExecutor(log);
...
function initAutonomyEnricher(cfg, log, injectedDeps) {
  ...
  wireEventListeners2();
  logger9.info("BrainAgent AutonomyEnricher: initialized (memory-driven autonomy)");
}
function stopAutonomyEnricher() {
  ...
  unsubscribers5.length = 0;
  logger9?.info("BrainAgent AutonomyEnricher: stopped.");
}
...
function enrichWithMemories(motivation) {
  ...
  if (uniqueEpisodic.length === 0 && uniqueSemantic.length === 0) {
    logger9?.info("BrainAgent AutonomyEnricher: no relevant memories found, skipping enrichment");
    return;
  }
...
function startDreamMode(config7, logger11, neuroClawConfig) {
  if (dreamInterval) return;
  storedConfig = config7;
  storedLogger = logger11;
  ...
  if (config7.circadian?.enabled) {
    setConsolidationCallback(async () => {
      await runConsolidation(config7, logger11, neuroClawConfig, true);
    });
    logger11?.info("BrainAgent DreamMode: registered with circadian rhythm system");
  }
  setTimeout(() => void runConsolidation(config7, logger11, neuroClawConfig, false), 3e4);
  dreamInterval = setInterval(() => {
    void runConsolidation(config7, logger11, neuroClawConfig, false);
  }, intervalMs);
}
...
async function runConsolidation(config7, logger11, neuroClawConfig, circadianTriggered = false) {
  ...
  try {
    ...
    const result = await consolidate(config7, neuroClawConfig, logger11, intensityMultiplier, skipAI);
    ...
    logger11?.info(`BrainAgent DreamMode: error during consolidation \u2014 ${String(err)}`);
  } finally {
    isConsolidating = false;
  }
}
async function forceConsolidation(config7, logger11, neuroClawConfig, intensity) {
  const sleepSettings = getSleepSettings();
  const effectiveIntensity = intensity ?? sleepSettings.consolidationIntensity;
  const result = await consolidate(config7, neuroClawConfig, logger11, effectiveIntensity);
  lastConsolidation = Date.now();
  if (logger11) {
    logger11.info(
...
function initProactiveFeedback(workspaceDir, cfg, log) {
  storageDir19 = join30(workspaceDir, ".brainagent", "proactive-feedback");
  if (!existsSync29(storageDir19)) {
    mkdirSync29(storageDir19, { recursive: true });
  }
  stateFile = join30(storageDir19, "state.json");
...
function stopProactiveFeedback() {
  ...
  logger10?.info("BrainAgent ProactiveFeedback: stopped.");
}
...
function createRewardLedger(workspaceDir, config7) {
  const storageDir20 = join31(workspaceDir, ".brainagent", "reward");
  if (!existsSync30(storageDir20)) {
    mkdirSync30(storageDir20, { recursive: true });
  }
  const storageFile2 = join31(storageDir20, "ledger.json");
...
function initRewardLedger(workspaceDir, config7) {
  active13?.dispose();
  active13 = createRewardLedger(workspaceDir, config7);
}
...
function attachLlmBridge(ctx, preferredModel) {
  ...
  setCallLLMBackend(async (systemPrompt, userText, _config, logger11, maxTokens) => {
    ...
    try {
      ...
      logger11?.info(`BrainAgent LLM bridge: ctx.llm call failed \u2014 ${String(error)}`);
      return void 0;
    }
  });
  void resolveRoute();
...
function apply(ctx, config7) {
  ...
  const state8 = createAutonomyState();
  function pickAgent() {
    const agents = ctx.agents.list();
    if (agents.length === 0) return void 0;
    return agents.find((a) => String(a.id) === state8.lastActiveAgentId) ?? agents[agents.length - 1];
  }
  ...
  initEmbeddings(hostConfig(), logger11);
  if (brainConfig.modules.workingMemory) {
  ...
  if (brainConfig.modules.neuralPathways) {
    initNeuralPathways(dataDir, brainConfig, logger11);
  }
  if (config7.modules.structuralPlasticity) {
    initStructuralPlasticity(dataDir, brainConfig, logger11);
  }
  if (brainConfig.modules.emotionalMemory) {
    initEmotionalMemory(dataDir, brainConfig);
  }
  if (brainConfig.modules.sessionBridge) {
    initSessionBridge(dataDir, brainConfig, logger11);
  }
  if (brainConfig.modules.dmn) {
    initDMN(dataDir, brainConfig, logger11);
  }
  if (brainConfig.modules.goalStack) {
  ...
  if (brainConfig.modules.socialDrive) {
    initSocialDrive(dataDir, brainConfig, logger11, {
      ...driveInitBase,
  ...
  if (brainConfig.modules.cognitiveHunger) {
    initCognitiveHunger(dataDir, brainConfig, logger11, {
      ...driveInitBase,
  ...
  if (brainConfig.modules.creativeDrive) {
    initCreativeDrive(dataDir, brainConfig, logger11, {
      ...driveInitBase,
  ...
  if (brainConfig.modules.masteryDrive) {
    initMasteryDrive(dataDir, brainConfig, logger11, {
      ...driveInitBase,
  ...
  if (brainConfig.modules.vitalImpulse) {
    initVitalImpulse(dataDir, brainConfig, logger11, {
  ...
  if (brainConfig.modules.goalStack) {
    initGoalExecutor(brainConfig, logger11);
  }
  if (brainConfig.modules.actionDispatcher && brainConfig.modules.vitalImpulse) {
    initAutonomyEnricher(brainConfig, logger11, {
  ...
  if (brainConfig.modules.autonomousResearch) {
    initAutonomousResearch(brainConfig, logger11, {
      callLLM,
      storeFact,
  ...
  if (brainConfig.circadian.enabled) {
    initCircadianRhythm(dataDir, brainConfig, logger11);
  }
  if (brainConfig.modules.dreamMode) {
    startDreamMode(brainConfig, logger11, hostConfig());
  }
  if (brainConfig.modules.metabolicBudget) {
    initMetabolicBudget(dataDir, brainConfig, logger11);
  }
  if (brainConfig.modules.emergentModules) {
    initEmergentModules(dataDir, brainConfig, logger11);
  }
  if (brainConfig.modules.proactiveFeedback) {
    initProactiveFeedback(dataDir, brainConfig, logger11);
  }
  ...
  if (brainConfig.modules.temporalAwareness) {
    initTemporalAwareness(dataDir, brainConfig, logger11);
  }
  if (brainConfig.modules.interoception) {
    initInteroception(
      {
  ...
  ctx.on("session/event", (_session, event) => {
    ...
    if (event.type === "user/message") {
      ...
      if (isUserMessage && state8.previousCycleWasAutonomous && state8.lastAutonomousEpisodeId) {
        let reactionSignal = "neutral";
        if (brainConfig.modules.proactiveFeedback) {
          reactionSignal = recordProactiveReaction(state8.lastAutonomousDomain, text);
        }
        if (brainConfig.modules.hippocampus) {
          ...
          storeEpisode(
            ...
            reactionEmotion,
            ["proactive_feedback", state8.lastAutonomousEpisodeId, reactionSignal],
            reactionSalience
          );
        }
      }
      state8.previousCycleWasAutonomous = false;
      state8.lastAutonomousEpisodeId = void 0;
    ...
    if (event.type === "turn/end") {
      void endCycle3(key).catch((err) => logger11.warn(`BrainAgent endCycle: ${String(err)}`));
    }
  });
  ...
  ctx.on("tools/pre-execute", async (exec, next) => {
    ...
    const agentId = exec.agent ? String(exec.agent.id) : state8.lastActiveAgentId;
  ...
  logger11.info("BrainAgent: cognitive architecture initialized");
  logger11.info(
