...
var CorpusCallosum = class {
  ...
  on(event, handler, priority = 0) {
    ...
    return () => {
      const current27 = this.listeners.get(event);
      if (!current27) return;
      const idx = current27.indexOf(entry);
      if (idx !== -1) current27.splice(idx, 1);
      if (current27.length === 0) this.listeners.delete(event);
    };
  }
  /**
...
var AUTONOMY_MEMORIES_PREFIX = "<autonomy-memories>";
var AUTONOMY_PRIORITY_PREFIX = "<autonomy-priority>";
...
function moduleFlagsSchema() {
  ...
  return Schema.object(fields).default(
...
var Config = Schema.object({
  ...
  autonomousUserSilenceMs: Schema.number().default(3 * 60 * 1e3).description("Minimum user silence before proactive (autonomous) messages, ms"),
...
function findUnknownBrainKeys(brain) {
...
function mergeBrainConfig(config) {
  ...
  const brainOverride = config.brain ? Object.fromEntries(
  ...
  for (const [key, value] of Object.entries(config.modules)) {
    if (typeof value !== "boolean") continue;
    const targetKey = MODULE_FLAG_MAP[key] ?? key;
...
function deepMergeConfig(base, override) {
  ...
  for (const [key, value] of Object.entries(override)) {
    ...
    if (typeof value === "object" && !Array.isArray(value) && typeof prev === "object" && prev !== null && !Array.isArray(prev)) {
...
function persistNow(filePath, serialize) {
  try {
...
function createHippocampus(workspaceDir) {
  ...
  function recallEpisodes(query, limit = 5, embeddingResults, emotionBonus) {
    ...
    const scored = episodicStore.map((ep) => {
      ...
      const score = vectorScore + recencyScore * 0.2 + salienceScore * 0.2 + (emotionBonus?.(ep.emotionalContext) ?? 0);
  ...
  function pruneWeakWorkflows2(minSteps = 2) {
    const keep = [];
    let removed = 0;
    for (const proc of proceduralStore) {
      const uniqueSteps = new Set(
        proc.steps.map((s) => s.trim()).filter((s) => s.length >= 3).map((s) => s.toLowerCase())
      );
  ...
  function stepsFingerprint(stepsArr) {
    return stepsArr.map((s) => s.trim().toLowerCase()).filter((s) => s.length >= 3).sort().join("\0");
  }
  function mergeDuplicateWorkflows2() {
  ...
  function recallAll2(query, episodicLimit = 3, semanticLimit = 5, emotionBonus) {
  ...
  async function recallAllAsync2(query, episodicLimit = 3, semanticLimit = 5, emotionBonus) {
    if (!embeddingsAvailable) {
      return recallAll2(query, episodicLimit, semanticLimit, emotionBonus);
    }
    const [epScores, semScores] = await Promise.all([
      searchWithEmbeddings(query, "episodic", 15),
      searchWithEmbeddings(query, "semantic", 15)
    ]);
    const episodic = recallEpisodes(query, episodicLimit, epScores ?? void 0, emotionBonus);
  ...
  return {
    ...
    pruneWeakWorkflows: pruneWeakWorkflows2,
    mergeDuplicateWorkflows: mergeDuplicateWorkflows2,
...
function pruneWeakWorkflows(minSteps = 2) {
  return current3().pruneWeakWorkflows(minSteps);
}
function mergeDuplicateWorkflows() {
  return current3().mergeDuplicateWorkflows();
}
function recallAll(query, episodicLimit = 3, semanticLimit = 5, emotionBonus) {
  return current3().recallAll(query, episodicLimit, semanticLimit, emotionBonus);
}
async function recallAllAsync(query, episodicLimit = 3, semanticLimit = 5, emotionBonus) {
  return current3().recallAllAsync(query, episodicLimit, semanticLimit, emotionBonus);
}
...
function createMirrorNeurons(workspaceDir) {
  ...
  try {
    ...
    if (storageDir && existsSync3(indexPath)) {
  ...
  function persistModels() {
    ...
    try {
      ...
      atomicWrite(join3(storageDir, "index.json"), JSON.stringify(data, null, 2));
    } catch {
    }
  }
...
import { existsSync as existsSync4, mkdirSync as mkdirSync3, readFileSync as readFileSync3 } from "node:fs";
...
function createBasalGanglia(workspaceDir) {
  ...
  if (storageDir && !existsSync5(storageDir)) {
  ...
  function persistHabits() {
    if (!storageDir) return;
    try {
      atomicWrite(join5(storageDir, "habits.json"), JSON.stringify(habits, null, 2));
    } catch {
    }
  }
...
import { existsSync as existsSync8, mkdirSync as mkdirSync7, readFileSync as readFileSync7 } from "node:fs";
import { join as join8 } from "node:path";

// src/modules/dopamine-system.ts
import { existsSync as existsSync7, mkdirSync as mkdirSync6, readFileSync as readFileSync6 } from "node:fs";
...
import { existsSync as existsSync9, mkdirSync as mkdirSync8, readFileSync as readFileSync8 } from "node:fs";
...
function createWorkingMemory(workspaceDir, opts) {
  ...
  let entries = [];
  if (storageDir && !existsSync9(storageDir)) {
  ...
  function persistState() {
    if (!storageDir) return;
    try {
      atomicWrite(join9(storageDir, "state.json"), JSON.stringify(entries, null, 2));
    } catch {
    }
  }
  loadState();
...
function parseFactsFromResponse(content) {
  try {
    ...
    for (const item of parsed) {
      ...
      if (!Number.isFinite(conf) || conf < 0.5 || conf > 1) continue;
      facts.push({
        content: factContent,
        category,
        confidence: conf
...
function checkLanguageConsistency(response, userModel, issues, _suggestions) {
  const prose = response.replace(/```[\s\S]*?```/g, " ").replace(/`[^`\n]*`/g, " ").replace(/https?:\/\/\S+/g, " ");
...
var PLACEHOLDER_STEP = /^(?:\.{2,}|…|any|anything|something|none|n\/a|todo|шаг\s*\d*|step\s*\d*|[-—–]+|любое|что-нибудь|что-то)\.?$/i;
function isMeaningfulStep(step, trigger) {
  const s = step.trim();
...
function isStorableProcedure(pattern) {
  if (pattern.confidence <= 0.5) return false;
  const uniqueSteps = new Set(
    pattern.steps.filter((s) => isMeaningfulStep(s, pattern.triggerPattern)).map((s) => s.trim().toLowerCase())
  );
...
\u0412\u0430\u0436\u043D\u043E: \u043E\u0434\u0438\u043D\u043E\u0447\u043D\u0430\u044F \u043A\u043E\u043C\u0430\u043D\u0434\u0430 \u0431\u0435\u0437 \u043F\u043E\u0441\u043B\u0435\u0434\u043E\u0432\u0430\u0442\u0435\u043B\u044C\u043D\u043E\u0441\u0442\u0438 \u0434\u0435\u0439\u0441\u0442\u0432\u0438\u0439 (\xAB\u0441\u043E\u0437\u0434\u0430\u0439 \u043D\u0430\u043F\u043E\u043C\u0438\u043D\u0430\u043D\u0438\u0435\xBB, \xAB\u0437\u0430\u043F\u0443\u0441\u0442\u0438 \u0441\u0435\u0440\u0432\u0435\u0440\xBB) \u2014 \u041D\u0415 \u043F\u0440\u043E\u0446\u0435\u0434\u0443\u0440\u0430.
...
import { existsSync as existsSync10, mkdirSync as mkdirSync9, readFileSync as readFileSync9 } from "node:fs";
...
function createStructuralPlasticity(workspaceDir, config, log) {
  ...
  function saveState() {
    if (!storageDir) return;
    try {
      atomicWrite(join10(storageDir, "state.json"), JSON.stringify(state, null, 2));
    } catch {
    }
  }
...
function stopStructuralPlasticity() {
  active11 = null;
}
...
import { existsSync as existsSync11, mkdirSync as mkdirSync10, readFileSync as readFileSync10 } from "node:fs";
  ...
  function generateInsights(cfg) {
    ...
    for (let i = 0; i < moduleNames.length; i++) {
      for (let j = i + 1; j < moduleNames.length; j++) {
        ...
        if (rewardsA.every((v, i2) => v === rewardsB[i2])) continue;
...
function identifyImplicatedModule(issue) {
  ...
  if (lower.includes("language") || lower.includes("mismatch")) return void 0;
...
function stopLearningCoordinator() {
  active12?.stop();
  active12 = void 0;
}
...
import { existsSync as existsSync12, mkdirSync as mkdirSync11, readFileSync as readFileSync11 } from "node:fs";
...
function createEmergentModules(workspaceDir, config, log) {
  ...
  function saveState() {
    ...
    try {
      ...
      atomicWrite(join12(storageDir, "state.json"), JSON.stringify(persisted, null, 2));
    } catch {
    }
  }
  ...
  function getRecommendedModulesForDomain2(domain) {
  ...
  return {
    ...
    getRecommendedModulesForDomain: getRecommendedModulesForDomain2
  };
}
var active13 = null;
function current10() {
...
function getRecommendedModulesForDomain(domain) {
  return current10().getRecommendedModulesForDomain(domain);
}
var lastDomainRecommendationAt = /* @__PURE__ */ new Map();
var DOMAIN_RECOMMENDATION_DEDUP_MS = 30 * 60 * 1e3;
function buildDomainRecommendationContext(domain, nowMs = Date.now()) {
  ...
  if (nowMs - last < DOMAIN_RECOMMENDATION_DEDUP_MS) return void 0;
  lastDomainRecommendationAt.set(domain, nowMs);
  return [
    "## \u041F\u0440\u043E\u0432\u0435\u0440\u0435\u043D\u043D\u0430\u044F \u0441\u0432\u044F\u0437\u043A\u0430 \u043C\u043E\u0434\u0443\u043B\u0435\u0439 (\u044D\u043C\u0435\u0440\u0434\u0436\u0435\u043D\u0442\u043D\u044B\u0435 \u043F\u0430\u0442\u0442\u0435\u0440\u043D\u044B)",
    `\u0414\u043B\u044F \u0434\u043E\u043C\u0435\u043D\u0430 \xAB${domain}\xBB \u0441\u0442\u0430\u0431\u0438\u043B\u044C\u043D\u043E \u0445\u043E\u0440\u043E\u0448\u0438\u0439 \u0440\u0435\u0437\u0443\u043B\u044C\u0442\u0430\u0442 \u0434\u0430\u0432\u0430\u043B\u0430 \u043A\u043E\u043C\u0431\u0438\u043D\u0430\u0446\u0438\u044F: ${participants.join(", ")}. \u041E\u043F\u0438\u0440\u0430\u0439\u0441\u044F \u043D\u0430 \u043D\u0435\u0451 \u0432 \u043F\u0435\u0440\u0432\u0443\u044E \u043E\u0447\u0435\u0440\u0435\u0434\u044C.`
  ].join("\n");
}

// src/modules/metabolic-budget.ts
import { existsSync as existsSync13, mkdirSync as mkdirSync12, readFileSync as readFileSync12 } from "node:fs";
...
function createMetabolicBudget(workspaceDir, config, log) {
  ...
  function saveState() {
    try {
      atomicWrite(join13(storageDir, "state.json"), JSON.stringify(state, null, 2));
    } catch {
    }
  }
  loadState();
...
import { existsSync as existsSync14, mkdirSync as mkdirSync13, readFileSync as readFileSync13 } from "node:fs";
...
import { existsSync as existsSync15, mkdirSync as mkdirSync14, readFileSync as readFileSync14 } from "node:fs";
...
function createEmotionalMemory(workspaceDir, config) {
  ...
  if (storageDir && !existsSync15(storageDir)) {
  ...
  function computeEmotionMatchBonus2(queryEmotion, memoryEmotion, matchConfig) {
...
${neuroDesc}`;
  ...
  return {
    ...
    computeEmotionMatchBonus: computeEmotionMatchBonus2,
...
function computeEmotionMatchBonus(queryEmotion, memoryEmotion, config) {
  return current12().computeEmotionMatchBonus(queryEmotion, memoryEmotion, config);
}
...
import { existsSync as existsSync16, mkdirSync as mkdirSync15, readFileSync as readFileSync15 } from "node:fs";
...
// src/modules/dmn.ts
import { existsSync as existsSync17, mkdirSync as mkdirSync16, readFileSync as readFileSync16 } from "node:fs";
...
function createDMN(workspaceDir, config, log) {
  ...
  function prepareProactiveContext2(predictions) {
...
${lines}
  ...
  return {
    ...
    prepareProactiveContext: prepareProactiveContext2,
    getStats: getStats2,
...
function prepareProactiveContext(predictions) {
  return current14().prepareProactiveContext(predictions);
}
...
import { existsSync as existsSync18, mkdirSync as mkdirSync17, readFileSync as readFileSync17 } from "node:fs";
...
function createTemporalBinding(workspaceDir, opts) {
  ...
  function persistState() {
    if (!storageDir) return;
    try {
      atomicWrite(join18(storageDir, "state.json"), JSON.stringify(moments, null, 2));
    } catch {
    }
  }
  loadState();
...
import { existsSync as existsSync19, mkdirSync as mkdirSync18, readFileSync as readFileSync18 } from "node:fs";
...
function createSessionBridge(workspaceDir, config, _logger) {
  ...
  if (storageDir && !existsSync19(storageDir)) {
  ...
  function persistLast() {
    if (!storageDir) return;
    try {
      atomicWrite(join19(storageDir, "last.json"), JSON.stringify(lastSession, null, 2));
    } catch {
    }
  }
...
import { existsSync as existsSync20, mkdirSync as mkdirSync19, readFileSync as readFileSync19 } from "node:fs";
...
function extractGapTopic(input) {
  ...
  const REMINDER_COMMAND_RE = /(поставь|создай)?\s*(напоминание|напомни)\s.*(через|в)\s/i;
  let candidate = text;
...
function stopCuriosityDrive() {
  active21?.stop();
  active21 = null;
}

// src/modules/goal-stack.ts
import { existsSync as existsSync21, mkdirSync as mkdirSync20 } from "node:fs";
...
function createGoalStack(workspaceDir, config) {
  ...
  function loadState() {
    if (!storageDir) return;
    try {
      const raw = loadJsonFile(join21(storageDir, "state.json"), void 0);
  ...
  function persistState() {
    if (!storageDir) return;
    try {
      saveJsonFile(join21(storageDir, "state.json"), { goals, desires, decisionLog });
    } catch {
    }
  }
  ...
  async function extractGoalsFromConversation3(userMessage, config2, logger) {
    ...
    const response = await callLLM(buildGoalExtractionPrompt(), userMessage, config2, logger, 300);
    if (!response) {
    ...
    for (const raw of parsed.slice(0, 3)) {
      ...
      let type = validTypes.includes(triggerType) ? triggerType : "topic";
      let condition = triggerCondition;
      let goalTTLMs;
      if (type === "time") {
        const parsed2 = parseTimeTriggerCondition(triggerCondition);
        if (parsed2 === void 0) {
          logger?.info(
        ...
        condition = String(parsed2);
        const untilTriggerMs = parsed2 - Date.now() + 60 * 60 * 1e3;
  ...
  bus.on("goal:expired", (data) => {
    ...
    for (const desire of desires) {
      if (desire.strength > 0.05) {
        desire.strength = Math.round(desire.strength * 0.85 * 1e3) / 1e3;
        changed = true;
      }
    }
    if (changed) {
...
function parseTimeTriggerCondition(condition) {
  const trimmed = condition.trim();
  if (!trimmed) return void 0;
  const minSane = Date.UTC(2e3, 0, 1);
  ...
  if (dotMatch) {
    ...
    if (local.getDate() !== Number(dd) || local.getMonth() !== Number(mm) - 1 || local.getHours() !== Number(hh) || local.getMinutes() !== Number(mi) || local.getSeconds() !== Number(ss)) {
      return void 0;
    }
    const asDate2 = local.getTime();
    return Number.isNaN(asDate2) || asDate2 < minSane ? void 0 : asDate2;
  }
  if (!/^\d{4}-\d{2}(-\d{2})?([T ]\d.*)?$/.test(trimmed)) return void 0;
  const asDate = Date.parse(trimmed);
  if (!Number.isNaN(asDate) && asDate >= minSane) return asDate;
  return void 0;
}
function buildGoalExtractionPrompt(now = /* @__PURE__ */ new Date()) {
...
var GOAL_INTENT_PATTERN = /планирую|запланир|моя цель|мои цели|поставь цель|поставь(?: мне)? напоминание|напоминание (?:на|через|о|об|про)|надо не забыть|не забудь|напомни(?:те)?(?: мне)?|хочу успеть|задачи на (?:неделю|сегодня|завтра)|\bgoal\b|\bplan\b|\btodo\b|\bremind\b/i;
function hasGoalIntent(text) {
  return GOAL_INTENT_PATTERN.test(text);
}

// src/modules/drive-engine.ts
import { existsSync as existsSync23, mkdirSync as mkdirSync22, readFileSync as readFileSync21 } from "node:fs";
import { join as join23 } from "node:path";

// src/modules/circadian-rhythm.ts
import { existsSync as existsSync22, mkdirSync as mkdirSync21, readFileSync as readFileSync20 } from "node:fs";
...
function createCircadianRhythm(workspaceDir, config, log) {
  ...
  function persistState() {
    try {
      atomicWrite(join22(storageDir, "state.json"), JSON.stringify(state, null, 2));
    } catch {
    }
  }
...
var DriveEngine = class {
  ...
  emitNeedSignals() {
    ...
    if (this.currentNeedLevel === "urgent") {
      this.emitDriveEvent(`${this.spec.id}:urge`, {
        ...
        [this.spec.urgeTimeField]: this.lastInteractionTime > 0 ? now - this.lastInteractionTime : 0
      });
  ...
  loadState() {
    try {
      if (!existsSync23(this.stateFile)) {
        this.satiation = this.config.initialSatiation;
        this.lastDecayEvaluationTime = Date.now();
        return;
      }
      const raw = JSON.parse(readFileSync21(this.stateFile, "utf-8"));
...
import { existsSync as existsSync24, mkdirSync as mkdirSync23, readFileSync as readFileSync22 } from "node:fs";
...
function initGoalExecutor(_cfg, log) {
  active28 = createGoalExecutor(log);
...
function recordGoalExecution(count) {
  active28?.record(count);
}
function getGoalExecutorStats() {
  return active28?.getStats() ?? {
    totalChecks: 0,
...
function createAutonomousDeliverer(deps) {
  ...
  return function enqueueAutonomousIntent(text) {
    ...
    if (!priority) {
      ...
      if (state.lastUserMessageAt > 0 && Date.now() - state.lastUserMessageAt < minUserSilenceMs) {
    ...
    state.pendingMemoryContext = void 0;
...
function pinIdentityFacts(recalled, identityFacts, maxPinned = 3) {
...
import { existsSync as existsSync25, mkdirSync as mkdirSync24, readFileSync as readFileSync23 } from "node:fs";
...
function createStrategyBandit(workspaceDir, config) {
  ...
  let lastChoices = {};
  function loadState() {
    if (!existsSync25(storageFile)) return;
    try {
      const data = JSON.parse(readFileSync23(storageFile, "utf-8"));
...
function initStrategyBandit(workspaceDir, config) {
  active29?.dispose();
  active29 = createStrategyBandit(workspaceDir, config);
}
...
function chooseArm(decisionPoint, arms) {
  ...
  return active29.chooseArm(decisionPoint, arms);
}

// src/modules/vital-impulse.ts
import { existsSync as existsSync26, mkdirSync as mkdirSync25, readFileSync as readFileSync24 } from "node:fs";
...
function initVitalImpulse(workspaceDir, cfg, log, injectedDeps) {
  active30?.stop();
  active30 = createVitalImpulse(workspaceDir, cfg, log, injectedDeps);
}
...
import { existsSync as existsSync27, mkdirSync as mkdirSync26, readFileSync as readFileSync25 } from "node:fs";
...
function initDriveArbiter(workspaceDir, cfg, getters, log) {
  active31?.stop();
  active31 = createDriveArbiter(workspaceDir, cfg, getters, log);
}
...
function initInteroception(getters, log) {
  active32?.dispose();
  active32 = createInteroception(getters, log);
}
...
import { existsSync as existsSync28, mkdirSync as mkdirSync27, readFileSync as readFileSync26 } from "node:fs";
...
function initTemporalAwareness(workspaceDir, cfg, log) {
  active33?.dispose();
  active33 = createTemporalAwareness(workspaceDir, cfg, log);
}
...
function recordInteraction() {
  active33?.recordInteraction();
}
function getTemporalAwarenessStats() {
  return active33?.getStats() ?? {
    typicalGapMs: 0,
    lastInteractionTime: 0,
    currentGapMs: 0,
...
function buildTemporalContext2() {
  return active33?.buildContext() ?? null;
}
...
import { existsSync as existsSync29, mkdirSync as mkdirSync28, readFileSync as readFileSync27 } from "node:fs";
...
function createAttentionGate(workspaceDir) {
  ...
  if (storageDir && !existsSync29(storageDir)) {
...
var active34;
function current21() {
  return active34 ?? (active34 = createAttentionGate(""));
}
function initAttentionGate(workspaceDir, _config) {
  active34 = createAttentionGate(workspaceDir);
}
...
function readDeepSeekKeyFromCredentialsStore() {
  try {
    const home = process.env.USERPROFILE ?? process.env.HOME ?? "";
    if (!home) return null;
    const raw = readFileSync28(join30(home, ".dsh", ".credentials.yaml"), "utf-8");
...
function salvageExtraction(raw) {
  ...
  const facts = [];
  const seen = /* @__PURE__ */ new Set();
  const RE_CONTENT_FIRST = /\{\s*"content"\s*:\s*"((?:[^"\\]|\\.)*)"\s*,\s*"category"\s*:\s*"((?:[^"\\]|\\.)*)"\s*\}/g;
  const RE_CATEGORY_FIRST = /\{\s*"category"\s*:\s*"((?:[^"\\]|\\.)*)"\s*,\s*"content"\s*:\s*"((?:[^"\\]|\\.)*)"\s*\}/g;
  const collect = (re, swapped) => {
    let m;
...
${knownContext}` : `Topic: ${topic}`;
  ...
  function resolveApiKey(provider) {
    ...
    switch (provider) {
      ...
      case "deepseek": {
        const dsCfg = searchCfg?.deepseek;
...
var RESEARCH_INJECTION_MAX_FACTS = 8;
var RESEARCH_INJECTION_FACT_CHARS = 280;
function buildResearchInjection(result, opts) {
  if (!result.summary && result.factsStored === 0) return void 0;
  const searchBlocked = opts?.searchBlocked ?? true;
  const lines = [
    "## Research Results (Autonomous Research Pipeline)",
    "",
    searchBlocked ? "\u0418\u043D\u0441\u0442\u0440\u0443\u043A\u0446\u0438\u044F: \u043F\u043E\u0438\u0441\u043A \u0443\u0436\u0435 \u0432\u044B\u043F\u043E\u043B\u043D\u0435\u043D \u0438\u0437\u043E\u043B\u0438\u0440\u043E\u0432\u0430\u043D\u043D\u044B\u043C \u043A\u043E\u043D\u0432\u0435\u0439\u0435\u0440\u043E\u043C \u2014 \u043D\u0435 \u0432\u044B\u0437\u044B\u0432\u0430\u0439 \u0438\u043D\u0441\u0442\u0440\u0443\u043C\u0435\u043D\u0442\u044B \u043F\u043E\u0438\u0441\u043A\u0430 (web_search, web_fetch) \u043F\u043E\u0432\u0442\u043E\u0440\u043D\u043E \u0432 \u044D\u0442\u043E\u043C \u0445\u043E\u0434\u0435." : "\u0418\u043D\u0441\u0442\u0440\u0443\u043A\u0446\u0438\u044F: \u043F\u043E\u0438\u0441\u043A \u0443\u0436\u0435 \u0432\u044B\u043F\u043E\u043B\u043D\u0435\u043D \u0438\u0437\u043E\u043B\u0438\u0440\u043E\u0432\u0430\u043D\u043D\u044B\u043C \u043A\u043E\u043D\u0432\u0435\u0439\u0435\u0440\u043E\u043C; \u043F\u043E\u0432\u0442\u043E\u0440\u043D\u044B\u0439 \u043F\u043E\u0438\u0441\u043A \u0432\u044B\u0437\u044B\u0432\u0430\u0439 \u0442\u043E\u043B\u044C\u043A\u043E \u0435\u0441\u043B\u0438 \u0438\u0442\u043E\u0433\u043E\u0432 \u043D\u0435\u0434\u043E\u0441\u0442\u0430\u0442\u043E\u0447\u043D\u043E.",
  ...
  if (top.length > 0) {
    ...
    for (const fact of top) {
      const clipped = fact.length > RESEARCH_INJECTION_FACT_CHARS ? fact.slice(0, RESEARCH_INJECTION_FACT_CHARS).trimEnd() + "\u2026" : fact;
      lines.push(`- ${clipped}`);
    }
  }
  lines.push(
...
var active35 = null;
function current22() {
  if (!active35) active35 = createAutonomousResearch();
  return active35;
}
function initAutonomousResearch(cfg, log, injectedDeps) {
  active35 = createAutonomousResearch(cfg, log, injectedDeps);
}
...
function buildClockContext(now = /* @__PURE__ */ new Date()) {
...
function preserveResearchBlock(filtered, researchBlock) {
...
function createPreStepHandler(deps) {
  ...
  return async (payload, next) => {
    ...
    if (brainConfig.modules.autonomousResearch && isAutonomousCycle && isResearchIntent(state.lastAutonomousSource, input)) {
      ...
      const searchBlocked = brainConfig.autonomousResearch.blockedToolsInAutonomous.includes("web_search");
      const researchBlock = result ? buildResearchInjection(result, { searchBlocked }) : void 0;
      if (researchBlock) {
    ...
    const emotionBonus = brainConfig.modules.emotionalMemory && cyc.assessment ? (memoryEmotion) => computeEmotionMatchBonus(cyc.assessment.emotion, memoryEmotion, brainConfig) : void 0;
...
function createMetricsCollector(workspaceDir, options = {}) {
  const providers = /* @__PURE__ */ new Map();
  const startedAt = /* @__PURE__ */ new Date();
  const filePath = workspaceDir ? join31(workspaceDir, ".brainagent", "metrics.json") : "";
  let updates = 0;
  if (filePath) {
    mkdirSync29(join31(workspaceDir, ".brainagent"), { recursive: true });
  }
  function collect() {
    const sections = {};
    const errors = {};
    for (const [name2, provider] of providers) {
      try {
        sections[name2] = provider();
      } catch (error) {
        errors[name2] = error.message;
      }
    }
    return {
  ...
  let last = collect();
  function register(section, provider) {
...
// src/modules/goal-reminder.ts
var DEFAULT_REMINDER_CHECK_MS = 3e4;
function buildReminderText(triggered, goalCtx) {
  const lines = [
...
function startGoalReminderScheduler(deps) {
  const rawEnv = process.env.BRAINAGENT_REMINDER_CHECK_MS;
  const parsed = rawEnv !== void 0 && rawEnv !== "" ? Number(rawEnv) : NaN;
  const intervalMs = Number.isFinite(parsed) && parsed >= 5e3 ? parsed : DEFAULT_REMINDER_CHECK_MS;
  const timer = setInterval(() => {
    try {
      ...
      deps.logger?.info(
        `BrainAgent GoalReminder: \u043E\u0448\u0438\u0431\u043A\u0430 \u043F\u0440\u043E\u0432\u0435\u0440\u043A\u0438 \u0446\u0435\u043B\u0435\u0439 \u2014 ${error.message}`
      );
    }
  }, intervalMs);
  timer.unref?.();
...
function createGoalRoundGuard(logWarn, windowMs = 6e4, maxRounds = 2) {
  let arrivals = [];
  let warned = false;
  return (source) => {
    if (warned) return;
    ...
    if (arrivals.length > maxRounds) {
      warned = true;
      logWarn(
        "BrainAgent: \u0433\u043E\u043B-\u0440\u0430\u0443\u043D\u0434\u044B \u0445\u043E\u0441\u0442\u0430 \u043F\u0440\u0438\u0445\u043E\u0434\u044F\u0442 \u0447\u0430\u0449\u0435 \u0440\u0430\u0437\u0430 \u0432 \u043C\u0438\u043D\u0443\u0442\u0443 \u2014 \u043F\u0430\u0442\u0447 \u043F\u0435\u0439\u0441\u0438\u043D\u0433\u0430 \u043D\u0435 \u043F\u0440\u0438\u043C\u0435\u043D\u0451\u043D. \u0412\u044B\u043F\u043E\u043B\u043D\u0438\u0442\u0435: node brainagent/host-patches/apply-goal-round-pacing.cjs"
      );
    }
  };
}

// src/modules/goal-round-pacer.ts
var DEFAULT_GOAL_ROUND_MIN_IDLE_MS = 18e4;
function isGoalRoundSource(source) {
  return !!source && source.kind === "goal" && typeof source.round === "number" && source.round > 0;
}
function resolveGoalRoundMinIdleMs(env = process.env) {
  const raw = env.DSH_GOAL_ROUND_MIN_IDLE_MS;
  if (raw === void 0 || raw === "") return DEFAULT_GOAL_ROUND_MIN_IDLE_MS;
  const parsed = Number(raw);
  return Number.isFinite(parsed) && parsed >= 0 ? parsed : DEFAULT_GOAL_ROUND_MIN_IDLE_MS;
}
function startGoalRoundPacer(deps) {
  const now = deps.now ?? Date.now;
  const getGoals = () => typeof deps.goals === "function" ? deps.goals() : deps.goals;
  const minIdleMs = resolveGoalRoundMinIdleMs(deps.env ?? process.env);
  if (minIdleMs <= 0) return void 0;
  const schedule = deps.schedule ?? ((fn, ms) => {
    const timer = setTimeout(fn, ms);
    timer.unref?.();
    return { cancel: () => clearTimeout(timer) };
  });
  const lastRoundAt = /* @__PURE__ */ new Map();
  const resumeTimers = /* @__PURE__ */ new Map();
  let stopped = false;
  function scheduleResume(agent, waitMs) {
    if (resumeTimers.has(agent.id)) return;
    const timer = schedule(() => {
      ...
      try {
        ...
        deps.logger?.info(
          `BrainAgent GoalRoundPacer: \u043D\u0435 \u0443\u0434\u0430\u043B\u043E\u0441\u044C \u0432\u0435\u0440\u043D\u0443\u0442\u044C \u0446\u0435\u043B\u044C \u2014 ${error.message}`
        );
      }
    }, waitMs);
  ...
  const off = deps.onInserted(({ agent, message }) => {
    ...
    if (last === void 0 || at - last >= minIdleMs) {
    ...
    try {
      ...
      deps.logger?.info(
        `BrainAgent GoalRoundPacer: \u043D\u0435 \u0443\u0434\u0430\u043B\u043E\u0441\u044C \u0443\u0434\u0430\u043B\u0438\u0442\u044C \u0440\u0430\u0443\u043D\u0434 \u0438\u0437 inbox \u2014 ${error.message}`
      );
      return;
    }
    try {
      ...
      deps.logger?.info(
        `BrainAgent GoalRoundPacer: \u043D\u0435 \u0443\u0434\u0430\u043B\u043E\u0441\u044C \u043F\u043E\u0441\u0442\u0430\u0432\u0438\u0442\u044C \u0446\u0435\u043B\u044C \u043D\u0430 \u043F\u0430\u0443\u0437\u0443 \u2014 ${error.message}`
      );
    }
  });
  deps.logger?.info(
...
import { existsSync as existsSync30, mkdirSync as mkdirSync30, readFileSync as readFileSync29 } from "node:fs";
import { join as join32 } from "node:path";
function createProactiveFeedback(workspaceDir, cfg, log) {
  const storageDir = workspaceDir ? join32(workspaceDir, ".brainagent", "proactive-feedback") : "";
  const stateFile = storageDir ? join32(storageDir, "state.json") : "";
  ...
  function loadState() {
    if (!stateFile) return;
    try {
      if (!existsSync30(stateFile)) return;
...
import { existsSync as existsSync31, mkdirSync as mkdirSync31, readFileSync as readFileSync30 } from "node:fs";
...
function createRewardLedger(workspaceDir, config) {
  const storageDir = join33(workspaceDir, ".brainagent", "reward");
  if (!existsSync31(storageDir)) {
    mkdirSync31(storageDir, { recursive: true });
  }
  const storageFile = join33(storageDir, "ledger.json");
  cancelPersist(storageFile);
  const maxEntries = config.learningLoop.rewardLedger.maxEntries;
  let entries = [];
  let idCounter = 0;
  if (existsSync31(storageFile)) {
    try {
...
function createThalamicGate(config, signalProviders) {
  ...
  function shouldActivateCortex2(ctx, nowMs) {
  ...
  return { shouldActivateCortex: shouldActivateCortex2, getStats: getStats2, reset };
}
var active40 = null;
function current25() {
...
function stopThalamicGate() {
  active40 = null;
}
function shouldActivateCortex(ctx, nowMs) {
  return current25().shouldActivateCortex(ctx, nowMs);
}
...
var HARNESS_SYSTEM_PREFIXES = [
  "<system-reminder>",
  "Current runtime context.",
  "Current runtime context: none."
];
function isHarnessSystemMessage(text) {
  const trimmed = text.trimStart();
  ...
  setCallLLMBackend(async (systemPrompt, userText, _config, logger, maxTokens) => {
    ...
    try {
      const chunks = ctx.llm.stream({
        ...
        maxTokens: Math.max(1e3, (maxTokens ?? 500) * 4)
      });
      let text = "";
...
function apply(ctx, config) {
  ...
  if (config.brain) {
    const unknownKeys = findUnknownBrainKeys(config.brain);
  ...
  mkdirSync32(dataDir, { recursive: true });
  const optional = (enabled, getter) => enabled ? getter : void 0;
  const disposers = [];
  ...
  const pendingDeliveryRetries = /* @__PURE__ */ new Set();
  function pickAgent() {
  ...
  const enqueueAutonomousIntent = createAutonomousDeliverer({
    ...
    minUserSilenceMs: config.autonomousUserSilenceMs ?? 3 * 60 * 1e3,
    logger,
    pickAgent,
    deliver: (agent, framed) => {
      ...
      try {
        ...
        logger.info(
          `BrainAgent Autonomy: delivery deferred \u2014 ${error.message}`
        );
        const retryDelivery = (attempt) => {
          const timer = setTimeout(() => {
            ...
            try {
              ...
              if (attempt < 3) {
                ...
                logger.info(
                  `BrainAgent Autonomy: delivery failed after ${attempt} attempts \u2014 ${retryError.message}`
                );
              }
            }
          }, 25 * attempt);
    ...
    thalamic: brainConfig.modules.thalamicGate ? { shouldActivateCortex } : void 0
  });
  disposers.push(() => {
  ...
  if (brainConfig.modules.thalamicGate) {
    initThalamicGate(brainConfig.thalamicGate, {
      ...
      getVitalImpulseStats: optional(brainConfig.modules.vitalImpulse, () => {
        ...
        if (s.lastFireTime > 0 && Date.now() - s.lastFireTime <= 1e4) {
      ...
      getSocialDriveSatiation: optional(brainConfig.modules.socialDrive, getSatiation),
  ...
  ctx.effect(() => {
    ...
    const metricsInterval = setInterval(() => metrics.update(), 6e4);
    return () => {
