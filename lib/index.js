...
function createMirrorNeurons(workspaceDir) {
  const storageDir15 = workspaceDir ? join3(workspaceDir, ".brainagent", "users") : "";
  const userModels = /* @__PURE__ */ new Map();
  if (storageDir15 && !existsSync2(storageDir15)) {
    mkdirSync2(storageDir15, { recursive: true });
  }
  try {
    const indexPath = join3(storageDir15, "index.json");
    if (storageDir15 && existsSync2(indexPath)) {
  ...
  function persistModels() {
    ...
    try {
      ...
      writeFileSync3(join3(storageDir15, "index.json"), JSON.stringify(data, null, 2), "utf-8");
    } catch {
    }
  }
...
function createPredictiveEngine(workspaceDir) {
  const storageDir15 = workspaceDir ? join4(workspaceDir, ".brainagent", "predictions") : "";
  ...
  let lastDomain;
  if (storageDir15 && !existsSync3(storageDir15)) {
    mkdirSync3(storageDir15, { recursive: true });
  }
  function loadPatterns() {
    if (!storageDir15) return;
    try {
      const tPath = join4(storageDir15, "temporal.json");
    ...
    try {
      const sPath = join4(storageDir15, "sequential.json");
    ...
    try {
      const cPath = join4(storageDir15, "contextual.json");
...
function createBasalGanglia(workspaceDir) {
  const storageDir15 = workspaceDir ? join5(workspaceDir, ".brainagent", "habits") : "";
  let habits = [];
  ...
  if (storageDir15 && !existsSync4(storageDir15)) {
    mkdirSync4(storageDir15, { recursive: true });
  }
  function loadHabits() {
    if (!storageDir15) return;
    try {
      const path = join5(storageDir15, "habits.json");
      if (existsSync4(path)) {
  ...
  function persistHabits() {
    if (!storageDir15) return;
    try {
      writeFileSync5(join5(storageDir15, "habits.json"), JSON.stringify(habits, null, 2), "utf-8");
    } catch {
    }
  }
...
function createDopamineSystem(workspaceDir) {
  const storageDir15 = join7(workspaceDir, ".brainagent", "neuromodulators");
  if (!existsSync6(storageDir15)) {
    mkdirSync6(storageDir15, { recursive: true });
  }
  cancelPersist(join7(storageDir15, "state.json"));
  let state4 = {
    dopamine: 0.5,
    serotonin: 0.6,
    norepinephrine: 0.3,
  ...
  function loadState15() {
    try {
      const path = join7(storageDir15, "state.json");
      if (existsSync6(path)) {
  ...
  function updateDopamine(predictionError, config7) {
    ...
    state4.dopamine = state4.dopamine * (1 - decay) + (baseline + spike) * decay;
    state4.dopamine = Math.max(0, Math.min(1, state4.dopamine));
  }
  function updateSerotonin(reward) {
    const alpha = 0.15;
    positiveOutcomeRatio = positiveOutcomeRatio * (1 - alpha) + (reward > 0 ? 1 : 0) * alpha;
    state4.serotonin = 0.3 + positiveOutcomeRatio * 0.5;
    state4.serotonin = Math.max(0.1, Math.min(0.95, state4.serotonin));
  }
  function updateNorepinephrine(complexity, emotion) {
    ...
    const alpha = 0.3;
    state4.norepinephrine = state4.norepinephrine * (1 - alpha) + target * alpha;
    state4.norepinephrine = Math.max(0.05, Math.min(0.95, state4.norepinephrine));
  }
  function updateAcetylcholine(_domain, _config) {
    ...
    const alpha = 0.2;
    state4.acetylcholine = state4.acetylcholine * (1 - alpha) + target * alpha;
    state4.acetylcholine = Math.max(0.1, Math.min(0.9, state4.acetylcholine));
  }
  function processOutcome(params, config7) {
    ...
    bus.emitSync("neuromodulator:state-changed", { ...state4 });
  ...
  return {
    ...
    getEffectiveLearningRate: (baseLearningRate, config7) => {
      const achBoost = 1 + (state4.acetylcholine - 0.5) * (config7.neuromodulators.acetylcholineLearningBoost - 1);
      const dopamineBoost = 0.7 + state4.dopamine * 0.6;
      return baseLearningRate * achBoost * dopamineBoost;
    },
    getRiskTolerance: () => state4.serotonin,
    getAttentionLevel: () => state4.norepinephrine,
    getStats: getStats2,
    stop: () => {
    },
    dispose: () => {
...
function createNeuralPathways(workspaceDir, config7, logger9) {
  const storageDir15 = join8(workspaceDir, ".brainagent", "synapses");
  if (!existsSync7(storageDir15)) {
    mkdirSync7(storageDir15, { recursive: true });
  }
  ...
  function loadSynapticState() {
    try {
      const path = join8(storageDir15, "weights.json");
      if (existsSync7(path)) {
  ...
  function saveSynapticState() {
    try {
      writeFileSync7(
        join8(storageDir15, "weights.json"),
...
function initNeuralPathways(workspaceDir, config7, logger9) {
  active5?.dispose();
  active5 = createNeuralPathways(workspaceDir, config7, logger9);
}
function resetCycleState() {
  active5?.resetCycleState();
}
function getPathwayStats() {
  return active5?.getPathwayStats() ?? {
    pathwayCount: 8,
...
function getSynapticStats() {
  return active5?.getSynapticStats() ?? computeSynapticStats(createDefaultSynapticState());
}
function buildNeuromodulatorContext() {
  return active5?.buildNeuromodulatorContext();
}
...
function createWorkingMemory(workspaceDir, opts) {
  const storageDir15 = workspaceDir ? join9(workspaceDir, ".brainagent", "working-memory") : "";
  const maxEntries = opts.maxEntries;
  const summaryMaxLength = opts.summaryMaxLength;
  let entries = [];
  if (storageDir15 && !existsSync8(storageDir15)) {
    mkdirSync8(storageDir15, { recursive: true });
  }
  function loadState15() {
    if (!storageDir15) return;
    try {
      const path = join9(storageDir15, "state.json");
      if (existsSync8(path)) {
  ...
  function persistState12() {
    if (!storageDir15) return;
    try {
      writeFileSync8(join9(storageDir15, "state.json"), JSON.stringify(entries, null, 2), "utf-8");
    } catch {
    }
  }
  loadState15();
...
function createLearningCoordinator(workspaceDir, config7) {
  const storageDir15 = join11(workspaceDir, ".brainagent", "learning");
  if (!existsSync10(storageDir15)) {
    mkdirSync10(storageDir15, { recursive: true });
  }
  ...
  function loadState15() {
    try {
      const path = join11(storageDir15, "coordinator.json");
  ...
  function persistState12() {
    try {
      ...
      writeFileSync10(
        join11(storageDir15, "coordinator.json"),
        JSON.stringify(
          {
...
function initLearningCoordinator(workspaceDir, config7) {
  active7?.dispose();
  active7 = createLearningCoordinator(workspaceDir, config7);
}
...
function buildLearningContext() {
  return active7?.buildContext();
}
function recordRecurringIssue(issueType) {
  return active7?.recordRecurringIssue(issueType);
}
function recordDomainPerformance(domain, reward, errorIssues = []) {
  active7?.recordDomainPerformance(domain, reward, errorIssues);
}
function buildCapabilityContext(currentDomain) {
  return active7?.buildCapabilityContext(currentDomain);
}
...
function createMetabolicBudget(workspaceDir, config7, log) {
  const storageDir15 = join13(workspaceDir, ".brainagent", "metabolic");
  if (!existsSync12(storageDir15)) {
    mkdirSync12(storageDir15, { recursive: true });
  }
  const state4 = createDefaultState3();
  ...
  function loadState15() {
    try {
      const path = join13(storageDir15, "state.json");
  ...
  function saveState3() {
    try {
      writeFileSync12(join13(storageDir15, "state.json"), JSON.stringify(state4, null, 2), "utf-8");
    } catch {
    }
  }
  loadState15();
  ...
  function endCycle3() {
    ...
    if (state4.cyclesSinceRebalance >= config7.metabolicBudget.rebalanceInterval) {
      rebalanceEnergy();
  ...
  return {
    ...
    endCycle: endCycle3,
    getModuleEnergy: (module) => state4.moduleEnergies[module]?.energy ?? 1,
    isModuleLowPower: (module) => state4.moduleEnergies[module]?.lowPowerMode ?? false,
    getStats: getStats2,
...
function initMetabolicBudget(workspaceDir, config7, log) {
  active8?.dispose();
  active8 = createMetabolicBudget(workspaceDir, config7, log);
}
function consumeEnergy(module, amount) {
  return active8?.consumeEnergy(module, amount) ?? true;
}
function recordPerformance(module, reward) {
  active8?.recordPerformance(module, reward);
}
function endCycle2() {
  active8?.endCycle();
}
...
function initEmotionalMemory(workspaceDir, _config) {
  storageDir5 = join15(workspaceDir, ".brainagent", "emotional-memory");
  if (!existsSync14(storageDir5)) {
    mkdirSync14(storageDir5, { recursive: true });
  }
...
function loadState5() {
  ...
  try {
    const path = join15(storageDir5, "state.json");
...
function initQualiaSimulator(workspaceDir, config7) {
  storageDir6 = join16(workspaceDir, ".brainagent", "qualia-simulator");
  if (!existsSync15(storageDir6)) {
    mkdirSync15(storageDir6, { recursive: true });
  }
...
function loadState6() {
  ...
  try {
    const path = join16(storageDir6, "state.json");
...
function describeNeuromodulatorFeel(state4) {
  const { dopamine, serotonin, norepinephrine, acetylcholine } = state4;
  const parts = [];
...
function initDMN(workspaceDir, config7, log) {
  storageDir7 = join17(workspaceDir, ".brainagent", "dmn");
  if (!existsSync16(storageDir7)) {
    mkdirSync16(storageDir7, { recursive: true });
  }
...
function loadState7() {
  ...
  try {
    const path = join17(storageDir7, "state.json");
...
function createTemporalBinding(workspaceDir, opts) {
  const storageDir15 = workspaceDir ? join18(workspaceDir, ".brainagent", "temporal-binding") : "";
  const maxMoments = opts.maxMoments;
  let moments = [];
  let idCounter4 = 0;
  if (storageDir15 && !existsSync17(storageDir15)) {
    mkdirSync17(storageDir15, { recursive: true });
  }
  function loadState15() {
    if (!storageDir15) return;
    try {
      const path = join18(storageDir15, "state.json");
  ...
  function persistState12() {
    if (!storageDir15) return;
    try {
      writeFileSync17(join18(storageDir15, "state.json"), JSON.stringify(moments, null, 2), "utf-8");
    } catch {
    }
  }
  loadState15();
...
var active9;
function current5() {
  return active9 ?? (active9 = createTemporalBinding("", { maxMoments: DEFAULT_MAX_MOMENTS }));
}
function initTemporalBinding(workspaceDir, config7) {
  active9 = createTemporalBinding(workspaceDir, { maxMoments: config7.temporalBinding.maxMoments });
}
...
function initSessionBridge(workspaceDir, config7, _logger) {
  storageDir8 = join19(workspaceDir, ".brainagent", "sessions");
  if (!existsSync18(storageDir8)) {
    mkdirSync18(storageDir8, { recursive: true });
  }
...
function loadState8() {
  ...
  try {
    const currentPath = join19(storageDir8, "current.json");
  ...
  try {
    const lastPath = join19(storageDir8, "last.json");
...
function persistCurrent() {
  ...
  try {
    writeFileSync18(
      join19(storageDir8, "current.json"),
...
function persistLast() {
  if (!storageDir8) return;
  try {
    writeFileSync18(join19(storageDir8, "last.json"), JSON.stringify(lastSession, null, 2), "utf-8");
  } catch {
  }
}
...
function initCuriosityDrive(workspaceDir, config7) {
  storageDir9 = join20(workspaceDir, ".brainagent", "curiosity");
...
function initGoalStack(workspaceDir, config7) {
  storageDir10 = join21(workspaceDir, ".brainagent", "goals");
  if (!existsSync20(storageDir10)) {
    mkdirSync20(storageDir10, { recursive: true });
  }
...
function loadState10() {
  ...
  try {
    const path = join21(storageDir10, "state.json");
...
function persistState7() {
  ...
  try {
    writeFileSync20(
      join21(storageDir10, "state.json"),
...
function expireGoals() {
  ...
  if (goals.length > 60) {
    const active20 = goals.filter((g) => g.status === "pending" || g.status === "triggered");
    const inactive = goals.filter((g) => g.status === "completed" || g.status === "expired").slice(-30);
    goals = [...active20, ...inactive];
    changed = true;
  }
...
function createCircadianRhythm(workspaceDir, config7, log) {
  const storageDir15 = join22(workspaceDir, ".brainagent", "circadian");
  ...
  let state4 = createDefaultState4();
  ...
  function loadState15() {
    try {
      ...
      if (existsSync21(path)) {
        ...
        state4 = { ...createDefaultState4(), ...data };
  ...
  function persistState12() {
    try {
      writeFileSync21(join22(storageDir15, "state.json"), JSON.stringify(state4, null, 2), "utf-8");
    } catch {
    }
  }
  ...
  log?.info(
    `Circadian: initialized in ${state4.phase} phase (idle threshold: ${config7.circadian.idleThresholdMs / 1e3}s)`
  );
  function stopTimer() {
  ...
  return {
    ...
    getSleepSettings: () => ({ ...state4.sleepSettings }),
    isInSleepPhase: () => state4.phase === "sleep" || state4.phase === "transition-to-sleep",
    isInWakePhase: () => state4.phase === "wake" || state4.phase === "transition-to-wake",
    forcePhase: (phase) => {
...
function initCircadianRhythm(workspaceDir, config7, log) {
  active10?.dispose();
  active10 = createCircadianRhythm(workspaceDir, config7, log);
  if (pendingConsolidationCallback) {
    active10.setConsolidationCallback(pendingConsolidationCallback);
  }
}
...
function setConsolidationCallback(callback) {
  ...
  active10?.setConsolidationCallback(callback);
}
function recordActivity() {
  active10?.recordActivity();
}
function getCircadianState() {
  return active10?.getState() ?? createDefaultState4();
}
function getSleepSettings() {
  return active10?.getSleepSettings() ?? {
    consolidationIntensity: 0.5,
    pruningAggressiveness: 0.3,
    synapticNormalization: false
  };
...
function isInSleepPhase() {
  return active10?.isInSleepPhase() ?? false;
}
function isInWakePhase() {
  return active10?.isInWakePhase() ?? true;
}
function forcePhase(phase) {
  active10?.forcePhase(phase);
}
function getCircadianStats() {
  const st = active10?.getState() ?? createDefaultState4();
  return {
    phase: st.phase,
...
var DriveEngine = class {
  constructor(spec, config7, workspaceDir, circadianEnabled3, deps5, logger9) {
    ...
    this.deps = deps5;
    this.logger = logger9;
    const storageDir15 = join23(workspaceDir, ".brainagent", spec.id);
    if (!existsSync22(storageDir15)) {
      mkdirSync22(storageDir15, { recursive: true });
    }
    this.stateFile = join23(storageDir15, "state.json");
  ...
  persistState() {
    schedulePersist(this.stateFile, () => {
      ...
      return JSON.stringify(state4, null, 2);
    });
  }
};
...
function initSocialDrive(workspaceDir, cfg, log, injectedDeps) {
  active11?.dispose();
  active11 = createSocialDrive(workspaceDir, cfg, log, injectedDeps);
}
...
function initCognitiveHunger(workspaceDir, cfg, log, injectedDeps) {
  active12?.dispose();
  active12 = createCognitiveHunger(workspaceDir, cfg, log, injectedDeps);
}
...
function getCognitiveHungerSatiation() {
  return active12?.getSatiation() ?? 0;
}
...
function initCreativeDrive(workspaceDir, cfg, log, injectedDeps) {
  active13?.dispose();
  active13 = createCreativeDrive(workspaceDir, cfg, log, injectedDeps);
}
...
function getCreativeDriveSatiation() {
  return active13?.getSatiation() ?? 0;
}
...
function initMasteryDrive(workspaceDir, cfg, log, injectedDeps) {
  storageDir11 = join24(workspaceDir, ".brainagent", "mastery-drive");
  if (!existsSync23(storageDir11)) {
    mkdirSync23(storageDir11, { recursive: true });
  }
  ...
  totalNeedSignals = 0;
  adaptiveDecayModifier = 1;
  cancelPersist(join24(storageDir11, "state.json"));
  loadState11();
  wireEventListeners();
  logger4.info(
...
function stopMasteryDrive() {
  ...
  unsubscribers.length = 0;
  persistState8();
  flushPersist(join24(storageDir11, "state.json"));
...
function loadState11() {
  try {
    const filePath = join24(storageDir11, "state.json");
...
function persistState8() {
  schedulePersist(join24(storageDir11, "state.json"), () => {
    const domainMap = {};
    for (const [domain, mastery] of domainSatiations) {
...
function collectActiveDrives(drives, threshold) {
  ...
  if (drives.social) {
    ...
    if (s.need >= threshold) active20.push({ name: "social", need: s.need, needLevel: s.needLevel });
  }
  if (drives.cognitive) {
    const c = drives.cognitive();
    if (c.need >= threshold) active20.push({ name: "cognitive", need: c.need, needLevel: c.needLevel });
  }
  if (drives.creative) {
    const c = drives.creative();
    if (c.need >= threshold) active20.push({ name: "creative", need: c.need, needLevel: c.needLevel });
  }
  if (drives.mastery) {
    const m = drives.mastery();
    if (m.need >= threshold) active20.push({ name: "mastery", need: m.need, needLevel: m.needLevel });
  }
  return active20;
}
function synthesizeAutonomousCycleState(brainConfig, drives, cycle) {
  const active20 = collectActiveDrives(drives, 0.5);
  if (active20.length === 0) return;
  const ranked = active20.map((d) => {
...
function createAutonomousDeliverer(deps5) {
  const { state: state4, brainConfig, minGapMs, logger: logger9 } = deps5;
  return function enqueueAutonomousIntent(text) {
    const trimmed = text.trim();
    if (!trimmed) return;
    if (state4.previousCycleWasAutonomous) {
      logger9.info("BrainAgent Autonomy: intent suppressed \u2014 previous cycle was autonomous");
      return;
    }
    if (Date.now() - state4.lastAutonomousDeliveryAt < minGapMs) {
    ...
    state4.lastAutonomousDeliveryAt = Date.now();
    deps5.deliver(agent, framed);
  };
}
function createAutonomousIntentResolver(deps5) {
  const { state: state4, brainConfig, drives } = deps5;
  return () => {
    if (brainConfig.modules.goalStack && deps5.goalStack) {
      ...
      if (stats2.pending > 0) {
        ...
        if (triggered.length > 0) {
          ...
          if (goalCtx) {
            state4.lastAutonomousSource = `goal:${triggered[0].id}`;
            return {
              text: [
                AUTONOMOUS_TAG,
    ...
    if (brainConfig.modules.dmn && deps5.dmn) {
      ...
      if (novel.length > 0) {
        ...
        state4.lastAutonomousSource = "dmn:insight";
        return {
          text: [
            AUTONOMOUS_TAG,
            best.insightText,
    ...
    if (brainConfig.modules.goalStack && deps5.goalStack) {
      ...
      if (strong.length > 0) {
        ...
        state4.lastAutonomousSource = `desire:${top.id}`;
        return {
          text: [
            AUTONOMOUS_TAG,
            top.description,
    ...
    const active20 = collectActiveDrives(drives, 0.8);
    if (active20.length > 0) {
      const strongest = active20.sort((a, b) => b.need - a.need)[0];
      state4.lastAutonomousSource = `drive:${strongest.name}`;
      return {
        text: [
          AUTONOMOUS_TAG,
...
function createCycleEngine(deps5) {
  const { config: config7, brainConfig, getHostConfig, logger: logger9, markActivation, state: state4 } = deps5;
  ...
  async function endCycle3(key) {
    ...
    if (brainConfig.modules.hippocampus && input.length > 5) {
      if (isAutonomousCycle && cycle.responseText.trim()) {
        ...
        state4.lastAutonomousEpisodeId = episode.id;
        state4.lastAutonomousDomain = cycle.classification?.domain ?? "unknown";
        state4.previousCycleWasAutonomous = true;
...
function createStrategyBandit(workspaceDir, config7) {
  const storageDir15 = join25(workspaceDir, ".brainagent", "bandit");
  if (!existsSync24(storageDir15)) {
    mkdirSync24(storageDir15, { recursive: true });
  }
  const storageFile = join25(storageDir15, "state.json");
  ...
  function getArmStats(decisionPoint) {
    const point = state4[decisionPoint] ?? {};
    const out = {};
  ...
  function getBanditStats() {
    let totalPlays = 0;
    for (const point of Object.values(state4)) {
      for (const rec of Object.values(point)) {
        totalPlays += rec.plays;
      }
    }
    return { decisionPoints: Object.keys(state4).length, totalPlays, initialized: true };
  }
  return {
...
function initStrategyBandit(workspaceDir, config7) {
  active14?.dispose();
  active14 = createStrategyBandit(workspaceDir, config7);
}
...
function chooseArm(decisionPoint, arms) {
  ...
  return active14.chooseArm(decisionPoint, arms);
}
...
function initVitalImpulse(workspaceDir, cfg, log, injectedDeps) {
  storageDir12 = join26(workspaceDir, ".brainagent", "vital-impulse");
  if (!existsSync25(storageDir12)) {
    mkdirSync25(storageDir12, { recursive: true });
  }
  ...
  cancelPersist(join26(storageDir12, "state.json"));
  loadState12();
  wireSignalListeners();
  logger5.info(
...
function stopVitalImpulse() {
  ...
  unsubscribers2.length = 0;
  persistState9();
  flushPersist(join26(storageDir12, "state.json"));
...
function loadState12() {
  try {
    const filePath = join26(storageDir12, "state.json");
...
function persistState9() {
  schedulePersist(join26(storageDir12, "state.json"), () => {
    const state4 = {
      lastFireTime,
      totalFires,
      totalSignalsReceived,
...
function initDriveArbiter(workspaceDir, cfg, getters, log) {
  ...
  storageDir13 = join27(workspaceDir, ".brainagent");
  if (!existsSync26(storageDir13)) {
    mkdirSync26(storageDir13, { recursive: true });
  }
  cancelPersist(join27(storageDir13, "drive-arbiter.json"));
  loadState13();
  const driveEvents = [
...
function stopDriveArbiter() {
  ...
  unsubscribers3.length = 0;
  persistState10();
  flushPersist(join27(storageDir13, "drive-arbiter.json"));
...
function loadState13() {
  try {
    const path = join27(storageDir13, "drive-arbiter.json");
...
function persistState10() {
  ...
  schedulePersist(join27(storageDir13, "drive-arbiter.json"), () => {
    const data = {
      driveWeights,
      lastSelectedDrive,
      lastSelectionTime,
...
function createInteroception(getters, log) {
  ...
  function evaluate() {
    ...
    const changed = !lastState || lastState.pattern !== state4.pattern || Math.abs(lastState.confidence - state4.confidence) > 0.15;
    lastState = state4;
    if (changed) {
      bus.emitSync("interoception:state-updated", {
        pattern: state4.pattern,
        confidence: state4.confidence,
        description: state4.description,
        aggregateNeed: state4.aggregateNeed
      });
      log?.info(
        `BrainAgent Interoception: ${state4.pattern} (confidence=${(state4.confidence * 100).toFixed(0)}%) \u2014 ${state4.description}`
      );
    }
  }
  unsubscribers5.push(
...
function initInteroception(getters, log) {
  active15?.dispose();
  active15 = createInteroception(getters, log);
}
...
function getInteroceptiveState() {
  return active15?.getState() ?? null;
}
function buildInteroceptionContext() {
  return active15?.buildContext() ?? null;
}
...
function createTemporalAwareness(workspaceDir, cfg, log) {
  ...
  let typicalGapMs = 0;
  let totalInteractions = 0;
  const storageDir15 = join28(workspaceDir, ".brainagent");
  if (!existsSync27(storageDir15)) {
    mkdirSync27(storageDir15, { recursive: true });
  }
  try {
    const path = join28(storageDir15, "temporal-awareness.json");
  ...
  function persistState12() {
    try {
      ...
      writeFileSync22(
        join28(storageDir15, "temporal-awareness.json"),
...
function initTemporalAwareness(workspaceDir, cfg, log) {
  active16?.dispose();
  active16 = createTemporalAwareness(workspaceDir, cfg, log);
}
...
function buildTemporalContext2() {
  return active16?.buildContext() ?? null;
}
...
function createAttentionGate(workspaceDir) {
  const storageDir15 = workspaceDir ? join29(workspaceDir, ".brainagent", "attention") : "";
  let totalProcessed = 0;
  let totalDropped = 0;
  let totalRelevanceSum = 0;
  if (storageDir15 && !existsSync28(storageDir15)) {
    mkdirSync28(storageDir15, { recursive: true });
  }
  function loadState15() {
    if (!storageDir15) return;
    try {
      const path = join29(storageDir15, "state.json");
  ...
  function persistState12() {
    ...
    try {
      writeFileSync23(
        join29(storageDir15, "state.json"),
...
var active17;
function current6() {
  return active17 ?? (active17 = createAttentionGate(""));
}
function initAttentionGate(workspaceDir, _config) {
  active17 = createAttentionGate(workspaceDir);
}
...
function assembleContext(state4) {
  const sections = [];
  if (state4.classification) {
    const meta = buildMetacognitiveInstruction(state4.classification);
    if (meta) sections.push(meta);
  }
  if (state4.priority) {
    const empathy = buildEmpathyGuidance(state4.priority);
    if (empathy) sections.push(empathy);
  }
  if (state4.userModel) {
    const userCtx = buildUserContext(state4.userModel);
    if (userCtx) sections.push(userCtx);
  }
  const memCtx = buildMemoryContext(state4.relevantMemories);
  if (memCtx) sections.push(memCtx);
  if (state4.relevantMemories.procedural.length > 0) {
    const procCtx = buildProceduralContext(state4.relevantMemories.procedural);
    if (procCtx) sections.push(procCtx);
  }
  if (state4.contextInjections.length > 0) {
    sections.push(...state4.contextInjections);
  }
...
function createInjectionMetrics() {
  const state4 = zeroState();
  function record(sectionCount, chars, budgetChars) {
    state4.totalCycles++;
    state4.totalChars += chars;
    state4.totalSections += sectionCount;
    state4.lastChars = chars;
    state4.lastSections = sectionCount;
    if (chars > state4.maxChars) state4.maxChars = chars;
    if (sectionCount > state4.maxSections) state4.maxSections = sectionCount;
    if (budgetChars !== void 0 && chars > budgetChars) state4.overBudgetCycles++;
  }
  function get() {
    const avgChars = state4.totalCycles > 0 ? Math.round(state4.totalChars / state4.totalCycles) : 0;
    const avgSections = state4.totalCycles > 0 ? Math.round(state4.totalSections / state4.totalCycles * 10) / 10 : 0;
    return {
      cycles: state4.totalCycles,
      avgChars,
      maxChars: state4.maxChars,
      lastChars: state4.lastChars,
      avgSections,
      maxSections: state4.maxSections,
      lastSections: state4.lastSections,
      avgEstTokens: Math.ceil(avgChars / 4),
      overBudgetCycles: state4.overBudgetCycles
    };
  }
  function reset() {
    Object.assign(state4, zeroState());
  }
...
function createPreStepHandler(deps5) {
  const { config: config7, brainConfig, getHostConfig, logger: logger9, state: state4, cycles, startCycle } = deps5;
  return async (payload, next) => {
    ...
    const cyc = cycle;
    const isAutonomousCycle = isAutonomousInput(input);
    if (brainConfig.modules.autonomousResearch && isAutonomousCycle && isResearchIntent(state4.lastAutonomousSource, input)) {
      ...
      logger9.info(
        `BrainAgent AutonomousResearch: detected research intent (source=${state4.lastAutonomousSource}), running isolated pipeline for "${topic}"`
      );
...
function initGoalExecutor(_cfg, log) {
  active18 = createGoalExecutor(log);
...
function initProactiveFeedback(workspaceDir, cfg, log) {
  storageDir14 = join30(workspaceDir, ".brainagent", "proactive-feedback");
  if (!existsSync29(storageDir14)) {
    mkdirSync29(storageDir14, { recursive: true });
  }
  stateFile = join30(storageDir14, "state.json");
...
function createRewardLedger(workspaceDir, config7) {
  const storageDir15 = join31(workspaceDir, ".brainagent", "reward");
  if (!existsSync30(storageDir15)) {
    mkdirSync30(storageDir15, { recursive: true });
  }
  const storageFile = join31(storageDir15, "ledger.json");
  cancelPersist(storageFile);
...
function initRewardLedger(workspaceDir, config7) {
  active19?.dispose();
  active19 = createRewardLedger(workspaceDir, config7);
}
...
function apply(ctx, config7) {
  ...
  const state4 = createAutonomyState();
  function pickAgent() {
    const agents = ctx.agents.list();
    if (agents.length === 0) return void 0;
    return agents.find((a) => String(a.id) === state4.lastActiveAgentId) ?? agents[agents.length - 1];
  }
  ...
  ctx.on("session/event", (_session, event) => {
    ...
    if (event.type === "user/message") {
      ...
      if (isUserMessage && state4.previousCycleWasAutonomous && state4.lastAutonomousEpisodeId) {
        let reactionSignal = "neutral";
        if (brainConfig.modules.proactiveFeedback) {
          reactionSignal = recordProactiveReaction(state4.lastAutonomousDomain, text);
        }
        if (brainConfig.modules.hippocampus) {
          ...
          storeEpisode(
            ...
            reactionEmotion,
            ["proactive_feedback", state4.lastAutonomousEpisodeId, reactionSignal],
            reactionSalience
          );
        }
      }
      state4.previousCycleWasAutonomous = false;
      state4.lastAutonomousEpisodeId = void 0;
  ...
  ctx.on("tools/pre-execute", async (exec, next) => {
    ...
    const agentId = exec.agent ? String(exec.agent.id) : state4.lastActiveAgentId;
