...
function buildHostConfig(options = {}) {
  ...
  const config6 = { models: { providers } };
  if (options.model) {
    config6.agents = { defaults: { model: options.model } };
  }
  return config6;
}
...
function mergeBrainConfig(config6) {
  const modules = { ...DEFAULT_CONFIG.modules };
  for (const [key, value] of Object.entries(config6.modules)) {
  ...
  return {
    ...
    dualProcess: {
      ...
      ...config6.dualProcess?.fastModel ? { fastModel: config6.dualProcess.fastModel } : {},
      ...config6.dualProcess?.slowModel ? { slowModel: config6.dualProcess.slowModel } : {}
    },
    circadian: {
      ...DEFAULT_CONFIG.circadian,
      enabled: config6.circadian.enabled
    },
    contextInjection: {
      ...DEFAULT_CONFIG.contextInjection,
      ...config6.contextInjection
    },
    learningLoop: {
      rewardLedger: {
        ...DEFAULT_CONFIG.learningLoop.rewardLedger,
        ...config6.learningLoop?.rewardLedger ?? {}
      },
      strategyBandit: {
        ...DEFAULT_CONFIG.learningLoop.strategyBandit,
        ...config6.learningLoop?.strategyBandit ?? {}
      }
    }
  };
}
...
function parseUserModelSelection(config6) {
  const agents = config6.agents;
...
function resolveProvider(config6) {
  const providers = config6.models?.providers;
  if (!providers) return null;
  const userSelection = parseUserModelSelection(config6);
  if (userSelection) {
...
function isAIProviderAvailable(config6) {
  return (availabilityHook?.() ?? false) || resolveProvider(config6) !== null;
}
var callBackend;
...
async function callLLM(systemPrompt, userText, config6, logger7, maxTokens = 500) {
  if (callBackend) {
    try {
      const bridged = await callBackend(systemPrompt, userText, config6, logger7, maxTokens);
      if (bridged !== void 0) return bridged;
    } catch (error) {
      logger7?.info(`BrainAgent LLM: bridge failed, falling back \u2014 ${String(error)}`);
    }
  }
  const provider = resolveProvider(config6);
  if (!provider) {
    logger7?.info("BrainAgent LLM: no AI provider configured, skipping");
    return null;
  }
  const userSelection = parseUserModelSelection(config6);
  if (userSelection) {
    logger7?.info(`BrainAgent LLM: calling ${provider.name} (${provider.model}) [user-selected]`);
  } else {
    logger7?.info(`BrainAgent LLM: calling ${provider.name} (${provider.model}) [auto-detected]`);
  }
  try {
    let response;
...
${userText}` }] }],
      ...
      logger7?.info(`BrainAgent LLM: Google error ${response.status}: ${errorText}`);
      return null;
    } else {
    ...
    if (!response.ok) {
      const errorText = await response.text();
      logger7?.info(`BrainAgent LLM: ${provider.name} error ${response.status}: ${errorText}`);
      return null;
    }
    ...
    logger7?.info(`BrainAgent LLM: error \u2014 ${String(error)}`);
    return null;
  }
}
...
function resolveEmbeddingProvider(config6) {
  const providers = config6.models?.providers;
  if (!providers) return null;
  const userSelection = parseUserModelSelection(config6);
  if (userSelection) {
...
async function getEmbedding(text, config6, logger7) {
  const result = await getEmbeddings([text], config6, logger7);
  return result?.[0] ?? null;
}
async function getEmbeddings(texts, config6, logger7) {
  const provider = resolveEmbeddingProvider(config6);
  if (!provider) {
    return null;
  }
  ...
  try {
    const result = provider.format === "google" ? await fetchGoogleEmbeddings(texts, provider, logger7) : await fetchOpenAIEmbeddings(texts, provider, logger7);
    if (result === null && provider.name === "Ollama") {
      ollamaUnreachable = true;
    }
    return result;
  } catch (error) {
    logger7?.info(`BrainAgent Embeddings: ${provider.name} error \u2014 ${String(error)}`);
    if (provider.name === "Ollama") ollamaUnreachable = true;
    return null;
  }
}
async function fetchOpenAIEmbeddings(texts, provider, logger7) {
...
async function fetchGoogleEmbeddings(texts, provider, logger7) {
  ...
  if (!response.ok) {
    const errorText = await response.text();
    logger7?.info(`BrainAgent Embeddings: Google API error ${response.status}: ${errorText}`);
    return null;
  }
  const data = await response.json();
...
function createHippocampus(workspaceDir) {
  ...
  function nextId(prefix) {
    return `${prefix}-${Date.now()}-${++idCounter2}`;
  }
  ...
  function scheduleEmbeddingBackfill() {
    ...
    const config6 = embeddingsConfig;
    const missing = [];
    ...
    void (async () => {
      for (let i = 0; i < missing.length; i++) {
        ...
        try {
          const vec = await getEmbedding(item.text, config6, embeddingsLogger);
          if (vec) {
  ...
  function initEmbeddings2(config6, logger7) {
    const provider = resolveEmbeddingProvider(config6);
    if (!provider) {
      logger7?.info("BrainAgent Hippocampus: no embedding provider \u2014 using TF-IDF");
      return;
    }
    embeddingsConfig = config6;
    embeddingsLogger = logger7;
  ...
  function updateEmbeddingsConfig2(config6) {
  ...
  function storeEpisode2(event, summary, emotionalContext = "neutral", entities = [], emotionIntensity = 0) {
    ...
    if (embeddingsAvailable && embeddingsConfig) {
      const config6 = embeddingsConfig;
      void getEmbedding(`${event} ${summary}`, config6, embeddingsLogger).then((vec) => {
        if (vec) {
  ...
  function storeFact2(content, category, sourceEpisodeIds = [], relatedIds = []) {
    ...
    if (embeddingsAvailable && embeddingsConfig) {
      const config6 = embeddingsConfig;
      void getEmbedding(`${content} ${category}`, config6, embeddingsLogger).then((vec) => {
        if (vec) {
  ...
  function reviseFact(existing, newContent, newConfidence, sourceEpisodeIds, reason) {
    ...
    if (embeddingsAvailable && embeddingsConfig) {
      const config6 = embeddingsConfig;
      void getEmbedding(`${newContent} ${existing.category}`, config6, embeddingsLogger).then(
        (vec) => {
          if (vec) {
  ...
  async function consolidate2(config6, neuroClawConfig, logger7, intensity = 0.5, skipAIReview = false) {
    let merged = 0;
    let pruned = 0;
    let strengthened = 0;
    ...
    if (semanticStore.length > config6.memory.maxSemanticMemories) {
      semanticStore.sort((a, b) => b.confidence - a.confidence);
      const removed = semanticStore.splice(config6.memory.maxSemanticMemories);
    ...
    if (!skipAIReview && neuroClawConfig && isAIProviderAvailable(neuroClawConfig) && semanticStore.length >= 5) {
      try {
        ...
        logger7?.info(`BrainAgent Consolidation: AI review error \u2014 ${String(err)}`);
      }
    }
    persistEpisodic();
    persistSemantic();
...
function initEmbeddings(config6, logger7) {
  current().initEmbeddings(config6, logger7);
}
function updateEmbeddingsConfig(config6) {
  current().updateEmbeddingsConfig(config6);
}
...
async function consolidate(config6, neuroClawConfig, logger7, intensity = 0.5, skipAIReview = false) {
  return current().consolidate(config6, neuroClawConfig, logger7, intensity, skipAIReview);
}
function getStats() {
...
async function assessWithAI(text, config6, logger7) {
  const content = await callLLM(EMOTION_PROMPT, text, config6, logger7, 100);
...
function createMirrorNeurons(workspaceDir) {
  const storageDir9 = workspaceDir ? join3(workspaceDir, ".brainagent", "users") : "";
  const userModels = /* @__PURE__ */ new Map();
  if (storageDir9 && !existsSync2(storageDir9)) {
    mkdirSync2(storageDir9, { recursive: true });
  }
  try {
    const indexPath = join3(storageDir9, "index.json");
    if (storageDir9 && existsSync2(indexPath)) {
  ...
  function persistModels() {
    if (!storageDir9) return;
    try {
      const data = Object.fromEntries(userModels);
      writeFileSync3(join3(storageDir9, "index.json"), JSON.stringify(data, null, 2), "utf-8");
    } catch {
    }
  }
  ...
  function observe2(userId, text, amygdalaResult, config6) {
    ...
    if (model.emotionHistory.length > config6.empathy.emotionHistoryLength) {
      model.emotionHistory = model.emotionHistory.slice(-config6.empathy.emotionHistoryLength);
    }
    ...
    applyTheoryOfMindUpdates(model, text, amygdalaResult, config6);
    persistModels();
  ...
  async function observeWithAI2(userId, text, amygdalaResult, config6, neuroConfig, logger7) {
    const model = getOrCreateModel(userId);
    model.emotionHistory.push({
      timestamp: Date.now(),
      emotion: amygdalaResult.emotion,
    ...
    const alpha = 0.3;
    model.stressLevel = model.stressLevel * (1 - alpha) + (isStressed ? amygdalaResult.emotionIntensity : 0) * alpha;
    const content = await callLLM(STYLE_PROMPT, text, neuroConfig, logger7, 100);
    if (content) {
      try {
        const jsonMatch = content.match(/\{[\s\S]*\}/);
        if (jsonMatch) {
...
function applyTheoryOfMindUpdates(model, text, amygdalaResult, config6) {
  ensureToMFields(model);
  const maxIntentHistory = config6.empathy.maxIntentHistory ?? 20;
  const domainLimit = config6.empathy.knowledgeModelDomainLimit ?? 15;
...
function updateKnowledgeModelToM(model, text, domainLimit) {
  ...
  for (const domain of domainSignals) {
    const current15 = model.knowledgeModel[domain];
    if (!current15) {
      model.knowledgeModel[domain] = "beginner";
    } else if (current15 === "beginner") {
...
var active2;
function current2() {
  return active2 ?? (active2 = createMirrorNeurons(""));
}
function initMirrorStorage(workspaceDir) {
  active2 = createMirrorNeurons(workspaceDir);
}
function getUserModel(userId) {
  return current2().getUserModel(userId);
}
function observe(userId, text, amygdalaResult, config6) {
  return current2().observe(userId, text, amygdalaResult, config6);
}
function processStyleReward(userId, reward, activeStyle) {
  current2().processStyleReward(userId, reward, activeStyle);
}
function getStyleRecommendation(userId) {
  return current2().getStyleRecommendation(userId);
}
function observeWithAI(userId, text, amygdalaResult, config6, neuroConfig, logger7) {
  return current2().observeWithAI(userId, text, amygdalaResult, config6, neuroConfig, logger7);
}
...
function createPredictiveEngine(workspaceDir) {
  const storageDir9 = workspaceDir ? join4(workspaceDir, ".brainagent", "predictions") : "";
  ...
  let lastDomain;
  if (storageDir9 && !existsSync3(storageDir9)) {
    mkdirSync3(storageDir9, { recursive: true });
  }
  function loadPatterns() {
    if (!storageDir9) return;
    try {
      const tPath = join4(storageDir9, "temporal.json");
    ...
    try {
      const sPath = join4(storageDir9, "sequential.json");
    ...
    try {
      const cPath = join4(storageDir9, "contextual.json");
...
var active3;
function current3() {
  return active3 ?? (active3 = createPredictiveEngine(""));
}
function initPredictiveStorage(workspaceDir) {
  active3 = createPredictiveEngine(workspaceDir);
}
function observeInteraction(domain, keywords, context) {
  current3().observeInteraction(domain, keywords, context);
}
function predict(currentContext) {
  return current3().predict(currentContext);
}
...
function createBasalGanglia(workspaceDir) {
  const storageDir9 = workspaceDir ? join5(workspaceDir, ".brainagent", "habits") : "";
  let habits = [];
  ...
  if (storageDir9 && !existsSync4(storageDir9)) {
    mkdirSync4(storageDir9, { recursive: true });
  }
  function loadHabits() {
    if (!storageDir9) return;
    try {
      const path = join5(storageDir9, "habits.json");
      if (existsSync4(path)) {
  ...
  function persistHabits() {
    if (!storageDir9) return;
    try {
      writeFileSync5(join5(storageDir9, "habits.json"), JSON.stringify(habits, null, 2), "utf-8");
    } catch {
    }
  }
...
async function detectReinforcementWithAI(text, config6, logger7) {
  const content = await callLLM(REINFORCEMENT_PROMPT, text, config6, logger7, 100);
...
var active4;
function current4() {
  return active4 ?? (active4 = createBasalGanglia(""));
}
function initBasalStorage(workspaceDir) {
  active4 = createBasalGanglia(workspaceDir);
}
function findHabit(input, domain) {
  return current4().findHabit(input, domain);
}
function recordPattern(cue, routine, domain, exampleResponse) {
  return current4().recordPattern(cue, routine, domain, exampleResponse);
}
function reinforce(habitId, signal) {
  current4().reinforce(habitId, signal);
}
function getBasalStats() {
  return current4().getStats();
}
...
function createIntrospection(workspaceDir, config6) {
  const storageDir9 = workspaceDir ? join6(workspaceDir, ".brainagent", "introspection") : "";
  const traces = [];
  let currentTrace = null;
  let maxTraces = config6?.introspection.maxTraces ?? 3;
  const injectConfidence = config6?.introspection.injectConfidence ?? true;
  const selfDialogue = [];
  const metaSnapshots = [];
  let maxSelfDialogue = config6?.introspection.maxSelfDialogue ?? 10;
  let maxMetaSnapshots = config6?.introspection.maxMetaSnapshots ?? 5;
  function loadState9() {
    if (!storageDir9) return;
    try {
      const path = join6(storageDir9, "traces.json");
      if (existsSync5(path)) {
  ...
  if (storageDir9) {
    if (!existsSync5(storageDir9)) {
      mkdirSync5(storageDir9, { recursive: true });
    }
    maxTraces = config6?.introspection.maxTraces ?? maxTraces;
    maxSelfDialogue = config6?.introspection.maxSelfDialogue ?? maxSelfDialogue;
    maxMetaSnapshots = config6?.introspection.maxMetaSnapshots ?? maxMetaSnapshots;
    loadState9();
  }
  return {
...
var active5 = null;
function current5() {
  if (!active5) active5 = createIntrospection("");
  return active5;
}
function initIntrospection(workspaceDir, config6) {
  active5 = createIntrospection(workspaceDir, config6);
}
function startTrace(input) {
  current5().startTrace(input);
}
function addTraceStep(module, hook, output) {
  current5().addTraceStep(module, hook, output);
}
function completeTrace(cerebellumPassed, issues, reward) {
  current5().completeTrace(cerebellumPassed, issues, reward);
}
function buildConfidenceContext() {
  return current5().buildConfidenceContext();
}
function getLastTrace() {
  return current5().getLastTrace();
}
function getIntrospectionStats() {
  return current5().getIntrospectionStats();
}
function reflectOnConsciousness() {
  return current5().reflectOnConsciousness();
}
...
function createDopamineSystem(workspaceDir) {
  const storageDir9 = join7(workspaceDir, ".brainagent", "neuromodulators");
  if (!existsSync6(storageDir9)) {
    mkdirSync6(storageDir9, { recursive: true });
  }
  cancelPersist(join7(storageDir9, "state.json"));
  let state3 = {
    dopamine: 0.5,
    serotonin: 0.6,
    norepinephrine: 0.3,
  ...
  function loadState9() {
    try {
      const path = join7(storageDir9, "state.json");
      if (existsSync6(path)) {
  ...
  loadState9();
  function updateDopamine(predictionError, config6) {
    const baseline = config6.neuromodulators.baselineDopamine;
    const decay = config6.neuromodulators.dopamineDecayRate;
    const spike = predictionError * 0.5;
    state3.dopamine = state3.dopamine * (1 - decay) + (baseline + spike) * decay;
    state3.dopamine = Math.max(0, Math.min(1, state3.dopamine));
  }
  function updateSerotonin(reward) {
    const alpha = 0.15;
    positiveOutcomeRatio = positiveOutcomeRatio * (1 - alpha) + (reward > 0 ? 1 : 0) * alpha;
    state3.serotonin = 0.3 + positiveOutcomeRatio * 0.5;
    state3.serotonin = Math.max(0.1, Math.min(0.95, state3.serotonin));
  }
  function updateNorepinephrine(complexity, emotion) {
    ...
    const alpha = 0.3;
    state3.norepinephrine = state3.norepinephrine * (1 - alpha) + target * alpha;
    state3.norepinephrine = Math.max(0.05, Math.min(0.95, state3.norepinephrine));
  }
  function updateAcetylcholine(_domain, _config) {
    ...
    const alpha = 0.2;
    state3.acetylcholine = state3.acetylcholine * (1 - alpha) + target * alpha;
    state3.acetylcholine = Math.max(0.1, Math.min(0.9, state3.acetylcholine));
  }
  function processOutcome(params, config6) {
    totalInteractions++;
    let reward = 0;
    ...
    updateDopamine(predictionError, config6);
    updateSerotonin(reward);
    updateNorepinephrine(params.complexity, params.emotion);
    updateAcetylcholine(params.domain, config6);
    ...
    bus.emitSync("neuromodulator:state-changed", { ...state3 });
  ...
  return {
    ...
    getState: () => ({ ...state3 }),
    getEffectiveLearningRate: (baseLearningRate, config6) => {
      const achBoost = 1 + (state3.acetylcholine - 0.5) * (config6.neuromodulators.acetylcholineLearningBoost - 1);
      const dopamineBoost = 0.7 + state3.dopamine * 0.6;
      return baseLearningRate * achBoost * dopamineBoost;
    },
    getRiskTolerance: () => state3.serotonin,
    getAttentionLevel: () => state3.norepinephrine,
    getStats: getStats2,
    stop: () => {
    },
    dispose: () => {
...
function initDopamineSystem(workspaceDir) {
  active6?.dispose();
  active6 = createDopamineSystem(workspaceDir);
}
function processInteractionOutcome(params, config6) {
  if (!active6) {
    return {
      reward: 0,
      predictionError: 0,
  ...
  return active6.processOutcome(params, config6);
}
function markNovelty() {
  active6?.markNovelty();
}
function getNeuromodulatorState() {
  return active6?.getState() ?? {
    dopamine: 0.5,
    serotonin: 0.6,
    norepinephrine: 0.3,
    acetylcholine: 0.4
...
function getAttentionLevel() {
  return active6?.getAttentionLevel() ?? 0.3;
}
function getDopamineStats() {
  return active6?.getStats() ?? {
...
function createNeuralPathways(workspaceDir, config6, logger7) {
  const storageDir9 = join8(workspaceDir, ".brainagent", "synapses");
  if (!existsSync7(storageDir9)) {
    mkdirSync7(storageDir9, { recursive: true });
  }
  ...
  function applyHebbianLearning(reward) {
    ...
    const { learningRate, decayRate, minWeight, maxWeight } = config6.synapticPlasticity;
    const now = Date.now();
    for (const pathwayName of activatedPathways) {
      ...
      if (weight.recentActivations.length > config6.synapticPlasticity.activationHistorySize) {
        weight.recentActivations = weight.recentActivations.slice(
          -config6.synapticPlasticity.activationHistorySize
        );
      }
  ...
  function loadSynapticState() {
    try {
      const path = join8(storageDir9, "weights.json");
      if (existsSync7(path)) {
  ...
  function saveSynapticState() {
    try {
      writeFileSync7(
        join8(storageDir9, "weights.json"),
  ...
  loadSynapticState();
  synapticState.learningRate = config6.synapticPlasticity.learningRate;
  synapticState.decayRate = config6.synapticPlasticity.decayRate;
  const unsubs = [];
  unsubs.push(
...
var active7;
function initNeuralPathways(workspaceDir, config6, logger7) {
  active7?.dispose();
  active7 = createNeuralPathways(workspaceDir, config6, logger7);
}
function resetCycleState() {
  active7?.resetCycleState();
}
function getPathwayStats() {
  return active7?.getPathwayStats() ?? {
    pathwayCount: 8,
...
function getSynapticStats() {
  return active7?.getSynapticStats() ?? computeSynapticStats(createDefaultSynapticState());
}
function buildNeuromodulatorContext() {
  return active7?.buildNeuromodulatorContext();
}
...
function createWorkingMemory(workspaceDir, opts) {
  const storageDir9 = workspaceDir ? join9(workspaceDir, ".brainagent", "working-memory") : "";
  const maxEntries = opts.maxEntries;
  const summaryMaxLength = opts.summaryMaxLength;
  let entries = [];
  if (storageDir9 && !existsSync8(storageDir9)) {
    mkdirSync8(storageDir9, { recursive: true });
  }
  function loadState9() {
    if (!storageDir9) return;
    try {
      const path = join9(storageDir9, "state.json");
      if (existsSync8(path)) {
  ...
  function persistState7() {
    if (!storageDir9) return;
    try {
      writeFileSync8(join9(storageDir9, "state.json"), JSON.stringify(entries, null, 2), "utf-8");
    } catch {
    }
  }
  loadState9();
...
var active8;
function current6() {
  return active8 ?? (active8 = createWorkingMemory("", {
    maxEntries: DEFAULT_MAX_ENTRIES,
    summaryMaxLength: DEFAULT_SUMMARY_MAX_LENGTH
  }));
}
function initWorkingMemoryStorage(workspaceDir, config6) {
  active8 = createWorkingMemory(workspaceDir, {
    maxEntries: config6.workingMemory.maxEntries,
    summaryMaxLength: config6.workingMemory.summaryMaxLength
  });
}
function storeCompletedCycle(entry) {
  current6().storeCompletedCycle(entry);
}
function buildWorkingMemoryContext(currentInput) {
  return current6().buildContext(currentInput);
}
function getWorkingMemoryStats() {
  return current6().getStats();
}
function truncateForWorkingMemory(text) {
  return current6().truncate(text);
}
...
async function extractFactsWithAI(text, config6, logger7) {
  const content = await callLLM(EXTRACTION_PROMPT, text, config6, logger7, 500);
  if (!content) return [];
  const facts = parseFactsFromResponse(content);
  if (facts.length > 0) {
    logger7?.info(`BrainAgent AI Extractor: found ${facts.length} facts`);
  }
  return facts;
}
...
function isAIProviderAvailable2(config6) {
  return isAIProviderAvailable(config6);
}
...
async function validateAsync(response, originalInput, config6, classification, assessment, userModel, logger7) {
  const heuristicResult = validate(response, originalInput, classification, assessment, userModel);
  if (!isAIProviderAvailable(config6)) {
...
\u041E\u0442\u0432\u0435\u0442: ${response}`;
    const aiResponse = await callLLM(QUALITY_PROMPT, userText, config6, logger7, 300);
    if (aiResponse) {
    ...
    logger7?.info(`BrainAgent Cerebellum: AI validation error \u2014 ${String(err)}`);
  }
  return heuristicResult;
}
...
async function extractProcedureAsync(text, config6, classification, logger7) {
  if (isAIProviderAvailable(config6)) {
    try {
      const aiResponse = await callLLM(PROCEDURE_PROMPT, text, config6, logger7, 300);
      if (aiResponse) {
        const parsed = parseProcedureResponse(aiResponse, classification);
        if (parsed) return parsed;
      }
    } catch (err) {
      logger7?.info(`BrainAgent Procedural: AI extraction error \u2014 ${String(err)}`);
    }
  }
...
function initStructuralPlasticity(workspaceDir, config6, log) {
  currentConfig = config6;
  logger = log;
  storageDir = join10(workspaceDir, ".brainagent", "structural");
...
function createLearningCoordinator(workspaceDir, config6) {
  const storageDir9 = join11(workspaceDir, ".brainagent", "learning");
  ...
  function loadState9() {
    try {
      const path = join11(storageDir9, "coordinator.json");
  ...
  function persistState7() {
    try {
      ...
      writeFileSync10(
        join11(storageDir9, "coordinator.json"),
        JSON.stringify(
          {
  ...
  function processRewardSignal(signal) {
    ...
    if (cycleCount >= config6.learning.minCyclesForInsights && cycleCount % 5 === 0) {
      generateInsights(config6);
    }
    if (cycleCount % 10 === 0) {
...
var active9;
function initLearningCoordinator(workspaceDir, config6) {
  active9?.dispose();
  active9 = createLearningCoordinator(workspaceDir, config6);
}
function getLearningStats() {
  return active9?.getStats() ?? {
    cycleCount: 0,
    moduleCount: 0,
    activeInsights: 0,
    recentInsights: [],
...
function buildLearningContext() {
  return active9?.buildContext();
}
function recordRecurringIssue(issueType) {
  return active9?.recordRecurringIssue(issueType);
}
function recordDomainPerformance(domain, reward, errorIssues = []) {
  active9?.recordDomainPerformance(domain, reward, errorIssues);
}
function buildCapabilityContext(currentDomain) {
  return active9?.buildCapabilityContext(currentDomain);
}
...
var logger2;
var state2 = createDefaultState2();
var patternHistory = [];
...
function initEmergentModules(workspaceDir, config6, log) {
  currentConfig2 = config6;
  logger2 = log;
  storageDir2 = join12(workspaceDir, ".brainagent", "emergent");
  if (!existsSync11(storageDir2)) {
    mkdirSync11(storageDir2, { recursive: true });
  }
  state2.minOccurrences = config6.emergentModules.minOccurrences;
  state2.minReward = config6.emergentModules.minRewardForEstablishment;
  loadState2();
  logger2?.info(`EmergentModules: initialized with ${state2.modules.length} discovered patterns`);
}
function recordPattern2(participants, domain, reward) {
  if (!currentConfig2 || participants.length < 2) return;
  const now = Date.now();
...
function updateExistingModule(module, reward) {
  ...
  if (module.status === "emerging" && module.occurrences >= state2.minOccurrences * 2 && module.avgReward >= state2.minReward) {
...
function checkForNewModule(participants, domain) {
  if (!currentConfig2) return;
  const sortedParticipants = [...participants].sort();
  const patternKey = sortedParticipants.join("+");
  const matchingPatterns = patternHistory.filter((p) => {
    const sortedP = [...p.participants].sort();
    return sortedP.length === sortedParticipants.length && sortedP.every((m, i) => m === sortedParticipants[i]) && p.reward >= state2.minReward * 0.8;
  });
  if (matchingPatterns.length < state2.minOccurrences) return;
  const activeModules = state2.modules.filter((m) => m.status !== "deprecated").length;
  if (activeModules >= currentConfig2.emergentModules.maxEmergentModules) {
    const weakest = state2.modules.filter((m) => m.status !== "deprecated").sort((a, b) => a.avgReward - b.avgReward)[0];
    if (weakest && weakest.avgReward < state2.minReward * 0.5) {
  ...
  if (avgReward < state2.minReward * 0.8) return;
  const newModule = {
...
function deprecateUnusedModules() {
  ...
  for (const module of state2.modules) {
    ...
    if (recentUses.length === 0 && module.status === "established") {
      ...
      logger2?.info(`EmergentModule: "${module.name}" demoted to EMERGING (no recent use)`);
      ...
      logger2?.info(`EmergentModule: "${module.name}" DEPRECATED (unused)`);
    }
  }
}
function getEmergentStats() {
  const emerging = state2.modules.filter((m) => m.status === "emerging").length;
  const established = state2.modules.filter((m) => m.status === "established").length;
  const deprecated = state2.modules.filter((m) => m.status === "deprecated").length;
  const topModules = state2.modules.filter((m) => m.status !== "deprecated").sort((a, b) => b.avgReward - a.avgReward).slice(0, 5).map((m) => ({
    name: m.name,
  ...
  return {
    totalDiscovered: state2.modules.length,
    emerging,
    established,
    deprecated,
    topModules
...
function loadState2() {
  try {
    const path = join12(storageDir2, "state.json");
    if (existsSync11(path)) {
      const data = JSON.parse(readFileSync11(path, "utf-8"));
      state2.modules = data.modules ?? [];
    }
  } catch {
  }
}
function saveState2() {
  try {
    writeFileSync11(join12(storageDir2, "state.json"), JSON.stringify(state2, null, 2), "utf-8");
  } catch {
  }
}
...
function createMetabolicBudget(workspaceDir, config6, log) {
  const storageDir9 = join13(workspaceDir, ".brainagent", "metabolic");
  if (!existsSync12(storageDir9)) {
    mkdirSync12(storageDir9, { recursive: true });
  }
  const state3 = createDefaultState3();
  state3.totalBudget = config6.metabolicBudget.totalBudget;
  state3.regenRate = config6.metabolicBudget.regenRate;
  const periodStats = {};
  ...
  function loadState9() {
    try {
      const path = join13(storageDir9, "state.json");
      if (existsSync12(path)) {
        const data = JSON.parse(readFileSync12(path, "utf-8"));
        for (const module of ALL_MODULES2) {
          if (data.moduleEnergies[module]) {
            state3.moduleEnergies[module] = data.moduleEnergies[module];
          }
        }
        state3.cyclesSinceRebalance = data.cyclesSinceRebalance ?? 0;
      }
    } catch {
    }
  }
  function saveState3() {
    try {
      writeFileSync12(join13(storageDir9, "state.json"), JSON.stringify(state3, null, 2), "utf-8");
    } catch {
    }
  }
  loadState9();
  ...
  function consumeEnergy2(module, amount) {
    const moduleEnergy = state3.moduleEnergies[module];
    ...
    if (moduleEnergy.energy < config6.metabolicBudget.lowPowerThreshold) {
  ...
  function rebalanceEnergy() {
    ...
    for (const perf of performances) {
      const moduleEnergy = state3.moduleEnergies[perf.module];
      const normalizedScore = Math.max(0.1, perf.score + 1) / totalScore;
      const newBaseEnergy = state3.totalBudget / ALL_MODULES2.length * (0.5 + normalizedScore);
  ...
  function endCycle3() {
    state3.cyclesSinceRebalance++;
    for (const module of ALL_MODULES2) {
      const moduleEnergy = state3.moduleEnergies[module];
      const regenAmount = state3.regenRate * moduleEnergy.performance;
      moduleEnergy.energy = Math.min(moduleEnergy.baseEnergy, moduleEnergy.energy + regenAmount);
      if (moduleEnergy.lowPowerMode && moduleEnergy.energy > config6.metabolicBudget.lowPowerThreshold * 2) {
    ...
    if (state3.cyclesSinceRebalance >= config6.metabolicBudget.rebalanceInterval) {
      rebalanceEnergy();
      state3.cyclesSinceRebalance = 0;
      saveState3();
    }
  }
  function getStats2() {
    const usedEnergy = ALL_MODULES2.reduce(
      (sum, m) => sum + (state3.moduleEnergies[m]?.baseEnergy ?? 0) - (state3.moduleEnergies[m]?.energy ?? 0),
      0
    );
    const modules = ALL_MODULES2.map((m) => ({
      name: m,
      energy: state3.moduleEnergies[m].energy,
      baseEnergy: state3.moduleEnergies[m].baseEnergy,
      performance: state3.moduleEnergies[m].performance,
      lowPowerMode: state3.moduleEnergies[m].lowPowerMode
    }));
    const lowPowerModules = ALL_MODULES2.filter((m) => state3.moduleEnergies[m].lowPowerMode);
    const topPerformers = [...ALL_MODULES2].sort((a, b) => state3.moduleEnergies[b].performance - state3.moduleEnergies[a].performance).slice(0, 3);
    return {
      totalBudget: state3.totalBudget,
      usedEnergy,
      cyclesSinceRebalance: state3.cyclesSinceRebalance,
      modules,
      lowPowerModules,
      topPerformers
    };
  ...
  log?.info(
    `MetabolicBudget: initialized with total budget ${state3.totalBudget.toFixed(1)} energy units`
  );
  return {
    ...
    endCycle: endCycle3,
    getModuleEnergy: (module) => state3.moduleEnergies[module]?.energy ?? 1,
    isModuleLowPower: (module) => state3.moduleEnergies[module]?.lowPowerMode ?? false,
    getStats: getStats2,
...
var active10;
function initMetabolicBudget(workspaceDir, config6, log) {
  active10?.dispose();
  active10 = createMetabolicBudget(workspaceDir, config6, log);
}
function consumeEnergy(module, amount) {
  return active10?.consumeEnergy(module, amount) ?? true;
}
function recordPerformance(module, reward) {
  active10?.recordPerformance(module, reward);
}
function endCycle2() {
  active10?.endCycle();
}
function getMetabolicStats() {
  return active10?.getStats() ?? {
    totalBudget: 10,
    usedEnergy: 0,
    cyclesSinceRebalance: 0,
...
function initAgentIdentity(workspaceDir, config6) {
  storageDir3 = join14(workspaceDir, ".brainagent", "identity");
  if (!existsSync13(storageDir3)) {
    mkdirSync13(storageDir3, { recursive: true });
  }
  snapshotInterval = config6.agentIdentity.snapshotInterval;
  maxSnapshots = config6.agentIdentity.maxSnapshots;
  maxAutobioMemories = config6.agentIdentity.maxAutobiographicalMemories ?? 100;
  significantRewardThreshold = config6.agentIdentity.significantRewardThreshold ?? 0.8;
  significantEmotionThreshold = config6.agentIdentity.significantEmotionThreshold ?? 0.7;
  capabilities = {};
  snapshots = [];
  lessonsLearned = [];
  totalCycles = 0;
...
function loadState3() {
  ...
  try {
    const path = join14(storageDir3, "state.json");
...
function createEmotionalMemory(workspaceDir, config6) {
  const storageDir9 = workspaceDir ? join15(workspaceDir, ".brainagent", "emotional-memory") : "";
  if (storageDir9 && !existsSync14(storageDir9)) {
    mkdirSync14(storageDir9, { recursive: true });
  }
  let state3 = { flashbulbCount: 0, emotionMatchBoosts: 0 };
  let qualiaHistory = [];
  const maxQualiaHistory = config6.emotionalMemory.maxQualiaHistory ?? 10;
  const metaphorCache = /* @__PURE__ */ new Map();
  let lastLLMCallTimestamp = 0;
  function loadState9() {
    if (!storageDir9) return;
    try {
      const path = join15(storageDir9, "state.json");
  ...
  async function generateQualiaAsync2(emotion, intensity, domain, neuromodulators, llmConfig, logger7) {
...
${neuroDesc}`;
    try {
      ...
      const response = await callLLM(QUALIA_METAPHOR_PROMPT, userMessage, llmConfig, logger7, 200);
      if (!response) {
        logger7?.info("BrainAgent Qualia: LLM returned null, falling back to template");
        return generateQualia(emotion, intensity, domain, neuromodulators);
      }
      const parsed = parseLLMQualiaResponse(response);
      if (!parsed) {
        logger7?.info("BrainAgent Qualia: failed to parse LLM response, falling back to template");
        return generateQualia(emotion, intensity, domain, neuromodulators);
      }
      const qualia = {
        timestamp: now,
      ...
      persistState7();
      logger7?.info(`BrainAgent Qualia: LLM-generated metaphor for ${emotion}/${domain}`);
      return qualia;
    } catch (err) {
      logger7?.info(`BrainAgent Qualia: AI metaphor generation failed \u2014 ${String(err)}`);
      return generateQualia(emotion, intensity, domain, neuromodulators);
    }
  }
  function dispose() {
...
function current7() {
  if (!active11) {
    active11 = createEmotionalMemory("", DEFAULT_CONFIG);
  }
  return active11;
}
function initEmotionalMemory(workspaceDir, config6) {
  active11?.dispose();
  active11 = createEmotionalMemory(workspaceDir, config6);
}
function tagEmotionalContext(emotion, intensity) {
  return current7().tagEmotionalContext(emotion, intensity);
}
async function generateQualiaAsync(emotion, intensity, domain, neuromodulators, config6, logger7) {
  return current7().generateQualiaAsync(emotion, intensity, domain, neuromodulators, config6, logger7);
}
...
function initQualiaSimulator(workspaceDir, config6) {
  storageDir4 = join16(workspaceDir, ".brainagent", "qualia-simulator");
  if (!existsSync15(storageDir4)) {
    mkdirSync15(storageDir4, { recursive: true });
  }
  minIntensityForInjection = config6.qualiaSimulator.minIntensityForInjection;
  maxLog = 20;
  currentQualia = null;
  qualiaLog = [];
  loadState4();
...
function loadState4() {
  ...
  try {
    const path = join16(storageDir4, "state.json");
...
function describeNeuromodulatorFeel(state3) {
  const { dopamine, serotonin, norepinephrine, acetylcholine } = state3;
  const parts = [];
...
function createDMN(workspaceDir, config6, log) {
  const storageDir9 = workspaceDir ? join17(workspaceDir, ".brainagent", "dmn") : "";
  const insights = [];
  let lastRunTimestamp = 0;
  let totalAssociationsFound = 0;
  let currentConfig3 = null;
  let logger7 = log;
  const innerMonologue = [];
  let maxBackgroundThoughts = config6?.dmn?.maxBackgroundThoughts ?? 20;
  function loadState9() {
    if (!storageDir9) return;
    try {
      const path = join17(storageDir9, "state.json");
  ...
  async function runAssociationFinding2(cfg) {
    ...
    persistState7();
    logger7?.info(`DMN: found ${newInsights.length} cross-domain associations`);
    return newInsights;
  }
...
${insight.insightText}
  ...
  function generateBackgroundThoughts2(cfg, unresolvedQuestions, recentEmotions, knowledgeGaps) {
    ...
    persistState7();
    logger7?.info(`DMN: generated ${newThoughts.length} background thoughts`);
    return newThoughts;
  }
...
${lines}
  ...
  if (storageDir9) {
    if (!existsSync16(storageDir9)) {
      mkdirSync16(storageDir9, { recursive: true });
    }
    currentConfig3 = config6 ?? null;
    loadState9();
  }
  return {
    init,
...
var active12 = null;
function current8() {
  if (!active12) active12 = createDMN("");
  return active12;
}
function initDMN(workspaceDir, config6, log) {
  active12?.stop();
  active12 = createDMN(workspaceDir, config6, log);
}
function runAssociationFinding(config6) {
  return current8().runAssociationFinding(config6);
}
function getDMNStats() {
  return current8().getStats();
}
function generateBackgroundThoughts(config6, unresolvedQuestions, recentEmotions, knowledgeGaps) {
  return current8().generateBackgroundThoughts(
    config6,
    unresolvedQuestions,
    recentEmotions,
    knowledgeGaps
...
function buildBackgroundThoughtContext() {
  return current8().buildBackgroundThoughtContext();
}
function getRecentUnusedInsights(maxAge = 24 * 60 * 60 * 1e3) {
  return current8().getRecentUnusedInsights(maxAge);
}
...
function createTemporalBinding(workspaceDir, opts) {
  const storageDir9 = workspaceDir ? join18(workspaceDir, ".brainagent", "temporal-binding") : "";
  const maxMoments = opts.maxMoments;
  let moments = [];
  let idCounter2 = 0;
  if (storageDir9 && !existsSync17(storageDir9)) {
    mkdirSync17(storageDir9, { recursive: true });
  }
  function loadState9() {
    if (!storageDir9) return;
    try {
      const path = join18(storageDir9, "state.json");
  ...
  function persistState7() {
    if (!storageDir9) return;
    try {
      writeFileSync17(join18(storageDir9, "state.json"), JSON.stringify(moments, null, 2), "utf-8");
    } catch {
    }
  }
  loadState9();
...
var active13;
function current9() {
  return active13 ?? (active13 = createTemporalBinding("", { maxMoments: DEFAULT_MAX_MOMENTS }));
}
function initTemporalBinding(workspaceDir, config6) {
  active13 = createTemporalBinding(workspaceDir, { maxMoments: config6.temporalBinding.maxMoments });
}
function createMoment(input, thoughts, emotion, emotionIntensity, activeMemoryIds, intentions, confidence, domain) {
  return current9().createMoment(
    input,
    thoughts,
    emotion,
    emotionIntensity,
...
function buildTemporalContext(n = 3) {
  return current9().buildContext(n);
}
...
function createSessionBridge(workspaceDir, config6, _logger) {
  const storageDir9 = workspaceDir ? join19(workspaceDir, ".brainagent", "sessions") : "";
  if (storageDir9 && !existsSync18(storageDir9)) {
    mkdirSync18(storageDir9, { recursive: true });
  }
  const gapThresholdMs = config6.sessionBridge.gapThresholdMs;
  const maxSummaryTopics = config6.sessionBridge.maxSummaryTopics;
  ...
  function loadState9() {
    if (!storageDir9) return;
    try {
      const currentPath = join19(storageDir9, "current.json");
    ...
    try {
      const lastPath = join19(storageDir9, "last.json");
  ...
  function persistCurrent() {
    ...
    try {
      writeFileSync18(
        join19(storageDir9, "current.json"),
  ...
  function persistLast() {
    if (!storageDir9) return;
    try {
      writeFileSync18(join19(storageDir9, "last.json"), JSON.stringify(lastSession, null, 2), "utf-8");
    } catch {
    }
  }
...
function current10() {
  if (!active14) {
    active14 = createSessionBridge("", DEFAULT_CONFIG);
  }
  return active14;
}
function initSessionBridge(workspaceDir, config6, logger7) {
  active14?.dispose();
  active14 = createSessionBridge(workspaceDir, config6, logger7);
}
function recordCycleForSession(input, classification, assessment, reward) {
  current10().recordCycleForSession(input, classification, assessment, reward);
}
function checkSessionGap() {
  return current10().checkSessionGap();
}
function buildSessionBridgeContext() {
  return current10().buildSessionBridgeContext();
}
function getSessionBridgeStats() {
  return current10().getSessionBridgeStats();
}
...
function createCuriosityDrive(workspaceDir, config6) {
  const storageDir9 = workspaceDir ? join20(workspaceDir, ".brainagent", "curiosity") : "";
  const gaps = [];
  let totalDetected = 0;
  let questionsGenerated = 0;
  let gapsFilled = 0;
  let maxGaps = config6?.curiosity?.maxGaps ?? 15;
  let minGapConfidence = config6?.curiosity?.minGapConfidence ?? 0.3;
  let askProbability = config6?.curiosity?.askProbability ?? 0.1;
  let idCounter2 = 0;
  function loadState9() {
    if (!storageDir9) return;
    try {
      const path = join20(storageDir9, "state.json");
...
${question}`;
  ...
  if (storageDir9) {
    if (!existsSync19(storageDir9)) {
      mkdirSync19(storageDir9, { recursive: true });
    }
    if (config6) {
      maxGaps = config6.curiosity.maxGaps;
      minGapConfidence = config6.curiosity.minGapConfidence;
      askProbability = config6.curiosity.askProbability;
    }
    loadState9();
  }
  return {
...
var active15 = null;
function current11() {
  if (!active15) active15 = createCuriosityDrive("");
  return active15;
}
function initCuriosityDrive(workspaceDir, config6) {
  active15?.stop();
  active15 = createCuriosityDrive(workspaceDir, config6);
}
function detectKnowledgeGap(topic, domain, recallWasEmpty, predictionConfidence) {
  current11().detectKnowledgeGap(topic, domain, recallWasEmpty, predictionConfidence);
}
function buildCuriosityContext(serotoninLevel, _acetylcholineLevel) {
  return current11().buildCuriosityContext(serotoninLevel, _acetylcholineLevel);
}
function markGapFilled(topic) {
  current11().markGapFilled(topic);
}
function getCuriosityStats() {
  return current11().getStats();
}
function getOpenGaps() {
  return current11().getOpenGaps();
}
...
function initGoalStack(workspaceDir, config6) {
  storageDir5 = join21(workspaceDir, ".brainagent", "goals");
  if (!existsSync20(storageDir5)) {
    mkdirSync20(storageDir5, { recursive: true });
  }
  maxGoals = config6.goalStack.maxGoals;
  defaultTTLMs = config6.goalStack.defaultTTLMs;
  maxDesires = config6.goalStack.maxDesires;
  maxDecisionLog = config6.goalStack.maxDecisionLog;
  explorationRate = config6.goalStack.explorationRate;
  goals = [];
  desires = [];
  decisionLog = [];
  idCounter = 0;
...
function loadState5() {
  ...
  try {
    const path = join21(storageDir5, "state.json");
...
function checkGoalTriggers(input, currentEmotion, currentDomain) {
  ...
  if (triggered.length > 0) persistState3();
  return triggered;
}
function expireGoals() {
  ...
  if (goals.length > 60) {
    const active29 = goals.filter((g) => g.status === "pending" || g.status === "triggered");
    const inactive = goals.filter((g) => g.status === "completed" || g.status === "expired").slice(-30);
    goals = [...active29, ...inactive];
    changed = true;
  }
  if (changed) persistState3();
}
...
function weakenDesiresAfterFire(consecutiveFires) {
  ...
  if (desires.length !== before || changed) persistState3();
}
...
async function extractGoalsFromConversation(userMessage, config6, logger7) {
  if (!isAIProviderAvailable(config6)) {
    logger7?.info("BrainAgent GoalStack: no AI provider available, skipping goal extraction");
    return [];
  }
  logger7?.info("BrainAgent GoalStack: extracting goals from conversation...");
  const response = await callLLM(GOAL_EXTRACTION_PROMPT, userMessage, config6, logger7, 300);
  if (!response) {
    logger7?.info("BrainAgent GoalStack: LLM returned null/empty response");
    return [];
  }
  logger7?.info(`BrainAgent GoalStack: LLM response received (${response.length} chars)`);
  let parsed;
  try {
    let jsonStr = response.trim();
    const jsonMatch = jsonStr.match(/\[[\s\S]*\]/);
    ...
    logger7?.info("BrainAgent GoalStack: failed to parse LLM goal extraction response");
    return [];
  }
  const createdGoals = [];
...
function createCircadianRhythm(workspaceDir, config6, log) {
  const storageDir9 = join22(workspaceDir, ".brainagent", "circadian");
  if (!existsSync21(storageDir9)) {
    mkdirSync21(storageDir9, { recursive: true });
  }
  const maxSleepConsolidations = config6.circadian.maxSleepConsolidations ?? 5;
  let state3 = createDefaultState4();
  let lastActivityTime = Date.now();
  let activityCounter = 0;
  let consolidationCallback = null;
  function loadState9() {
    try {
      const path = join22(storageDir9, "state.json");
      if (existsSync21(path)) {
        const data = JSON.parse(readFileSync21(path, "utf-8"));
        state3 = { ...createDefaultState4(), ...data };
        state3.phaseStartedAt = Date.now();
        state3.idleTime = 0;
  ...
  function persistState7() {
    try {
      writeFileSync21(join22(storageDir9, "state.json"), JSON.stringify(state3, null, 2), "utf-8");
    } catch {
    }
  }
  function evaluatePhase() {
    if (!config6.circadian.enabled) return;
    const now = Date.now();
    const idleTime = now - lastActivityTime;
    state3.idleTime = idleTime;
    const activityWindow = config6.circadian.activityWindowMs;
    state3.activityLevel = Math.max(0, 1 - idleTime / activityWindow);
    const phaseDuration = now - state3.phaseStartedAt;
    const { idleThresholdMs, minWakeDurationMs, minSleepDurationMs, transitionDurationMs } = config6.circadian;
    switch (state3.phase) {
      case "wake":
      ...
      case "transition-to-sleep":
        state3.phaseProgress = Math.min(1, phaseDuration / transitionDurationMs);
        updateModulationFromPhase();
        if (activityCounter > 0) {
          activityCounter = 0;
          transitionTo("wake");
        } else if (state3.phaseProgress >= 1) {
          transitionTo("sleep");
        }
        break;
      case "sleep":
        if (consolidationCallback && phaseDuration > 3e4 && state3.sleepConsolidations < maxSleepConsolidations) {
          const consolidationInterval = config6.circadian.sleepConsolidationIntervalMs ?? 6e4;
          const triggerWindow = consolidationInterval / 3;
          const shouldConsolidate = phaseDuration % consolidationInterval < triggerWindow && state3.sleepConsolidations === 0;
          if (shouldConsolidate || phaseDuration > state3.sleepConsolidations * consolidationInterval) {
  ...
  function transitionTo(newPhase) {
    const oldPhase = state3.phase;
    if (oldPhase === newPhase) return;
    log?.info(`Circadian: ${oldPhase} \u2192 ${newPhase}`);
    state3.phase = newPhase;
    state3.phaseStartedAt = Date.now();
    state3.phaseProgress = 0;
    if (newPhase === "wake") {
      state3.wakeInteractions = 0;
      bus.emitSync("circadian:wake-started", { idleTime: state3.idleTime });
    } else if (newPhase === "sleep") {
      state3.sleepConsolidations = 0;
      bus.emitSync("circadian:sleep-started", { wakeInteractions: state3.wakeInteractions });
    }
    updateModulationFromPhase();
  ...
  function updateModulationFromPhase() {
    const cfg = config6.circadian;
    switch (state3.phase) {
      case "wake":
        state3.wakeModulation = {
      ...
      case "transition-to-sleep": {
        ...
        const sleepProgress = state3.phaseProgress;
        state3.wakeModulation = {
      ...
      case "transition-to-wake": {
        ...
        const wakeProgress = state3.phaseProgress;
        state3.wakeModulation = {
  ...
  async function triggerSleepConsolidation() {
    ...
    state3.sleepConsolidations++;
    log?.info(
      `Circadian: sleep consolidation #${state3.sleepConsolidations} (intensity: ${(state3.sleepSettings.consolidationIntensity * 100).toFixed(0)}%)`
    );
    try {
  ...
  loadState9();
  updateModulationFromPhase();
  const evalInterval = config6.circadian.evaluationIntervalMs ?? 3e4;
  const evaluationTimer = setInterval(() => evaluatePhase(), evalInterval);
  log?.info(
    `Circadian: initialized in ${state3.phase} phase (idle threshold: ${config6.circadian.idleThresholdMs / 1e3}s)`
  );
  function stopTimer() {
  ...
  return {
    recordActivity: () => {
      lastActivityTime = Date.now();
      activityCounter++;
      state3.wakeInteractions++;
      bus.emitSync("circadian:activity-detected", { activityLevel: state3.activityLevel });
      if (state3.phase === "sleep" || state3.phase === "transition-to-sleep") {
        const phaseDuration = Date.now() - state3.phaseStartedAt;
        const minSleep = config6.circadian.minSleepDurationMs ?? 6e4;
        if (state3.phase === "transition-to-sleep" || phaseDuration >= minSleep) {
          transitionTo("transition-to-wake");
        }
      }
    },
    getState: () => ({ ...state3 }),
    getModulation: () => ({
      dopamine: state3.wakeModulation.dopamineBoost,
      serotonin: state3.wakeModulation.serotoninBoost,
      acetylcholine: state3.wakeModulation.acetylcholineBoost,
      norepinephrine: state3.wakeModulation.norepinephrineBoost
    }),
    getSleepSettings: () => ({ ...state3.sleepSettings }),
    isInSleepPhase: () => state3.phase === "sleep" || state3.phase === "transition-to-sleep",
    isInWakePhase: () => state3.phase === "wake" || state3.phase === "transition-to-wake",
    forcePhase: (phase) => {
    ...
    getStats: () => ({
      phase: state3.phase,
      phaseProgress: state3.phaseProgress,
      phaseDuration: Date.now() - state3.phaseStartedAt,
      idleTime: state3.idleTime,
      activityLevel: state3.activityLevel,
      wakeInteractions: state3.wakeInteractions,
      sleepConsolidations: state3.sleepConsolidations,
      modulation: {
        dopamine: state3.wakeModulation.dopamineBoost,
        serotonin: state3.wakeModulation.serotoninBoost,
        acetylcholine: state3.wakeModulation.acetylcholineBoost,
      ...
      sleepSettings: { ...state3.sleepSettings }
    }),
...
function initCircadianRhythm(workspaceDir, config6, log) {
  active16?.dispose();
  active16 = createCircadianRhythm(workspaceDir, config6, log);
  if (pendingConsolidationCallback) {
    active16.setConsolidationCallback(pendingConsolidationCallback);
  }
}
...
function setConsolidationCallback(callback) {
  ...
  active16?.setConsolidationCallback(callback);
}
function recordActivity() {
  active16?.recordActivity();
}
function getCircadianState() {
  return active16?.getState() ?? createDefaultState4();
}
function getSleepSettings() {
  return active16?.getSleepSettings() ?? {
    consolidationIntensity: 0.5,
    pruningAggressiveness: 0.3,
    synapticNormalization: false
  };
...
function isInSleepPhase() {
  return active16?.isInSleepPhase() ?? false;
}
function isInWakePhase() {
  return active16?.isInWakePhase() ?? true;
}
function forcePhase(phase) {
  active16?.forcePhase(phase);
}
function getCircadianStats() {
  const st = active16?.getState() ?? createDefaultState4();
  return {
    phase: st.phase,
...
var DriveEngine = class {
  constructor(spec, config6, workspaceDir, circadianEnabled3, deps5, logger7) {
    this.spec = spec;
    this.config = config6;
    this.circadianEnabled = circadianEnabled3;
    this.deps = deps5;
    this.logger = logger7;
    const storageDir9 = join23(workspaceDir, ".brainagent", spec.id);
    if (!existsSync22(storageDir9)) {
      mkdirSync22(storageDir9, { recursive: true });
    }
    this.stateFile = join23(storageDir9, "state.json");
    cancelPersist(this.stateFile);
    this.loadState();
    this.wireCoreListeners();
    logger7.info(
      `BrainAgent ${spec.logName}: initialized (satiation=${this.satiation.toFixed(2)}, decay=${config6.baseDecayRate}/${config6.decayIntervalMs}ms, domains=${config6.rewardDomains.join(",")})`
    );
  }
  spec;
  config;
  ...
  persistState() {
    schedulePersist(this.stateFile, () => {
      ...
      return JSON.stringify(state3, null, 2);
    });
  }
};
...
function initSocialDrive(workspaceDir, cfg, log, injectedDeps) {
  active17?.dispose();
  active17 = createSocialDrive(workspaceDir, cfg, log, injectedDeps);
}
...
function getSatiation() {
  return active17?.getSatiation() ?? 0;
}
...
function initCognitiveHunger(workspaceDir, cfg, log, injectedDeps) {
  active18?.dispose();
  active18 = createCognitiveHunger(workspaceDir, cfg, log, injectedDeps);
}
...
function getCognitiveHungerSatiation() {
  return active18?.getSatiation() ?? 0;
}
...
function initCreativeDrive(workspaceDir, cfg, log, injectedDeps) {
  active19?.dispose();
  active19 = createCreativeDrive(workspaceDir, cfg, log, injectedDeps);
}
...
function getCreativeDriveSatiation() {
  return active19?.getSatiation() ?? 0;
}
...
function initMasteryDrive(workspaceDir, cfg, log, injectedDeps) {
  storageDir6 = join24(workspaceDir, ".brainagent", "mastery-drive");
  if (!existsSync23(storageDir6)) {
    mkdirSync23(storageDir6, { recursive: true });
  }
  ...
  totalNeedSignals = 0;
  adaptiveDecayModifier = 1;
  cancelPersist(join24(storageDir6, "state.json"));
  loadState6();
  wireEventListeners();
  logger3.info(
...
function stopMasteryDrive() {
  ...
  unsubscribers.length = 0;
  persistState4();
  flushPersist(join24(storageDir6, "state.json"));
  logger3?.info("BrainAgent MasteryDrive: stopped.");
}
...
function biasDMNToMasteryThoughts(weakestDomain) {
  ...
  logger3?.info(`BrainAgent MasteryDrive: biased DMN toward ${topics.length} mastery topic(s)`);
}
...
function loadState6() {
  try {
    const filePath = join24(storageDir6, "state.json");
...
function persistState4() {
  schedulePersist(join24(storageDir6, "state.json"), () => {
    const domainMap = {};
    for (const [domain, mastery] of domainSatiations) {
...
function collectActiveDrives(drives, threshold) {
  ...
  if (drives.social) {
    ...
    if (s.need >= threshold) active29.push({ name: "social", need: s.need, needLevel: s.needLevel });
  }
  if (drives.cognitive) {
    const c = drives.cognitive();
    if (c.need >= threshold) active29.push({ name: "cognitive", need: c.need, needLevel: c.needLevel });
  }
  if (drives.creative) {
    const c = drives.creative();
    if (c.need >= threshold) active29.push({ name: "creative", need: c.need, needLevel: c.needLevel });
  }
  if (drives.mastery) {
    const m = drives.mastery();
    if (m.need >= threshold) active29.push({ name: "mastery", need: m.need, needLevel: m.needLevel });
  }
  return active29;
}
function synthesizeAutonomousCycleState(brainConfig, drives, cycle) {
  const active29 = collectActiveDrives(drives, 0.5);
  if (active29.length === 0) return;
  const ranked = active29.map((d) => {
...
function createAutonomousDeliverer(deps5) {
  const { state: state3, brainConfig, minGapMs, logger: logger7 } = deps5;
  return function enqueueAutonomousIntent(text) {
    const trimmed = text.trim();
    if (!trimmed) return;
    if (state3.previousCycleWasAutonomous) {
      logger7.info("BrainAgent Autonomy: intent suppressed \u2014 previous cycle was autonomous");
      return;
    }
    if (Date.now() - state3.lastAutonomousDeliveryAt < minGapMs) {
      logger7.info("BrainAgent Autonomy: intent suppressed \u2014 minimum gap not elapsed");
      return;
    }
    if (brainConfig.modules.proactiveFeedback) {
      const intentDomain = deps5.classifyDomain(trimmed).domain;
      if (deps5.isDomainSuppressed(intentDomain)) {
        logger7.info(
    ...
    const agent = deps5.pickAgent();
    if (!agent) {
      logger7.warn("BrainAgent Autonomy: no live agent \u2014 autonomous intent dropped");
      return;
    }
    const rejectionHints = brainConfig.modules.proactiveFeedback ? deps5.getSuppressedDomainHints() : [];
    ...
    state3.lastAutonomousDeliveryAt = Date.now();
    deps5.deliver(agent, framed);
  };
}
function createAutonomousIntentResolver(deps5) {
  const { state: state3, brainConfig, drives } = deps5;
  return () => {
    if (brainConfig.modules.goalStack && deps5.goalStack) {
      const stats2 = deps5.goalStack.getGoalStackStats();
      if (stats2.pending > 0) {
        const idleMs = brainConfig.circadian.enabled ? deps5.circadian?.getCircadianState().idleTime : void 0;
        const triggered = deps5.goalStack.checkAutonomousGoals(idleMs);
        if (triggered.length > 0) {
          const goalCtx = deps5.goalStack.buildGoalContext(triggered);
          if (goalCtx) {
            state3.lastAutonomousSource = `goal:${triggered[0].id}`;
            return {
              text: [
                AUTONOMOUS_TAG,
    ...
    if (brainConfig.modules.dmn && deps5.dmn) {
      const unused = deps5.dmn.getRecentUnusedInsights(30 * 60 * 1e3);
      const novel = unused.filter((i) => i.confidence > 0.5);
      if (novel.length > 0) {
        const best = novel[novel.length - 1];
        state3.lastAutonomousSource = "dmn:insight";
        return {
          text: [
            AUTONOMOUS_TAG,
            best.insightText,
    ...
    if (brainConfig.modules.goalStack && deps5.goalStack) {
      const strong = deps5.goalStack.getDesires().filter((d) => d.strength >= 0.9);
      if (strong.length > 0) {
        const top = strong.sort((a, b) => b.strength - a.strength)[0];
        state3.lastAutonomousSource = `desire:${top.id}`;
        return {
          text: [
            AUTONOMOUS_TAG,
            top.description,
    ...
    const active29 = collectActiveDrives(drives, 0.8);
    if (active29.length > 0) {
      const strongest = active29.sort((a, b) => b.need - a.need)[0];
      state3.lastAutonomousSource = `drive:${strongest.name}`;
      return {
        text: [
          AUTONOMOUS_TAG,
...
function createCycleEngine(deps5) {
  const { config: config6, brainConfig, getHostConfig, logger: logger7, markActivation, state: state3 } = deps5;
  ...
  async function endCycle3(key) {
    ...
    const aiAvailable = config6.modules.aiEnrichment && isAIProviderAvailable2(getHostConfig());
    if (brainConfig.modules.cerebellum && cycle.responseText.trim()) {
      ...
      if (result.issues.length > 0) {
        logger7.warn(`BrainAgent Cerebellum: quality issues \u2014 ${result.issues.join("; ")}`);
    ...
    if (brainConfig.modules.hippocampus && input.length > 5) {
      if (isAutonomousCycle && cycle.responseText.trim()) {
        ...
        state3.lastAutonomousEpisodeId = episode.id;
        state3.lastAutonomousDomain = cycle.classification?.domain ?? "unknown";
        state3.previousCycleWasAutonomous = true;
    ...
    if (config6.modules.semanticExtraction && semanticSource.length > 15 && isFactWorthy(semanticSource, cycle.classification)) {
      let factsStored = false;
      if (aiAvailable) {
        try {
          const aiFacts = await extractFactsWithAI(semanticSource, getHostConfig(), logger7);
          if (aiFacts.length > 0) {
          ...
          logger7.info(`BrainAgent Semantic: AI extraction failed, falling back \u2014 ${String(err)}`);
        }
      }
      if (!factsStored) {
      ...
      if (factsStored && brainConfig.modules.curiosityDrive) {
        ...
        for (const gap of getOpenGaps()) {
          if (inputLower.includes(gap.topic.toLowerCase())) {
            ...
            logger7.info(`BrainAgent Curiosity: gap filled for "${gap.topic}" via new fact storage`);
          }
        }
      }
    }
    if (config6.modules.proceduralExtraction && input.length > 10 && isProcedural(input, cycle.classification)) {
      const procedure = await extractProcedureAsync(input, getHostConfig(), cycle.classification, logger7);
      if (procedure && procedure.confidence > 0.5) {
        storeWorkflow(procedure.description, procedure.triggerPattern, procedure.steps);
        logger7.info(`BrainAgent Procedural: stored workflow "${procedure.description}"`);
      }
    }
    ...
    if (brainConfig.modules.neuromodulatorSystem && input.length > 5) {
      ...
      if (config6.modules.structuralPlasticity) {
        endCycle(reward);
      }
    ...
    if (brainConfig.modules.dmn) {
      ...
      if (wakeInteractionCount >= brainConfig.dmn.wakeThoughtInterval) {
        ...
        void runAssociationFinding(brainConfig).catch((err) => {
          logger7.info(`BrainAgent DMN: association finding error: ${err}`);
        });
      }
    }
    if (brainConfig.modules.goalStack && input.length > 10) {
      ...
      if (goalExtractionCounter >= brainConfig.goalStack.extractionInterval) {
        ...
        void extractGoalsFromConversation(input, getHostConfig(), logger7).catch((err) => {
          logger7.info(`BrainAgent GoalStack: extraction error: ${err}`);
        });
      }
    }
    if (brainConfig.modules.goalStack) {
...
function createStrategyBandit(workspaceDir, config6) {
  const storageDir9 = join25(workspaceDir, ".brainagent", "bandit");
  if (!existsSync24(storageDir9)) {
    mkdirSync24(storageDir9, { recursive: true });
  }
  const storageFile = join25(storageDir9, "state.json");
  const explorationConstant = config6.learningLoop.strategyBandit.explorationConstant;
  const attributionWindowMs = config6.learningLoop.strategyBandit.attributionWindowMs;
  let state3 = {};
  let lastChoices = {};
  function loadState9() {
    ...
    try {
      ...
      state3 = data && typeof data === "object" ? data : {};
    } catch {
    }
  }
  function persistState7() {
    schedulePersist(storageFile, () => JSON.stringify(state3, null, 2));
  }
  function recordOutcome(decisionPoint, arm, reward) {
    const point = state3[decisionPoint] ?? (state3[decisionPoint] = {});
  ...
  function getArmStats(decisionPoint) {
    const point = state3[decisionPoint] ?? {};
    const out = {};
  ...
  function getBanditStats() {
    let totalPlays = 0;
    for (const point of Object.values(state3)) {
      for (const rec of Object.values(point)) {
        totalPlays += rec.plays;
      }
    }
    return { decisionPoints: Object.keys(state3).length, totalPlays, initialized: true };
  }
  return {
...
var active20;
function initStrategyBandit(workspaceDir, config6) {
  active20?.dispose();
  active20 = createStrategyBandit(workspaceDir, config6);
}
...
function chooseArm(decisionPoint, arms) {
  ...
  return active20.chooseArm(decisionPoint, arms);
}
...
function initVitalImpulse(workspaceDir, cfg, log, injectedDeps) {
  storageDir7 = join26(workspaceDir, ".brainagent", "vital-impulse");
  if (!existsSync25(storageDir7)) {
    mkdirSync25(storageDir7, { recursive: true });
  }
  config2 = cfg.vitalImpulse;
  ...
  adaptiveSignalWeights = { ...DEFAULT_SIGNAL_WEIGHTS, ...config2.signalWeights };
  hebbianLearningRate = cfg.synapticPlasticity?.learningRate ?? 0.1;
  cancelPersist(join26(storageDir7, "state.json"));
  loadState7();
  wireSignalListeners();
  logger4.info(
    `BrainAgent VitalImpulse: initialized (threshold=${config2.firingThreshold}, refractory=${config2.refractoryPeriodMs}ms, decay=${config2.decayRate}/${config2.decayIntervalMs}ms)`
  );
}
function stopVitalImpulse() {
  ...
  unsubscribers2.length = 0;
  persistState5();
  flushPersist(join26(storageDir7, "state.json"));
  logger4?.info("BrainAgent VitalImpulse: stopped.");
}
...
function applyDecay() {
  ...
  if (currentPressure > 0 && referenceTime > 0) {
    const elapsedIntervals = (now - referenceTime) / config2.decayIntervalMs;
    if (elapsedIntervals > 0) {
      const decayFactor = Math.pow(1 - config2.decayRate, elapsedIntervals);
  ...
  if (habituationLevel > 0 && lastFireTime > 0) {
    ...
    const halfLife = config2.habituationHalfLifeMinutes ?? 5;
...
function onSignal(eventName, weight, description) {
  ...
  if (recentSignals.length > config2.maxRecentSignals) {
    recentSignals.shift();
  }
...
function evaluateFiring() {
  ...
  applyDecay();
  const now = Date.now();
  if (lastFireTime > 0 && now - lastFireTime < config2.refractoryPeriodMs) {
    return;
  }
  let effectiveThreshold = config2.firingThreshold;
  if (circadianEnabled2) {
    if (isInWakePhase()) {
      effectiveThreshold *= config2.circadianWakeModifier;
    } else if (isInSleepPhase()) {
      effectiveThreshold *= config2.circadianSleepModifier;
    }
  }
  ...
  } else if (firedPressure >= effectiveThreshold * (config2.genericFireMultiplier ?? 1.6)) {
...
function reinforceSignalWeights(reward) {
  ...
  if (elapsed > (config2?.hebbianWindowMs ?? 6e4)) return;
...
function forceImpulse(reason) {
  ...
  recentSignals = [];
  persistState5();
  logger4?.info("BrainAgent VitalImpulse: forced impulse fired.");
}
...
function getVitalImpulseStats() {
  applyDecay();
  let effectiveThreshold = config2?.firingThreshold ?? 0.7;
  if (circadianEnabled2) {
    if (isInWakePhase()) {
      effectiveThreshold *= config2?.circadianWakeModifier ?? 0.8;
    } else if (isInSleepPhase()) {
      effectiveThreshold *= config2?.circadianSleepModifier ?? 1.5;
    }
  }
  effectiveThreshold *= 1 + habituationLevel;
  const refractoryMs = config2?.refractoryPeriodMs ?? 0;
...
function loadState7() {
  try {
    const filePath = join26(storageDir7, "state.json");
...
function persistState5() {
  schedulePersist(join26(storageDir7, "state.json"), () => {
    const state3 = {
      lastFireTime,
      totalFires,
      totalSignalsReceived,
...
function createDriveArbiter(workspaceDir, cfg, getters, log) {
  const config6 = cfg.driveArbiter;
  const statGetters = getters;
  const logger7 = log;
  const unsubscribers4 = [];
  const storageDir9 = join27(workspaceDir, ".brainagent");
  if (!existsSync26(storageDir9)) {
    mkdirSync26(storageDir9, { recursive: true });
  }
  let driveWeights = {
    social: 1,
    cognitive: 1,
  ...
  function loadState9() {
    try {
      const path = join27(storageDir9, "drive-arbiter.json");
  ...
  function persistState7() {
    if (!storageDir9) return;
    schedulePersist(join27(storageDir9, "drive-arbiter.json"), () => {
      const data = {
        driveWeights,
        lastSelectedDrive,
        lastSelectionTime,
  ...
  cancelPersist(join27(storageDir9, "drive-arbiter.json"));
  loadState9();
  function arbitrate() {
    if (!config6 || isArbitrating) return;
    isArbitrating = true;
    try {
      arbitrateInner();
    } finally {
  ...
  function arbitrateInner() {
    ...
    const drives = [];
    if (social && social.need >= config6.minDriveNeed) {
      drives.push({ id: "social", need: social.need });
    }
    if (cognitive && cognitive.need >= config6.minDriveNeed) {
      drives.push({ id: "cognitive", need: cognitive.need });
    }
    if (creative && creative.need >= config6.minDriveNeed) {
      drives.push({ id: "creative", need: creative.need });
    }
    if (mastery && mastery.need >= config6.minDriveNeed) {
    ...
    const scores = drives.map((d) => {
      ...
      if (d.id === lastSelectedDrive && lastSelectionTime > 0) {
        ...
        const decayFactor = Math.pow(config6.recencyDecay, timeSinceLast / (5 * 60 * 1e3));
    ...
    let winner = scores[0];
    if (scores.length > 1 && Math.random() < config6.explorationRate) {
    ...
    if (conflictLog.length > (config6.maxConflictLog ?? 50)) {
      conflictLog = conflictLog.slice(-config6.maxConflictLog);
    }
  ...
  function processReward(signal) {
    if (!config6 || !lastSelectedDrive) return;
    ...
    const lr = config6.rewardLearningRate;
  ...
  logger7?.info("BrainAgent DriveArbiter: initialized");
  function getStats2() {
    return {
  ...
  function stop() {
    ...
    unsubscribers4.length = 0;
    persistState7();
    flushPersist(join27(storageDir9, "drive-arbiter.json"));
    logger7?.info("BrainAgent DriveArbiter: stopped.");
  }
  return {
    getStats: getStats2,
...
function initDriveArbiter(workspaceDir, cfg, getters, log) {
  active21?.stop();
  active21 = createDriveArbiter(workspaceDir, cfg, getters, log);
}
...
function buildArbiterContext() {
  return active21?.buildContext() ?? null;
}
...
function createInteroception(getters, log) {
  ...
  function evaluate() {
    ...
    const changed = !lastState || lastState.pattern !== state3.pattern || Math.abs(lastState.confidence - state3.confidence) > 0.15;
    lastState = state3;
    if (changed) {
      bus.emitSync("interoception:state-updated", {
        pattern: state3.pattern,
        confidence: state3.confidence,
        description: state3.description,
        aggregateNeed: state3.aggregateNeed
      });
      log?.info(
        `BrainAgent Interoception: ${state3.pattern} (confidence=${(state3.confidence * 100).toFixed(0)}%) \u2014 ${state3.description}`
      );
    }
  }
  unsubscribers4.push(
...
function initInteroception(getters, log) {
  active22?.dispose();
  active22 = createInteroception(getters, log);
}
...
function getInteroceptiveState() {
  return active22?.getState() ?? null;
}
function buildInteroceptionContext() {
  return active22?.buildContext() ?? null;
}
...
function createTemporalAwareness(workspaceDir, cfg, log) {
  const config6 = cfg.temporalAwareness;
  let timestamps = [];
  let typicalGapMs = 0;
  let totalInteractions = 0;
  const storageDir9 = join28(workspaceDir, ".brainagent");
  if (!existsSync27(storageDir9)) {
    mkdirSync27(storageDir9, { recursive: true });
  }
  try {
    const path = join28(storageDir9, "temporal-awareness.json");
  ...
  function persistState7() {
    try {
      ...
      writeFileSync22(
        join28(storageDir9, "temporal-awareness.json"),
...
function initTemporalAwareness(workspaceDir, cfg, log) {
  active23?.dispose();
  active23 = createTemporalAwareness(workspaceDir, cfg, log);
}
...
function recordInteraction() {
  active23?.recordInteraction();
}
function getTemporalAwarenessStats() {
  return active23?.getStats() ?? {
    typicalGapMs: 0,
    lastInteractionTime: 0,
    currentGapMs: 0,
...
function buildTemporalContext2() {
  return active23?.buildContext() ?? null;
}
...
function createAttentionGate(workspaceDir) {
  const storageDir9 = workspaceDir ? join29(workspaceDir, ".brainagent", "attention") : "";
  let totalProcessed = 0;
  let totalDropped = 0;
  let totalRelevanceSum = 0;
  if (storageDir9 && !existsSync28(storageDir9)) {
    mkdirSync28(storageDir9, { recursive: true });
  }
  function loadState9() {
    if (!storageDir9) return;
    try {
      const path = join29(storageDir9, "state.json");
  ...
  loadState9();
  function filter(injections, currentInput, norepinephrineLevel, config6, maxTokenBudget) {
    ...
    const effectiveThreshold = config6.attentionGate.minRelevanceScore * (1 - norepinephrineLevel * 0.5);
    scored.sort((a, b) => b.score - a.score);
    let kept = [];
    let dropped = 0;
    for (const item of scored) {
      if (kept.length < config6.attentionGate.maxContextSections && item.score >= effectiveThreshold) {
        kept.push(item.text);
...
var active24;
function current12() {
  return active24 ?? (active24 = createAttentionGate(""));
}
function initAttentionGate(workspaceDir, _config) {
  active24 = createAttentionGate(workspaceDir);
}
function filterContextInjections(injections, currentInput, norepinephrineLevel, config6, maxTokenBudget) {
  return current12().filter(injections, currentInput, norepinephrineLevel, config6, maxTokenBudget);
}
function getAttentionStats() {
  return current12().getStats();
}
...
function decideProcessingPath(classification, config6) {
  const threshold = COMPLEXITY_ORDER2[config6.dualProcess.system2Threshold];
  ...
  const modelOverride = useSlow ? config6.dualProcess.slowModel : config6.dualProcess.fastModel;
  const result = {
...
function assembleContext(state3) {
  const sections = [];
  if (state3.classification) {
    const meta = buildMetacognitiveInstruction(state3.classification);
    if (meta) sections.push(meta);
  }
  if (state3.priority) {
    const empathy = buildEmpathyGuidance(state3.priority);
    if (empathy) sections.push(empathy);
  }
  if (state3.userModel) {
    const userCtx = buildUserContext(state3.userModel);
    if (userCtx) sections.push(userCtx);
  }
  const memCtx = buildMemoryContext(state3.relevantMemories);
  if (memCtx) sections.push(memCtx);
  if (state3.relevantMemories.procedural.length > 0) {
    const procCtx = buildProceduralContext(state3.relevantMemories.procedural);
    if (procCtx) sections.push(procCtx);
  }
  if (state3.contextInjections.length > 0) {
    sections.push(...state3.contextInjections);
  }
...
function createInjectionMetrics() {
  const state3 = zeroState();
  function record(sectionCount, chars, budgetChars) {
    state3.totalCycles++;
    state3.totalChars += chars;
    state3.totalSections += sectionCount;
    state3.lastChars = chars;
    state3.lastSections = sectionCount;
    if (chars > state3.maxChars) state3.maxChars = chars;
    if (sectionCount > state3.maxSections) state3.maxSections = sectionCount;
    if (budgetChars !== void 0 && chars > budgetChars) state3.overBudgetCycles++;
  }
  function get() {
    const avgChars = state3.totalCycles > 0 ? Math.round(state3.totalChars / state3.totalCycles) : 0;
    const avgSections = state3.totalCycles > 0 ? Math.round(state3.totalSections / state3.totalCycles * 10) / 10 : 0;
    return {
      cycles: state3.totalCycles,
      avgChars,
      maxChars: state3.maxChars,
      lastChars: state3.lastChars,
      avgSections,
      maxSections: state3.maxSections,
      lastSections: state3.lastSections,
      avgEstTokens: Math.ceil(avgChars / 4),
      overBudgetCycles: state3.overBudgetCycles
    };
  }
  function reset() {
    Object.assign(state3, zeroState());
  }
...
function initAutonomousResearch(cfg, log, injectedDeps) {
  config3 = cfg.autonomousResearch;
  deps3 = injectedDeps;
  stats = {
    totalCycles: 0,
    totalFactsExtracted: 0,
...
function stopAutonomousResearch() {
  deps3?.logger.info("BrainAgent AutonomousResearch: stopped.");
}
...
async function executeResearch(topic) {
  if (!config3?.enabled || !deps3) return null;
  const now = Date.now();
  if (now - stats.lastResearchTime < config3.cooldownMs) {
  ...
  const existingFacts = deps3.recallFacts(topic, 5);
...
${existingFacts.map((f) => `- ${f.content}`).join("\n")}` : "";
  ...
  if (!queries || queries.length === 0) {
    deps3.logger.info("BrainAgent AutonomousResearch: no queries planned, skipping");
    return null;
  }
  const provider = resolveProvider2(deps3.gatewayConfig);
  deps3.logger.info(`BrainAgent AutonomousResearch: using search provider "${provider}"`);
  const searchResponse = await searchWeb(queries, provider);
  if (!searchResponse) {
    deps3.logger.info("BrainAgent AutonomousResearch: search returned nothing, skipping");
    return null;
  }
  let content;
  let pagesRead = 0;
  if (searchResponse.type === "text") {
    content = searchResponse.content.slice(0, config3.maxTotalChars);
    if (searchResponse.citations.length > 0) {
      content += "\n\nSources:\n" + searchResponse.citations.map((c) => `- ${c}`).join("\n");
    }
  } else {
    if (searchResponse.results.length === 0) {
      deps3.logger.info("BrainAgent AutonomousResearch: no search results, skipping");
      return null;
    }
...
${knownContext}` : `Topic: ${topic}`;
  const result = await deps3.callLLM(systemPrompt, userText, deps3.gatewayConfig, deps3.logger, 300);
  if (!result) return null;
  try {
    const cleaned = result.replace(/```json?\s*/g, "").replace(/```/g, "").trim();
    const parsed = JSON.parse(cleaned);
    if (Array.isArray(parsed)) {
      return parsed.filter((q) => typeof q === "string" && q.length > 2).slice(0, config3.maxSearchQueries);
    }
  } catch {
    const lines = result.split("\n").map((l) => l.replace(/^[-*\d.)\s]+/, "").trim()).filter((l) => l.length > 3 && l.length < 200);
    if (lines.length > 0) return lines.slice(0, config3.maxSearchQueries);
  }
  return null;
}
...
async function searchBrave(queries) {
  ...
  if (!apiKey) {
    deps3.logger.info("BrainAgent AutonomousResearch: no Brave API key available");
    return null;
  }
  const allResults = [];
  for (const query of queries) {
    try {
      const params = new URLSearchParams({
        q: query,
        count: String(config3.maxPagesPerQuery)
      });
      ...
      if (!response.ok) {
        deps3.logger.info(`BrainAgent AutonomousResearch: Brave search failed (${response.status})`);
        continue;
      }
      const data = await response.json();
...
async function searchTavily(queries) {
  ...
  if (!apiKey) {
    deps3.logger.info("BrainAgent AutonomousResearch: no Tavily API key available");
    return null;
  }
  const allResults = [];
  for (const query of queries) {
    try {
      const response = await fetch("https://api.tavily.com/search", {
        ...
        body: JSON.stringify({
          ...
          max_results: config3.maxPagesPerQuery
        }),
        signal: AbortSignal.timeout(1e4)
      });
      if (!response.ok) {
...
async function searchPerplexity(queries) {
  ...
  if (!apiKey) {
    deps3.logger.info("BrainAgent AutonomousResearch: no Perplexity API key available");
    return null;
  }
  const combinedQuery = queries.join("; ");
  const baseUrl = resolvePerplexityBaseUrl(apiKey, deps3.gatewayConfig);
  const model = resolvePerplexityModel(deps3.gatewayConfig);
  try {
    ...
    deps3.logger.info(`BrainAgent AutonomousResearch: Perplexity error \u2014 ${String(err)}`);
    return null;
  }
}
async function searchGrok(queries) {
  ...
  if (!apiKey) {
    deps3.logger.info("BrainAgent AutonomousResearch: no xAI/Grok API key available");
    return null;
  }
  const combinedQuery = queries.join("; ");
  const model = resolveGrokModel(deps3.gatewayConfig);
  try {
    ...
    if (!response.ok) {
      deps3.logger.info(`BrainAgent AutonomousResearch: Grok search failed (${response.status})`);
      return null;
    }
    const data = await response.json();
    ...
    deps3.logger.info(`BrainAgent AutonomousResearch: Grok error \u2014 ${String(err)}`);
    return null;
  }
}
function resolveApiKey(provider) {
  if (!deps3) return null;
  const search = deps3.gatewayConfig.tools;
...
async function fetchPages(results) {
  ...
  const pages = [];
  for (const result of results) {
    if (totalChars >= config3.maxTotalChars) break;
    try {
      ...
      const truncated = text.slice(0, config3.maxPageChars);
...
async function extractAndStore(topic, content, queriesExecuted, pagesRead) {
  ...
  const truncatedContent = content.slice(0, config3.maxTotalChars);
  const userText = `Topic: ${topic}

Web content:
${truncatedContent}`;
  const result = await deps3.callLLM(
    systemPrompt,
    userText,
    deps3.gatewayConfig,
    deps3.logger,
    config3.extractMaxTokens
  );
  if (!result) {
    deps3.logger.info("BrainAgent AutonomousResearch: extraction LLM call failed");
    return { summary: "", factsStored: 0, queriesExecuted, pagesRead };
  }
  let facts = [];
  let summary = "";
  try {
    ...
    deps3.logger.info("BrainAgent AutonomousResearch: extraction JSON parse failed, using raw");
  }
  for (const fact of facts) {
    deps3.storeFact(fact.content, fact.category || "knowledge", [], ["autonomous-research"]);
  }
  stats.totalCycles++;
...
function createPreStepHandler(deps5) {
  const { config: config6, brainConfig, getHostConfig, logger: logger7, state: state3, cycles, startCycle } = deps5;
  return async (payload, next) => {
    ...
    const cyc = cycle;
    const isAutonomousCycle = isAutonomousInput(input);
    if (brainConfig.modules.autonomousResearch && isAutonomousCycle && isResearchIntent(state3.lastAutonomousSource, input)) {
      ...
      logger7.info(
        `BrainAgent AutonomousResearch: detected research intent (source=${state3.lastAutonomousSource}), running isolated pipeline for "${topic}"`
      );
    ...
    if (config6.modules.aiEnrichment && isAIProviderAvailable2(getHostConfig())) {
      const hc = getHostConfig();
      const tasks = [];
      if (brainConfig.modules.amygdala) {
        tasks.push(
          assessWithAI(input, hc, logger7).then((aiAssessment) => {
      ...
      if (brainConfig.modules.mirrorNeurons && cyc.assessment) {
        ...
        tasks.push(
          observeWithAI("default", input, snapshot, brainConfig, hc, logger7).then((model) => {
            cyc.userModel = model;
          }).catch(() => {
          })
        );
      ...
      if (brainConfig.modules.basalGanglia) {
        tasks.push(
          detectReinforcementWithAI(input, hc, logger7).then((aiSignal) => {
      ...
      ]).catch((err) => logger7.warn(String(err)));
    }
    ...
    const episodicLimit = Math.max(1, Math.round(config6.recall.episodicLimit * (0.5 + attentionLevel)));
    const semanticLimit = Math.max(1, Math.round(config6.recall.semanticLimit * (0.5 + attentionLevel)));
    ...
    if (brainConfig.modules.basalGanglia && cyc.classification) {
      ...
      if (habitMatch) {
        ...
        deps5.sessionHabits.set(key, habitMatch.habit.id);
...
function initGoalExecutor(_cfg, log) {
  active25 = createGoalExecutor(log);
...
function initAutonomyEnricher(cfg, log, injectedDeps) {
  ...
  wireEventListeners2();
  logger5.info("BrainAgent AutonomyEnricher: initialized (memory-driven autonomy)");
}
function stopAutonomyEnricher() {
  ...
  unsubscribers3.length = 0;
  logger5?.info("BrainAgent AutonomyEnricher: stopped.");
}
...
function enrichWithMemories(motivation) {
  if (!deps4 || !config4) return;
  const desires2 = deps4.getDesires();
  ...
  const allSemantic = [];
  for (const query of queries) {
    const recalled = deps4.recallMemories(query, 2, 3);
  ...
  if (uniqueEpisodic.length === 0 && uniqueSemantic.length === 0) {
    logger5?.info("BrainAgent AutonomyEnricher: no relevant memories found, skipping enrichment");
    return;
  }
  const memoryContext = buildMemoryContext2(uniqueEpisodic, uniqueSemantic);
  deps4.enqueueSystemEvent(memoryContext, { contextKey: "autonomy-enricher" });
  logger5?.info(
...
function createDreamMode() {
  ...
  async function runConsolidation(config6, logger7, neuroClawConfig, circadianTriggered = false) {
    ...
    try {
      ...
      const result = await consolidate(config6, neuroClawConfig, logger7, intensityMultiplier, skipAI);
      ...
      logger7?.info(`BrainAgent DreamMode: error during consolidation \u2014 ${String(err)}`);
    } finally {
      isConsolidating = false;
    }
  }
  function start(config6, logger7, neuroClawConfig) {
    if (dreamInterval) return;
    storedConfig = config6;
    storedLogger = logger7;
    storedNeuroClawConfig = neuroClawConfig;
    const intervalMs = config6.memory.dreamIntervalMinutes * 60 * 1e3;
    logger7?.info(
      `BrainAgent DreamMode: starting (interval: ${config6.memory.dreamIntervalMinutes}min)`
    );
    if (config6.circadian?.enabled) {
      setConsolidationCallback(async () => {
        await runConsolidation(config6, logger7, neuroClawConfig, true);
      });
      logger7?.info("BrainAgent DreamMode: registered with circadian rhythm system");
    }
    setTimeout(() => void runConsolidation(config6, logger7, neuroClawConfig, false), 3e4);
    dreamInterval = setInterval(() => {
      void runConsolidation(config6, logger7, neuroClawConfig, false);
    }, intervalMs);
  }
  function stop() {
    if (dreamInterval) {
  ...
  async function forceConsolidation2(config6, logger7, neuroClawConfig, intensity) {
    const sleepSettings = getSleepSettings();
    const effectiveIntensity = intensity ?? sleepSettings.consolidationIntensity;
    const result = await consolidate(config6, neuroClawConfig, logger7, effectiveIntensity);
    lastConsolidation = Date.now();
    if (logger7) {
      logger7.info(
...
var active26 = null;
function current13() {
  if (!active26) active26 = createDreamMode();
  return active26;
}
function startDreamMode(config6, logger7, neuroClawConfig) {
  current13().start(config6, logger7, neuroClawConfig);
}
function stopDreamMode() {
  current13().stop();
}
function forceConsolidation(config6, logger7, neuroClawConfig, intensity) {
  return current13().forceConsolidation(config6, logger7, neuroClawConfig, intensity);
}
...
function initProactiveFeedback(workspaceDir, cfg, log) {
  storageDir8 = join30(workspaceDir, ".brainagent", "proactive-feedback");
  if (!existsSync29(storageDir8)) {
    mkdirSync29(storageDir8, { recursive: true });
  }
  stateFile = join30(storageDir8, "state.json");
  config5 = cfg.proactiveFeedback;
  logger6 = log;
  domainFeedback = /* @__PURE__ */ new Map();
  cancelPersist(stateFile);
  loadState8();
  logger6.info(
    `BrainAgent ProactiveFeedback: initialized (threshold=${config5.suppressionThreshold}, cooldown=${config5.cooldownMs}ms, decay=${config5.decayPerDay}/day, tracked=${domainFeedback.size})`
  );
}
function stopProactiveFeedback() {
  flushPersist(stateFile);
  logger6?.info("BrainAgent ProactiveFeedback: stopped.");
}
function recordProactiveReaction(domain, reactionText) {
  if (!config5) return "neutral";
  ...
  if (signal === "rejection") {
    entry.rejections += 1;
    entry.suppressionScore += config5.rejectionStep;
    entry.lastRejectionTime = Date.now();
    entry.lastHits = classification.hits;
  } else if (signal === "negative") {
    entry.rejections += 1;
    entry.suppressionScore += config5.negativeStep;
    entry.lastRejectionTime = Date.now();
    entry.lastHits = classification.hits;
  } else if (signal === "positive") {
    entry.accepts += 1;
    entry.suppressionScore = Math.max(0, entry.suppressionScore - config5.positiveStep);
  }
...
function isDomainSuppressed(domain) {
  if (!config5) return false;
  applyDecay2();
  const entry = domainFeedback.get(domain);
  if (!entry) return false;
  if (entry.suppressionScore < config5.suppressionThreshold) return false;
  return Date.now() - entry.lastRejectionTime < config5.cooldownMs;
}
...
function applyDecay2() {
  ...
  for (const entry of domainFeedback.values()) {
    ...
    entry.suppressionScore = Math.max(0, entry.suppressionScore - days * config5.decayPerDay);
    entry.lastDecayTime = now;
  }
}
function enforceDomainLimit() {
  if (!config5 || domainFeedback.size <= config5.maxTrackedDomains) return;
  const sorted = [...domainFeedback.entries()].sort(
    (a, b) => a[1].suppressionScore - b[1].suppressionScore
  );
  while (domainFeedback.size > config5.maxTrackedDomains) {
...
function createRewardLedger(workspaceDir, config6) {
  const storageDir9 = join31(workspaceDir, ".brainagent", "reward");
  if (!existsSync30(storageDir9)) {
    mkdirSync30(storageDir9, { recursive: true });
  }
  const storageFile = join31(storageDir9, "ledger.json");
  cancelPersist(storageFile);
  const maxEntries = config6.learningLoop.rewardLedger.maxEntries;
  let entries = [];
  let idCounter2 = 0;
...
var active27;
function initRewardLedger(workspaceDir, config6) {
  active27?.dispose();
  active27 = createRewardLedger(workspaceDir, config6);
}
...
function createThalamicGate(config6, signalProviders) {
  let gateConfig = config6;
  ...
  function collectSignals() {
    ...
    try {
      const goals2 = providers.getGoalStackStats?.();
      if (goals2 && goals2.triggered > 0) {
        signals.push({
...
var active28 = null;
function current14() {
  if (!active28) active28 = createThalamicGate();
  return active28;
}
function initThalamicGate(config6, signalProviders) {
  active28 = createThalamicGate(config6, signalProviders);
}
...
function attachLlmBridge(ctx, preferredModel) {
  ...
  setCallLLMBackend(async (systemPrompt, userText, _config, logger7, maxTokens) => {
    ...
    try {
      ...
      logger7?.info(`BrainAgent LLM bridge: ctx.llm call failed \u2014 ${String(error)}`);
      return void 0;
    }
  });
  void resolveRoute();
...
function registerBrainAgentCommands(api, config6) {
  api.registerCommand({
    name: "brainagent",
    ...
    handler: async (ctx) => {
      ...
      if (args === "status" || args === "") {
        return buildStatusReport(config6);
      }
      if (args === "dream") {
        const result = await forceConsolidation(config6, api.logger, api.config);
        return {
          text: [
...
function buildStatusReport(config6) {
  ...
  const lines = [
    ...
    "",
    "**Core Modules:**",
    `  Thalamus (classifier):        ${config6.modules.thalamus ? "ON" : "OFF"}`,
    `  Amygdala (emotion/priority):   ${config6.modules.amygdala ? "ON" : "OFF"}`,
    `  Hippocampus (memory):          ${config6.modules.hippocampus ? "ON" : "OFF"}`,
    `  Prefrontal Cortex (reasoning): ${config6.modules.prefrontalCortex ? "ON" : "OFF"}`,
    `  Cerebellum (quality):          ${config6.modules.cerebellum ? "ON" : "OFF"}`,
    `  Mirror Neurons (empathy):      ${config6.modules.mirrorNeurons ? "ON" : "OFF"}`,
    `  Predictive Engine (anticip.):  ${config6.modules.predictiveEngine ? "ON" : "OFF"}`,
    `  Basal Ganglia (habits):        ${config6.modules.basalGanglia ? "ON" : "OFF"}`,
    `  Dream Mode (consolidation):    ${config6.modules.dreamMode ? "ON" : "OFF"}`,
    "",
    "**Integration Modules:**",
    `  Dopamine System (reward):      ${config6.modules.neuromodulatorSystem ? "ON" : "OFF"}`,
    `  Learning Coordinator (meta):   ${config6.modules.learningCoordinator ? "ON" : "OFF"}`,
    `  Neural Pathways (connections):  ${config6.modules.neuralPathways ? "ON" : "OFF"}`,
    "",
    "**Consciousness Modules:**",
    `  Working Memory (continuity):   ${config6.modules.workingMemory ? "ON" : "OFF"}`,
    `  Session Bridge (cross-sess.):  ${config6.modules.sessionBridge ? "ON" : "OFF"}`,
    `  Emotional Memory (flashbulb):  ${config6.modules.emotionalMemory ? "ON" : "OFF"}`,
    `  Attention Gate (filtering):    ${config6.modules.attentionGate ? "ON" : "OFF"}`,
    `  DMN (idle thinking):           ${config6.modules.dmn ? "ON" : "OFF"}`,
    `  Introspection (self-trace):    ${config6.modules.introspection ? "ON" : "OFF"}`,
    `  Agent Identity (self-model):   ${config6.modules.agentIdentity ? "ON" : "OFF"}`,
    `  Goal Stack (proactive):        ${config6.modules.goalStack ? "ON" : "OFF"}`,
    `  Curiosity Drive (gaps):        ${config6.modules.curiosityDrive ? "ON" : "OFF"}`,
    `  Temporal Binding (stream):     ${config6.modules.temporalBinding ? "ON" : "OFF"}`,
    `  Qualia Simulator (subjective): ${config6.modules.qualiaSimulator ? "ON" : "OFF"}`,
    `  Vital Impulse (autonomous):   ${config6.modules.vitalImpulse ? "ON" : "OFF"}`,
    `  Goal Executor (autonomous):   ${config6.modules.goalStack ? "ON" : "OFF"}`,
    "",
    "**Autonomic Modules:**",
    `  Metabolic Budget (energy):     ${config6.modules.metabolicBudget ? "ON" : "OFF"}`,
    `  Emergent Modules (patterns):   ${config6.modules.emergentModules ? "ON" : "OFF"}`,
    `  Interoception (inner state):   ${config6.modules.interoception ? "ON" : "OFF"}`,
    `  Proactive Feedback (\xAB\u043D\u0435 \u0437\u0430\u0448\u043B\u043E\xBB): ${config6.modules.proactiveFeedback ? "ON" : "OFF"}`,
    "",
    "**Memory:**",
  ...
  if (config6.modules.neuromodulatorSystem) {
    const dStats = getDopamineStats();
    const ns = dStats.currentState;
    lines.push(
      "",
  ...
  if (config6.modules.learningCoordinator) {
    const lStats = getLearningStats();
    lines.push(
      "",
  ...
  if (identityStatsGetter && config6.modules.agentIdentity) {
  ...
  if (qualiaSimulatorStatsGetter && config6.modules.qualiaSimulator) {
  ...
  if (vitalImpulseStatsGetter && config6.modules.vitalImpulse) {
    const vi = vitalImpulseStatsGetter();
  ...
  if (goalExecutorStatsGetter && config6.modules.goalStack) {
  ...
  if (config6.modules.proactiveFeedback) {
  ...
  if (socialDriveStatsGetter && config6.modules.socialDrive) {
  ...
  if (cognitiveHungerStatsGetter && config6.modules.cognitiveHunger) {
  ...
  if (creativeDriveStatsGetter && config6.modules.creativeDrive) {
  ...
  if (masteryDriveStatsGetter && config6.modules.masteryDrive) {
  ...
  if (driveArbiterStatsGetter && config6.modules.driveArbiter) {
  ...
  if (temporalAwarenessStatsGetter && config6.modules.temporalAwareness) {
  ...
  if (autonomousResearchStatsGetter && config6.modules.autonomousResearch) {
...
function apply(ctx, config6) {
  const logger7 = {
    info: (msg) => ctx.logger.info(msg),
    warn: (msg) => ctx.logger.warn(msg),
    error: (msg) => ctx.logger.error(msg)
  };
  const brainConfig = mergeBrainConfig(config6);
  const dataDir = config6.dataDir;
  mkdirSync31(dataDir, { recursive: true });
  const hostConfig = () => buildHostConfig({
    providers: config6.providers,
    ...config6.model ? { model: config6.model } : {}
  });
  const state3 = createAutonomyState();
  function pickAgent() {
    const agents = ctx.agents.list();
    if (agents.length === 0) return void 0;
    return agents.find((a) => String(a.id) === state3.lastActiveAgentId) ?? agents[agents.length - 1];
  }
  const enqueueAutonomousIntent = createAutonomousDeliverer({
    state: state3,
    brainConfig,
    minGapMs: config6.autonomousMinGapMs ?? 10 * 60 * 1e3,
    logger: logger7,
    pickAgent,
  ...
  initEmbeddings(hostConfig(), logger7);
  if (brainConfig.modules.workingMemory) {
  ...
  if (brainConfig.modules.neuralPathways) {
    initNeuralPathways(dataDir, brainConfig, logger7);
  }
  if (config6.modules.structuralPlasticity) {
    initStructuralPlasticity(dataDir, brainConfig, logger7);
  }
  if (brainConfig.modules.emotionalMemory) {
    initEmotionalMemory(dataDir, brainConfig);
  }
  if (brainConfig.modules.sessionBridge) {
    initSessionBridge(dataDir, brainConfig, logger7);
  }
  if (brainConfig.modules.dmn) {
    initDMN(dataDir, brainConfig, logger7);
  }
  if (brainConfig.modules.goalStack) {
  ...
  if (brainConfig.modules.socialDrive) {
    initSocialDrive(dataDir, brainConfig, logger7, {
      ...driveInitBase,
  ...
  if (brainConfig.modules.cognitiveHunger) {
    initCognitiveHunger(dataDir, brainConfig, logger7, {
      ...driveInitBase,
  ...
  if (brainConfig.modules.creativeDrive) {
    initCreativeDrive(dataDir, brainConfig, logger7, {
      ...driveInitBase,
  ...
  if (brainConfig.modules.masteryDrive) {
    initMasteryDrive(dataDir, brainConfig, logger7, {
      ...driveInitBase,
  ...
  if (brainConfig.modules.vitalImpulse) {
    initVitalImpulse(dataDir, brainConfig, logger7, {
  ...
  if (brainConfig.modules.goalStack) {
    initGoalExecutor(brainConfig, logger7);
  }
  if (brainConfig.modules.actionDispatcher && brainConfig.modules.vitalImpulse) {
    initAutonomyEnricher(brainConfig, logger7, {
  ...
  if (brainConfig.modules.autonomousResearch) {
    initAutonomousResearch(brainConfig, logger7, {
      callLLM,
      storeFact,
  ...
  if (brainConfig.circadian.enabled) {
    initCircadianRhythm(dataDir, brainConfig, logger7);
  }
  if (brainConfig.modules.dreamMode) {
    startDreamMode(brainConfig, logger7, hostConfig());
  }
  if (brainConfig.modules.metabolicBudget) {
    initMetabolicBudget(dataDir, brainConfig, logger7);
  }
  if (brainConfig.modules.emergentModules) {
    initEmergentModules(dataDir, brainConfig, logger7);
  }
  if (brainConfig.modules.proactiveFeedback) {
    initProactiveFeedback(dataDir, brainConfig, logger7);
  }
  ...
  if (brainConfig.modules.temporalAwareness) {
    initTemporalAwareness(dataDir, brainConfig, logger7);
  }
  if (brainConfig.modules.interoception) {
    initInteroception(
      {
  ...
  ctx.on("session/event", (_session, event) => {
    ...
    if (event.type === "user/message") {
      ...
      if (isUserMessage && state3.previousCycleWasAutonomous && state3.lastAutonomousEpisodeId) {
        let reactionSignal = "neutral";
        if (brainConfig.modules.proactiveFeedback) {
          reactionSignal = recordProactiveReaction(state3.lastAutonomousDomain, text);
        }
        if (brainConfig.modules.hippocampus) {
          ...
          storeEpisode(
            ...
            reactionEmotion,
            ["proactive_feedback", state3.lastAutonomousEpisodeId, reactionSignal],
            reactionSalience
          );
        }
      }
      state3.previousCycleWasAutonomous = false;
      state3.lastAutonomousEpisodeId = void 0;
    ...
    if (event.type === "turn/end") {
      void endCycle3(key).catch((err) => logger7.warn(`BrainAgent endCycle: ${String(err)}`));
    }
  });
  ...
  ctx.on("tools/pre-execute", async (exec, next) => {
    ...
    const agentId = exec.agent ? String(exec.agent.id) : state3.lastActiveAgentId;
