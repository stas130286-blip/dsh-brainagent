...
function stopSocialDrive() {
  engine?.stop();
  engine = void 0;
  logger6?.info("BrainAgent SocialDrive: stopped.");
}
...
function stopCognitiveHunger() {
  engine2?.stop();
  engine2 = void 0;
  logger7?.info("BrainAgent CognitiveHunger: stopped.");
}
...
function stopCreativeDrive() {
  engine3?.stop();
  engine3 = void 0;
  logger8?.info("BrainAgent CreativeDrive: stopped.");
}
...
function stopMasteryDrive() {
  ...
  logger9?.info("BrainAgent MasteryDrive: stopped.");
}
...
function biasDMNToMasteryThoughts(weakestDomain) {
  ...
  logger9?.info(`BrainAgent MasteryDrive: biased DMN toward ${topics.length} mastery topic(s)`);
}
...
function createAutonomousDeliverer(deps5) {
  const { state: state8, brainConfig, minGapMs, logger: logger17 } = deps5;
...
function createCycleEngine(deps5) {
  const { config: config8, brainConfig, getHostConfig, logger: logger17, markActivation, state: state8 } = deps5;
  ...
  async function endCycle3(key) {
    ...
    if (config8.modules.semanticExtraction && semanticSource.length > 15 && isFactWorthy(semanticSource, cycle.classification)) {
      ...
      if (aiAvailable) {
        try {
          const aiFacts = await extractFactsWithAI(semanticSource, getHostConfig(), logger17);
          if (aiFacts.length > 0) {
    ...
    if (config8.modules.proceduralExtraction && input.length > 10 && isProcedural(input, cycle.classification)) {
      const procedure = await extractProcedureAsync(input, getHostConfig(), cycle.classification, logger17);
    ...
    if (brainConfig.modules.goalStack && input.length > 10) {
      ...
      if (goalExtractionCounter >= brainConfig.goalStack.extractionInterval) {
        ...
        void extractGoalsFromConversation(input, getHostConfig(), logger17).catch((err) => {
...
function stopVitalImpulse() {
  ...
  logger10?.info("BrainAgent VitalImpulse: stopped.");
}
...
function forceImpulse(reason) {
  ...
  recentSignals = [];
  persistState15();
  logger10?.info("BrainAgent VitalImpulse: forced impulse fired.");
}
...
function initDriveArbiter(workspaceDir, cfg, getters, log) {
  ...
  logger11?.info("BrainAgent DriveArbiter: initialized");
}
function stopDriveArbiter() {
  ...
  logger11?.info("BrainAgent DriveArbiter: stopped.");
}
function arbitrate() {
...
function initInteroception(getters, log) {
  ...
  logger12?.info("BrainAgent Interoception: initialized");
}
function stopInteroception() {
  ...
  unsubscribers5.length = 0;
  logger12?.info("BrainAgent Interoception: stopped.");
}
function evaluate() {
...
function stopTemporalAwareness() {
  persistState17();
  logger13?.info("BrainAgent TemporalAwareness: stopped.");
}
...
function createPreStepHandler(deps5) {
  const { config: config8, brainConfig, getHostConfig, logger: logger17, state: state8, cycles, startCycle } = deps5;
...
function initGoalExecutor(_cfg, log) {
  ...
  logger14.info("BrainAgent GoalExecutor: initialized (goals checked via vital impulse)");
}
function stopGoalExecutor() {
  logger14?.info("BrainAgent GoalExecutor: stopped.");
}
...
function initAutonomyEnricher(cfg, log, injectedDeps) {
  ...
  wireEventListeners2();
  logger15.info("BrainAgent AutonomyEnricher: initialized (memory-driven autonomy)");
}
function stopAutonomyEnricher() {
  ...
  unsubscribers6.length = 0;
  logger15?.info("BrainAgent AutonomyEnricher: stopped.");
}
...
function enrichWithMemories(motivation) {
  ...
  if (uniqueEpisodic.length === 0 && uniqueSemantic.length === 0) {
    logger15?.info("BrainAgent AutonomyEnricher: no relevant memories found, skipping enrichment");
    return;
  }
...
function stopProactiveFeedback() {
  ...
  logger16?.info("BrainAgent ProactiveFeedback: stopped.");
}
