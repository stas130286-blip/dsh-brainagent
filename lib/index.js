...
function initMirrorStorage(workspaceDir) {
  userModels.clear();
  storageDir = join3(workspaceDir, ".brainagent", "users");
...
async function observeWithAI(userId, text, amygdalaResult, config8, neuroConfig, logger13) {
  const model = getOrCreateModel(userId);
  model.emotionHistory.push({
    timestamp: Date.now(),
    emotion: amygdalaResult.emotion,
...
function updateKnowledgeModelToM(model, text, domainLimit) {
  ...
  for (const domain of domainSignals) {
    const current4 = model.knowledgeModel[domain];
    if (!current4) {
      model.knowledgeModel[domain] = "beginner";
    } else if (current4 === "beginner") {
...
function initPredictiveStorage(workspaceDir) {
  storageDir2 = join4(workspaceDir, ".brainagent", "predictions");
...
function initBasalStorage(workspaceDir) {
  storageDir3 = join5(workspaceDir, ".brainagent", "habits");
...
function loadHabits() {
  ...
  try {
    const path = join5(storageDir3, "habits.json");
    if (existsSync4(path)) {
...
function persistHabits() {
  if (!storageDir3) return;
  try {
    writeFileSync5(join5(storageDir3, "habits.json"), JSON.stringify(habits, null, 2), "utf-8");
  } catch {
  }
}
...
function initIntrospection(workspaceDir, config8) {
  storageDir4 = join6(workspaceDir, ".brainagent", "introspection");
...
function loadState() {
  ...
  try {
    const path = join6(storageDir4, "traces.json");
    if (existsSync5(path)) {
...
function initDopamineSystem(workspaceDir) {
  storageDir5 = join7(workspaceDir, ".brainagent", "neuromodulators");
  if (!existsSync6(storageDir5)) {
    mkdirSync6(storageDir5, { recursive: true });
  }
  cancelPersist(join7(storageDir5, "state.json"));
  loadState2();
}
function loadState2() {
  if (!storageDir5) return;
  try {
    const path = join7(storageDir5, "state.json");
    if (existsSync6(path)) {
...
function initNeuralPathways(workspaceDir, config8, logger13) {
  ...
  storageDir6 = join8(workspaceDir, ".brainagent", "synapses");
...
function loadSynapticState() {
  ...
  try {
    const path = join8(storageDir6, "weights.json");
    if (existsSync7(path)) {
...
function saveSynapticState() {
  ...
  try {
    writeFileSync7(
      join8(storageDir6, "weights.json"),
...
function createWorkingMemory(workspaceDir, opts) {
  const storageDir24 = workspaceDir ? join9(workspaceDir, ".brainagent", "working-memory") : "";
  const maxEntries2 = opts.maxEntries;
  const summaryMaxLength = opts.summaryMaxLength;
  let entries2 = [];
  if (storageDir24 && !existsSync8(storageDir24)) {
    mkdirSync8(storageDir24, { recursive: true });
  }
  function loadState21() {
    if (!storageDir24) return;
    try {
      const path = join9(storageDir24, "state.json");
      if (existsSync8(path)) {
  ...
  function persistState17() {
    if (!storageDir24) return;
    try {
      writeFileSync8(join9(storageDir24, "state.json"), JSON.stringify(entries2, null, 2), "utf-8");
    } catch {
    }
  }
  loadState21();
...
var active;
function current() {
  return active ?? (active = createWorkingMemory("", {
...
function initStructuralPlasticity(workspaceDir, config8, log) {
  ...
  logger = log;
  storageDir7 = join10(workspaceDir, ".brainagent", "structural");
  if (!existsSync9(storageDir7)) {
    mkdirSync9(storageDir7, { recursive: true });
  }
  loadState3();
  logger?.info(
...
function loadState3() {
  try {
    const path = join10(storageDir7, "state.json");
    if (existsSync9(path)) {
...
function saveState() {
  try {
    writeFileSync9(join10(storageDir7, "state.json"), JSON.stringify(state2, null, 2), "utf-8");
  } catch {
  }
}
...
function initLearningCoordinator(workspaceDir, config8) {
  storageDir8 = join11(workspaceDir, ".brainagent", "learning");
  if (!existsSync10(storageDir8)) {
    mkdirSync10(storageDir8, { recursive: true });
  }
...
function loadState4() {
  ...
  try {
    const path = join11(storageDir8, "coordinator.json");
...
function persistState3() {
  ...
  try {
    ...
    writeFileSync10(
      join11(storageDir8, "coordinator.json"),
      JSON.stringify(
        {
...
function initEmergentModules(workspaceDir, config8, log) {
  ...
  logger2 = log;
  storageDir9 = join12(workspaceDir, ".brainagent", "emergent");
  if (!existsSync11(storageDir9)) {
    mkdirSync11(storageDir9, { recursive: true });
  }
...
function loadState5() {
  try {
    const path = join12(storageDir9, "state.json");
...
function saveState2() {
  try {
    writeFileSync11(join12(storageDir9, "state.json"), JSON.stringify(state3, null, 2), "utf-8");
  } catch {
  }
}
...
function initMetabolicBudget(workspaceDir, config8, log) {
  ...
  logger3 = log;
  storageDir10 = join13(workspaceDir, ".brainagent", "metabolic");
...
function saveState3() {
  try {
    writeFileSync12(join13(storageDir10, "state.json"), JSON.stringify(state4, null, 2), "utf-8");
  } catch {
  }
}
...
function initAgentIdentity(workspaceDir, config8) {
  storageDir11 = join14(workspaceDir, ".brainagent", "identity");
  if (!existsSync13(storageDir11)) {
    mkdirSync13(storageDir11, { recursive: true });
  }
...
function loadState7() {
  ...
  try {
    const path = join14(storageDir11, "state.json");
...
function persistState4() {
  ...
  try {
    writeFileSync13(
      join14(storageDir11, "state.json"),
      JSON.stringify(
        {
          capabilities,
...
function initEmotionalMemory(workspaceDir, _config) {
  storageDir12 = join15(workspaceDir, ".brainagent", "emotional-memory");
  if (!existsSync14(storageDir12)) {
...
function initQualiaSimulator(workspaceDir, config8) {
  storageDir13 = join16(workspaceDir, ".brainagent", "qualia-simulator");
  if (!existsSync15(storageDir13)) {
    mkdirSync15(storageDir13, { recursive: true });
  }
...
function loadState9() {
  ...
  try {
    const path = join16(storageDir13, "state.json");
...
function persistState6() {
  ...
  try {
    writeFileSync15(
      join16(storageDir13, "state.json"),
...
function initDMN(workspaceDir, config8, log) {
  storageDir14 = join17(workspaceDir, ".brainagent", "dmn");
  if (!existsSync16(storageDir14)) {
    mkdirSync16(storageDir14, { recursive: true });
  }
...
function loadState10() {
  ...
  try {
    const path = join17(storageDir14, "state.json");
...
function persistState7() {
  ...
  try {
    writeFileSync16(
      join17(storageDir14, "state.json"),
      JSON.stringify(
        {
...
function createTemporalBinding(workspaceDir, opts) {
  const storageDir24 = workspaceDir ? join18(workspaceDir, ".brainagent", "temporal-binding") : "";
  const maxMoments = opts.maxMoments;
  let moments = [];
  let idCounter6 = 0;
  if (storageDir24 && !existsSync17(storageDir24)) {
    mkdirSync17(storageDir24, { recursive: true });
  }
  function loadState21() {
    if (!storageDir24) return;
    try {
      const path = join18(storageDir24, "state.json");
  ...
  function persistState17() {
    if (!storageDir24) return;
    try {
      writeFileSync17(join18(storageDir24, "state.json"), JSON.stringify(moments, null, 2), "utf-8");
    } catch {
    }
  }
  loadState21();
...
var active2;
function current2() {
  return active2 ?? (active2 = createTemporalBinding("", { maxMoments: DEFAULT_MAX_MOMENTS }));
}
function initTemporalBinding(workspaceDir, config8) {
  active2 = createTemporalBinding(workspaceDir, { maxMoments: config8.temporalBinding.maxMoments });
}
function createMoment(input, thoughts, emotion, emotionIntensity, activeMemoryIds, intentions, confidence, domain) {
  return current2().createMoment(
    input,
    thoughts,
    emotion,
    emotionIntensity,
...
function buildTemporalContext(n = 3) {
  return current2().buildContext(n);
}
...
function initSessionBridge(workspaceDir, config8, _logger) {
  storageDir15 = join19(workspaceDir, ".brainagent", "sessions");
  if (!existsSync18(storageDir15)) {
    mkdirSync18(storageDir15, { recursive: true });
  }
...
function loadState11() {
  ...
  try {
    const currentPath = join19(storageDir15, "current.json");
  ...
  try {
    const lastPath = join19(storageDir15, "last.json");
...
function persistCurrent() {
  ...
  try {
    writeFileSync18(
      join19(storageDir15, "current.json"),
...
function persistLast() {
  if (!storageDir15) return;
  try {
    writeFileSync18(join19(storageDir15, "last.json"), JSON.stringify(lastSession, null, 2), "utf-8");
  } catch {
  }
}
...
function initCuriosityDrive(workspaceDir, config8) {
  storageDir16 = join20(workspaceDir, ".brainagent", "curiosity");
  if (!existsSync19(storageDir16)) {
    mkdirSync19(storageDir16, { recursive: true });
  }
...
function loadState12() {
  if (!storageDir16) return;
  try {
    const path = join20(storageDir16, "state.json");
...
function persistState8() {
  ...
  try {
    writeFileSync19(
      join20(storageDir16, "state.json"),
      JSON.stringify(
...
function initGoalStack(workspaceDir, config8) {
  storageDir17 = join21(workspaceDir, ".brainagent", "goals");
  if (!existsSync20(storageDir17)) {
    mkdirSync20(storageDir17, { recursive: true });
  }
...
function loadState13() {
  ...
  try {
    const path = join21(storageDir17, "state.json");
...
function persistState9() {
  ...
  try {
    writeFileSync20(
      join21(storageDir17, "state.json"),
...
function expireGoals() {
  ...
  if (goals.length > 60) {
    const active8 = goals.filter((g) => g.status === "pending" || g.status === "triggered");
    const inactive = goals.filter((g) => g.status === "completed" || g.status === "expired").slice(-30);
    goals = [...active8, ...inactive];
    changed = true;
  }
...
function initCircadianRhythm(workspaceDir, config8, log) {
  storageDir18 = join22(workspaceDir, ".brainagent", "circadian");
  if (!existsSync21(storageDir18)) {
    mkdirSync21(storageDir18, { recursive: true });
  }
...
function loadState14() {
  ...
  try {
    const path = join22(storageDir18, "state.json");
...
function persistState10() {
  if (!storageDir18) return;
  try {
    writeFileSync21(join22(storageDir18, "state.json"), JSON.stringify(state6, null, 2), "utf-8");
  } catch {
  }
}
...
var DriveEngine = class {
  constructor(spec, config8, workspaceDir, circadianEnabled3, deps5, logger13) {
    ...
    const storageDir24 = join23(workspaceDir, ".brainagent", spec.id);
    if (!existsSync22(storageDir24)) {
      mkdirSync22(storageDir24, { recursive: true });
    }
    this.stateFile = join23(storageDir24, "state.json");
...
function initSocialDrive(workspaceDir, cfg, log, injectedDeps) {
  active3?.dispose();
  active3 = createSocialDrive(workspaceDir, cfg, log, injectedDeps);
}
...
function initCognitiveHunger(workspaceDir, cfg, log, injectedDeps) {
  active4?.dispose();
  active4 = createCognitiveHunger(workspaceDir, cfg, log, injectedDeps);
}
...
function initCreativeDrive(workspaceDir, cfg, log, injectedDeps) {
  active5?.dispose();
  active5 = createCreativeDrive(workspaceDir, cfg, log, injectedDeps);
}
...
function initMasteryDrive(workspaceDir, cfg, log, injectedDeps) {
  storageDir19 = join24(workspaceDir, ".brainagent", "mastery-drive");
  if (!existsSync23(storageDir19)) {
    mkdirSync23(storageDir19, { recursive: true });
  }
  ...
  totalNeedSignals = 0;
  adaptiveDecayModifier = 1;
  cancelPersist(join24(storageDir19, "state.json"));
  loadState15();
  wireEventListeners();
  logger6.info(
...
function stopMasteryDrive() {
  ...
  unsubscribers.length = 0;
  persistState11();
  flushPersist(join24(storageDir19, "state.json"));
...
function loadState15() {
  try {
    const filePath = join24(storageDir19, "state.json");
...
function persistState11() {
  schedulePersist(join24(storageDir19, "state.json"), () => {
    const domainMap = {};
    for (const [domain, mastery] of domainSatiations) {
...
function collectActiveDrives(drives, threshold) {
  ...
  if (drives.social) {
    ...
    if (s.need >= threshold) active8.push({ name: "social", need: s.need, needLevel: s.needLevel });
  }
  if (drives.cognitive) {
    const c = drives.cognitive();
    if (c.need >= threshold) active8.push({ name: "cognitive", need: c.need, needLevel: c.needLevel });
  }
  if (drives.creative) {
    const c = drives.creative();
    if (c.need >= threshold) active8.push({ name: "creative", need: c.need, needLevel: c.needLevel });
  }
  if (drives.mastery) {
    const m = drives.mastery();
    if (m.need >= threshold) active8.push({ name: "mastery", need: m.need, needLevel: m.needLevel });
  }
  return active8;
}
function synthesizeAutonomousCycleState(brainConfig, drives, cycle) {
  const active8 = collectActiveDrives(drives, 0.5);
  if (active8.length === 0) return;
  const ranked = active8.map((d) => {
...
function createAutonomousIntentResolver(deps5) {
  ...
  return () => {
    ...
    const active8 = collectActiveDrives(drives, 0.8);
    if (active8.length > 0) {
      const strongest = active8.sort((a, b) => b.need - a.need)[0];
...
function initStrategyBandit(workspaceDir, config8) {
  stopStrategyBandit();
  const storageDir24 = join25(workspaceDir, ".brainagent", "bandit");
  if (!existsSync24(storageDir24)) {
    mkdirSync24(storageDir24, { recursive: true });
  }
  storageFile = join25(storageDir24, "state.json");
...
function initVitalImpulse(workspaceDir, cfg, log, injectedDeps) {
  storageDir20 = join26(workspaceDir, ".brainagent", "vital-impulse");
  if (!existsSync25(storageDir20)) {
    mkdirSync25(storageDir20, { recursive: true });
  }
  ...
  cancelPersist(join26(storageDir20, "state.json"));
  loadState17();
  wireSignalListeners();
  logger7.info(
...
function stopVitalImpulse() {
  ...
  unsubscribers3.length = 0;
  persistState13();
  flushPersist(join26(storageDir20, "state.json"));
...
function loadState17() {
  try {
    const filePath = join26(storageDir20, "state.json");
...
function persistState13() {
  schedulePersist(join26(storageDir20, "state.json"), () => {
    const state8 = {
      lastFireTime,
      totalFires,
      totalSignalsReceived,
...
function initDriveArbiter(workspaceDir, cfg, getters, log) {
  ...
  storageDir21 = join27(workspaceDir, ".brainagent");
  if (!existsSync26(storageDir21)) {
    mkdirSync26(storageDir21, { recursive: true });
  }
  cancelPersist(join27(storageDir21, "drive-arbiter.json"));
  loadState18();
  const driveEvents = [
...
function stopDriveArbiter() {
  ...
  unsubscribers4.length = 0;
  persistState14();
  flushPersist(join27(storageDir21, "drive-arbiter.json"));
...
function loadState18() {
  try {
    const path = join27(storageDir21, "drive-arbiter.json");
...
function persistState14() {
  ...
  schedulePersist(join27(storageDir21, "drive-arbiter.json"), () => {
    const data = {
      driveWeights,
      lastSelectedDrive,
      lastSelectionTime,
...
function initTemporalAwareness(workspaceDir, cfg, log) {
  ...
  storageDir22 = join28(workspaceDir, ".brainagent");
  if (!existsSync27(storageDir22)) {
    mkdirSync27(storageDir22, { recursive: true });
  }
  loadState19();
  logger10?.info(
...
function loadState19() {
  try {
    const path = join28(storageDir22, "temporal-awareness.json");
...
function persistState15() {
  ...
  try {
    ...
    writeFileSync22(
      join28(storageDir22, "temporal-awareness.json"),
...
function createAttentionGate(workspaceDir) {
  const storageDir24 = workspaceDir ? join29(workspaceDir, ".brainagent", "attention") : "";
  let totalProcessed = 0;
  let totalDropped = 0;
  let totalRelevanceSum = 0;
  if (storageDir24 && !existsSync28(storageDir24)) {
    mkdirSync28(storageDir24, { recursive: true });
  }
  function loadState21() {
    if (!storageDir24) return;
    try {
      const path = join29(storageDir24, "state.json");
  ...
  function persistState17() {
    ...
    try {
      writeFileSync23(
        join29(storageDir24, "state.json"),
...
var active6;
function current3() {
  return active6 ?? (active6 = createAttentionGate(""));
}
function initAttentionGate(workspaceDir, _config) {
  active6 = createAttentionGate(workspaceDir);
}
function filterContextInjections(injections, currentInput, norepinephrineLevel, config8, maxTokenBudget) {
  return current3().filter(injections, currentInput, norepinephrineLevel, config8, maxTokenBudget);
}
...
function initGoalExecutor(_cfg, log) {
  active7 = createGoalExecutor(log);
...
function initProactiveFeedback(workspaceDir, cfg, log) {
  storageDir23 = join30(workspaceDir, ".brainagent", "proactive-feedback");
  if (!existsSync29(storageDir23)) {
    mkdirSync29(storageDir23, { recursive: true });
  }
  stateFile = join30(storageDir23, "state.json");
...
function initRewardLedger(workspaceDir, config8) {
  stopRewardLedger();
  const storageDir24 = join31(workspaceDir, ".brainagent", "reward");
  if (!existsSync30(storageDir24)) {
    mkdirSync30(storageDir24, { recursive: true });
  }
  storageFile2 = join31(storageDir24, "ledger.json");
