...
function buildHostConfig(options = {}) {
  ...
  const config7 = { models: { providers: providers2 } };
  if (options.model) {
    config7.agents = { defaults: { model: options.model } };
  }
  return config7;
}
...
function mergeBrainConfig(config7) {
  const modules = { ...DEFAULT_CONFIG.modules };
  for (const [key, value] of Object.entries(config7.modules)) {
  ...
  return {
    ...
    dualProcess: {
      ...
      ...config7.dualProcess?.fastModel ? { fastModel: config7.dualProcess.fastModel } : {},
      ...config7.dualProcess?.slowModel ? { slowModel: config7.dualProcess.slowModel } : {}
    },
    circadian: {
      ...DEFAULT_CONFIG.circadian,
      enabled: config7.circadian.enabled
    },
    contextInjection: {
      ...DEFAULT_CONFIG.contextInjection,
      ...config7.contextInjection
    },
    learningLoop: {
      rewardLedger: {
        ...DEFAULT_CONFIG.learningLoop.rewardLedger,
        ...config7.learningLoop?.rewardLedger ?? {}
      },
      strategyBandit: {
        ...DEFAULT_CONFIG.learningLoop.strategyBandit,
        ...config7.learningLoop?.strategyBandit ?? {}
      }
    }
  };
}
...
function parseUserModelSelection(config7) {
  const agents = config7.agents;
...
function resolveProvider(config7) {
  const providers2 = config7.models?.providers;
  if (!providers2) return null;
  const userSelection = parseUserModelSelection(config7);
  if (userSelection) {
...
function isAIProviderAvailable(config7) {
  return (availabilityHook?.() ?? false) || resolveProvider(config7) !== null;
}
var callBackend;
...
async function callLLM(systemPrompt, userText, config7, logger8, maxTokens = 500) {
  if (callBackend) {
    try {
      const bridged = await callBackend(systemPrompt, userText, config7, logger8, maxTokens);
      if (bridged !== void 0) return bridged;
    } catch (error) {
      logger8?.info(`BrainAgent LLM: bridge failed, falling back \u2014 ${String(error)}`);
    }
  }
  const provider = resolveProvider(config7);
  if (!provider) {
    logger8?.info("BrainAgent LLM: no AI provider configured, skipping");
    return null;
  }
  const userSelection = parseUserModelSelection(config7);
  if (userSelection) {
    logger8?.info(`BrainAgent LLM: calling ${provider.name} (${provider.model}) [user-selected]`);
  } else {
    logger8?.info(`BrainAgent LLM: calling ${provider.name} (${provider.model}) [auto-detected]`);
  }
  try {
    let response;
...
${userText}` }] }],
      ...
      logger8?.info(`BrainAgent LLM: Google error ${response.status}: ${errorText}`);
      return null;
    } else {
    ...
    if (!response.ok) {
      const errorText = await response.text();
      logger8?.info(`BrainAgent LLM: ${provider.name} error ${response.status}: ${errorText}`);
      return null;
    }
    ...
    logger8?.info(`BrainAgent LLM: error \u2014 ${String(error)}`);
    return null;
  }
}
...
function resolveEmbeddingProvider(config7) {
  const providers2 = config7.models?.providers;
  if (!providers2) return null;
  const userSelection = parseUserModelSelection(config7);
  if (userSelection) {
...
async function getEmbedding(text, config7, logger8) {
  const result = await getEmbeddings([text], config7, logger8);
  return result?.[0] ?? null;
}
async function getEmbeddings(texts, config7, logger8) {
  const provider = resolveEmbeddingProvider(config7);
  if (!provider) {
    return null;
  }
  ...
  try {
    const result = provider.format === "google" ? await fetchGoogleEmbeddings(texts, provider, logger8) : await fetchOpenAIEmbeddings(texts, provider, logger8);
    if (result === null && provider.name === "Ollama") {
      ollamaUnreachable = true;
    }
    return result;
  } catch (error) {
    logger8?.info(`BrainAgent Embeddings: ${provider.name} error \u2014 ${String(error)}`);
    if (provider.name === "Ollama") ollamaUnreachable = true;
    return null;
  }
}
async function fetchOpenAIEmbeddings(texts, provider, logger8) {
...
async function fetchGoogleEmbeddings(texts, provider, logger8) {
  ...
  if (!response.ok) {
    const errorText = await response.text();
    logger8?.info(`BrainAgent Embeddings: Google API error ${response.status}: ${errorText}`);
    return null;
  }
  const data = await response.json();
...
function createHippocampus(workspaceDir) {
  ...
  function nextId(prefix) {
    return `${prefix}-${Date.now()}-${++idCounter3}`;
  }
  ...
  function scheduleEmbeddingBackfill() {
    ...
    const config7 = embeddingsConfig;
    const missing = [];
    ...
    void (async () => {
      for (let i = 0; i < missing.length; i++) {
        ...
        try {
          const vec = await getEmbedding(item.text, config7, embeddingsLogger);
          if (vec) {
  ...
  function initEmbeddings2(config7, logger8) {
    const provider = resolveEmbeddingProvider(config7);
    if (!provider) {
      logger8?.info("BrainAgent Hippocampus: no embedding provider \u2014 using TF-IDF");
      return;
    }
    embeddingsConfig = config7;
    embeddingsLogger = logger8;
  ...
  function updateEmbeddingsConfig2(config7) {
  ...
  function storeEpisode2(event, summary, emotionalContext = "neutral", entities = [], emotionIntensity = 0) {
    ...
    if (embeddingsAvailable && embeddingsConfig) {
      const config7 = embeddingsConfig;
      void getEmbedding(`${event} ${summary}`, config7, embeddingsLogger).then((vec) => {
        if (vec) {
  ...
  function storeFact2(content, category, sourceEpisodeIds = [], relatedIds = []) {
    ...
    if (embeddingsAvailable && embeddingsConfig) {
      const config7 = embeddingsConfig;
      void getEmbedding(`${content} ${category}`, config7, embeddingsLogger).then((vec) => {
        if (vec) {
  ...
  function reviseFact(existing, newContent, newConfidence, sourceEpisodeIds, reason) {
    ...
    if (embeddingsAvailable && embeddingsConfig) {
      const config7 = embeddingsConfig;
      void getEmbedding(`${newContent} ${existing.category}`, config7, embeddingsLogger).then(
        (vec) => {
          if (vec) {
  ...
  async function consolidate2(config7, neuroClawConfig, logger8, intensity = 0.5, skipAIReview = false) {
    let merged = 0;
    let pruned = 0;
    let strengthened = 0;
    ...
    if (semanticStore.length > config7.memory.maxSemanticMemories) {
      semanticStore.sort((a, b) => b.confidence - a.confidence);
      const removed = semanticStore.splice(config7.memory.maxSemanticMemories);
    ...
    if (!skipAIReview && neuroClawConfig && isAIProviderAvailable(neuroClawConfig) && semanticStore.length >= 5) {
      try {
        ...
        logger8?.info(`BrainAgent Consolidation: AI review error \u2014 ${String(err)}`);
      }
    }
    persistEpisodic();
    persistSemantic();
...
function initEmbeddings(config7, logger8) {
  current().initEmbeddings(config7, logger8);
}
function updateEmbeddingsConfig(config7) {
  current().updateEmbeddingsConfig(config7);
}
...
async function consolidate(config7, neuroClawConfig, logger8, intensity = 0.5, skipAIReview = false) {
  return current().consolidate(config7, neuroClawConfig, logger8, intensity, skipAIReview);
}
function getStats() {
...
async function assessWithAI(text, config7, logger8) {
  const content = await callLLM(EMOTION_PROMPT, text, config7, logger8, 100);
...
function createMirrorNeurons(workspaceDir) {
  const storageDir12 = workspaceDir ? join3(workspaceDir, ".brainagent", "users") : "";
  const userModels = /* @__PURE__ */ new Map();
  if (storageDir12 && !existsSync2(storageDir12)) {
    mkdirSync2(storageDir12, { recursive: true });
  }
  try {
    const indexPath = join3(storageDir12, "index.json");
    if (storageDir12 && existsSync2(indexPath)) {
  ...
  function persistModels() {
    ...
    try {
      ...
      writeFileSync3(join3(storageDir12, "index.json"), JSON.stringify(data, null, 2), "utf-8");
    } catch {
    }
  }
  ...
  function observe2(userId, text, amygdalaResult, config7) {
    ...
    if (model.emotionHistory.length > config7.empathy.emotionHistoryLength) {
      model.emotionHistory = model.emotionHistory.slice(-config7.empathy.emotionHistoryLength);
    }
    ...
    applyTheoryOfMindUpdates(model, text, amygdalaResult, config7);
    persistModels();
  ...
  async function observeWithAI2(userId, text, amygdalaResult, config7, neuroConfig, logger8) {
    const model = getOrCreateModel(userId);
    model.emotionHistory.push({
      timestamp: Date.now(),
      emotion: amygdalaResult.emotion,
    ...
    const alpha = 0.3;
    model.stressLevel = model.stressLevel * (1 - alpha) + (isStressed ? amygdalaResult.emotionIntensity : 0) * alpha;
    const content = await callLLM(STYLE_PROMPT, text, neuroConfig, logger8, 100);
    if (content) {
      try {
        const jsonMatch = content.match(/\{[\s\S]*\}/);
        if (jsonMatch) {
...
function applyTheoryOfMindUpdates(model, text, amygdalaResult, config7) {
  ensureToMFields(model);
  const maxIntentHistory = config7.empathy.maxIntentHistory ?? 20;
  const domainLimit = config7.empathy.knowledgeModelDomainLimit ?? 15;
...
function updateKnowledgeModelToM(model, text, domainLimit) {
  ...
  for (const domain of domainSignals) {
    const current12 = model.knowledgeModel[domain];
    if (!current12) {
      model.knowledgeModel[domain] = "beginner";
    } else if (current12 === "beginner") {
...
function observe(userId, text, amygdalaResult, config7) {
  return current2().observe(userId, text, amygdalaResult, config7);
}
...
function observeWithAI(userId, text, amygdalaResult, config7, neuroConfig, logger8) {
  return current2().observeWithAI(userId, text, amygdalaResult, config7, neuroConfig, logger8);
}
...
function createPredictiveEngine(workspaceDir) {
  const storageDir12 = workspaceDir ? join4(workspaceDir, ".brainagent", "predictions") : "";
  ...
  let lastDomain;
  if (storageDir12 && !existsSync3(storageDir12)) {
    mkdirSync3(storageDir12, { recursive: true });
  }
  function loadPatterns() {
    if (!storageDir12) return;
    try {
      const tPath = join4(storageDir12, "temporal.json");
    ...
    try {
      const sPath = join4(storageDir12, "sequential.json");
    ...
    try {
      const cPath = join4(storageDir12, "contextual.json");
...
function createBasalGanglia(workspaceDir) {
  const storageDir12 = workspaceDir ? join5(workspaceDir, ".brainagent", "habits") : "";
  let habits = [];
  let habitIndex = new VectorIndex();
  let idCounter3 = 0;
  function nextHabitId() {
    return `hab-${Date.now()}-${++idCounter3}`;
  }
  if (storageDir12 && !existsSync4(storageDir12)) {
    mkdirSync4(storageDir12, { recursive: true });
  }
  function loadHabits() {
    if (!storageDir12) return;
    try {
      const path = join5(storageDir12, "habits.json");
      if (existsSync4(path)) {
  ...
  function persistHabits() {
    if (!storageDir12) return;
    try {
      writeFileSync5(join5(storageDir12, "habits.json"), JSON.stringify(habits, null, 2), "utf-8");
    } catch {
    }
  }
...
async function detectReinforcementWithAI(text, config7, logger8) {
  const content = await callLLM(REINFORCEMENT_PROMPT, text, config7, logger8, 100);
...
function initIntrospection(workspaceDir, config7) {
  storageDir = join6(workspaceDir, ".brainagent", "introspection");
  if (!existsSync5(storageDir)) {
    mkdirSync5(storageDir, { recursive: true });
  }
  maxTraces = config7.introspection.maxTraces;
  injectConfidence = config7.introspection.injectConfidence;
  maxSelfDialogue = config7.introspection.maxSelfDialogue;
  maxMetaSnapshots = config7.introspection.maxMetaSnapshots;
  traces = [];
  currentTrace = null;
  selfDialogue = [];
  metaSnapshots = [];
...
function createDopamineSystem(workspaceDir) {
  const storageDir12 = join7(workspaceDir, ".brainagent", "neuromodulators");
  if (!existsSync6(storageDir12)) {
    mkdirSync6(storageDir12, { recursive: true });
  }
  cancelPersist(join7(storageDir12, "state.json"));
  let state3 = {
    dopamine: 0.5,
    serotonin: 0.6,
    norepinephrine: 0.3,
  ...
  function loadState12() {
    try {
      const path = join7(storageDir12, "state.json");
      if (existsSync6(path)) {
  ...
  loadState12();
  function updateDopamine(predictionError, config7) {
    const baseline = config7.neuromodulators.baselineDopamine;
    const decay = config7.neuromodulators.dopamineDecayRate;
  ...
  function processOutcome(params, config7) {
    totalInteractions++;
    let reward = 0;
    ...
    updateDopamine(predictionError, config7);
    updateSerotonin(reward);
    updateNorepinephrine(params.complexity, params.emotion);
    updateAcetylcholine(params.domain, config7);
  ...
  return {
    ...
    getState: () => ({ ...state3 }),
    getEffectiveLearningRate: (baseLearningRate, config7) => {
      const achBoost = 1 + (state3.acetylcholine - 0.5) * (config7.neuromodulators.acetylcholineLearningBoost - 1);
...
function processInteractionOutcome(params, config7) {
  if (!active5) {
    return {
      reward: 0,
      predictionError: 0,
  ...
  return active5.processOutcome(params, config7);
}
...
function createNeuralPathways(workspaceDir, config7, logger8) {
  const storageDir12 = join8(workspaceDir, ".brainagent", "synapses");
  if (!existsSync7(storageDir12)) {
    mkdirSync7(storageDir12, { recursive: true });
  }
  ...
  function applyHebbianLearning(reward) {
    ...
    const { learningRate, decayRate, minWeight, maxWeight } = config7.synapticPlasticity;
    const now = Date.now();
    for (const pathwayName of activatedPathways) {
      ...
      if (weight.recentActivations.length > config7.synapticPlasticity.activationHistorySize) {
        weight.recentActivations = weight.recentActivations.slice(
          -config7.synapticPlasticity.activationHistorySize
        );
      }
  ...
  function loadSynapticState() {
    try {
      const path = join8(storageDir12, "weights.json");
      if (existsSync7(path)) {
  ...
  function saveSynapticState() {
    try {
      writeFileSync7(
        join8(storageDir12, "weights.json"),
  ...
  loadSynapticState();
  synapticState.learningRate = config7.synapticPlasticity.learningRate;
  synapticState.decayRate = config7.synapticPlasticity.decayRate;
  const unsubs = [];
  unsubs.push(
...
var active6;
function initNeuralPathways(workspaceDir, config7, logger8) {
  active6?.dispose();
  active6 = createNeuralPathways(workspaceDir, config7, logger8);
}
...
function createWorkingMemory(workspaceDir, opts) {
  const storageDir12 = workspaceDir ? join9(workspaceDir, ".brainagent", "working-memory") : "";
  const maxEntries = opts.maxEntries;
  const summaryMaxLength = opts.summaryMaxLength;
  let entries = [];
  if (storageDir12 && !existsSync8(storageDir12)) {
    mkdirSync8(storageDir12, { recursive: true });
  }
  function loadState12() {
    if (!storageDir12) return;
    try {
      const path = join9(storageDir12, "state.json");
      if (existsSync8(path)) {
  ...
  function persistState10() {
    if (!storageDir12) return;
    try {
      writeFileSync8(join9(storageDir12, "state.json"), JSON.stringify(entries, null, 2), "utf-8");
    } catch {
    }
  }
  loadState12();
...
function initWorkingMemoryStorage(workspaceDir, config7) {
  active7 = createWorkingMemory(workspaceDir, {
    maxEntries: config7.workingMemory.maxEntries,
    summaryMaxLength: config7.workingMemory.summaryMaxLength
  });
}
...
async function extractFactsWithAI(text, config7, logger8) {
  const content = await callLLM(EXTRACTION_PROMPT, text, config7, logger8, 500);
  if (!content) return [];
  const facts = parseFactsFromResponse(content);
  if (facts.length > 0) {
    logger8?.info(`BrainAgent AI Extractor: found ${facts.length} facts`);
  }
  return facts;
}
...
function isAIProviderAvailable2(config7) {
  return isAIProviderAvailable(config7);
}
...
async function validateAsync(response, originalInput, config7, classification, assessment, userModel, logger8) {
  const heuristicResult = validate(response, originalInput, classification, assessment, userModel);
  if (!isAIProviderAvailable(config7)) {
...
\u041E\u0442\u0432\u0435\u0442: ${response}`;
    const aiResponse = await callLLM(QUALITY_PROMPT, userText, config7, logger8, 300);
    if (aiResponse) {
    ...
    logger8?.info(`BrainAgent Cerebellum: AI validation error \u2014 ${String(err)}`);
  }
  return heuristicResult;
}
...
async function extractProcedureAsync(text, config7, classification, logger8) {
  if (isAIProviderAvailable(config7)) {
    try {
      const aiResponse = await callLLM(PROCEDURE_PROMPT, text, config7, logger8, 300);
      if (aiResponse) {
        const parsed = parseProcedureResponse(aiResponse, classification);
        if (parsed) return parsed;
      }
    } catch (err) {
      logger8?.info(`BrainAgent Procedural: AI extraction error \u2014 ${String(err)}`);
    }
  }
...
function initStructuralPlasticity(workspaceDir, config7, log) {
...
function createLearningCoordinator(workspaceDir, config7) {
  const storageDir12 = join11(workspaceDir, ".brainagent", "learning");
  if (!existsSync10(storageDir12)) {
    mkdirSync10(storageDir12, { recursive: true });
  }
  ...
  function loadState12() {
    try {
      const path = join11(storageDir12, "coordinator.json");
  ...
  function persistState10() {
    try {
      ...
      writeFileSync10(
        join11(storageDir12, "coordinator.json"),
        JSON.stringify(
          {
  ...
  function processRewardSignal(signal) {
    ...
    if (cycleCount >= config7.learning.minCyclesForInsights && cycleCount % 5 === 0) {
      generateInsights(config7);
    }
    if (cycleCount % 10 === 0) {
...
var active8;
function initLearningCoordinator(workspaceDir, config7) {
  active8?.dispose();
  active8 = createLearningCoordinator(workspaceDir, config7);
}
...
function initEmergentModules(workspaceDir, config7, log) {
  currentConfig2 = config7;
  logger2 = log;
  storageDir3 = join12(workspaceDir, ".brainagent", "emergent");
  if (!existsSync11(storageDir3)) {
    mkdirSync11(storageDir3, { recursive: true });
  }
  state2.minOccurrences = config7.emergentModules.minOccurrences;
  state2.minReward = config7.emergentModules.minRewardForEstablishment;
  loadState3();
...
function createMetabolicBudget(workspaceDir, config7, log) {
  const storageDir12 = join13(workspaceDir, ".brainagent", "metabolic");
  if (!existsSync12(storageDir12)) {
    mkdirSync12(storageDir12, { recursive: true });
  }
  const state3 = createDefaultState3();
  state3.totalBudget = config7.metabolicBudget.totalBudget;
  state3.regenRate = config7.metabolicBudget.regenRate;
  const periodStats = {};
  ...
  function saveState3() {
    try {
      writeFileSync12(join13(storageDir12, "state.json"), JSON.stringify(state3, null, 2), "utf-8");
    } catch {
    }
  }
  loadState12();
  ...
  function consumeEnergy2(module, amount) {
    ...
    if (moduleEnergy.energy < config7.metabolicBudget.lowPowerThreshold) {
  ...
  function endCycle3() {
    ...
    for (const module of ALL_MODULES2) {
      ...
      if (moduleEnergy.lowPowerMode && moduleEnergy.energy > config7.metabolicBudget.lowPowerThreshold * 2) {
    ...
    if (state3.cyclesSinceRebalance >= config7.metabolicBudget.rebalanceInterval) {
      rebalanceEnergy();
...
var active9;
function initMetabolicBudget(workspaceDir, config7, log) {
  active9?.dispose();
  active9 = createMetabolicBudget(workspaceDir, config7, log);
}
...
function initAgentIdentity(workspaceDir, config7) {
  storageDir4 = join14(workspaceDir, ".brainagent", "identity");
  if (!existsSync13(storageDir4)) {
    mkdirSync13(storageDir4, { recursive: true });
  }
  snapshotInterval = config7.agentIdentity.snapshotInterval;
  maxSnapshots = config7.agentIdentity.maxSnapshots;
  maxAutobioMemories = config7.agentIdentity.maxAutobiographicalMemories ?? 100;
  significantRewardThreshold = config7.agentIdentity.significantRewardThreshold ?? 0.8;
  significantEmotionThreshold = config7.agentIdentity.significantEmotionThreshold ?? 0.7;
  capabilities = {};
  snapshots = [];
  lessonsLearned = [];
  totalCycles = 0;
...
function createEmotionalMemory(workspaceDir, config7) {
  const storageDir12 = workspaceDir ? join15(workspaceDir, ".brainagent", "emotional-memory") : "";
  if (storageDir12 && !existsSync14(storageDir12)) {
    mkdirSync14(storageDir12, { recursive: true });
  }
  let state3 = { flashbulbCount: 0, emotionMatchBoosts: 0 };
  let qualiaHistory = [];
  const maxQualiaHistory = config7.emotionalMemory.maxQualiaHistory ?? 10;
  const metaphorCache = /* @__PURE__ */ new Map();
  let lastLLMCallTimestamp = 0;
  function loadState12() {
    if (!storageDir12) return;
    try {
      const path = join15(storageDir12, "state.json");
  ...
  function persistState10() {
    ...
    try {
      writeFileSync14(
        join15(storageDir12, "state.json"),
  ...
  async function generateQualiaAsync2(emotion, intensity, domain, neuromodulators, llmConfig, logger8) {
...
${neuroDesc}`;
    try {
      ...
      const response = await callLLM(QUALIA_METAPHOR_PROMPT, userMessage, llmConfig, logger8, 200);
      if (!response) {
        logger8?.info("BrainAgent Qualia: LLM returned null, falling back to template");
        return generateQualia(emotion, intensity, domain, neuromodulators);
      }
      const parsed = parseLLMQualiaResponse(response);
      if (!parsed) {
        logger8?.info("BrainAgent Qualia: failed to parse LLM response, falling back to template");
        return generateQualia(emotion, intensity, domain, neuromodulators);
      }
      const qualia = {
        timestamp: now,
      ...
      persistState10();
      logger8?.info(`BrainAgent Qualia: LLM-generated metaphor for ${emotion}/${domain}`);
      return qualia;
    } catch (err) {
      logger8?.info(`BrainAgent Qualia: AI metaphor generation failed \u2014 ${String(err)}`);
      return generateQualia(emotion, intensity, domain, neuromodulators);
    }
  }
  function dispose() {
...
function initEmotionalMemory(workspaceDir, config7) {
  active10?.dispose();
  active10 = createEmotionalMemory(workspaceDir, config7);
}
function tagEmotionalContext(emotion, intensity) {
  return current6().tagEmotionalContext(emotion, intensity);
}
async function generateQualiaAsync(emotion, intensity, domain, neuromodulators, config7, logger8) {
  return current6().generateQualiaAsync(emotion, intensity, domain, neuromodulators, config7, logger8);
}
...
function initQualiaSimulator(workspaceDir, config7) {
  ...
  minIntensityForInjection = config7.qualiaSimulator.minIntensityForInjection;
  maxLog = 20;
  currentQualia = null;
  qualiaLog = [];
  loadState5();
...
function createDMN(workspaceDir, config7, log) {
  const storageDir12 = workspaceDir ? join17(workspaceDir, ".brainagent", "dmn") : "";
  const insights = [];
  ...
  let maxBackgroundThoughts = config7?.dmn?.maxBackgroundThoughts ?? 20;
  function loadState12() {
    if (!storageDir12) return;
    try {
      const path = join17(storageDir12, "state.json");
  ...
  function persistState10() {
    ...
    try {
      writeFileSync16(
        join17(storageDir12, "state.json"),
        JSON.stringify(
          {
  ...
  async function runAssociationFinding2(cfg) {
    ...
    persistState10();
    logger8?.info(`DMN: found ${newInsights.length} cross-domain associations`);
    return newInsights;
  }
...
${insight.insightText}
  ...
  function generateBackgroundThoughts2(cfg, unresolvedQuestions, recentEmotions, knowledgeGaps) {
    ...
    persistState10();
    logger8?.info(`DMN: generated ${newThoughts.length} background thoughts`);
    return newThoughts;
  }
...
${lines}
  ...
  if (storageDir12) {
    if (!existsSync16(storageDir12)) {
      mkdirSync16(storageDir12, { recursive: true });
    }
    currentConfig3 = config7 ?? null;
    loadState12();
  }
  return {
    init,
...
function initDMN(workspaceDir, config7, log) {
  active11?.stop();
  active11 = createDMN(workspaceDir, config7, log);
}
function runAssociationFinding(config7) {
  return current7().runAssociationFinding(config7);
}
function getDMNStats() {
  return current7().getStats();
}
function generateBackgroundThoughts(config7, unresolvedQuestions, recentEmotions, knowledgeGaps) {
...
function createTemporalBinding(workspaceDir, opts) {
  const storageDir12 = workspaceDir ? join18(workspaceDir, ".brainagent", "temporal-binding") : "";
  const maxMoments = opts.maxMoments;
  let moments = [];
  let idCounter3 = 0;
  if (storageDir12 && !existsSync17(storageDir12)) {
    mkdirSync17(storageDir12, { recursive: true });
  }
  function loadState12() {
    if (!storageDir12) return;
    try {
      const path = join18(storageDir12, "state.json");
  ...
  function persistState10() {
    if (!storageDir12) return;
    try {
      writeFileSync17(join18(storageDir12, "state.json"), JSON.stringify(moments, null, 2), "utf-8");
    } catch {
    }
  }
  loadState12();
...
function initTemporalBinding(workspaceDir, config7) {
  active12 = createTemporalBinding(workspaceDir, { maxMoments: config7.temporalBinding.maxMoments });
}
...
function createSessionBridge(workspaceDir, config7, _logger) {
  const storageDir12 = workspaceDir ? join19(workspaceDir, ".brainagent", "sessions") : "";
  if (storageDir12 && !existsSync18(storageDir12)) {
    mkdirSync18(storageDir12, { recursive: true });
  }
  const gapThresholdMs = config7.sessionBridge.gapThresholdMs;
  const maxSummaryTopics = config7.sessionBridge.maxSummaryTopics;
  ...
  function loadState12() {
    if (!storageDir12) return;
    try {
      const currentPath = join19(storageDir12, "current.json");
    ...
    try {
      const lastPath = join19(storageDir12, "last.json");
  ...
  function persistCurrent() {
    ...
    try {
      writeFileSync18(
        join19(storageDir12, "current.json"),
  ...
  function persistLast() {
    if (!storageDir12) return;
    try {
      writeFileSync18(join19(storageDir12, "last.json"), JSON.stringify(lastSession, null, 2), "utf-8");
    } catch {
    }
  }
...
function initSessionBridge(workspaceDir, config7, logger8) {
  active13?.dispose();
  active13 = createSessionBridge(workspaceDir, config7, logger8);
}
...
var idCounter = 0;
function initCuriosityDrive(workspaceDir, config7) {
  storageDir6 = join20(workspaceDir, ".brainagent", "curiosity");
  if (!existsSync19(storageDir6)) {
    mkdirSync19(storageDir6, { recursive: true });
  }
  maxGaps = config7.curiosity.maxGaps;
  minGapConfidence = config7.curiosity.minGapConfidence;
  askProbability = config7.curiosity.askProbability;
  gaps = [];
  totalDetected = 0;
...
function loadState6() {
  ...
  try {
    const path = join20(storageDir6, "state.json");
...
function initGoalStack(workspaceDir, config7) {
  storageDir7 = join21(workspaceDir, ".brainagent", "goals");
  if (!existsSync20(storageDir7)) {
    mkdirSync20(storageDir7, { recursive: true });
  }
  maxGoals = config7.goalStack.maxGoals;
  defaultTTLMs = config7.goalStack.defaultTTLMs;
  maxDesires = config7.goalStack.maxDesires;
  maxDecisionLog = config7.goalStack.maxDecisionLog;
  explorationRate = config7.goalStack.explorationRate;
  goals = [];
  desires = [];
  decisionLog = [];
  idCounter2 = 0;
...
function loadState7() {
  ...
  try {
    const path = join21(storageDir7, "state.json");
...
function checkGoalTriggers(input, currentEmotion, currentDomain) {
  ...
  if (triggered.length > 0) persistState5();
  return triggered;
}
function expireGoals() {
  ...
  if (goals.length > 60) {
    const active25 = goals.filter((g) => g.status === "pending" || g.status === "triggered");
    const inactive = goals.filter((g) => g.status === "completed" || g.status === "expired").slice(-30);
    goals = [...active25, ...inactive];
    changed = true;
  }
  if (changed) persistState5();
}
...
function weakenDesiresAfterFire(consecutiveFires) {
  ...
  if (desires.length !== before || changed) persistState5();
}
...
async function extractGoalsFromConversation(userMessage, config7, logger8) {
  if (!isAIProviderAvailable(config7)) {
    logger8?.info("BrainAgent GoalStack: no AI provider available, skipping goal extraction");
    return [];
  }
  logger8?.info("BrainAgent GoalStack: extracting goals from conversation...");
  const response = await callLLM(GOAL_EXTRACTION_PROMPT, userMessage, config7, logger8, 300);
  if (!response) {
    logger8?.info("BrainAgent GoalStack: LLM returned null/empty response");
    return [];
  }
  logger8?.info(`BrainAgent GoalStack: LLM response received (${response.length} chars)`);
  let parsed;
  try {
    let jsonStr = response.trim();
    const jsonMatch = jsonStr.match(/\[[\s\S]*\]/);
    ...
    logger8?.info("BrainAgent GoalStack: failed to parse LLM goal extraction response");
    return [];
  }
  const createdGoals = [];
...
function createCircadianRhythm(workspaceDir, config7, log) {
  const storageDir12 = join22(workspaceDir, ".brainagent", "circadian");
  if (!existsSync21(storageDir12)) {
    mkdirSync21(storageDir12, { recursive: true });
  }
  const maxSleepConsolidations = config7.circadian.maxSleepConsolidations ?? 5;
  ...
  function loadState12() {
    try {
      const path = join22(storageDir12, "state.json");
  ...
  function persistState10() {
    try {
      writeFileSync21(join22(storageDir12, "state.json"), JSON.stringify(state3, null, 2), "utf-8");
    } catch {
    }
  }
  function evaluatePhase() {
    if (!config7.circadian.enabled) return;
    const now = Date.now();
    const idleTime = now - lastActivityTime;
    state3.idleTime = idleTime;
    const activityWindow = config7.circadian.activityWindowMs;
    state3.activityLevel = Math.max(0, 1 - idleTime / activityWindow);
    const phaseDuration = now - state3.phaseStartedAt;
    const { idleThresholdMs, minWakeDurationMs, minSleepDurationMs, transitionDurationMs } = config7.circadian;
    switch (state3.phase) {
      case "wake":
      ...
      case "sleep":
        if (consolidationCallback && phaseDuration > 3e4 && state3.sleepConsolidations < maxSleepConsolidations) {
          const consolidationInterval = config7.circadian.sleepConsolidationIntervalMs ?? 6e4;
  ...
  loadState12();
  updateModulationFromPhase();
  const evalInterval = config7.circadian.evaluationIntervalMs ?? 3e4;
  const evaluationTimer = setInterval(() => evaluatePhase(), evalInterval);
  log?.info(
    `Circadian: initialized in ${state3.phase} phase (idle threshold: ${config7.circadian.idleThresholdMs / 1e3}s)`
  );
  function stopTimer() {
  ...
  return {
    recordActivity: () => {
      ...
      if (state3.phase === "sleep" || state3.phase === "transition-to-sleep") {
        ...
        const minSleep = config7.circadian.minSleepDurationMs ?? 6e4;
...
function initCircadianRhythm(workspaceDir, config7, log) {
  active14?.dispose();
  active14 = createCircadianRhythm(workspaceDir, config7, log);
  if (pendingConsolidationCallback) {
    active14.setConsolidationCallback(pendingConsolidationCallback);
  }
}
...
function setConsolidationCallback(callback) {
  ...
  active14?.setConsolidationCallback(callback);
}
function recordActivity() {
  active14?.recordActivity();
}
function getCircadianState() {
  return active14?.getState() ?? createDefaultState4();
}
function getSleepSettings() {
  return active14?.getSleepSettings() ?? {
    consolidationIntensity: 0.5,
    pruningAggressiveness: 0.3,
    synapticNormalization: false
  };
...
function isInSleepPhase() {
  return active14?.isInSleepPhase() ?? false;
}
function isInWakePhase() {
  return active14?.isInWakePhase() ?? true;
}
function forcePhase(phase) {
  active14?.forcePhase(phase);
}
function getCircadianStats() {
  const st = active14?.getState() ?? createDefaultState4();
  return {
    phase: st.phase,
...
var DriveEngine = class {
  constructor(spec, config7, workspaceDir, circadianEnabled3, deps5, logger8) {
    this.spec = spec;
    this.config = config7;
    this.circadianEnabled = circadianEnabled3;
    this.deps = deps5;
    this.logger = logger8;
    const storageDir12 = join23(workspaceDir, ".brainagent", spec.id);
    if (!existsSync22(storageDir12)) {
      mkdirSync22(storageDir12, { recursive: true });
    }
    this.stateFile = join23(storageDir12, "state.json");
    cancelPersist(this.stateFile);
    this.loadState();
    this.wireCoreListeners();
    logger8.info(
      `BrainAgent ${spec.logName}: initialized (satiation=${this.satiation.toFixed(2)}, decay=${config7.baseDecayRate}/${config7.decayIntervalMs}ms, domains=${config7.rewardDomains.join(",")})`
    );
  }
  spec;
  config;
...
function initSocialDrive(workspaceDir, cfg, log, injectedDeps) {
  active15?.dispose();
  active15 = createSocialDrive(workspaceDir, cfg, log, injectedDeps);
}
...
function getSatiation() {
  return active15?.getSatiation() ?? 0;
}
...
function initCognitiveHunger(workspaceDir, cfg, log, injectedDeps) {
  active16?.dispose();
  active16 = createCognitiveHunger(workspaceDir, cfg, log, injectedDeps);
}
...
function initCreativeDrive(workspaceDir, cfg, log, injectedDeps) {
  active17?.dispose();
  active17 = createCreativeDrive(workspaceDir, cfg, log, injectedDeps);
}
...
function initMasteryDrive(workspaceDir, cfg, log, injectedDeps) {
  storageDir8 = join24(workspaceDir, ".brainagent", "mastery-drive");
  if (!existsSync23(storageDir8)) {
    mkdirSync23(storageDir8, { recursive: true });
  }
  ...
  totalNeedSignals = 0;
  adaptiveDecayModifier = 1;
  cancelPersist(join24(storageDir8, "state.json"));
  loadState8();
  wireEventListeners();
  logger3.info(
...
function stopMasteryDrive() {
  ...
  unsubscribers.length = 0;
  persistState6();
  flushPersist(join24(storageDir8, "state.json"));
...
function loadState8() {
  try {
    const filePath = join24(storageDir8, "state.json");
...
function persistState6() {
  schedulePersist(join24(storageDir8, "state.json"), () => {
    const domainMap = {};
    for (const [domain, mastery] of domainSatiations) {
...
function collectActiveDrives(drives, threshold) {
  ...
  if (drives.social) {
    ...
    if (s.need >= threshold) active25.push({ name: "social", need: s.need, needLevel: s.needLevel });
  }
  if (drives.cognitive) {
    const c = drives.cognitive();
    if (c.need >= threshold) active25.push({ name: "cognitive", need: c.need, needLevel: c.needLevel });
  }
  if (drives.creative) {
    const c = drives.creative();
    if (c.need >= threshold) active25.push({ name: "creative", need: c.need, needLevel: c.needLevel });
  }
  if (drives.mastery) {
    const m = drives.mastery();
    if (m.need >= threshold) active25.push({ name: "mastery", need: m.need, needLevel: m.needLevel });
  }
  return active25;
}
function synthesizeAutonomousCycleState(brainConfig, drives, cycle) {
  const active25 = collectActiveDrives(drives, 0.5);
  if (active25.length === 0) return;
  const ranked = active25.map((d) => {
...
function createAutonomousDeliverer(deps5) {
  const { state: state3, brainConfig, minGapMs, logger: logger8 } = deps5;
  return function enqueueAutonomousIntent(text) {
    const trimmed = text.trim();
    if (!trimmed) return;
    if (state3.previousCycleWasAutonomous) {
      logger8.info("BrainAgent Autonomy: intent suppressed \u2014 previous cycle was autonomous");
      return;
    }
    if (Date.now() - state3.lastAutonomousDeliveryAt < minGapMs) {
      logger8.info("BrainAgent Autonomy: intent suppressed \u2014 minimum gap not elapsed");
      return;
    }
    ...
    if (!agent) {
      logger8.warn("BrainAgent Autonomy: no live agent \u2014 autonomous intent dropped");
      return;
    }
...
function createAutonomousIntentResolver(deps5) {
  ...
  return () => {
    ...
    const active25 = collectActiveDrives(drives, 0.8);
    if (active25.length > 0) {
      const strongest = active25.sort((a, b) => b.need - a.need)[0];
...
function createCycleEngine(deps5) {
  const { config: config7, brainConfig, getHostConfig, logger: logger8, markActivation, state: state3 } = deps5;
  ...
  async function endCycle3(key) {
    ...
    const aiAvailable = config7.modules.aiEnrichment && isAIProviderAvailable2(getHostConfig());
    if (brainConfig.modules.cerebellum && cycle.responseText.trim()) {
      ...
      if (result.issues.length > 0) {
        logger8.warn(`BrainAgent Cerebellum: quality issues \u2014 ${result.issues.join("; ")}`);
    ...
    if (config7.modules.semanticExtraction && semanticSource.length > 15 && isFactWorthy(semanticSource, cycle.classification)) {
      let factsStored = false;
      if (aiAvailable) {
        try {
          const aiFacts = await extractFactsWithAI(semanticSource, getHostConfig(), logger8);
          if (aiFacts.length > 0) {
          ...
          logger8.info(`BrainAgent Semantic: AI extraction failed, falling back \u2014 ${String(err)}`);
        }
      }
      if (!factsStored) {
      ...
      if (factsStored && brainConfig.modules.curiosityDrive) {
        ...
        for (const gap of getOpenGaps()) {
          if (inputLower.includes(gap.topic.toLowerCase())) {
            ...
            logger8.info(`BrainAgent Curiosity: gap filled for "${gap.topic}" via new fact storage`);
          }
        }
      }
    }
    if (config7.modules.proceduralExtraction && input.length > 10 && isProcedural(input, cycle.classification)) {
      const procedure = await extractProcedureAsync(input, getHostConfig(), cycle.classification, logger8);
      if (procedure && procedure.confidence > 0.5) {
        storeWorkflow(procedure.description, procedure.triggerPattern, procedure.steps);
        logger8.info(`BrainAgent Procedural: stored workflow "${procedure.description}"`);
      }
    }
    ...
    if (brainConfig.modules.neuromodulatorSystem && input.length > 5) {
      ...
      if (config7.modules.structuralPlasticity) {
        endCycle(reward);
      }
    ...
    if (brainConfig.modules.dmn) {
      ...
      if (wakeInteractionCount >= brainConfig.dmn.wakeThoughtInterval) {
        ...
        void runAssociationFinding(brainConfig).catch((err) => {
          logger8.info(`BrainAgent DMN: association finding error: ${err}`);
        });
      }
    }
    if (brainConfig.modules.goalStack && input.length > 10) {
      ...
      if (goalExtractionCounter >= brainConfig.goalStack.extractionInterval) {
        ...
        void extractGoalsFromConversation(input, getHostConfig(), logger8).catch((err) => {
          logger8.info(`BrainAgent GoalStack: extraction error: ${err}`);
        });
      }
    }
    if (brainConfig.modules.goalStack) {
...
function createStrategyBandit(workspaceDir, config7) {
  const storageDir12 = join25(workspaceDir, ".brainagent", "bandit");
  if (!existsSync24(storageDir12)) {
    mkdirSync24(storageDir12, { recursive: true });
  }
  const storageFile = join25(storageDir12, "state.json");
  const explorationConstant = config7.learningLoop.strategyBandit.explorationConstant;
  const attributionWindowMs = config7.learningLoop.strategyBandit.attributionWindowMs;
  let state3 = {};
  let lastChoices = {};
  function loadState12() {
...
var active18;
function initStrategyBandit(workspaceDir, config7) {
  active18?.dispose();
  active18 = createStrategyBandit(workspaceDir, config7);
}
...
function chooseArm(decisionPoint, arms) {
  ...
  return active18.chooseArm(decisionPoint, arms);
}
...
function initVitalImpulse(workspaceDir, cfg, log, injectedDeps) {
  storageDir9 = join26(workspaceDir, ".brainagent", "vital-impulse");
  if (!existsSync25(storageDir9)) {
    mkdirSync25(storageDir9, { recursive: true });
  }
  ...
  cancelPersist(join26(storageDir9, "state.json"));
  loadState9();
  wireSignalListeners();
  logger4.info(
...
function stopVitalImpulse() {
  ...
  unsubscribers2.length = 0;
  persistState7();
  flushPersist(join26(storageDir9, "state.json"));
...
function loadState9() {
  try {
    const filePath = join26(storageDir9, "state.json");
...
function persistState7() {
  schedulePersist(join26(storageDir9, "state.json"), () => {
    const state3 = {
      lastFireTime,
      totalFires,
      totalSignalsReceived,
...
function initDriveArbiter(workspaceDir, cfg, getters, log) {
  ...
  storageDir10 = join27(workspaceDir, ".brainagent");
  ...
  logger5?.info("BrainAgent DriveArbiter: initialized");
}
function stopDriveArbiter() {
  ...
  logger5?.info("BrainAgent DriveArbiter: stopped.");
}
function arbitrate() {
  if (!config3 || isArbitrating) return;
  isArbitrating = true;
  try {
    arbitrateInner();
  } finally {
...
function processReward(signal) {
  if (!config3 || !lastSelectedDrive) return;
  ...
  const lr = config3.rewardLearningRate;
...
function initInteroception(getters, log) {
  active19?.dispose();
  active19 = createInteroception(getters, log);
}
...
function getInteroceptiveState() {
  return active19?.getState() ?? null;
}
function buildInteroceptionContext() {
  return active19?.buildContext() ?? null;
}
...
function createTemporalAwareness(workspaceDir, cfg, log) {
  const config7 = cfg.temporalAwareness;
  let timestamps = [];
  let typicalGapMs = 0;
  let totalInteractions = 0;
  const storageDir12 = join28(workspaceDir, ".brainagent");
  if (!existsSync27(storageDir12)) {
    mkdirSync27(storageDir12, { recursive: true });
  }
  try {
    const path = join28(storageDir12, "temporal-awareness.json");
  ...
  function persistState10() {
    try {
      ...
      writeFileSync22(
        join28(storageDir12, "temporal-awareness.json"),
  ...
  function computeDensity(now) {
    ...
    const windowStart = now - config7.densityWindowMs;
  ...
  function recordInteraction2() {
    ...
    timestamps.push(now);
    if (timestamps.length > config7.gapHistorySize) {
      timestamps = timestamps.slice(-config7.gapHistorySize);
    }
    totalInteractions++;
    ...
    if (lastTime > 0 && gapMs > 0) {
      if (typicalGapMs === 0) {
        ...
        typicalGapMs = typicalGapMs * (1 - config7.gapEmaAlpha) + gapMs * config7.gapEmaAlpha;
      }
      const temporalSurprise = typicalGapMs > 0 ? gapMs / typicalGapMs : 1;
      if (temporalSurprise >= config7.longAbsenceMultiplier && gapMs > 60 * 1e3) {
    ...
    if (density >= config7.highDensityThreshold) {
  ...
  function buildContext() {
    ...
    const density = computeDensity(now);
    const lines = [];
    if (temporalSurprise >= config7.longAbsenceMultiplier && currentGapMs > 60 * 1e3) {
      lines.push(
...
function initTemporalAwareness(workspaceDir, cfg, log) {
  active20?.dispose();
  active20 = createTemporalAwareness(workspaceDir, cfg, log);
}
...
function createAttentionGate(workspaceDir) {
  const storageDir12 = workspaceDir ? join29(workspaceDir, ".brainagent", "attention") : "";
  let totalProcessed = 0;
  let totalDropped = 0;
  let totalRelevanceSum = 0;
  if (storageDir12 && !existsSync28(storageDir12)) {
    mkdirSync28(storageDir12, { recursive: true });
  }
  function loadState12() {
    if (!storageDir12) return;
    try {
      const path = join29(storageDir12, "state.json");
  ...
  function persistState10() {
    ...
    try {
      writeFileSync23(
        join29(storageDir12, "state.json"),
  ...
  loadState12();
  function filter(injections, currentInput, norepinephrineLevel, config7, maxTokenBudget) {
    ...
    const effectiveThreshold = config7.attentionGate.minRelevanceScore * (1 - norepinephrineLevel * 0.5);
    scored.sort((a, b) => b.score - a.score);
    let kept = [];
    let dropped = 0;
    for (const item of scored) {
      if (kept.length < config7.attentionGate.maxContextSections && item.score >= effectiveThreshold) {
        kept.push(item.text);
...
var active21;
function current10() {
  return active21 ?? (active21 = createAttentionGate(""));
}
function initAttentionGate(workspaceDir, _config) {
  active21 = createAttentionGate(workspaceDir);
}
function filterContextInjections(injections, currentInput, norepinephrineLevel, config7, maxTokenBudget) {
  return current10().filter(injections, currentInput, norepinephrineLevel, config7, maxTokenBudget);
}
...
function decideProcessingPath(classification, config7) {
  const threshold = COMPLEXITY_ORDER2[config7.dualProcess.system2Threshold];
  ...
  const modelOverride = useSlow ? config7.dualProcess.slowModel : config7.dualProcess.fastModel;
  const result = {
...
function initAutonomousResearch(cfg, log, injectedDeps) {
  config4 = cfg.autonomousResearch;
  deps3 = injectedDeps;
  stats = {
    totalCycles: 0,
    totalFactsExtracted: 0,
...
async function executeResearch(topic) {
  if (!config4?.enabled || !deps3) return null;
  const now = Date.now();
  if (now - stats.lastResearchTime < config4.cooldownMs) {
...
${existingFacts.map((f) => `- ${f.content}`).join("\n")}` : "";
  ...
  if (searchResponse.type === "text") {
    content = searchResponse.content.slice(0, config4.maxTotalChars);
...
${knownContext}` : `Topic: ${topic}`;
  ...
  try {
    ...
    if (Array.isArray(parsed)) {
      return parsed.filter((q) => typeof q === "string" && q.length > 2).slice(0, config4.maxSearchQueries);
    }
  } catch {
    const lines = result.split("\n").map((l) => l.replace(/^[-*\d.)\s]+/, "").trim()).filter((l) => l.length > 3 && l.length < 200);
    if (lines.length > 0) return lines.slice(0, config4.maxSearchQueries);
  }
  return null;
}
  ...
  for (const query of queries) {
    try {
      const params = new URLSearchParams({
        q: query,
        count: String(config4.maxPagesPerQuery)
      });
    ...
    try {
      const response = await fetch("https://api.tavily.com/search", {
        ...
        body: JSON.stringify({
          ...
          max_results: config4.maxPagesPerQuery
        }),
        signal: AbortSignal.timeout(1e4)
      });
      if (!response.ok) {
...
async function fetchPages(results) {
  ...
  const pages = [];
  for (const result of results) {
    if (totalChars >= config4.maxTotalChars) break;
    try {
      ...
      const truncated = text.slice(0, config4.maxPageChars);
...
async function extractAndStore(topic, content, queriesExecuted, pagesRead) {
  ...
  const truncatedContent = content.slice(0, config4.maxTotalChars);
...
function createPreStepHandler(deps5) {
  const { config: config7, brainConfig, getHostConfig, logger: logger8, state: state3, cycles, startCycle } = deps5;
  return async (payload, next) => {
    ...
    if (config7.modules.aiEnrichment && isAIProviderAvailable2(getHostConfig())) {
      const hc = getHostConfig();
      const tasks = [];
      if (brainConfig.modules.amygdala) {
        tasks.push(
          assessWithAI(input, hc, logger8).then((aiAssessment) => {
      ...
      if (brainConfig.modules.mirrorNeurons && cyc.assessment) {
        ...
        tasks.push(
          observeWithAI("default", input, snapshot, brainConfig, hc, logger8).then((model) => {
            cyc.userModel = model;
          }).catch(() => {
          })
        );
      ...
      if (brainConfig.modules.basalGanglia) {
        tasks.push(
          detectReinforcementWithAI(input, hc, logger8).then((aiSignal) => {
      ...
      ]).catch((err) => logger8.warn(String(err)));
    }
    ...
    const episodicLimit = Math.max(1, Math.round(config7.recall.episodicLimit * (0.5 + attentionLevel)));
    const semanticLimit = Math.max(1, Math.round(config7.recall.semanticLimit * (0.5 + attentionLevel)));
...
function initGoalExecutor(_cfg, log) {
  active22 = createGoalExecutor(log);
...
function initAutonomyEnricher(cfg, log, injectedDeps) {
  ...
  wireEventListeners2();
  logger6.info("BrainAgent AutonomyEnricher: initialized (memory-driven autonomy)");
}
function stopAutonomyEnricher() {
  ...
  unsubscribers4.length = 0;
  logger6?.info("BrainAgent AutonomyEnricher: stopped.");
}
...
function enrichWithMemories(motivation) {
  ...
  if (uniqueEpisodic.length === 0 && uniqueSemantic.length === 0) {
    logger6?.info("BrainAgent AutonomyEnricher: no relevant memories found, skipping enrichment");
    return;
  }
...
function createDreamMode() {
  ...
  async function runConsolidation(config7, logger8, neuroClawConfig, circadianTriggered = false) {
    ...
    try {
      ...
      const result = await consolidate(config7, neuroClawConfig, logger8, intensityMultiplier, skipAI);
      ...
      logger8?.info(`BrainAgent DreamMode: error during consolidation \u2014 ${String(err)}`);
    } finally {
      isConsolidating = false;
    }
  }
  function start(config7, logger8, neuroClawConfig) {
    if (dreamInterval) return;
    storedConfig = config7;
    storedLogger = logger8;
    storedNeuroClawConfig = neuroClawConfig;
    const intervalMs = config7.memory.dreamIntervalMinutes * 60 * 1e3;
    logger8?.info(
      `BrainAgent DreamMode: starting (interval: ${config7.memory.dreamIntervalMinutes}min)`
    );
    if (config7.circadian?.enabled) {
      setConsolidationCallback(async () => {
        await runConsolidation(config7, logger8, neuroClawConfig, true);
      });
      logger8?.info("BrainAgent DreamMode: registered with circadian rhythm system");
    }
    setTimeout(() => void runConsolidation(config7, logger8, neuroClawConfig, false), 3e4);
    dreamInterval = setInterval(() => {
      void runConsolidation(config7, logger8, neuroClawConfig, false);
    }, intervalMs);
  }
  function stop() {
    if (dreamInterval) {
  ...
  async function forceConsolidation2(config7, logger8, neuroClawConfig, intensity) {
    const sleepSettings = getSleepSettings();
    const effectiveIntensity = intensity ?? sleepSettings.consolidationIntensity;
    const result = await consolidate(config7, neuroClawConfig, logger8, effectiveIntensity);
    lastConsolidation = Date.now();
    if (logger8) {
      logger8.info(
...
var active23 = null;
function current11() {
  if (!active23) active23 = createDreamMode();
  return active23;
}
function startDreamMode(config7, logger8, neuroClawConfig) {
  current11().start(config7, logger8, neuroClawConfig);
}
function stopDreamMode() {
  current11().stop();
}
function forceConsolidation(config7, logger8, neuroClawConfig, intensity) {
  return current11().forceConsolidation(config7, logger8, neuroClawConfig, intensity);
}
...
function initProactiveFeedback(workspaceDir, cfg, log) {
  storageDir11 = join30(workspaceDir, ".brainagent", "proactive-feedback");
  if (!existsSync29(storageDir11)) {
    mkdirSync29(storageDir11, { recursive: true });
  }
  stateFile = join30(storageDir11, "state.json");
  config6 = cfg.proactiveFeedback;
  logger7 = log;
  domainFeedback = /* @__PURE__ */ new Map();
  cancelPersist(stateFile);
  loadState11();
  logger7.info(
    `BrainAgent ProactiveFeedback: initialized (threshold=${config6.suppressionThreshold}, cooldown=${config6.cooldownMs}ms, decay=${config6.decayPerDay}/day, tracked=${domainFeedback.size})`
  );
}
function stopProactiveFeedback() {
  flushPersist(stateFile);
  logger7?.info("BrainAgent ProactiveFeedback: stopped.");
}
function recordProactiveReaction(domain, reactionText) {
  if (!config6) return "neutral";
  ...
  if (signal === "rejection") {
    entry.rejections += 1;
    entry.suppressionScore += config6.rejectionStep;
    entry.lastRejectionTime = Date.now();
    entry.lastHits = classification.hits;
  } else if (signal === "negative") {
    entry.rejections += 1;
    entry.suppressionScore += config6.negativeStep;
    entry.lastRejectionTime = Date.now();
    entry.lastHits = classification.hits;
  } else if (signal === "positive") {
    entry.accepts += 1;
    entry.suppressionScore = Math.max(0, entry.suppressionScore - config6.positiveStep);
  }
...
function isDomainSuppressed(domain) {
  if (!config6) return false;
  applyDecay2();
  const entry = domainFeedback.get(domain);
  if (!entry) return false;
  if (entry.suppressionScore < config6.suppressionThreshold) return false;
  return Date.now() - entry.lastRejectionTime < config6.cooldownMs;
}
...
function applyDecay2() {
  ...
  for (const entry of domainFeedback.values()) {
    ...
    entry.suppressionScore = Math.max(0, entry.suppressionScore - days * config6.decayPerDay);
    entry.lastDecayTime = now;
  }
}
function enforceDomainLimit() {
  if (!config6 || domainFeedback.size <= config6.maxTrackedDomains) return;
  const sorted = [...domainFeedback.entries()].sort(
    (a, b) => a[1].suppressionScore - b[1].suppressionScore
  );
  while (domainFeedback.size > config6.maxTrackedDomains) {
...
function createRewardLedger(workspaceDir, config7) {
  const storageDir12 = join31(workspaceDir, ".brainagent", "reward");
  if (!existsSync30(storageDir12)) {
    mkdirSync30(storageDir12, { recursive: true });
  }
  const storageFile = join31(storageDir12, "ledger.json");
  cancelPersist(storageFile);
  const maxEntries = config7.learningLoop.rewardLedger.maxEntries;
  let entries = [];
  let idCounter3 = 0;
...
var active24;
function initRewardLedger(workspaceDir, config7) {
  active24?.dispose();
  active24 = createRewardLedger(workspaceDir, config7);
}
...
function initThalamicGate(config7, signalProviders) {
  gateConfig = config7;
...
function attachLlmBridge(ctx, preferredModel) {
  ...
  setCallLLMBackend(async (systemPrompt, userText, _config, logger8, maxTokens) => {
    ...
    try {
      ...
      logger8?.info(`BrainAgent LLM bridge: ctx.llm call failed \u2014 ${String(error)}`);
      return void 0;
    }
  });
  void resolveRoute();
...
function registerBrainAgentCommands(api, config7) {
  api.registerCommand({
    name: "brainagent",
    ...
    handler: async (ctx) => {
      ...
      if (args === "status" || args === "") {
        return buildStatusReport(config7);
      }
      if (args === "dream") {
        const result = await forceConsolidation(config7, api.logger, api.config);
        return {
          text: [
...
function buildStatusReport(config7) {
  ...
  const lines = [
    ...
    "",
    "**Core Modules:**",
    `  Thalamus (classifier):        ${config7.modules.thalamus ? "ON" : "OFF"}`,
    `  Amygdala (emotion/priority):   ${config7.modules.amygdala ? "ON" : "OFF"}`,
    `  Hippocampus (memory):          ${config7.modules.hippocampus ? "ON" : "OFF"}`,
    `  Prefrontal Cortex (reasoning): ${config7.modules.prefrontalCortex ? "ON" : "OFF"}`,
    `  Cerebellum (quality):          ${config7.modules.cerebellum ? "ON" : "OFF"}`,
    `  Mirror Neurons (empathy):      ${config7.modules.mirrorNeurons ? "ON" : "OFF"}`,
    `  Predictive Engine (anticip.):  ${config7.modules.predictiveEngine ? "ON" : "OFF"}`,
    `  Basal Ganglia (habits):        ${config7.modules.basalGanglia ? "ON" : "OFF"}`,
    `  Dream Mode (consolidation):    ${config7.modules.dreamMode ? "ON" : "OFF"}`,
    "",
    "**Integration Modules:**",
    `  Dopamine System (reward):      ${config7.modules.neuromodulatorSystem ? "ON" : "OFF"}`,
    `  Learning Coordinator (meta):   ${config7.modules.learningCoordinator ? "ON" : "OFF"}`,
    `  Neural Pathways (connections):  ${config7.modules.neuralPathways ? "ON" : "OFF"}`,
    "",
    "**Consciousness Modules:**",
    `  Working Memory (continuity):   ${config7.modules.workingMemory ? "ON" : "OFF"}`,
    `  Session Bridge (cross-sess.):  ${config7.modules.sessionBridge ? "ON" : "OFF"}`,
    `  Emotional Memory (flashbulb):  ${config7.modules.emotionalMemory ? "ON" : "OFF"}`,
    `  Attention Gate (filtering):    ${config7.modules.attentionGate ? "ON" : "OFF"}`,
    `  DMN (idle thinking):           ${config7.modules.dmn ? "ON" : "OFF"}`,
    `  Introspection (self-trace):    ${config7.modules.introspection ? "ON" : "OFF"}`,
    `  Agent Identity (self-model):   ${config7.modules.agentIdentity ? "ON" : "OFF"}`,
    `  Goal Stack (proactive):        ${config7.modules.goalStack ? "ON" : "OFF"}`,
    `  Curiosity Drive (gaps):        ${config7.modules.curiosityDrive ? "ON" : "OFF"}`,
    `  Temporal Binding (stream):     ${config7.modules.temporalBinding ? "ON" : "OFF"}`,
    `  Qualia Simulator (subjective): ${config7.modules.qualiaSimulator ? "ON" : "OFF"}`,
    `  Vital Impulse (autonomous):   ${config7.modules.vitalImpulse ? "ON" : "OFF"}`,
    `  Goal Executor (autonomous):   ${config7.modules.goalStack ? "ON" : "OFF"}`,
    "",
    "**Autonomic Modules:**",
    `  Metabolic Budget (energy):     ${config7.modules.metabolicBudget ? "ON" : "OFF"}`,
    `  Emergent Modules (patterns):   ${config7.modules.emergentModules ? "ON" : "OFF"}`,
    `  Interoception (inner state):   ${config7.modules.interoception ? "ON" : "OFF"}`,
    `  Proactive Feedback (\xAB\u043D\u0435 \u0437\u0430\u0448\u043B\u043E\xBB): ${config7.modules.proactiveFeedback ? "ON" : "OFF"}`,
    "",
    "**Memory:**",
  ...
  if (config7.modules.neuromodulatorSystem) {
    const dStats = getDopamineStats();
    const ns = dStats.currentState;
    lines.push(
      "",
  ...
  if (config7.modules.learningCoordinator) {
    const lStats = getLearningStats();
    lines.push(
      "",
  ...
  if (identityStatsGetter && config7.modules.agentIdentity) {
  ...
  if (qualiaSimulatorStatsGetter && config7.modules.qualiaSimulator) {
  ...
  if (vitalImpulseStatsGetter && config7.modules.vitalImpulse) {
    const vi = vitalImpulseStatsGetter();
  ...
  if (goalExecutorStatsGetter && config7.modules.goalStack) {
  ...
  if (config7.modules.proactiveFeedback) {
  ...
  if (socialDriveStatsGetter && config7.modules.socialDrive) {
  ...
  if (cognitiveHungerStatsGetter && config7.modules.cognitiveHunger) {
  ...
  if (creativeDriveStatsGetter && config7.modules.creativeDrive) {
  ...
  if (masteryDriveStatsGetter && config7.modules.masteryDrive) {
  ...
  if (driveArbiterStatsGetter && config7.modules.driveArbiter) {
  ...
  if (temporalAwarenessStatsGetter && config7.modules.temporalAwareness) {
  ...
  if (autonomousResearchStatsGetter && config7.modules.autonomousResearch) {
...
function apply(ctx, config7) {
  const logger8 = {
    info: (msg) => ctx.logger.info(msg),
    warn: (msg) => ctx.logger.warn(msg),
    error: (msg) => ctx.logger.error(msg)
  };
  const brainConfig = mergeBrainConfig(config7);
  const dataDir = config7.dataDir;
  mkdirSync31(dataDir, { recursive: true });
  const hostConfig = () => buildHostConfig({
    providers: config7.providers,
    ...config7.model ? { model: config7.model } : {}
  });
  ...
  const enqueueAutonomousIntent = createAutonomousDeliverer({
    state: state3,
    brainConfig,
    minGapMs: config7.autonomousMinGapMs ?? 10 * 60 * 1e3,
    logger: logger8,
    pickAgent,
  ...
  initEmbeddings(hostConfig(), logger8);
  if (brainConfig.modules.workingMemory) {
  ...
  if (brainConfig.modules.neuralPathways) {
    initNeuralPathways(dataDir, brainConfig, logger8);
  }
  if (config7.modules.structuralPlasticity) {
    initStructuralPlasticity(dataDir, brainConfig, logger8);
  }
  if (brainConfig.modules.emotionalMemory) {
    initEmotionalMemory(dataDir, brainConfig);
  }
  if (brainConfig.modules.sessionBridge) {
    initSessionBridge(dataDir, brainConfig, logger8);
  }
  if (brainConfig.modules.dmn) {
    initDMN(dataDir, brainConfig, logger8);
  }
  if (brainConfig.modules.goalStack) {
  ...
  if (brainConfig.modules.socialDrive) {
    initSocialDrive(dataDir, brainConfig, logger8, {
      ...driveInitBase,
  ...
  if (brainConfig.modules.cognitiveHunger) {
    initCognitiveHunger(dataDir, brainConfig, logger8, {
      ...driveInitBase,
  ...
  if (brainConfig.modules.creativeDrive) {
    initCreativeDrive(dataDir, brainConfig, logger8, {
      ...driveInitBase,
  ...
  if (brainConfig.modules.masteryDrive) {
    initMasteryDrive(dataDir, brainConfig, logger8, {
      ...driveInitBase,
  ...
  if (brainConfig.modules.vitalImpulse) {
    initVitalImpulse(dataDir, brainConfig, logger8, {
  ...
  if (brainConfig.modules.goalStack) {
    initGoalExecutor(brainConfig, logger8);
  }
  if (brainConfig.modules.actionDispatcher && brainConfig.modules.vitalImpulse) {
    initAutonomyEnricher(brainConfig, logger8, {
  ...
  if (brainConfig.modules.autonomousResearch) {
    initAutonomousResearch(brainConfig, logger8, {
      callLLM,
      storeFact,
  ...
  if (brainConfig.circadian.enabled) {
    initCircadianRhythm(dataDir, brainConfig, logger8);
  }
  if (brainConfig.modules.dreamMode) {
    startDreamMode(brainConfig, logger8, hostConfig());
  }
  if (brainConfig.modules.metabolicBudget) {
    initMetabolicBudget(dataDir, brainConfig, logger8);
  }
  if (brainConfig.modules.emergentModules) {
    initEmergentModules(dataDir, brainConfig, logger8);
  }
  if (brainConfig.modules.proactiveFeedback) {
    initProactiveFeedback(dataDir, brainConfig, logger8);
  }
  ...
  if (brainConfig.modules.temporalAwareness) {
    initTemporalAwareness(dataDir, brainConfig, logger8);
  }
  if (brainConfig.modules.interoception) {
    initInteroception(
      {
  ...
  ctx.on("session/event", (_session, event) => {
    ...
    if (event.type === "turn/end") {
      void endCycle3(key).catch((err) => logger8.warn(`BrainAgent endCycle: ${String(err)}`));
    }
  });
