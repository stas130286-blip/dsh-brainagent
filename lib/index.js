...
function createAutonomousDeliverer(deps5) {
  const { state: state7, brainConfig, minGapMs, logger: logger17 } = deps5;
  return function enqueueAutonomousIntent(text) {
    const trimmed = text.trim();
    if (!trimmed) return;
    if (state7.previousCycleWasAutonomous) {
      logger17.info("BrainAgent Autonomy: intent suppressed \u2014 previous cycle was autonomous");
      return;
    }
    if (Date.now() - state7.lastAutonomousDeliveryAt < minGapMs) {
    ...
    state7.lastAutonomousDeliveryAt = Date.now();
    deps5.deliver(agent, framed);
  };
}
function createAutonomousIntentResolver(deps5) {
  const { state: state7, brainConfig, drives } = deps5;
  return () => {
    if (brainConfig.modules.goalStack && deps5.goalStack) {
      ...
      if (stats2.pending > 0) {
        ...
        if (triggered.length > 0) {
          ...
          if (goalCtx) {
            state7.lastAutonomousSource = `goal:${triggered[0].id}`;
            return {
              text: [
                AUTONOMOUS_TAG,
    ...
    if (brainConfig.modules.dmn && deps5.dmn) {
      ...
      if (novel.length > 0) {
        ...
        state7.lastAutonomousSource = "dmn:insight";
        return {
          text: [
            AUTONOMOUS_TAG,
            best.insightText,
    ...
    if (brainConfig.modules.goalStack && deps5.goalStack) {
      ...
      if (strong.length > 0) {
        ...
        state7.lastAutonomousSource = `desire:${top.id}`;
        return {
          text: [
            AUTONOMOUS_TAG,
            top.description,
    ...
    if (active.length > 0) {
      ...
      state7.lastAutonomousSource = `drive:${strongest.name}`;
      return {
        text: [
          AUTONOMOUS_TAG,
...
function createCycleEngine(deps5) {
  const { config: config8, brainConfig, getHostConfig, logger: logger17, markActivation, state: state7 } = deps5;
  ...
  async function endCycle3(key) {
    ...
    if (brainConfig.modules.hippocampus && input.length > 5) {
      if (isAutonomousCycle && cycle.responseText.trim()) {
        ...
        state7.lastAutonomousEpisodeId = episode.id;
        state7.lastAutonomousDomain = cycle.classification?.domain ?? "unknown";
        state7.previousCycleWasAutonomous = true;
...
function initVitalImpulse(workspaceDir, cfg, log, injectedDeps) {
  storageDir22 = join25(workspaceDir, ".brainagent", "vital-impulse");
  ...
  cancelPersist(join25(storageDir22, "state.json"));
  loadState18();
  wireSignalListeners();
  logger10.info(
...
function stopVitalImpulse() {
  ...
  unsubscribers2.length = 0;
  persistState14();
  flushPersist(join25(storageDir22, "state.json"));
...
function loadState18() {
  try {
    const filePath = join25(storageDir22, "state.json");
    if (existsSync24(filePath)) {
      const raw = JSON.parse(readFileSync24(filePath, "utf-8"));
...
function persistState14() {
  schedulePersist(join25(storageDir22, "state.json"), () => {
    const state7 = {
      lastFireTime,
      totalFires,
      totalSignalsReceived,
...
function initDriveArbiter(workspaceDir, cfg, getters, log) {
  ...
  storageDir23 = join26(workspaceDir, ".brainagent");
  if (!existsSync25(storageDir23)) {
    mkdirSync25(storageDir23, { recursive: true });
  }
  cancelPersist(join26(storageDir23, "drive-arbiter.json"));
  loadState19();
  const driveEvents = [
...
function stopDriveArbiter() {
  ...
  unsubscribers3.length = 0;
  persistState15();
  flushPersist(join26(storageDir23, "drive-arbiter.json"));
...
function loadState19() {
  try {
    const path = join26(storageDir23, "drive-arbiter.json");
...
function persistState15() {
  ...
  schedulePersist(join26(storageDir23, "drive-arbiter.json"), () => {
    const data = {
      driveWeights,
      lastSelectedDrive,
      lastSelectionTime,
...
import { existsSync as existsSync26, mkdirSync as mkdirSync26, readFileSync as readFileSync26, writeFileSync as writeFileSync22 } from "node:fs";
...
function initTemporalAwareness(workspaceDir, cfg, log) {
  ...
  storageDir24 = join27(workspaceDir, ".brainagent");
...
function loadState20() {
  try {
    const path = join27(storageDir24, "temporal-awareness.json");
...
function persistState16() {
  ...
  try {
    ...
    writeFileSync22(
      join27(storageDir24, "temporal-awareness.json"),
...
import { existsSync as existsSync27, mkdirSync as mkdirSync27, readFileSync as readFileSync27, writeFileSync as writeFileSync23 } from "node:fs";
...
function initAttentionGate(workspaceDir, _config) {
  storageDir25 = join28(workspaceDir, ".brainagent", "attention");
...
function loadState21() {
  ...
  try {
    const path = join28(storageDir25, "state.json");
...
function persistState17() {
  ...
  try {
    writeFileSync23(
      join28(storageDir25, "state.json"),
...
function createPreStepHandler(deps5) {
  const { config: config8, brainConfig, getHostConfig, logger: logger17, state: state7, cycles, startCycle } = deps5;
  return async (payload, next) => {
    ...
    const cyc = cycle;
    const isAutonomousCycle = input.startsWith(AUTONOMOUS_TAG);
    if (brainConfig.modules.autonomousResearch && isAutonomousCycle && isResearchIntent(state7.lastAutonomousSource, input)) {
      const topic = input.replace(/<\/?autonomous-intent>/g, "").split("\n").filter((l) => l.trim().length > 5)[0]?.trim() ?? "general exploration";
      logger17.info(
        `BrainAgent AutonomousResearch: detected research intent (source=${state7.lastAutonomousSource}), running isolated pipeline for "${topic}"`
      );
...
import { existsSync as existsSync28, mkdirSync as mkdirSync28, readFileSync as readFileSync28 } from "node:fs";
...
function initProactiveFeedback(workspaceDir, cfg, log) {
  storageDir26 = join29(workspaceDir, ".brainagent", "proactive-feedback");
  if (!existsSync28(storageDir26)) {
    mkdirSync28(storageDir26, { recursive: true });
  }
  stateFile = join29(storageDir26, "state.json");
...
function loadState22() {
  try {
    if (!existsSync28(stateFile)) return;
    const raw = JSON.parse(readFileSync28(stateFile, "utf-8"));
...
function apply(ctx, config8) {
  ...
  const state7 = createAutonomyState();
  function pickAgent() {
    const agents = ctx.agents.list();
    if (agents.length === 0) return void 0;
    return agents.find((a) => String(a.id) === state7.lastActiveAgentId) ?? agents[agents.length - 1];
  }
  ...
  ctx.on("session/event", (_session, event) => {
    ...
    if (event.type === "user/message") {
      ...
      if (isUserMessage && state7.previousCycleWasAutonomous && state7.lastAutonomousEpisodeId) {
        let reactionSignal = "neutral";
        if (brainConfig.modules.proactiveFeedback) {
          reactionSignal = recordProactiveReaction(state7.lastAutonomousDomain, text);
        }
        if (brainConfig.modules.hippocampus) {
          ...
          storeEpisode(
            ...
            reactionEmotion,
            ["proactive_feedback", state7.lastAutonomousEpisodeId, reactionSignal],
            reactionSalience
          );
        }
      }
      state7.previousCycleWasAutonomous = false;
      state7.lastAutonomousEpisodeId = void 0;
  ...
  ctx.on("tools/pre-execute", async (exec, next) => {
    ...
    const agentId = exec.agent ? String(exec.agent.id) : state7.lastActiveAgentId;
