...
function initWorkingMemoryStorage(workspaceDir, config8) {
  storageDir7 = join9(workspaceDir, ".brainagent", "working-memory");
...
function loadState3() {
  ...
  try {
    const path = join9(storageDir7, "state.json");
    if (existsSync8(path)) {
...
function persistState3() {
  if (!storageDir7) return;
  try {
    writeFileSync8(join9(storageDir7, "state.json"), JSON.stringify(entries, null, 2), "utf-8");
  } catch {
  }
}
...
function initStructuralPlasticity(workspaceDir, config8, log) {
  ...
  logger = log;
  storageDir8 = join10(workspaceDir, ".brainagent", "structural");
...
function loadState4() {
  try {
    const path = join10(storageDir8, "state.json");
    if (existsSync9(path)) {
...
function saveState() {
  try {
    writeFileSync9(join10(storageDir8, "state.json"), JSON.stringify(state2, null, 2), "utf-8");
  } catch {
  }
}
...
function initLearningCoordinator(workspaceDir, config8) {
  storageDir9 = join11(workspaceDir, ".brainagent", "learning");
...
function initEmergentModules(workspaceDir, config8, log) {
  ...
  logger2 = log;
  storageDir10 = join12(workspaceDir, ".brainagent", "emergent");
...
function loadState6() {
  try {
    const path = join12(storageDir10, "state.json");
...
function saveState2() {
  try {
    writeFileSync11(join12(storageDir10, "state.json"), JSON.stringify(state3, null, 2), "utf-8");
  } catch {
  }
}
...
function initMetabolicBudget(workspaceDir, config8, log) {
  ...
  logger3 = log;
  storageDir11 = join13(workspaceDir, ".brainagent", "metabolic");
...
function loadState7() {
  try {
    const path = join13(storageDir11, "state.json");
...
function saveState3() {
  try {
    writeFileSync12(join13(storageDir11, "state.json"), JSON.stringify(state4, null, 2), "utf-8");
  } catch {
  }
}
...
function initAgentIdentity(workspaceDir, config8) {
  storageDir12 = join14(workspaceDir, ".brainagent", "identity");
...
function loadState8() {
  ...
  try {
    const path = join14(storageDir12, "state.json");
...
function persistState5() {
  ...
  try {
    writeFileSync13(
      join14(storageDir12, "state.json"),
      JSON.stringify(
        {
          capabilities,
...
function initEmotionalMemory(workspaceDir, _config) {
  storageDir13 = join15(workspaceDir, ".brainagent", "emotional-memory");
...
function initQualiaSimulator(workspaceDir, config8) {
  storageDir14 = join16(workspaceDir, ".brainagent", "qualia-simulator");
...
function loadState10() {
  ...
  try {
    const path = join16(storageDir14, "state.json");
...
function persistState7() {
  ...
  try {
    writeFileSync15(
      join16(storageDir14, "state.json"),
...
function initDMN(workspaceDir, config8, log) {
  storageDir15 = join17(workspaceDir, ".brainagent", "dmn");
...
function loadState11() {
  ...
  try {
    const path = join17(storageDir15, "state.json");
...
function persistState8() {
  ...
  try {
    writeFileSync16(
      join17(storageDir15, "state.json"),
      JSON.stringify(
        {
...
function initTemporalBinding(workspaceDir, config8) {
  storageDir16 = join18(workspaceDir, ".brainagent", "temporal-binding");
...
function loadState12() {
  ...
  try {
    const path = join18(storageDir16, "state.json");
...
function persistState9() {
  if (!storageDir16) return;
  try {
    writeFileSync17(join18(storageDir16, "state.json"), JSON.stringify(moments, null, 2), "utf-8");
  } catch {
  }
}
...
function initSessionBridge(workspaceDir, config8, _logger) {
  storageDir17 = join19(workspaceDir, ".brainagent", "sessions");
...
function loadState13() {
  ...
  try {
    const currentPath = join19(storageDir17, "current.json");
  ...
  try {
    const lastPath = join19(storageDir17, "last.json");
...
function persistCurrent() {
  ...
  try {
    writeFileSync18(
      join19(storageDir17, "current.json"),
...
function persistLast() {
  if (!storageDir17) return;
  try {
    writeFileSync18(join19(storageDir17, "last.json"), JSON.stringify(lastSession, null, 2), "utf-8");
  } catch {
  }
}
...
function initCuriosityDrive(workspaceDir, config8) {
  storageDir18 = join20(workspaceDir, ".brainagent", "curiosity");
...
function loadState14() {
  ...
  try {
    const path = join20(storageDir18, "state.json");
...
function persistState10() {
  ...
  try {
    writeFileSync19(
      join20(storageDir18, "state.json"),
      JSON.stringify(
...
function initGoalStack(workspaceDir, config8) {
  storageDir19 = join21(workspaceDir, ".brainagent", "goals");
...
function loadState15() {
  ...
  try {
    const path = join21(storageDir19, "state.json");
...
function persistState11() {
  ...
  try {
    writeFileSync20(
      join21(storageDir19, "state.json"),
...
function checkGoalTriggers(input, currentEmotion, currentDomain) {
  ...
  if (triggered.length > 0) persistState11();
  return triggered;
}
function expireGoals() {
  ...
  if (goals.length > 60) {
    const active5 = goals.filter((g) => g.status === "pending" || g.status === "triggered");
    const inactive = goals.filter((g) => g.status === "completed" || g.status === "expired").slice(-30);
    goals = [...active5, ...inactive];
    changed = true;
  }
  if (changed) persistState11();
}
...
function weakenDesiresAfterFire(consecutiveFires) {
  ...
  if (desires.length !== before || changed) persistState11();
}
...
function initCircadianRhythm(workspaceDir, config8, log) {
  storageDir20 = join22(workspaceDir, ".brainagent", "circadian");
...
function persistState12() {
  if (!storageDir20) return;
  try {
    writeFileSync21(join22(storageDir20, "state.json"), JSON.stringify(state6, null, 2), "utf-8");
  } catch {
  }
}
...
var DriveEngine = class {
  constructor(spec, config8, workspaceDir, circadianEnabled3, deps5, logger13) {
    ...
    const storageDir27 = join23(workspaceDir, ".brainagent", spec.id);
    if (!existsSync22(storageDir27)) {
      mkdirSync22(storageDir27, { recursive: true });
    }
    this.stateFile = join23(storageDir27, "state.json");
...
function initCognitiveHunger(workspaceDir, cfg, log, injectedDeps) {
  active2?.dispose();
  active2 = createCognitiveHunger(workspaceDir, cfg, log, injectedDeps);
}
...
function initCreativeDrive(workspaceDir, cfg, log, injectedDeps) {
  active3?.dispose();
  active3 = createCreativeDrive(workspaceDir, cfg, log, injectedDeps);
}
...
function initMasteryDrive(workspaceDir, cfg, log, injectedDeps) {
  storageDir21 = join24(workspaceDir, ".brainagent", "mastery-drive");
  ...
  totalNeedSignals = 0;
  adaptiveDecayModifier = 1;
  cancelPersist(join24(storageDir21, "state.json"));
  loadState17();
  wireEventListeners();
  logger6.info(
...
function stopMasteryDrive() {
  ...
  unsubscribers.length = 0;
  persistState13();
  flushPersist(join24(storageDir21, "state.json"));
...
function loadState17() {
  try {
    const filePath = join24(storageDir21, "state.json");
...
function persistState13() {
  schedulePersist(join24(storageDir21, "state.json"), () => {
    const domainMap = {};
    for (const [domain, mastery] of domainSatiations) {
...
function collectActiveDrives(drives, threshold) {
  ...
  if (drives.social) {
    ...
    if (s.need >= threshold) active5.push({ name: "social", need: s.need, needLevel: s.needLevel });
  }
  if (drives.cognitive) {
    const c = drives.cognitive();
    if (c.need >= threshold) active5.push({ name: "cognitive", need: c.need, needLevel: c.needLevel });
  }
  if (drives.creative) {
    const c = drives.creative();
    if (c.need >= threshold) active5.push({ name: "creative", need: c.need, needLevel: c.needLevel });
  }
  if (drives.mastery) {
    const m = drives.mastery();
    if (m.need >= threshold) active5.push({ name: "mastery", need: m.need, needLevel: m.needLevel });
  }
  return active5;
}
function synthesizeAutonomousCycleState(brainConfig, drives, cycle) {
  const active5 = collectActiveDrives(drives, 0.5);
  if (active5.length === 0) return;
  const ranked = active5.map((d) => {
...
function createAutonomousIntentResolver(deps5) {
  ...
  return () => {
    ...
    const active5 = collectActiveDrives(drives, 0.8);
    if (active5.length > 0) {
      const strongest = active5.sort((a, b) => b.need - a.need)[0];
...
function initStrategyBandit(workspaceDir, config8) {
  stopStrategyBandit();
  const storageDir27 = join25(workspaceDir, ".brainagent", "bandit");
  if (!existsSync24(storageDir27)) {
    mkdirSync24(storageDir27, { recursive: true });
  }
  storageFile = join25(storageDir27, "state.json");
...
function initVitalImpulse(workspaceDir, cfg, log, injectedDeps) {
  storageDir22 = join26(workspaceDir, ".brainagent", "vital-impulse");
  if (!existsSync25(storageDir22)) {
    mkdirSync25(storageDir22, { recursive: true });
  }
  ...
  cancelPersist(join26(storageDir22, "state.json"));
  loadState19();
  wireSignalListeners();
  logger7.info(
...
function stopVitalImpulse() {
  ...
  unsubscribers3.length = 0;
  persistState15();
  flushPersist(join26(storageDir22, "state.json"));
...
function loadState19() {
  try {
    const filePath = join26(storageDir22, "state.json");
...
function persistState15() {
  schedulePersist(join26(storageDir22, "state.json"), () => {
    const state8 = {
      lastFireTime,
      totalFires,
      totalSignalsReceived,
...
function initDriveArbiter(workspaceDir, cfg, getters, log) {
  ...
  storageDir23 = join27(workspaceDir, ".brainagent");
  if (!existsSync26(storageDir23)) {
    mkdirSync26(storageDir23, { recursive: true });
  }
  cancelPersist(join27(storageDir23, "drive-arbiter.json"));
  loadState20();
  const driveEvents = [
...
function stopDriveArbiter() {
  ...
  unsubscribers4.length = 0;
  persistState16();
  flushPersist(join27(storageDir23, "drive-arbiter.json"));
...
function loadState20() {
  try {
    const path = join27(storageDir23, "drive-arbiter.json");
...
function persistState16() {
  ...
  schedulePersist(join27(storageDir23, "drive-arbiter.json"), () => {
    const data = {
      driveWeights,
      lastSelectedDrive,
      lastSelectionTime,
...
function initTemporalAwareness(workspaceDir, cfg, log) {
  ...
  storageDir24 = join28(workspaceDir, ".brainagent");
  if (!existsSync27(storageDir24)) {
    mkdirSync27(storageDir24, { recursive: true });
  }
  loadState21();
  logger10?.info(
...
function loadState21() {
  try {
    const path = join28(storageDir24, "temporal-awareness.json");
...
function persistState17() {
  ...
  try {
    ...
    writeFileSync22(
      join28(storageDir24, "temporal-awareness.json"),
...
function initAttentionGate(workspaceDir, _config) {
  storageDir25 = join29(workspaceDir, ".brainagent", "attention");
  if (!existsSync28(storageDir25)) {
    mkdirSync28(storageDir25, { recursive: true });
  }
  totalProcessed = 0;
  totalDropped = 0;
  totalRelevanceSum = 0;
...
function loadState22() {
  ...
  try {
    const path = join29(storageDir25, "state.json");
...
function persistState18() {
  ...
  try {
    writeFileSync23(
      join29(storageDir25, "state.json"),
...
function initGoalExecutor(_cfg, log) {
  active4 = createGoalExecutor(log);
...
function initProactiveFeedback(workspaceDir, cfg, log) {
  storageDir26 = join30(workspaceDir, ".brainagent", "proactive-feedback");
  if (!existsSync29(storageDir26)) {
    mkdirSync29(storageDir26, { recursive: true });
  }
  stateFile = join30(storageDir26, "state.json");
...
function initRewardLedger(workspaceDir, config8) {
  stopRewardLedger();
  const storageDir27 = join31(workspaceDir, ".brainagent", "reward");
  if (!existsSync30(storageDir27)) {
    mkdirSync30(storageDir27, { recursive: true });
  }
  storageFile2 = join31(storageDir27, "ledger.json");
  cancelPersist(storageFile2);
  maxEntries2 = config8.learningLoop.rewardLedger.maxEntries;
  entries2 = [];
  idCounter6 = 0;
  loadLedger();
  unsubscribers7.push(
...
function recordReward(source, contribution, context) {
  ...
  entries2.push({
    id: `rw_${Date.now()}_${++idCounter6}`,
    timestamp: Date.now(),
    reward: Math.round(reward * 1e3) / 1e3,
    source,
    ...context !== void 0 ? { context } : {}
  });
  if (entries2.length > maxEntries2) {
    entries2 = entries2.slice(-maxEntries2);
  }
  persistLedger();
...
function loadLedger() {
  ...
  try {
    ...
    entries2 = Array.isArray(data.entries) ? data.entries.slice(-maxEntries2) : [];
    idCounter6 = entries2.length;
  } catch {
  }
}
function persistLedger() {
  if (!storageFile2) return;
  schedulePersist(storageFile2, () => JSON.stringify({ entries: entries2 }, null, 2));
}
