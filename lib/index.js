...
function initMemoryStorage(workspaceDir) {
  memoryDir = join2(workspaceDir, ".brainagent", "memory");
  ensureDir(memoryDir);
...
function createMirrorNeurons(workspaceDir) {
  const storageDir17 = workspaceDir ? join3(workspaceDir, ".brainagent", "users") : "";
  const userModels = /* @__PURE__ */ new Map();
  if (storageDir17 && !existsSync2(storageDir17)) {
    mkdirSync2(storageDir17, { recursive: true });
  }
  try {
    const indexPath = join3(storageDir17, "index.json");
    if (storageDir17 && existsSync2(indexPath)) {
  ...
  function persistModels() {
    ...
    try {
      ...
      writeFileSync3(join3(storageDir17, "index.json"), JSON.stringify(data, null, 2), "utf-8");
    } catch {
    }
  }
...
function updateKnowledgeModelToM(model, text, domainLimit) {
  ...
  for (const domain of domainSignals) {
    const current7 = model.knowledgeModel[domain];
    if (!current7) {
      model.knowledgeModel[domain] = "beginner";
    } else if (current7 === "beginner") {
...
function observe(userId, text, amygdalaResult, config7) {
  return current().observe(userId, text, amygdalaResult, config7);
}
...
function observeWithAI(userId, text, amygdalaResult, config7, neuroConfig, logger9) {
  return current().observeWithAI(userId, text, amygdalaResult, config7, neuroConfig, logger9);
}
...
function createPredictiveEngine(workspaceDir) {
  const storageDir17 = workspaceDir ? join4(workspaceDir, ".brainagent", "predictions") : "";
  ...
  let lastDomain;
  if (storageDir17 && !existsSync3(storageDir17)) {
    mkdirSync3(storageDir17, { recursive: true });
  }
  function loadPatterns() {
    if (!storageDir17) return;
    try {
      const tPath = join4(storageDir17, "temporal.json");
    ...
    try {
      const sPath = join4(storageDir17, "sequential.json");
    ...
    try {
      const cPath = join4(storageDir17, "contextual.json");
...
var active2;
function current2() {
  return active2 ?? (active2 = createPredictiveEngine(""));
}
function initPredictiveStorage(workspaceDir) {
  active2 = createPredictiveEngine(workspaceDir);
}
function observeInteraction(domain, keywords, context) {
  current2().observeInteraction(domain, keywords, context);
}
function predict(currentContext) {
  return current2().predict(currentContext);
}
function getPredictiveStats() {
  return current2().getStats();
}
...
function createBasalGanglia(workspaceDir) {
  const storageDir17 = workspaceDir ? join5(workspaceDir, ".brainagent", "habits") : "";
  let habits = [];
  let habitIndex = new VectorIndex();
  let idCounter4 = 0;
  function nextHabitId() {
    return `hab-${Date.now()}-${++idCounter4}`;
  }
  if (storageDir17 && !existsSync4(storageDir17)) {
    mkdirSync4(storageDir17, { recursive: true });
  }
  function loadHabits() {
    if (!storageDir17) return;
    try {
      const path = join5(storageDir17, "habits.json");
      if (existsSync4(path)) {
  ...
  function persistHabits() {
    if (!storageDir17) return;
    try {
      writeFileSync5(join5(storageDir17, "habits.json"), JSON.stringify(habits, null, 2), "utf-8");
    } catch {
    }
  }
...
var active3;
function current3() {
  return active3 ?? (active3 = createBasalGanglia(""));
}
function initBasalStorage(workspaceDir) {
  active3 = createBasalGanglia(workspaceDir);
}
function findHabit(input, domain) {
  return current3().findHabit(input, domain);
}
function recordPattern(cue, routine, domain, exampleResponse) {
  return current3().recordPattern(cue, routine, domain, exampleResponse);
}
function reinforce(habitId, signal) {
  current3().reinforce(habitId, signal);
}
...
function createDopamineSystem(workspaceDir) {
  const storageDir17 = join7(workspaceDir, ".brainagent", "neuromodulators");
  if (!existsSync6(storageDir17)) {
    mkdirSync6(storageDir17, { recursive: true });
  }
  cancelPersist(join7(storageDir17, "state.json"));
  let state5 = {
    dopamine: 0.5,
    serotonin: 0.6,
    norepinephrine: 0.3,
  ...
  function loadState17() {
    try {
      const path = join7(storageDir17, "state.json");
      if (existsSync6(path)) {
  ...
  function updateDopamine(predictionError, config7) {
    ...
    state5.dopamine = state5.dopamine * (1 - decay) + (baseline + spike) * decay;
    state5.dopamine = Math.max(0, Math.min(1, state5.dopamine));
  }
  function updateSerotonin(reward) {
    const alpha = 0.15;
    positiveOutcomeRatio = positiveOutcomeRatio * (1 - alpha) + (reward > 0 ? 1 : 0) * alpha;
    state5.serotonin = 0.3 + positiveOutcomeRatio * 0.5;
    state5.serotonin = Math.max(0.1, Math.min(0.95, state5.serotonin));
  }
  function updateNorepinephrine(complexity, emotion) {
    ...
    const alpha = 0.3;
    state5.norepinephrine = state5.norepinephrine * (1 - alpha) + target * alpha;
    state5.norepinephrine = Math.max(0.05, Math.min(0.95, state5.norepinephrine));
  }
  function updateAcetylcholine(_domain, _config) {
    ...
    const alpha = 0.2;
    state5.acetylcholine = state5.acetylcholine * (1 - alpha) + target * alpha;
    state5.acetylcholine = Math.max(0.1, Math.min(0.9, state5.acetylcholine));
  }
  function processOutcome(params, config7) {
    ...
    bus.emitSync("neuromodulator:state-changed", { ...state5 });
  ...
  return {
    ...
    getEffectiveLearningRate: (baseLearningRate, config7) => {
      const achBoost = 1 + (state5.acetylcholine - 0.5) * (config7.neuromodulators.acetylcholineLearningBoost - 1);
      const dopamineBoost = 0.7 + state5.dopamine * 0.6;
      return baseLearningRate * achBoost * dopamineBoost;
    },
    getRiskTolerance: () => state5.serotonin,
    getAttentionLevel: () => state5.norepinephrine,
    getStats: getStats2,
    stop: () => {
    },
    dispose: () => {
...
function initDopamineSystem(workspaceDir) {
  active4?.dispose();
  active4 = createDopamineSystem(workspaceDir);
}
function processInteractionOutcome(params, config7) {
  ...
  return active4.processOutcome(params, config7);
}
function markNovelty() {
  active4?.markNovelty();
}
function getNeuromodulatorState() {
  return active4?.getState() ?? {
    dopamine: 0.5,
    serotonin: 0.6,
    norepinephrine: 0.3,
    acetylcholine: 0.4
...
function getAttentionLevel() {
  return active4?.getAttentionLevel() ?? 0.3;
}
function getDopamineStats() {
  return active4?.getStats() ?? {
...
function initNeuralPathways(workspaceDir, config7, logger9) {
  ...
  pathwayLogger = logger9;
  currentConfig = config7;
  storageDir2 = join8(workspaceDir, ".brainagent", "synapses");
  if (!existsSync7(storageDir2)) {
    mkdirSync7(storageDir2, { recursive: true });
  }
  loadSynapticState();
...
function applyHebbianLearning(reward) {
  if (!currentConfig || activatedPathways.size === 0) return;
  const { learningRate, decayRate, minWeight, maxWeight } = currentConfig.synapticPlasticity;
  const now = Date.now();
  for (const pathwayName of activatedPathways) {
    ...
    if (weight.recentActivations.length > currentConfig.synapticPlasticity.activationHistorySize) {
      weight.recentActivations = weight.recentActivations.slice(
        -currentConfig.synapticPlasticity.activationHistorySize
      );
    }
...
function loadSynapticState() {
  ...
  try {
    const path = join8(storageDir2, "weights.json");
    if (existsSync7(path)) {
...
function saveSynapticState() {
  ...
  try {
    writeFileSync7(
      join8(storageDir2, "weights.json"),
...
function createWorkingMemory(workspaceDir, opts) {
  const storageDir17 = workspaceDir ? join9(workspaceDir, ".brainagent", "working-memory") : "";
  const maxEntries = opts.maxEntries;
  const summaryMaxLength = opts.summaryMaxLength;
  let entries = [];
  if (storageDir17 && !existsSync8(storageDir17)) {
    mkdirSync8(storageDir17, { recursive: true });
  }
  function loadState17() {
    if (!storageDir17) return;
    try {
      const path = join9(storageDir17, "state.json");
      if (existsSync8(path)) {
  ...
  function persistState14() {
    if (!storageDir17) return;
    try {
      writeFileSync8(join9(storageDir17, "state.json"), JSON.stringify(entries, null, 2), "utf-8");
    } catch {
    }
  }
  loadState17();
...
function initStructuralPlasticity(workspaceDir, config7, log) {
  ...
  logger = log;
  storageDir3 = join10(workspaceDir, ".brainagent", "structural");
  if (!existsSync9(storageDir3)) {
    mkdirSync9(storageDir3, { recursive: true });
  }
  loadState2();
  logger?.info(
...
function endCycle(reward) {
  ...
  state.totalCycles++;
  checkForNewPathways();
  if (state.totalCycles % currentConfig2.structuralPlasticity.pruningThreshold === 0) {
    pruneUnusedPathways();
  }
...
function checkForNewPathways() {
  if (!currentConfig2) return;
  const { minCorrelationForPathway, minCyclesForPathway, maxDynamicPathways } = currentConfig2.structuralPlasticity;
...
function loadState2() {
  try {
    const path = join10(storageDir3, "state.json");
    if (existsSync9(path)) {
...
function saveState() {
  try {
    writeFileSync9(join10(storageDir3, "state.json"), JSON.stringify(state, null, 2), "utf-8");
  } catch {
  }
}
...
function initLearningCoordinator(workspaceDir, config7) {
  storageDir4 = join11(workspaceDir, ".brainagent", "learning");
  if (!existsSync10(storageDir4)) {
    mkdirSync10(storageDir4, { recursive: true });
  }
...
function loadState3() {
  ...
  try {
    const path = join11(storageDir4, "coordinator.json");
...
function persistState2() {
  ...
  try {
    ...
    writeFileSync10(
      join11(storageDir4, "coordinator.json"),
      JSON.stringify(
        {
...
function processRewardSignal(signal) {
  ...
  const config7 = currentConfig3;
  if (config7 && cycleCount >= config7.learning.minCyclesForInsights && cycleCount % 5 === 0) {
...
function generateInsights(config7) {
  const window = config7.learning.trendWindowSize;
  const newInsights = [];
...
function initEmergentModules(workspaceDir, config7, log) {
  currentConfig4 = config7;
  logger2 = log;
  storageDir5 = join12(workspaceDir, ".brainagent", "emergent");
  if (!existsSync11(storageDir5)) {
    mkdirSync11(storageDir5, { recursive: true });
  }
...
function recordPattern2(participants, domain, reward) {
  if (!currentConfig4 || participants.length < 2) return;
  const now = Date.now();
...
function checkForNewModule(participants, domain) {
  ...
  if (activeModules >= currentConfig4.emergentModules.maxEmergentModules) {
...
function loadState4() {
  try {
    const path = join12(storageDir5, "state.json");
...
function saveState2() {
  try {
    writeFileSync11(join12(storageDir5, "state.json"), JSON.stringify(state2, null, 2), "utf-8");
  } catch {
  }
}
...
function createMetabolicBudget(workspaceDir, config7, log) {
  const storageDir17 = join13(workspaceDir, ".brainagent", "metabolic");
  if (!existsSync12(storageDir17)) {
    mkdirSync12(storageDir17, { recursive: true });
  }
  const state5 = createDefaultState3();
  state5.totalBudget = config7.metabolicBudget.totalBudget;
  state5.regenRate = config7.metabolicBudget.regenRate;
  const periodStats = {};
  ...
  function loadState17() {
    try {
      const path = join13(storageDir17, "state.json");
  ...
  function saveState3() {
    try {
      writeFileSync12(join13(storageDir17, "state.json"), JSON.stringify(state5, null, 2), "utf-8");
    } catch {
    }
  }
  loadState17();
  ...
  function endCycle3() {
    ...
    if (state5.cyclesSinceRebalance >= config7.metabolicBudget.rebalanceInterval) {
      rebalanceEnergy();
  ...
  return {
    ...
    endCycle: endCycle3,
    getModuleEnergy: (module) => state5.moduleEnergies[module]?.energy ?? 1,
    isModuleLowPower: (module) => state5.moduleEnergies[module]?.lowPowerMode ?? false,
    getStats: getStats2,
...
function initMetabolicBudget(workspaceDir, config7, log) {
  active6?.dispose();
  active6 = createMetabolicBudget(workspaceDir, config7, log);
}
function consumeEnergy(module, amount) {
  return active6?.consumeEnergy(module, amount) ?? true;
}
function recordPerformance(module, reward) {
  active6?.recordPerformance(module, reward);
}
function endCycle2() {
  active6?.endCycle();
}
...
function initAgentIdentity(workspaceDir, config7) {
  storageDir6 = join14(workspaceDir, ".brainagent", "identity");
  if (!existsSync13(storageDir6)) {
    mkdirSync13(storageDir6, { recursive: true });
  }
...
function loadState5() {
  ...
  try {
    const path = join14(storageDir6, "state.json");
...
function initEmotionalMemory(workspaceDir, _config) {
  storageDir7 = join15(workspaceDir, ".brainagent", "emotional-memory");
  if (!existsSync14(storageDir7)) {
    mkdirSync14(storageDir7, { recursive: true });
  }
...
function loadState6() {
  ...
  try {
    const path = join15(storageDir7, "state.json");
...
async function generateQualiaAsync(emotion, intensity, domain, neuromodulators, config7, logger9) {
  if (intensity <= LLM_INTENSITY_THRESHOLD || !config7) {
...
${neuroDesc}`;
  try {
    ...
    const response = await callLLM(QUALIA_METAPHOR_PROMPT, userMessage, config7, logger9, 200);
    if (!response) {
...
function initQualiaSimulator(workspaceDir, config7) {
  storageDir8 = join16(workspaceDir, ".brainagent", "qualia-simulator");
  if (!existsSync15(storageDir8)) {
    mkdirSync15(storageDir8, { recursive: true });
  }
...
function loadState7() {
  ...
  try {
    const path = join16(storageDir8, "state.json");
...
function describeNeuromodulatorFeel(state5) {
  const { dopamine, serotonin, norepinephrine, acetylcholine } = state5;
  const parts = [];
...
function initDMN(workspaceDir, config7, log) {
  storageDir9 = join17(workspaceDir, ".brainagent", "dmn");
  if (!existsSync16(storageDir9)) {
    mkdirSync16(storageDir9, { recursive: true });
  }
  currentConfig5 = config7;
  logger3 = log;
  insights = [];
  lastRunTimestamp = 0;
...
function createTemporalBinding(workspaceDir, opts) {
  const storageDir17 = workspaceDir ? join18(workspaceDir, ".brainagent", "temporal-binding") : "";
  const maxMoments = opts.maxMoments;
  let moments = [];
  let idCounter4 = 0;
  if (storageDir17 && !existsSync17(storageDir17)) {
    mkdirSync17(storageDir17, { recursive: true });
  }
  function loadState17() {
    if (!storageDir17) return;
    try {
      const path = join18(storageDir17, "state.json");
  ...
  function persistState14() {
    if (!storageDir17) return;
    try {
      writeFileSync17(join18(storageDir17, "state.json"), JSON.stringify(moments, null, 2), "utf-8");
    } catch {
    }
  }
  loadState17();
...
var active7;
function current5() {
  return active7 ?? (active7 = createTemporalBinding("", { maxMoments: DEFAULT_MAX_MOMENTS }));
}
function initTemporalBinding(workspaceDir, config7) {
  active7 = createTemporalBinding(workspaceDir, { maxMoments: config7.temporalBinding.maxMoments });
}
function createMoment(input, thoughts, emotion, emotionIntensity, activeMemoryIds, intentions, confidence, domain) {
  return current5().createMoment(
    input,
    thoughts,
    emotion,
    emotionIntensity,
...
function buildTemporalContext(n = 3) {
  return current5().buildContext(n);
}
...
function initSessionBridge(workspaceDir, config7, _logger) {
  storageDir10 = join19(workspaceDir, ".brainagent", "sessions");
  if (!existsSync18(storageDir10)) {
    mkdirSync18(storageDir10, { recursive: true });
  }
  gapThresholdMs = config7.sessionBridge.gapThresholdMs;
  maxSummaryTopics = config7.sessionBridge.maxSummaryTopics;
...
function initCuriosityDrive(workspaceDir, config7) {
  storageDir11 = join20(workspaceDir, ".brainagent", "curiosity");
  if (!existsSync19(storageDir11)) {
    mkdirSync19(storageDir11, { recursive: true });
  }
...
function loadState10() {
  ...
  try {
    const path = join20(storageDir11, "state.json");
...
function persistState7() {
  ...
  try {
    writeFileSync19(
      join20(storageDir11, "state.json"),
      JSON.stringify(
...
function initGoalStack(workspaceDir, config7) {
  storageDir12 = join21(workspaceDir, ".brainagent", "goals");
  if (!existsSync20(storageDir12)) {
    mkdirSync20(storageDir12, { recursive: true });
  }
...
function loadState11() {
  ...
  try {
    const path = join21(storageDir12, "state.json");
...
function persistState8() {
  ...
  try {
    writeFileSync20(
      join21(storageDir12, "state.json"),
...
function checkGoalTriggers(input, currentEmotion, currentDomain) {
  ...
  if (triggered.length > 0) persistState8();
  return triggered;
}
function expireGoals() {
  ...
  if (goals.length > 60) {
    const active17 = goals.filter((g) => g.status === "pending" || g.status === "triggered");
    const inactive = goals.filter((g) => g.status === "completed" || g.status === "expired").slice(-30);
    goals = [...active17, ...inactive];
    changed = true;
  }
  if (changed) persistState8();
}
...
function weakenDesiresAfterFire(consecutiveFires) {
  ...
  if (desires.length !== before || changed) persistState8();
}
...
function createCircadianRhythm(workspaceDir, config7, log) {
  const storageDir17 = join22(workspaceDir, ".brainagent", "circadian");
  if (!existsSync21(storageDir17)) {
    mkdirSync21(storageDir17, { recursive: true });
  }
  const maxSleepConsolidations = config7.circadian.maxSleepConsolidations ?? 5;
  let state5 = createDefaultState4();
  let lastActivityTime = Date.now();
  let activityCounter = 0;
  let consolidationCallback = null;
  function loadState17() {
    try {
      const path = join22(storageDir17, "state.json");
      if (existsSync21(path)) {
        const data = JSON.parse(readFileSync21(path, "utf-8"));
        state5 = { ...createDefaultState4(), ...data };
        state5.phaseStartedAt = Date.now();
        state5.idleTime = 0;
  ...
  function persistState14() {
    try {
      writeFileSync21(join22(storageDir17, "state.json"), JSON.stringify(state5, null, 2), "utf-8");
    } catch {
    }
  }
  function evaluatePhase() {
    ...
    state5.idleTime = idleTime;
    const activityWindow = config7.circadian.activityWindowMs;
    state5.activityLevel = Math.max(0, 1 - idleTime / activityWindow);
    const phaseDuration = now - state5.phaseStartedAt;
    ...
    switch (state5.phase) {
      ...
      case "transition-to-sleep":
        state5.phaseProgress = Math.min(1, phaseDuration / transitionDurationMs);
        updateModulationFromPhase();
        if (activityCounter > 0) {
          activityCounter = 0;
          transitionTo("wake");
        } else if (state5.phaseProgress >= 1) {
          transitionTo("sleep");
        }
        break;
      case "sleep":
        if (consolidationCallback && phaseDuration > 3e4 && state5.sleepConsolidations < maxSleepConsolidations) {
          ...
          const shouldConsolidate = phaseDuration % consolidationInterval < triggerWindow && state5.sleepConsolidations === 0;
          if (shouldConsolidate || phaseDuration > state5.sleepConsolidations * consolidationInterval) {
  ...
  function transitionTo(newPhase) {
    const oldPhase = state5.phase;
    if (oldPhase === newPhase) return;
    log?.info(`Circadian: ${oldPhase} \u2192 ${newPhase}`);
    state5.phase = newPhase;
    state5.phaseStartedAt = Date.now();
    state5.phaseProgress = 0;
    if (newPhase === "wake") {
      state5.wakeInteractions = 0;
      bus.emitSync("circadian:wake-started", { idleTime: state5.idleTime });
    } else if (newPhase === "sleep") {
      state5.sleepConsolidations = 0;
      bus.emitSync("circadian:sleep-started", { wakeInteractions: state5.wakeInteractions });
    }
    updateModulationFromPhase();
  ...
  function updateModulationFromPhase() {
    ...
    switch (state5.phase) {
      ...
      case "transition-to-sleep": {
        ...
        const sleepProgress = state5.phaseProgress;
        state5.wakeModulation = {
      ...
      case "transition-to-wake": {
        ...
        const wakeProgress = state5.phaseProgress;
        state5.wakeModulation = {
  ...
  async function triggerSleepConsolidation() {
    ...
    state5.sleepConsolidations++;
    log?.info(
      `Circadian: sleep consolidation #${state5.sleepConsolidations} (intensity: ${(state5.sleepSettings.consolidationIntensity * 100).toFixed(0)}%)`
    );
    try {
  ...
  log?.info(
    `Circadian: initialized in ${state5.phase} phase (idle threshold: ${config7.circadian.idleThresholdMs / 1e3}s)`
  );
  function stopTimer() {
  ...
  return {
    recordActivity: () => {
      lastActivityTime = Date.now();
      activityCounter++;
      state5.wakeInteractions++;
      bus.emitSync("circadian:activity-detected", { activityLevel: state5.activityLevel });
      if (state5.phase === "sleep" || state5.phase === "transition-to-sleep") {
        const phaseDuration = Date.now() - state5.phaseStartedAt;
        const minSleep = config7.circadian.minSleepDurationMs ?? 6e4;
        if (state5.phase === "transition-to-sleep" || phaseDuration >= minSleep) {
          transitionTo("transition-to-wake");
        }
      }
    },
    getState: () => ({ ...state5 }),
    getModulation: () => ({
      dopamine: state5.wakeModulation.dopamineBoost,
      serotonin: state5.wakeModulation.serotoninBoost,
      acetylcholine: state5.wakeModulation.acetylcholineBoost,
      norepinephrine: state5.wakeModulation.norepinephrineBoost
    }),
    getSleepSettings: () => ({ ...state5.sleepSettings }),
    isInSleepPhase: () => state5.phase === "sleep" || state5.phase === "transition-to-sleep",
    isInWakePhase: () => state5.phase === "wake" || state5.phase === "transition-to-wake",
    forcePhase: (phase) => {
    ...
    getStats: () => ({
      phase: state5.phase,
      phaseProgress: state5.phaseProgress,
      phaseDuration: Date.now() - state5.phaseStartedAt,
      idleTime: state5.idleTime,
      activityLevel: state5.activityLevel,
      wakeInteractions: state5.wakeInteractions,
      sleepConsolidations: state5.sleepConsolidations,
      modulation: {
        dopamine: state5.wakeModulation.dopamineBoost,
        serotonin: state5.wakeModulation.serotoninBoost,
        acetylcholine: state5.wakeModulation.acetylcholineBoost,
      ...
      sleepSettings: { ...state5.sleepSettings }
    }),
...
function initCircadianRhythm(workspaceDir, config7, log) {
  active8?.dispose();
  active8 = createCircadianRhythm(workspaceDir, config7, log);
  if (pendingConsolidationCallback) {
    active8.setConsolidationCallback(pendingConsolidationCallback);
  }
}
...
function setConsolidationCallback(callback) {
  ...
  active8?.setConsolidationCallback(callback);
}
function recordActivity() {
  active8?.recordActivity();
}
function getCircadianState() {
  return active8?.getState() ?? createDefaultState4();
}
function getSleepSettings() {
  return active8?.getSleepSettings() ?? {
    consolidationIntensity: 0.5,
    pruningAggressiveness: 0.3,
    synapticNormalization: false
  };
...
function isInSleepPhase() {
  return active8?.isInSleepPhase() ?? false;
}
function isInWakePhase() {
  return active8?.isInWakePhase() ?? true;
}
function forcePhase(phase) {
  active8?.forcePhase(phase);
}
function getCircadianStats() {
  const st = active8?.getState() ?? createDefaultState4();
  return {
    phase: st.phase,
...
var DriveEngine = class {
  constructor(spec, config7, workspaceDir, circadianEnabled3, deps5, logger9) {
    ...
    this.deps = deps5;
    this.logger = logger9;
    const storageDir17 = join23(workspaceDir, ".brainagent", spec.id);
    if (!existsSync22(storageDir17)) {
      mkdirSync22(storageDir17, { recursive: true });
    }
    this.stateFile = join23(storageDir17, "state.json");
  ...
  persistState() {
    schedulePersist(this.stateFile, () => {
      ...
      return JSON.stringify(state5, null, 2);
    });
  }
};
...
function initSocialDrive(workspaceDir, cfg, log, injectedDeps) {
  active9?.dispose();
  active9 = createSocialDrive(workspaceDir, cfg, log, injectedDeps);
}
...
function initCognitiveHunger(workspaceDir, cfg, log, injectedDeps) {
  active10?.dispose();
  active10 = createCognitiveHunger(workspaceDir, cfg, log, injectedDeps);
}
...
function getCognitiveHungerSatiation() {
  return active10?.getSatiation() ?? 0;
}
...
function initCreativeDrive(workspaceDir, cfg, log, injectedDeps) {
  active11?.dispose();
  active11 = createCreativeDrive(workspaceDir, cfg, log, injectedDeps);
}
...
function getCreativeDriveSatiation() {
  return active11?.getSatiation() ?? 0;
}
...
function initMasteryDrive(workspaceDir, cfg, log, injectedDeps) {
  storageDir13 = join24(workspaceDir, ".brainagent", "mastery-drive");
  if (!existsSync23(storageDir13)) {
    mkdirSync23(storageDir13, { recursive: true });
  }
  ...
  totalNeedSignals = 0;
  adaptiveDecayModifier = 1;
  cancelPersist(join24(storageDir13, "state.json"));
  loadState12();
  wireEventListeners();
  logger4.info(
...
function stopMasteryDrive() {
  ...
  unsubscribers.length = 0;
  persistState9();
  flushPersist(join24(storageDir13, "state.json"));
...
function loadState12() {
  try {
    const filePath = join24(storageDir13, "state.json");
...
function persistState9() {
  schedulePersist(join24(storageDir13, "state.json"), () => {
    const domainMap = {};
    for (const [domain, mastery] of domainSatiations) {
...
function collectActiveDrives(drives, threshold) {
  ...
  if (drives.social) {
    ...
    if (s.need >= threshold) active17.push({ name: "social", need: s.need, needLevel: s.needLevel });
  }
  if (drives.cognitive) {
    const c = drives.cognitive();
    if (c.need >= threshold) active17.push({ name: "cognitive", need: c.need, needLevel: c.needLevel });
  }
  if (drives.creative) {
    const c = drives.creative();
    if (c.need >= threshold) active17.push({ name: "creative", need: c.need, needLevel: c.needLevel });
  }
  if (drives.mastery) {
    const m = drives.mastery();
    if (m.need >= threshold) active17.push({ name: "mastery", need: m.need, needLevel: m.needLevel });
  }
  return active17;
}
function synthesizeAutonomousCycleState(brainConfig, drives, cycle) {
  const active17 = collectActiveDrives(drives, 0.5);
  if (active17.length === 0) return;
  const ranked = active17.map((d) => {
...
function createAutonomousDeliverer(deps5) {
  const { state: state5, brainConfig, minGapMs, logger: logger9 } = deps5;
  return function enqueueAutonomousIntent(text) {
    const trimmed = text.trim();
    if (!trimmed) return;
    if (state5.previousCycleWasAutonomous) {
      logger9.info("BrainAgent Autonomy: intent suppressed \u2014 previous cycle was autonomous");
      return;
    }
    if (Date.now() - state5.lastAutonomousDeliveryAt < minGapMs) {
    ...
    state5.lastAutonomousDeliveryAt = Date.now();
    deps5.deliver(agent, framed);
  };
}
function createAutonomousIntentResolver(deps5) {
  const { state: state5, brainConfig, drives } = deps5;
  return () => {
    if (brainConfig.modules.goalStack && deps5.goalStack) {
      ...
      if (stats2.pending > 0) {
        ...
        if (triggered.length > 0) {
          ...
          if (goalCtx) {
            state5.lastAutonomousSource = `goal:${triggered[0].id}`;
            return {
              text: [
                AUTONOMOUS_TAG,
    ...
    if (brainConfig.modules.dmn && deps5.dmn) {
      ...
      if (novel.length > 0) {
        ...
        state5.lastAutonomousSource = "dmn:insight";
        return {
          text: [
            AUTONOMOUS_TAG,
            best.insightText,
    ...
    if (brainConfig.modules.goalStack && deps5.goalStack) {
      ...
      if (strong.length > 0) {
        ...
        state5.lastAutonomousSource = `desire:${top.id}`;
        return {
          text: [
            AUTONOMOUS_TAG,
            top.description,
    ...
    const active17 = collectActiveDrives(drives, 0.8);
    if (active17.length > 0) {
      const strongest = active17.sort((a, b) => b.need - a.need)[0];
      state5.lastAutonomousSource = `drive:${strongest.name}`;
      return {
        text: [
          AUTONOMOUS_TAG,
...
function createCycleEngine(deps5) {
  const { config: config7, brainConfig, getHostConfig, logger: logger9, markActivation, state: state5 } = deps5;
  ...
  async function endCycle3(key) {
    ...
    if (brainConfig.modules.hippocampus && input.length > 5) {
      if (isAutonomousCycle && cycle.responseText.trim()) {
        ...
        state5.lastAutonomousEpisodeId = episode.id;
        state5.lastAutonomousDomain = cycle.classification?.domain ?? "unknown";
        state5.previousCycleWasAutonomous = true;
...
var storageFile = "";
var state4 = {};
var explorationConstant = 1.4;
var attributionWindowMs = 5 * 60 * 1e3;
var unsubscribers2 = [];
var initialized = false;
var lastChoices = {};
function initStrategyBandit(workspaceDir, config7) {
  stopStrategyBandit();
  const storageDir17 = join25(workspaceDir, ".brainagent", "bandit");
  if (!existsSync24(storageDir17)) {
    mkdirSync24(storageDir17, { recursive: true });
  }
  storageFile = join25(storageDir17, "state.json");
  cancelPersist(storageFile);
  explorationConstant = config7.learningLoop.strategyBandit.explorationConstant;
  attributionWindowMs = config7.learningLoop.strategyBandit.attributionWindowMs;
  state4 = {};
  lastChoices = {};
  loadState13();
  unsubscribers2.push(
...
function chooseArm(decisionPoint, arms) {
  ...
  const point = state4[decisionPoint] ?? (state4[decisionPoint] = {});
...
function loadState13() {
  if (!storageFile || !existsSync24(storageFile)) return;
  try {
    const data = JSON.parse(readFileSync24(storageFile, "utf-8"));
    state4 = data && typeof data === "object" ? data : {};
  } catch {
  }
}
function persistState10() {
  if (!storageFile) return;
  schedulePersist(storageFile, () => JSON.stringify(state4, null, 2));
}
...
function initVitalImpulse(workspaceDir, cfg, log, injectedDeps) {
  storageDir14 = join26(workspaceDir, ".brainagent", "vital-impulse");
  if (!existsSync25(storageDir14)) {
    mkdirSync25(storageDir14, { recursive: true });
  }
  ...
  cancelPersist(join26(storageDir14, "state.json"));
  loadState14();
  wireSignalListeners();
  logger5.info(
...
function stopVitalImpulse() {
  ...
  unsubscribers3.length = 0;
  persistState11();
  flushPersist(join26(storageDir14, "state.json"));
...
function loadState14() {
  try {
    const filePath = join26(storageDir14, "state.json");
...
function persistState11() {
  schedulePersist(join26(storageDir14, "state.json"), () => {
    const state5 = {
      lastFireTime,
      totalFires,
      totalSignalsReceived,
...
function initDriveArbiter(workspaceDir, cfg, getters, log) {
  ...
  storageDir15 = join27(workspaceDir, ".brainagent");
  if (!existsSync26(storageDir15)) {
    mkdirSync26(storageDir15, { recursive: true });
  }
  cancelPersist(join27(storageDir15, "drive-arbiter.json"));
  loadState15();
  const driveEvents = [
...
function stopDriveArbiter() {
  ...
  unsubscribers4.length = 0;
  persistState12();
  flushPersist(join27(storageDir15, "drive-arbiter.json"));
...
function loadState15() {
  try {
    const path = join27(storageDir15, "drive-arbiter.json");
...
function persistState12() {
  ...
  schedulePersist(join27(storageDir15, "drive-arbiter.json"), () => {
    const data = {
      driveWeights,
      lastSelectedDrive,
      lastSelectionTime,
...
function createInteroception(getters, log) {
  ...
  function evaluate() {
    ...
    const changed = !lastState || lastState.pattern !== state5.pattern || Math.abs(lastState.confidence - state5.confidence) > 0.15;
    lastState = state5;
    if (changed) {
      bus.emitSync("interoception:state-updated", {
        pattern: state5.pattern,
        confidence: state5.confidence,
        description: state5.description,
        aggregateNeed: state5.aggregateNeed
      });
      log?.info(
        `BrainAgent Interoception: ${state5.pattern} (confidence=${(state5.confidence * 100).toFixed(0)}%) \u2014 ${state5.description}`
      );
    }
  }
  unsubscribers6.push(
...
function initInteroception(getters, log) {
  active12?.dispose();
  active12 = createInteroception(getters, log);
}
...
function getInteroceptiveState() {
  return active12?.getState() ?? null;
}
function buildInteroceptionContext() {
  return active12?.buildContext() ?? null;
}
...
function createTemporalAwareness(workspaceDir, cfg, log) {
  ...
  let typicalGapMs = 0;
  let totalInteractions = 0;
  const storageDir17 = join28(workspaceDir, ".brainagent");
  if (!existsSync27(storageDir17)) {
    mkdirSync27(storageDir17, { recursive: true });
  }
  try {
    const path = join28(storageDir17, "temporal-awareness.json");
  ...
  function persistState14() {
    try {
      ...
      writeFileSync22(
        join28(storageDir17, "temporal-awareness.json"),
...
function initTemporalAwareness(workspaceDir, cfg, log) {
  active13?.dispose();
  active13 = createTemporalAwareness(workspaceDir, cfg, log);
}
...
function recordInteraction() {
  active13?.recordInteraction();
}
function getTemporalAwarenessStats() {
  return active13?.getStats() ?? {
    typicalGapMs: 0,
    lastInteractionTime: 0,
    currentGapMs: 0,
...
function buildTemporalContext2() {
  return active13?.buildContext() ?? null;
}
...
function createAttentionGate(workspaceDir) {
  const storageDir17 = workspaceDir ? join29(workspaceDir, ".brainagent", "attention") : "";
  let totalProcessed = 0;
  let totalDropped = 0;
  let totalRelevanceSum = 0;
  if (storageDir17 && !existsSync28(storageDir17)) {
    mkdirSync28(storageDir17, { recursive: true });
  }
  function loadState17() {
    if (!storageDir17) return;
    try {
      const path = join29(storageDir17, "state.json");
  ...
  function persistState14() {
    ...
    try {
      writeFileSync23(
        join29(storageDir17, "state.json"),
...
var active14;
function current6() {
  return active14 ?? (active14 = createAttentionGate(""));
}
function initAttentionGate(workspaceDir, _config) {
  active14 = createAttentionGate(workspaceDir);
}
function filterContextInjections(injections, currentInput, norepinephrineLevel, config7, maxTokenBudget) {
  return current6().filter(injections, currentInput, norepinephrineLevel, config7, maxTokenBudget);
}
...
function assembleContext(state5) {
  const sections = [];
  if (state5.classification) {
    const meta = buildMetacognitiveInstruction(state5.classification);
    if (meta) sections.push(meta);
  }
  if (state5.priority) {
    const empathy = buildEmpathyGuidance(state5.priority);
    if (empathy) sections.push(empathy);
  }
  if (state5.userModel) {
    const userCtx = buildUserContext(state5.userModel);
    if (userCtx) sections.push(userCtx);
  }
  const memCtx = buildMemoryContext(state5.relevantMemories);
  if (memCtx) sections.push(memCtx);
  if (state5.relevantMemories.procedural.length > 0) {
    const procCtx = buildProceduralContext(state5.relevantMemories.procedural);
    if (procCtx) sections.push(procCtx);
  }
  if (state5.contextInjections.length > 0) {
    sections.push(...state5.contextInjections);
  }
...
function createInjectionMetrics() {
  const state5 = zeroState();
  function record(sectionCount, chars, budgetChars) {
    state5.totalCycles++;
    state5.totalChars += chars;
    state5.totalSections += sectionCount;
    state5.lastChars = chars;
    state5.lastSections = sectionCount;
    if (chars > state5.maxChars) state5.maxChars = chars;
    if (sectionCount > state5.maxSections) state5.maxSections = sectionCount;
    if (budgetChars !== void 0 && chars > budgetChars) state5.overBudgetCycles++;
  }
  function get() {
    const avgChars = state5.totalCycles > 0 ? Math.round(state5.totalChars / state5.totalCycles) : 0;
    const avgSections = state5.totalCycles > 0 ? Math.round(state5.totalSections / state5.totalCycles * 10) / 10 : 0;
    return {
      cycles: state5.totalCycles,
      avgChars,
      maxChars: state5.maxChars,
      lastChars: state5.lastChars,
      avgSections,
      maxSections: state5.maxSections,
      lastSections: state5.lastSections,
      avgEstTokens: Math.ceil(avgChars / 4),
      overBudgetCycles: state5.overBudgetCycles
    };
  }
  function reset() {
    Object.assign(state5, zeroState());
  }
...
function createPreStepHandler(deps5) {
  const { config: config7, brainConfig, getHostConfig, logger: logger9, state: state5, cycles, startCycle } = deps5;
  return async (payload, next) => {
    ...
    const cyc = cycle;
    const isAutonomousCycle = isAutonomousInput(input);
    if (brainConfig.modules.autonomousResearch && isAutonomousCycle && isResearchIntent(state5.lastAutonomousSource, input)) {
      ...
      logger9.info(
        `BrainAgent AutonomousResearch: detected research intent (source=${state5.lastAutonomousSource}), running isolated pipeline for "${topic}"`
      );
...
function initGoalExecutor(_cfg, log) {
  active15 = createGoalExecutor(log);
...
function initProactiveFeedback(workspaceDir, cfg, log) {
  storageDir16 = join30(workspaceDir, ".brainagent", "proactive-feedback");
  if (!existsSync29(storageDir16)) {
    mkdirSync29(storageDir16, { recursive: true });
  }
  stateFile = join30(storageDir16, "state.json");
...
function createRewardLedger(workspaceDir, config7) {
  const storageDir17 = join31(workspaceDir, ".brainagent", "reward");
  if (!existsSync30(storageDir17)) {
    mkdirSync30(storageDir17, { recursive: true });
  }
  const storageFile2 = join31(storageDir17, "ledger.json");
  cancelPersist(storageFile2);
  const maxEntries = config7.learningLoop.rewardLedger.maxEntries;
  let entries = [];
  let idCounter4 = 0;
  if (existsSync30(storageFile2)) {
    try {
      const data = JSON.parse(readFileSync30(storageFile2, "utf-8"));
      entries = Array.isArray(data.entries) ? data.entries.slice(-maxEntries) : [];
      idCounter4 = entries.length;
    } catch {
    }
  }
  function persistLedger() {
    schedulePersist(storageFile2, () => JSON.stringify({ entries }, null, 2));
  }
  function record(source, contribution, context) {
    const reward = clampReward(contribution);
    if (Math.abs(reward) < 1e-9) return;
    entries.push({
      id: `rw_${Date.now()}_${++idCounter4}`,
      timestamp: Date.now(),
...
function initRewardLedger(workspaceDir, config7) {
  active16?.dispose();
  active16 = createRewardLedger(workspaceDir, config7);
}
...
function apply(ctx, config7) {
  ...
  const state5 = createAutonomyState();
  function pickAgent() {
    const agents = ctx.agents.list();
    if (agents.length === 0) return void 0;
    return agents.find((a) => String(a.id) === state5.lastActiveAgentId) ?? agents[agents.length - 1];
  }
  ...
  ctx.on("session/event", (_session, event) => {
    ...
    if (event.type === "user/message") {
      ...
      if (isUserMessage && state5.previousCycleWasAutonomous && state5.lastAutonomousEpisodeId) {
        let reactionSignal = "neutral";
        if (brainConfig.modules.proactiveFeedback) {
          reactionSignal = recordProactiveReaction(state5.lastAutonomousDomain, text);
        }
        if (brainConfig.modules.hippocampus) {
          ...
          storeEpisode(
            ...
            reactionEmotion,
            ["proactive_feedback", state5.lastAutonomousEpisodeId, reactionSignal],
            reactionSalience
          );
        }
      }
      state5.previousCycleWasAutonomous = false;
      state5.lastAutonomousEpisodeId = void 0;
  ...
  ctx.on("tools/pre-execute", async (exec, next) => {
    ...
    const agentId = exec.agent ? String(exec.agent.id) : state5.lastActiveAgentId;
