...
function updateKnowledgeModelToM(model, text, domainLimit) {
  ...
  for (const domain of domainSignals) {
    const current26 = model.knowledgeModel[domain];
    if (!current26) {
      model.knowledgeModel[domain] = "beginner";
    } else if (current26 === "beginner") {
...
function createGoalStack(workspaceDir, config) {
  ...
  function expireGoals2() {
    ...
    if (goals.length > 60) {
      const active41 = goals.filter((g) => g.status === "pending" || g.status === "triggered");
      const inactive = goals.filter((g) => g.status === "completed" || g.status === "expired").slice(-30);
      goals = [...active41, ...inactive];
      changed = true;
    }
...
function collectActiveDrives(drives, threshold) {
  ...
  if (drives.social) {
    ...
    if (s.need >= threshold) active41.push({ name: "social", need: s.need, needLevel: s.needLevel });
  }
  if (drives.cognitive) {
    const c = drives.cognitive();
    if (c.need >= threshold) active41.push({ name: "cognitive", need: c.need, needLevel: c.needLevel });
  }
  if (drives.creative) {
    const c = drives.creative();
    if (c.need >= threshold) active41.push({ name: "creative", need: c.need, needLevel: c.needLevel });
  }
  if (drives.mastery) {
    const m = drives.mastery();
    if (m.need >= threshold) active41.push({ name: "mastery", need: m.need, needLevel: m.needLevel });
  }
  return active41;
}
function synthesizeAutonomousCycleState(brainConfig, drives, cycle) {
  const active41 = collectActiveDrives(drives, 0.5);
  if (active41.length === 0) return;
  const ranked = active41.map((d) => {
...
function createAutonomousIntentResolver(deps) {
  ...
  return () => {
    ...
    const active41 = collectActiveDrives(drives, 0.8);
    if (active41.length > 0) {
      const strongest = active41.sort((a, b) => b.need - a.need)[0];
