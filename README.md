...
~60 модулей, 6 точек интеграции с платформой, **793 unit-теста**, всё работает поверх штатных хуков dsh. Единственное исключение — анти-спам-патч хостового goal-round-driver: он лежит в `host-patches/` вместе с идемпотентным скриптом применения (повторно применяется после обновления dsh).
...
pnpm vitest run     # 50 файлов, 793 теста
```
