...
~60 модулей, 6 точек интеграции с платформой, **654 unit-теста**, всё работает поверх штатных хуков dsh — без единой правки ядра.
...
pnpm vitest run     # 33 файла, 654 теста
```
...
Единственная часть, которая действительно учится, — петля обучения
(`learningLoop`): журнал наград (reward-ledger) сводит оценки всех
источников в единый журнал, а многорукий бандит (strategy-bandit, UCB1)
по накопленной награде решает, какие блоки контекста подавать. Политика
хранится в таблицах, а не в весах — обучение с подкреплением без GPU.
